...
import gnnt.bank.platform.DataProcess;
...
public class PaltformProcessor extends CapitalProcessor
{
  public ReturnValue signedContract(CorrespondValue corr)
  {
    ...
    Vector vecF2f = null;
    try {
      vecF2f = DAO.getFirmID2SysFirmID(" where sysFirmID='" + corr.sysFirmID + "' and systemID='" + corr.systemID + "' and bankID='" + corr.bankID + "'");
    ...
    try { conn = DAO.getConnection();
      ...
      {
        Firm fm;
        Vector vecf2f2 = DAO.getFirmMapping(" where platformID='" + corr.firmID + "'");
      ...
      Vector vecCorrFirmID = DAO.getCorrespondList(" where firmID='" + corr.firmID + "' and bankID='" + corr.bankID + "'");
      if ((vecCorrFirmID != null) && (vecCorrFirmID.size() > 0)) {
        ...
        if ((!corrsv.account.equals(corr.account)) || (!corrsv.accountName.equals(corr.accountName)) || (!corrsv.cardType.equals(corr.cardType)) || (!corrsv.card.equals(corr.card))) {
          Tool.log("平台已经存在的对应关系中的银行账号[" + corrsv.account + "]银行账户名[" + corrsv.accountName + "]证件类型[" + corrsv.cardType + "]证件号[" + corrsv.card + "]和传入的银行账号[" + corrsv.account + "]银行账户名[" + corrsv.accountName + "]证件类型[" + corrsv.cardType + "]证件号[" + corrsv.card + "]不完全相符");
          result.result = -1L;
          result.remark = "银行账户信息不相符";
          conn.rollback();
          localReturnValue1 = result; return localReturnValue1;
        }
        CorrespondValue corrsv;
        Vector vecCorrFirmID;
        Firm fm;
        if (corrsv.isOpen == 1) {
      ...
      else
      {
        ...
        Vector vecCorr2 = DAO.getCorrespondList(" where contact='" + corr.firmID + "' and bankID='" + corr.bankID + "'");
      ...
      if (result.result == 0L) {
        ...
        Vector vecFirmID2FirmID = DAO.getFirmID2SysFirmID(" where firmID='" + corr.firmID + "' and bankID='" + corr.bankID + "'", conn);
        ...
        if ((vecFBF2 != null) && (vecFBF2.size() > 0)) {
          Vector vecFBF3 = DAO.getFirmBankFunds(" and f.firmID='" + corr.firmID + "' and f.primaryBankFlag=1", conn);
          if ((vecFBF3 == null) || (vecFBF3.size() <= 0) || (vecFBF3.size() > 1)) {
            ...
            vecOpen = DAO.getCorrespondList(" where firmid='" + corr.firmID + "' and bankid='" + ((FirmBankFunds)vecFBF3.get(0)).bankID + "'", conn);
            if ((vecOpen == null) || (((Vector)vecOpen).size() != 1)) {
              ...
              Tool.log("平台号[" + corr.firmID + "]之前签约的主银行[" + ((FirmBankFunds)vecFBF3.get(0)).bankID + "]没有最终签约成功，将本次签约的银行[" + corr.bankID + "]设置为主银行，原有主银行修改为次银行");
              ...
              if (DAO.modFirmBankFunds(oldMainBank, conn) != 1) {
                ...
                Vector vecFBF4 = DAO.getFirmBankFunds(" and f.firmID='" + corr.firmID + "' and f.bankcode='" + corr.bankID + "'", conn);
        ...
        if ((vecFirmID2FirmID == null) || (vecFirmID2FirmID.size() <= 0))
          ...
          for (vecOpen = vecFirmID2FirmID.iterator(); ((Iterator)vecOpen).hasNext(); ) { FirmID2SysFirmID ff = (FirmID2SysFirmID)((Iterator)vecOpen).next();
            noticePlatFirmID(ff.systemID, ff.firmID, ff.sysFirmID); }
      }
      else
      {
  ...
  {
    ReturnValue result = new ReturnValue();
    Vector vec = getCorrespondValue(filter);
    if (vec == null) {
      result.result = -1L;
      result.remark = "查询失败";
      return result;
  ...
  public ReturnValue modCorrespondValue(CorrespondValue corrLast, CorrespondValue corr)
  {
    ...
    if ((corrOld.bankID.equals(corr.bankID)) && (corr.accountName.equals(corrOld.accountName)) && (corr.account.equals(corrOld.account)) && (corr.cardType.equals(corrOld.cardType)) && (corr.card.equals(corrOld.card))) {
    ...
    try {
      ...
      conn.setAutoCommit(false);
      CorrespondValue corrNew = (CorrespondValue)DAO.getCorrespondList(" where firmID='" + corrLast.firmID + "' and bankID='" + corrLast.bankID + "'").get(0);
      ...
      if (!corrOld.bankID.equals(corrNew.bankID)) {
        Vector corrCheck = DAO.getCorrespondList(" where firmID='" + corr.firmID + "' and contact='" + corrOld.contact + "' and bankID='" + corr.bankID + "'", conn);
        if ((corrCheck != null) && (corrCheck.size() > 0)) {
          corrNew.contact = (corrNew.firmID + "QYN" + corr.bankID);
        }

        Vector vecF2f = DAO.getFirmID2SysFirmID(" where firmID='" + corrLast.firmID + "' and sysfirmid='" + corrLast.sysFirmID + "' and systemID='" + corrLast.systemID + "' and bankID='" + corrLast.bankID + "'", conn);
        if ((vecF2f == null) || (vecF2f.size() <= 0)) {
          Tool.log("在F_B_FIRMID_SYSFIRMID表中没有找到交易系统[" + corrLast.systemID + "]的交易商[" + corrLast.sysFirmID + "]和平台号[" + corrLast.firmID + "]有关银行[" + corrLast.bankID + "]的记录");
          result.result = -1L;
  ...
  public ReturnValue delCorrMarket(CorrespondValue corr)
  {
    ...
    try {
      ...
      Vector vecOpen = DAO.getCorrespondList(" where firmID='" + corr.firmID + "'", conn);
      ReturnValue localReturnValue1;
      ...
      Vector vecOpen;
      Vector vecF2f = DAO.getFirmID2SysFirmID(" where firmID='" + corr.firmID + "' and bankID='" + corr.bankID + "'", conn);
      if ((vecF2f == null) || (vecF2f.size() <= 0)) {
      ...
      Vector vecFFV = DAO.getFirmBankFunds(" and f.firmid='" + corr.firmID + "' and f.bankcode='" + corr.bankID + "'", conn);
  ...
  public ReturnValue delCorrPT2Bank(CorrespondValue corr)
  {
    ...
    try {
      ...
      Vector vecC = DAO.getCorrespondList(" where firmid='" + corr.firmID + "' and bankid='" + corr.bankID + "'", conn);
      ReturnValue localReturnValue1;
  ...
  public ReturnValue getRelationSystem(String firmID)
  {
    ...
    Vector vecSysm = null;
    try {
      vecSysm = DAO.getSystemMessages(" where systemID in (select distinct systemid from f_b_firmid_sysfirmid t where firmid='" + firmID + "')");
    } catch (SQLException e) {
  ...
  public ReturnValue inOutMoneyReqDo(InOutMoney inOutMoney)
  {
    ...
    Vector vecF2f = null;
    try {
      vecF2f = DAO.getFirmID2SysFirmID(" where firmID='" + inOutMoney.firmID + "' and systemid='" + inOutMoney.systemID + "' and bankID='" + inOutMoney.bankID + "'");
    } catch (SQLException e) {
  ...
  public ReturnValue inMoneyPtToBank(InOutMoney inMoney)
  {
    ...
    try {
      ...
      if ((inMoney.contact == null) || ("".equals(inMoney.contact))) {
        Vector vecCorr = DAO.getCorrespondList(" where firmID='" + inMoney.firmID + "' and bankID='" + inMoney.bankID + "'", conn);
      ...
      InMoneyVO inMoneyInfo = new InMoneyVO(inMoney.bankID, bv.bankName, inMoney.money, inMoney.firmID, inMoney.contact, payInfo, inMoney.accountPwd, receiveInfo, inMoney.remark, inMoney.actionID, inMoney.payChannel);
  ...
  {
    Tool.log("入金，交易系统发起");
    Tool.log("bankID[" + inMoney.bankID + "]firmID[" + inMoney.firmID + "]systemID[" + inMoney.systemID + "]sysFirmID[" + inMoney.sysFirmID + "]account[" + inMoney.account + "]accountPwd[" + inMoney.accountPwd + "]money[" + inMoney.money + "]remark[" + inMoney.remark + "]paychannel[" + inMoney.payChannel + "]");
    ...
    try {
      ...
      Vector vecf2f = DAO.getFirmID2SysFirmID(" where bankID='" + inMoney.bankID + "' and sysFirmID='" + inMoney.sysFirmID + "' and systemID='" + inMoney.systemID + "'");
      ReturnValue localReturnValue1;
      if ((vecf2f == null) || (vecf2f.size() <= 0)) {
        Tool.log("没有找到平台号[" + inMoney.firmID + "]和交易系统[" + inMoney.systemID + "]的交易商[" + inMoney.sysFirmID + "]关于银行[" + inMoney.bankID + "]的签约关系");
        result.result = -1L;
        result.remark = "签约关系不存在";
        localReturnValue1 = result; return localReturnValue1;
      }
      Vector vecf2f;
      Vector capitalVec = DAO.getCapitalInfoListPT(" where funid='" + inMoney.sysAciontID + "' and systemid='" + inMoney.systemID + "'");
  ...
  public ReturnValue outMoneyPtToBank(InOutMoney outMoney, double outRate)
  {
    ...
    try {
      ...
      if ((outMoney.contact == null) || ("".equals(outMoney.contact))) {
        Vector vecCorr = DAO.getCorrespondList(" where firmID='" + outMoney.firmID + "' and bankID='" + outMoney.bankID + "'", conn);
        if ((vecCorr == null) || (vecCorr.size() != 1)) {
      ...
      OutMoneyVO outMoneyInfo = new OutMoneyVO(outMoney.bankID, bv.bankName, outMoney.money, outMoney.firmID, outMoney.contact, payInfo, receiveInfo, outMoney.actionID, 0, null);
  ...
  public ReturnValue outMoneyMarket(InOutMoney outMoney)
  {
    ...
    try {
      ...
      Vector vecf2f = DAO.getFirmID2SysFirmID(" where bankID='" + outMoney.bankID + "' and sysFirmID='" + outMoney.sysFirmID + "' and systemID='" + outMoney.systemID + "'");
      ReturnValue localReturnValue1;
      if ((vecf2f == null) || (vecf2f.size() <= 0)) {
        Tool.log("没有找到平台号[" + outMoney.firmID + "]和交易系统[" + outMoney.systemID + "]的交易商[" + outMoney.sysFirmID + "]关于银行[" + outMoney.bankID + "]的签约关系");
        result.result = -1L;
        result.remark = "签约关系不存在";
        localReturnValue1 = result; return localReturnValue1;
      }
      Vector vecf2f;
      Vector capitalVec = DAO.getCapitalInfoListPT(" where funid='" + outMoney.sysAciontID + "' and systemid='" + outMoney.systemID + "'");
      if ((capitalVec != null) && (capitalVec.size() > 0)) {
  ...
  public ReturnValue transferFunds(String outSystemID, String inSystemID, String firmID, double money, String fundsPwd)
  {
    ...
    try {
      ...
      req.setParams(new Object[] { firmID, ((FirmID2SysFirmID)vecF2fOut.get(0)).sysFirmID, Double.valueOf(money), Integer.valueOf(1), String.valueOf(outActionID) });
      result = tradeProc.doWork(req);
      ...
      if (result.result == 0L) {
        ...
        RequestMsg req;
        TradeProcessorRMI tradeProc;
        req.setParams(new Object[] { firmID, ((FirmID2SysFirmID)vecF2fIn.get(0)).sysFirmID, Double.valueOf(money), Integer.valueOf(0), String.valueOf(inActionID) });
        result = tradeProc.doWork(req);
        if (result.result != 0L) {
  ...
  public ReturnValue outMoneyMarketAudit(CapitalValue cv, boolean flag)
  {
    ...
    try {
      ...
      try
      {
        ...
        if (flag) {
          dataProcess.updateFundsFull(cv.firmID, dataProcess.getOUTSUMMARY(), (String)dataProcess.getBANKSUB().get(cv.bankID), cv.money, cv.actionID, conn);
        }
  ...
  public ReturnValue outMoneyBankAudit(CapitalValue cv, boolean flag)
  {
    ...
    try {
      ...
      Vector vecCorr = DAO.getCorrespondList(" where firmID='" + cv.firmID + "' and bankID='" + cv.bankID + "'", conn);
      if ((vecCorr == null) || (vecCorr.size() != 1)) {
      ...
      Vector vecCorr;
      BankValue bv;
      TransferInfoValue receiveInfo;
      TransferInfoValue payInfo;
      BankAdapterRMI bankadapter;
      OutMoneyVO outMoneyInfo = new OutMoneyVO(cv.bankID, bv.bankName, cv.money, cv.firmID, ((CorrespondValue)vecCorr.get(0)).contact, payInfo, receiveInfo, cv.actionID, 0, cvOut.funID);
      OutMoneyVO outMoneyInfo;
      BankAdapterRMI bankadapter;
      if (flag) {
        try {
          conn.setAutoCommit(false);
  ...
  public ReturnValue getAllCapitalList(String firmID, String startDate, String endDate)
  {
    ...
    Vector vecCap = null;
    try {
      vecCap = DAO.getCapitalInfoListPT(" where firmID='" + firmID + "' and trunc(createTime)>=to_date('" + startDate + "','yyyy-MM-dd') and trunc(createTime)<=to_date('" + endDate + "','yyyy-MM-dd')");
    } catch (Exception e) {
  ...
  public ReturnValue getRZData(Date date)
  {
    ...
    try {
      ...
      try {
        ...
        {
          Vector vec = DAO.getRZQSData(date, sm.systemID);
  ...
  public ReturnValue getRZDataBySystemID(Date date, String systemID, Connection conn)
  {
    ...
    Vector vecRZData = (Vector)result.msg[0];
    try {
  ...
  public ReturnValue getRZDataBySystemID(Date date, String systemID)
  {
    ...
    try {
      ...
      try
      {
        Vector vec = DAO.getRZQSData(date, systemID);
        if ((vec != null) && (vec.size() > 0) && (((RZQS)vec.get(0)).doFlag != 0)) {
  ...
  public ReturnValue inMoneyAduitToBank(CapitalValue cvBank, CapitalValue cvSystem, boolean flag)
  {
    ...
    try {
      ...
      try {
        ...
        if (flag) {
          Tool.log("审核通过，增加资金");
          dataProcess.updateFundsFull(cvBank.firmID, dataProcess.getINSUMMARY(), (String)dataProcess.getBANKSUB().get(cvBank.bankID), cvBank.money, cvBank.actionID, conn);
        }
      ...
      try {
        ...
        if ("1".equals(cvBank.tradeSource))
        {
          if (cvSystem.status == 2) {
            ...
            if (flag) {
              dataProcess.updateFundsFull(cvSystem.firmID, dataProcess.getOUTSUMMARY(), (String)dataProcess.getBANKSUB().get(cvSystem.bankID), cvSystem.money, cvSystem.actionID, conn);
            }
          }
  ...
  public ReturnValue addOtherFirmMsg(String platformID, String sysFirmID, String systemID)
  {
    ...
    Vector vecFirmMapping = null;
    try {
      vecFirmMapping = DAO.getFirmMapping(" where platfirmid='" + platformID + "' and moduleid='" + sysFirmID + "' and firmid='" + systemID + "'");
    } catch (SQLException e) {
  ...
  public ReturnValue readCapitalInfos(String systemID, Date date)
  {
    ...
    Vector vecCap = (Vector)result.msg[0];
    Connection conn = null;
    try {
      conn = DAO.getConnection();
      try {
