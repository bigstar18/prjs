...
public class ComplexProcessor {
	...
	public ReturnValue getCapitals(Date date) {
		...
		try {
			Map map = new HashMap();
			map.put(" and type<>?", Integer.valueOf(2));
			map.put(" and status=?", Integer.valueOf(0));
	...
	public ReturnValue noticeOutMoneyResult(String sysActionID, String actionID, boolean flag) {
		...
		if (!actionID.equals(cv.funID)) {
			Tool.log("市场端记录的平台流水号[" + cv.funID + "]和通知传入的平台流水号[" + actionID + "]不相符");
			result.result = -1L;
			result.remark = "平台流水号不相符";
			return result;
		}
		if (cv.status != 2) {
			Tool.log("该笔流水已经被处理");
		...
		try {
			...
			try {
				...
				if (flag) {
					DAO.modCapitalInfoStatus(cv.iD, actionID, 0, bankTime, conn);
					DAO.modCapitalInfoNote(cv.iD, cv.note + "平台通知成功，出金成功", conn);

					dataProcess.updateFrozenFunds(cv.firmID, -1.0D * cv.money, conn);

					dataProcess.updateFundsFull(cv.firmID, "11004", "100201", cv.money, Long.parseLong(actionID), conn);
				} else {
					DAO.modCapitalInfoStatus(cv.iD, actionID, 1, bankTime, conn);
