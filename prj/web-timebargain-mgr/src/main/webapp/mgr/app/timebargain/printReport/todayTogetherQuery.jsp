...
<fieldset class="pickList" >
	...
	<table border="0" align="center" cellpadding="5" cellspacing="5" class="common">
	 ...
	 <tr>
	  ...
	  <tr>
        ...
        <td>
         <div align="left">
            <INPUT TYPE="hidden" NAME="queryType" value=""  style="ime-mode:disabled"  size="8" maxlength="10" onblur="fandealcontent2(this)" onkeypress="notSpace()">
            <SELECT  name="select6" onchange="dealcontent2(this)">
            <OPTION value="">请选择</OPTION>
	        <option value="1">查询全部</option>
	        <option value="2">仅有资金余额</option>
	        <option value="3">仅当日有资金变化</option>
          </SELECT>   
	...
	<table border="0" align="center" cellpadding="5" cellspacing="5" class="common">
		<tr>
				<td>
				<rightButton:rightButton name="查看" onclick="return query('pdf')" className="btn_sec" action="${basePath}/timebargain/todayTogetherReport/todayTogether.action" id="add"></rightButton:rightButton>
	     		</td>
	     		<td>
	     		<rightButton:rightButton name="保存为excel" onclick="return query('excel')" className="btn_sec1" action="${basePath}/timebargain/todayTogetherReport/todayTogetherExcel.action" id="saveExcel"></rightButton:rightButton>
	     		</td>
		</tr>
	</table>
	</fieldset>
...
{
    document.forms[0].queryType.value=sel.options[sel.selectedIndex].value;
}
...
function query(exportTo)
{
	...
	if (queryType == "") {
     ...
     if(exportTo=="excel"){
		//获取配置权限的 URL
		var url = "${mgrPath}/app/timebargain/printReport/todayTogetherReportExcel.jsp?startFirmID="+zStart +"&endFirmID="+ zEnd +"&cleardate="+zDate+"&title=综合日报表&brokerId="+brokerId+"&queryType="+queryType;	
		document.location.href = url;	
	 }else{
		//获取配置权限的 URL
		var url = "${mgrPath}/app/timebargain/printReport/todayTogetherReport.jsp?startFirmID="+zStart +"&endFirmID="+ zEnd +"&cleardate="+zDate+"&title=综合日报表&brokerId="+brokerId+"&queryType="+queryType;			
