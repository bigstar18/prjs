...
<script type="text/javascript">
	...
	function check_commodityMargin(id){
		//获取配置权限的 URL
		var detailUrl = "${basePath}/timebargain/check/updateCommodityMarginCheforward.action?entity.id=";
		//获取完整跳转URL
		var url = detailUrl + id;
		//弹出修改页面
		if(showDialog(url, "", 700, 650)){
			//如果修改成功，则刷新列表
...
<div id="main_body">
	<table class="table1_style" border="0" cellspacing="0" cellpadding="0">
		<tr>
			<td>
				<div class="div_cx">
					<form id="frm" name="frm" action="${basePath}/timebargain/check/commodityMarginCheList.action" method="post">
						...
						<tr>
							<td>
								<ec:table items="pageInfo.result" var="commodityMargin"
									action="${basePath}/timebargain/check/commodityMarginCheList.action"											
									...
									<ec:row>
										...
										<c:choose>
												<c:when test="${commodityMargin.STATUS == 1}">
													<%-- <a href="#" onclick="check_commodityMargin('<c:out value="${commodityMargin.ID}"/>')"><img height="20" title="审核与查看" src="<%=skinPath%>/image/app/timebargain/commodity.gif"/></a>--%>
													<rightHyperlink:rightHyperlink text="<img title='审核与查看' src='${skinPath}/image/app/timebargain/commodity.gif'/>" href="#" onclick="check_commodityMargin('${commodityMargin.ID}')" action="/timebargain/check/updateCommodityMarginCheforward.action"/>
												</c:when>
												<c:otherwise>
													<%-- <a href="#" onclick="check_commodityMargin('<c:out value="${commodityMargin.ID}"/>')"><img height="20" title="审核与查看" src="<%=skinPath%>/image/app/timebargain/commodity.gif"/></a>--%>
													<rightHyperlink:rightHyperlink text="<img title='审核与查看' src='${skinPath}/image/app/timebargain/commodity.gif'/>" href="#" onclick="check_commodityMargin('${commodityMargin.ID}')" action="/timebargain/check/updateCommodityMarginCheforward.action"/>
