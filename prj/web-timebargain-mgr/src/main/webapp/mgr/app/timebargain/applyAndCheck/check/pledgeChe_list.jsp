...
<script type="text/javascript">
	...
	function check_pledge(id){
		//获取配置权限的 URL
		var detailUrl = "${basePath}/timebargain/check/updatePledgeCheforward.action?entity.id=";
		//获取完整跳转URL
		var url = detailUrl + id;
		//弹出修改页面
		if(showDialog(url, "", 500, 550)){
			//如果修改成功，则刷新列表
...
<div id="main_body">
	<table class="table1_style" border="0" cellspacing="0" cellpadding="0">
		<tr>
			<td>
				<div class="div_cx">
					<form id="frm" name="frm" action="${basePath}/timebargain/check/pledgeCheList.action" method="post">
						...
						<tr>
							<td>
								<ec:table items="pageInfo.result" var="pledge"
									action="${basePath}/timebargain/check/pledgeCheList.action"											
									...
									<ec:row>
								        ...
								        <ec:column property="FIRMID" title="交易商代码" width="150" ellipsis="true"  style="text-align:center;">
								        </ec:column>
										...
										<c:choose>
											<c:when test="${pledge.STATUS == 1}">
												<a href="#" onclick="check_pledge('<c:out value="${pledge.ID}"/>')"><img height="20" title="查看" src="<%=skinPath%>/image/app/timebargain/commodity.gif"/></a>
											</c:when>
											<c:otherwise>
												<a href="#" onclick="check_pledge('<c:out value="${pledge.ID}"/>')"><img height="20" title="查看" src="<%=skinPath%>/image/app/timebargain/commodity.gif"/></a>
											</c:otherwise>
