...
function save_onclick()
{
	if (confirm("您确定要提交吗？")) {
    	...
    	var updateDataUrl = $("#editSpacMargin").attr("action");
...
function isFirmMargin(){
	var commodityId=document.forms(0).commodityID.value;
	var oldAjaxAsync = $.ajaxSettings.async;
	var url = "${basePath}/timebargain/xtwh/ycclFirmMargin.action?id=" + commodityId;
...
function checkFirmId(){
	var firmId=document.forms(0).customerID.value;
	var oldAjaxAsync = $.ajaxSettings.async;
	var url = "${basePath}/timebargain/xtwh/ycclCheckFirm.action?firmId=" + firmId;
	$.ajaxSettings.async = false;
	$.getJSON(url,null,function call(result){
		if(result=="false"){
			alert("交易商代码不存在");
						...
						<fieldset class="pickList" >
							...
							<table border="0" align="center" cellpadding="5" cellspacing="5"
								...
								<tr>
									...
									<td>
										<input type="text"  id="customerID" name="customerID" maxlength="10"  styleClass="text" onBlur="checkFirmId()" />
										<span class="req">*</span>
									</td>
									<td></td>
								</tr>
								<tr>
									...
									<td>
                                        ...
                                        <SELECT  name="select6" style="width:100px"  onchange="dealcontent1(this);commodity_onchange()">
								            ...
								            <c:forEach items="${spacMarginList}" var="result">
								...
								<tr>
									<td colspan="3" align="center">
										<rightButton:rightButton name="提交"   id="editSpacMargin" onclick="save_onclick();" className="button2" action="${basePath}/timebargain/xtwh/ycclEditSpacMargin.action" ></rightButton:rightButton>
