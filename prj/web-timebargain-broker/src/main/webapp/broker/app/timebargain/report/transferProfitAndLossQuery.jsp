...
<fieldset class="pickList" >
	<legend class="common">
		<b>转让盈亏明细表</b><input name="userType" type="hidden" value="${CurrentUserType }">
	</legend>
	...
	<table border="0" align="center" cellpadding="5" cellspacing="5" class="common">
		<tr>
				<td>
				<rightButton:rightButton name="查看" onclick="return query('pdf')" className="btn_sec" action="${basePath}/timebargain/transferProfitAndLoss/transferProfitAndLoss.action" id="add"></rightButton:rightButton>
	     		</td>
	     		<td>
	     		<rightButton:rightButton name="保存为excel" onclick="return query('excel')" className="btn_sec1" action="${basePath}/timebargain/transferProfitAndLoss/transferProfitAndLossExcel.action" id="saveExcel"></rightButton:rightButton>
	     		</td>
		</tr>
	</table>
	</fieldset>
...
function query(exportTo)
{
	...
	if (zDate == "") {
     ...
     if(exportTo=="excel"){
		//获取配置权限的 URL
		var url = "${mgrPath}/app/timebargain/report/transferProfitAndLossReportExcel.jsp?startFirmID="+zStart +"&endFirmID="+ zEnd +"&cleardate="+zDate+"&brokerageId="+zBrokerage+"&firmcategory="+firmcategory;
		document.location.href = url;
	 }else{
		//获取配置权限的 URL
		var url = "${mgrPath}/app/timebargain/report/transferProfitAndLossReport.jsp?startFirmID="+zStart +"&endFirmID="+ zEnd +"&cleardate="+zDate+"&brokerageId="+zBrokerage+"&firmcategory="+firmcategory;
