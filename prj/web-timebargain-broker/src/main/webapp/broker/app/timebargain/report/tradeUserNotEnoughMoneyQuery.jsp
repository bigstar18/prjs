...
<fieldset class="pickList" >
	<legend class="common">
		<b>资金不足交易商情况表</b><input name="userType" type="hidden" value="${CurrentUserType }">
	</legend>
  <table border="0" align="center" cellpadding="5" cellspacing="5" class="common">  
	 <tr>
      ...
      <tr>
        ...
        <td>
          <div align="left">
            <INPUT TYPE="" NAME="zEnd" value=""  style="ime-mode:disabled"  size=15 maxlength=10 onblur="fandealcontent1(this)" onkeypress="notSpace()" readonly="readonly">
	...
	<c:if test="${CurrentUserType=='0'}">
	 ...
	 <tr>
      ...
      <tr>
		...
		<tr>
				<td>
				<rightButton:rightButton name="查看" onclick="return query('pdf')" className="btn_sec" action="${basePath}/timebargain/report/tradeUserNotEnoughMoneyReport.action" id="add"></rightButton:rightButton>
	     		</td>
	     		<td>
	     		<rightButton:rightButton name="保存为excel" onclick="return query('excel')" className="btn_sec1" action="${basePath}/timebargain/report/tradeUserNotEnoughMoneyReportExcel.action" id="saveExcel"></rightButton:rightButton>
	     		</td>
		</tr>
	</table>
	</fieldset>
...
function query(exportTo)
{
	...
	if(exportTo=="excel"){
		//获取配置权限的 URL
		var url = "${mgrPath}/app/timebargain/report/tradeUserNotEnoughMoneyReportExcel.jsp?startFirmID="+zStart +"&endFirmID="+ zEnd +"&cleardate="+zDate+"&brokerageId="+zBrokerage+"&firmcategory="+firmcategory;
		document.location.href = url;
	 }else{
		//获取配置权限的 URL
		var url = "${mgrPath}/app/timebargain/report/tradeUserNotEnoughMoneyReport.jsp?startFirmID="+zStart +"&endFirmID="+ zEnd +"&cleardate="+zDate+"&brokerageId="+zBrokerage+"&firmcategory="+firmcategory;
