...
<fieldset class="pickList" >
	<legend class="common">
		<b>资金汇总表</b><input name="userType" type="hidden" value="${CurrentUserType }">
	</legend>
	...
	<table border="0" align="center" cellpadding="5" cellspacing="5" class="common">
		<tr>
				<td>
				<rightButton:rightButton name="查看" onclick="return query('pdf')" className="btn_sec" action="${basePath}/timebargain/fundCollectReport/fundCollect.action" id="add"></rightButton:rightButton>
	     		</td>
	     		<td>
	     		<rightButton:rightButton name="保存为excel" onclick="return query('excel')" className="btn_sec1" action="${basePath}/timebargain/fundCollectReport/fundCollectExcel.action" id="saveExcel"></rightButton:rightButton>
	     		</td>
		</tr>
	</table>
	</fieldset>
...
function query(exportTo)
{
   var zStart=document.forms[0].zStart.value;
	...
	if (zEndDate == "") {
     ...
     if(exportTo=="excel"){
		//获取配置权限的 URL
		var url = "${mgrPath}/app/timebargain/report/fundCollectReportExcel.jsp?startFirmID="+zStart +"&endFirmID="+ zEnd +"&startdate="+zDate +"&enddate="+zEndDate+"&brokerageId="+zBrokerage+"&firmcategory="+firmcategory;
		document.location.href = url;
	 }else{
		//获取配置权限的 URL
		var url = "${mgrPath}/app/timebargain/report/fundCollectReport.jsp?startFirmID="+zStart +"&endFirmID="+ zEnd +"&startdate="+zDate +"&enddate="+zEndDate+"&brokerageId="+zBrokerage+"&firmcategory="+firmcategory;
