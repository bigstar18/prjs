	  ...
	  <br>
		...
		<span>
			<table border="0" cellspacing="0" cellpadding="0" width="100%">
              ...
              <tr height="35">
				...
				<td align="right" class="tdstyle"> 合同状态：</td>
                <td align="left">
				  <select name="status">
		...
		<%
          ...
          %>
					...
					<a href="javascript:controlStatus('<%=commodityID%>','${marketId}','<%=rs.getString("str6")%>','<%=contractID%>')" class="normal">
					<%if(status==0){%>
					付款提货中
					<%}else if(status==1){%>
					提货完毕
					...
					<td class="underLine"><a href="javascript:liftGoods('<%=commodityID%>','${marketId}','<%=rs.getString("str6")%>','<%=contractID%>')" class="normal">查看</a></td>
