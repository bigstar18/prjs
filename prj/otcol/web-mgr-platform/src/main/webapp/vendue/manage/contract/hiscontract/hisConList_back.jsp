...
<%@ include file="/manage/globalDef.jsp"%> 
<!--用户身份判断-->
<%@ include file="/manage/public/session.jsp"%>
<!--用户权限判断-->
<%
String pd = gnnt.vendue.server.dao.TradeDAOFactory.getDAO().getPD();//xieying
	request.setAttribute("pd",pd); //xieying
%>
<jsp:include page="/manage/public/checkRight.jsp">
	<jsp:param name="comRight" value="<%=HISCONTRACT%>"/>
	<jsp:param name="userRight" value="${USERRIGHT}"/>
</jsp:include>
<!--操作-->
<c:if test='${not empty param.opt}'>
  ...
  <c:if test="${param.opt=='returnContract'}">
    <%
	  ...
	  try
      {
		String sql="select contractid,patchstatus from hisbargain where commodityid=";
		long contractid=0l;
		int patchstatus=0;
  ...
  <c:if test="${param.opt=='retFile'}"><!--合同归档-->
     ...
     <c:forEach var="v_value" items="${paramValues.ck}">
	   ...
	   <db:select var="row" table="hisbargain" columns="contractid" where="commodityid=${v_value}">
          <c:set var="contractid" value="${row.contractid}"/>
	   </db:select>
	   <!--判断是否有出库记录-->
       <c:set var="cnt" value="0"/>
       <!--判断是否有出库记录-->
       <db:select var="row" table="outlog" columns="count(contractid) as c" where="contractid=${contractid}">
       <c:set var="cnt" value="${row.c}"/>
       </db:select>
       <c:choose>
       <c:when test="${cnt<=0}">
         <c:set var="modFlag" value="false"/>
		 <c:set var="hintInfo" value="不存在出库记录"/>
	   </c:when>
	   <c:otherwise>
	     <c:set var="modFlag" value="true"/>
       </c:otherwise>
	   ...
	   <!--判断合同的出库记录都是完毕状态-->
       <db:select var="row" table="outlog" columns="contractid" where="contractid=${contractid} and finished<>2">
         <c:set var="modFlag" value="false"/>
		 <c:set var="hintInfo" value="的出库记录不是已完毕状态"/>
	   </db:select>
	   <!--判断合同是否是付款完毕状态，只有是付款完毕状态才可以置归档-->
	   <db:select var="row" table="hisbargain" columns="status" where="commodityid=${v_value}">
	   ...
	   <c:choose>
		 <c:when test="${modFlag=='true'}">
           <db:update table="hisbargain" status="2" where="commodityid=${v_value}"/>
		  </c:when>
	      <c:otherwise>
           <c:choose>
...
<%
	  ...
	  sql.append("select partitionid from syspartition where validflag=1 order by partitionid asc");
...
<%
	  ...
	  if("".equals(t_partitionID)){
	      sql=new StringBuffer();
	      sql.append("select partitionid from syspartition  where 1=1 "+tempfilter+" order by partitionid asc");
          ps = conn.prepareStatement(sql.toString());
	      rs=ps.executeQuery();
	      if(rs.next()){
...
%>
	  <jsp:include page="/manage/public/menu.jsp">
...
<c:choose> 
 ...
 <c:otherwise> 
   ...
   <c:choose>
   <c:when test="${partitionID!=allPartition}">
	 <%filter=filter+" and tradepartition = "+pageContext.getAttribute("partitionID").toString()+"";%>
   </c:when>
   <c:otherwise>
	 <%filter=filter+" and tradepartition in  (select partitionid from syspartition where validflag=1)";%>
   </c:otherwise>
   </c:choose>
 </c:otherwise> 
</c:choose>
	  <br>
			...
			<table border="0" cellspacing="0" cellpadding="0" width="100%">
              ...
              <tr height="35">
				...
				<c:choose>
                	...
                	<MEBS:calendar eltID="tradeDate" eltName="tradeDate" eltCSS="date" eltStyle="width:80px" eltImgPath="${CONTEXT}/manage/pic/" eltValue="${tradeDateAgr}"/>
                </td>
				<td align="left" class="tdstyle"> 合同状态 ：</td>
                <td align="left">
				  <select name="status">
                     <option value="" <c:if test="${empty param.status}">selected</c:if>>全部</option>
					 <option value="0" <c:if test="${param.status=='0'}">selected</c:if>>付款提货中</option>
					 <option value="1" <c:if test="${param.status=='1'}">selected</c:if>>提货完毕</option>
				  </select>
				</td>
              </tr>
			  <tr height="35">
			  <td align="left" class="tdstyle"> 合同到期 ：</td>
                <td align="left">
				  <select name="expire">
                     <option value="" <c:if test="${empty param.expire}">selected</c:if>>全部</option>
					 <option value="1" <c:if test="${param.expire==1}">selected</c:if>>到期</option>
					 <option value="2" <c:if test="${param.expire==2}">selected</c:if>>未到期</option>
				  </select>
				</td>
		...
		<table id="tb" border="0" cellspacing="0" cellpadding="0" width="100%" height="300">
	  		<tHead>
			    ...
			    <%if(pd.equals("true")){%>
			    <td class="panel_tHead_MB">数量(<%=DW%>)</td>
				<%}else{%>
				<td class="panel_tHead_MB">数量(<%=DWS%>)</td>
				<%}%>
		...
		<%
		   ...
		   gnnt.vendue.server.dao.TradeDAO to = gnnt.vendue.server.dao.TradeDAOFactory.getDAO();
		   sql=new StringBuffer();
		   sql.append("select tradepartition,b_lastBail,s_lastBail, code,contractid,price,amount,userid,tradedate,");
		   sql.append("section,b_bail,b_poundage,s_bail,s_poundage,status,lastamount,id,str1,str2,tradeunit,trademode,bluserid,");
		   ...
		   sql.append("tradedate,section,b_bail,b_poundage,s_bail,s_poundage,status,lastamount,id,str1,str2,tradeunit,trademode");
		   sql.append(",bluserid,str19 from( select tradepartition,b_lastBail,s_lastBail,code,contractid,price,amount,tradedate,");
		   sql.append("section,b_bail,b_poundage,s_bail,s_poundage,status,lastAmount,id,str1,str2,tradeunit,case when trademode=0");
		   ...
		   sql.append("u1.s_bail,u1.s_poundage,u1.status,u1.lastAmount,u3.id,u4.str1,u4.str2,u3.tradeunit,u3.trademode,");
		   sql.append("u3.userid bluserid,u4.str19 from hisbargain u1,commodity u3,commext u4 where u1.commodityid=");
	       ...
	       while(rs.next()){
			   ...
			   cvo = to.getCommodity(Long.valueOf(commodityID));
			   
          %>
		  <input type="hidden" name="tradeunit"+<%=commodityID%> id=<%=commodityID%> value=<%=cvo.tradeUnit%>>  <!-- xieying -->
					...
					<td class="underLineExt"><%=replaceNull(rs.getString("str1"))%></td>
					...
					<%}else{%>
					<td class="underLineExt" style="text-align:right;">
					<%=disInt(rs.getDouble("amount")*cvo.tradeUnit)%>
					</td>
					<%}%>
					<td class="underLineExt" style="text-align:right;">
					<%=disDouble(rs.getDouble("amount")*rs.getDouble("price")*cvo.tradeUnit)%>
					</td>
					<td class="underLineExt"><%=tradeDate.substring(0,10)%></td>
				    <td class="underLineExt">
					<%int status=rs.getInt("status");%>
					<a href="javascript:controlStatus('<%=commodityID%>','${marketId}')" class="normal">
					<%if(status==0){%>
					付款提货中
					<%}else if(status==1){%>
					提货完毕
					...
					<td class="underLine"><a href="javascript:viewDetail('<%=contractID%>')" class="normal">查看</a></td>
					<td class="underLine"><a href="javascript:liftGoods('<%=commodityID%>','${marketId}')" class="normal">查看</a></td>
		  	...
		  	<%
				...
				sql.append("price,amount,userid,tradedate,section,b_bail,b_poundage,s_bail,s_poundage,status,lastamount,id,str1");
				...
				sql.append("str1,str2,tradeunit,case when trademode=0 then userid else bluserid end userid,case when ");
				...
				sql.append(",u4.str1,u4.str2,u3.tradeunit,u3.trademode,u3.userid bluserid,u4.str19 from hisbargain u1,commodity");
				sql.append(" u3,commext u4 where u1.commodityid=u3.id and u3.id=u4.commid  and  u1.status<>2 ))");
			...
			<jsp:include page="/manage/public/pageTurn1.jsp">
     ...
     <table border="0" cellspacing="0" cellpadding="0" width="100%">
          <tr height="35">
			...
			<%
			  ...
			  userRight=session.getAttribute("USERRIGHT").toString();
			  CheckRight checkRight=new CheckRight();
              checkRight.JNDI=JNDI;
			  if(checkRight.checkRight(Long.parseLong(userRight),RETURNCONTRACT)){	
			%>
			<input type="button" onclick="return deleteRecExt(frm,tb,'ck',7);" class="btn" value="剩余资金转货款">
			<c:if test="${param.partitionID!=ALLPARTITION}">
			&nbsp;&nbsp;
			<input type="button" onclick="returnAllContract();" <%=FindData.judgeClose(JNDI)?"":"disabled"%> class="btn" value="剩余资金转货款(全部)">
			</c:if>
			<%
			  }
			%>
...
<%@ include file="/manage/public/pageTurn2.jsp"%>
<SCRIPT LANGUAGE="JavaScript">
...
function viewDetail(contractID){
	 url="detail.jsp?contractID="+contractID+"";
     PopWindow(url,900,600);
}

//合同状态设置
function controlStatus(commodityID,marketId){
	 ...
	 result=PopWindow("controlStatus.jsp?commodityID="+commodityID+"&marketId="+marketId+"&tradeunit="+tradeunit+"",650,600);
}

//提货记录
function liftGoods(commodityID,marketId){
	 //window.open("liftGoods.jsp?commodityID="+commodityID+"&marketId="+marketId+"","_blank","width=650,height=450,scrollbars=yes");
	 result=PopWindow("liftGoods.jsp?commodityID="+commodityID+"&marketId="+marketId+"",650,450);
}
...
function printMulti(tableList,checkName){
  ...
  for(var i=1;i<len-3;i++){
      ...
      //var obj=new Object();
  	  //obj.printContent=addPrint.value;
      //pagePrint(basePath,obj);
      //addPrint.value="";
	  //var sUrl =basePath+"/manage/public/printHisContract.jsp?commodityIds="+commodityIds+"";
	  //openDialog(sUrl,window,800,600);
	  frm.opt.value="print";
