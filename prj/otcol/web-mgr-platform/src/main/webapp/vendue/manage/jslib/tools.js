...
function exportToExcel( objTbl , rowIndex1 , colIndex , rowIndex)
{
	...
	for( var i = rowIndex ; i < rowCount ; i++ )
	{
		...
		{
			if(objTbl.rows[i].cells.length-1==j)
				{
				  	break;
				}
...
//判断是否为数字浮点型，且判断是否可以为空，是否可以为零,是否必需是小数  xieying
function checkNumber_xie( obj, isNotNull, isNotZero,isBZero, msg )
{
	//var obj = eval( valueName );
	var value = obj.value;
	...
	if(isBZero==true)
	{
		if(floatValue>100)
		{
			warningValue( msg + " 不能大于100%,请重新输入!", obj );
			return false;
		}
	}
...
//判断是否为数字浮点型，且判断是否可以为空，是否可以为零
function checkNumber_cao( obj, isNotNull, isNotZero, isFloat, msg )
{
	var value = obj.value;
	var floatValue = new Number( value );
...
function selectAll1( childName )
{
  ...
  //else
  //	var collCheck = document.all.namedItem(childName);
  var collCheck=document.all(childName);
  if(collCheck)
  {
	  if(!collCheck.length){
	  	collCheck.checked = (objCheck.checked==true)? true:false;
		selectAll1(''+collCheck.d_value+'');
	  }
	  else
	  {
	  	for(var i=0;i < collCheck.length;i++ )
		{
			collCheck[i].checked = (objCheck.checked==true)? true:false;
			selectAll1(''+collCheck[i].d_value+'');
		}
	  }  
  }
  else
...
function modRec(frm_delete,tableList,checkName)
{
	if(isSelNothing(tableList,checkName) == -1)
	{
		alert ( "没有可以操作的数据！" );
...
function printRec(frm_delete,tableList,checkName)
{
	...
	if(userCodeValue==null||userCodeValue==""){
	  ...
	  //	  alert("123");
		  ...
		  result=PopWindow("confirmList.jsp?userId="+userId+"",350,300);
		  //frm.submit();
	  //  }else{
	  //    return false;
	  //  }
...
{ 
  ...
  var popwin=window.showModalDialog(pageUrl,"", "dialogWidth="+WinWidth+"px; dialogHeight="+WinHeight+"px; status=yes;scroll=yes;help=no;");	
  return false; 
}
