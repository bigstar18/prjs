...
<br><br><br>
  ...
  <tr>
    <td colspan="2">
      ...
      <!--计算提货总数量-->
		...
		<%/*
        <db:select var="rowCacu" table="outrecord" columns="place,amount,variety" where="outlogid='${param.outlogId}' and amount>0">*/%>
      <%
	    DBBean bean=null;
		ResultSet rs=null;
		try{
			...
			//System.out.println(sql.toString()); //xieying
        ...
        <c:if test="${i==1}">
          <td style="width:2.5cm" rowspan='<c:if test="${recordTotal>7}">${recordTotal}</c:if><c:if test="${recordTotal<=7}">7</c:if>' class="td_printRd" style="text-align:right;" align="center"><fmt:formatNumber value="${curamount*price*tradeunit}" pattern="${FUNDPATTERN}"/></td>
        </c:if>
	...
	%>
	  ...
	  <tr height="22">
		...
		<td style="width:2cm" align="center" class="td_printMd">&nbsp;</td>
        <td style="width:2cm" rowspan='<c:if test="${recordTotal>7}">${recordTotal}</c:if><c:if test="${recordTotal<=7}">7</c:if>' class="td_printRd" style="text-align:right;" align="center"><fmt:formatNumber value="${curamount*price*tradeunit}" pattern="${FUNDPATTERN}"/></td>
      ...
      <tr height="30">
        ...
        <td align="center" style="font-size:14px;font-family:Verdana;text-align:right;"><fmt:formatNumber value="${curamount*price*tradeunit}" pattern="${FUNDPATTERN}"/></td>
      </tr>
    </table></td>
  </tr>
  <tr>
  ...
  <tr>
	...
	<tr>
	<td style="width:13.5cm;" align="right" class="fontSize">&nbsp;</td>
