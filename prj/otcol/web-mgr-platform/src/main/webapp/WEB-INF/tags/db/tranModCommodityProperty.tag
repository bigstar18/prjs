...
<%@ attribute name="reservedchar5" required="true" %>

<c_db:set var="modParam" value="name='${name}',type=${type},isnull=${isnull},charvartext='${charvartext}',reservedchar1='${reservedchar1}',reservedchar2='${reservedchar2}',reservedchar3='${reservedchar3}',reservedchar4='${reservedchar4}',reservedchar5='${reservedchar5}'"/>
<sql_db:transaction dataSource="${tags_db_dataSource}">

     
    <c_db:choose>
      <c_db:when test="${not empty modParam}">
         <c_db:if test="${not empty reservedchar1}"><c_db:set var="modParam" value="${modParam},reservedchar1='${reservedchar1}'"/></c_db:if>
      </c_db:when>
      <c_db:otherwise>
         <c_db:if test="${not empty reservedchar1}"><c_db:set var="modParam" value="${modParam}reservedchar1='${reservedchar1}'"/></c_db:if>
      </c_db:otherwise>
    ...
    <c_db:choose>
      <c_db:when test="${not empty modParam}">
         <c_db:if test="${not empty reservedchar5}"><c_db:set var="modParam" value="${modParam},reservedchar5='${reservedchar5}'"/></c_db:if>
      </c_db:when>
      <c_db:otherwise>
         <c_db:if test="${not empty reservedchar5}"><c_db:set var="modParam" value="${modParam}reservedchar5='${reservedchar5}'"/></c_db:if>
      </c_db:otherwise>
  ...
  <sql_db:update>
     update v_commodityproperty set ${modParam} where id=${id}
  </sql_db:update>
  </c_db:if> 
</sql_db:transaction>
