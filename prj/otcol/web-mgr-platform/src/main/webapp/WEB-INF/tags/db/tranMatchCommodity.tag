...
<%//更新父商品%>
<%@ attribute name="ids1" required="true" %>
...
<c_db:set var="values1" value="${id},'${code}',to_date('${firsttime}','yyyy-mm-dd'),to_date('${createtime}','yyyy-mm-dd'),${status},${splitid},${category},${beginprice},${stepprice},${amount},${tradeunit},${alertprice},${security},${fee},${minamount},'${userid}',${trademode}"/>
...
<sql_db:transaction dataSource="${tags_db_dataSource}">
  ...
  <sql_db:update>
    update v_commodity set status=4,splitid=${id} where id in ${ids1}
  </sql_db:update>
</sql_db:transaction>
