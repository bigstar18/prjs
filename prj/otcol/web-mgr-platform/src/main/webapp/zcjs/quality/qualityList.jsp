...
<html xmlns:MEBS>
	...
	<body onload="initBody('${returnRefresh}')">
		<form name="frm"
			action="<%=basePath%>qualityController.zcjs?funcflg=list"
			method="post">
					...
					<tr height="35">
						...
						<td align="left">
							<input id="breedName" name="_breedName[like]"
								value="<c:out value='${oldParams["breedName[like]"]}'/>" type=text
								class="text" style="width: 100px" onkeypress="notSpace()">
						</td>
...
<SCRIPT LANGUAGE="JavaScript">
	...
	function dealForbid(){
       frm.action="<%=basePath%>qualityController.zcjs?funcflg=mod&type="+2;
		modRec(frm,tableList,'delCheck');
	}	
	function dealNormal(){
       frm.action="<%=basePath%>qualityController.zcjs?funcflg=mod&type="+1;
		modRec(frm,tableList,'delCheck');
	}
	function modRec(frm,tableList,checkName)
	{
		...
		{
			alert ( "没有可以操作的数据！" );
			frm.action="<%=basePath%>qualityController.zcjs?funcflg=list"
		return false;
		}
