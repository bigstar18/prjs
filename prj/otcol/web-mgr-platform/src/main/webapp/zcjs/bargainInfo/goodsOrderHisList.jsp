...
<html xmlns:MEBS>
  ...
  <body onload="initBody('${returnRefresh}')">
  	<form name="frm" action="<%=basePath%>bargainInfoController.zcjs?funcflg=getH_GOConList" method="post">
		<input type="hidden" name="orderField" value="${orderFiled}">
		<input type="hidden" name="orderDesc" value="${orderType}">
		<input type="hidden" name="pageSize" value="<c:out value="${pageInfo.pageSize}"/>">
		<input type="hidden" id="pageNo" name="pageNo">
	<fieldset width="95%">
			<legend>历史挂单情况查询</legend>
	  ...
	  <%@ include file="goodsOrderHisTable.jsp"%>
...
function checkRate(input)    
	{    
  	...
  	function fmod(goodsOrderId){
  	 openDialog("<%=basePath%>/bargainInfoController.zcjs?funcflg=getH_GoodsOrderParameter&goodsOrderId="+goodsOrderId,"_blank",470,450);
