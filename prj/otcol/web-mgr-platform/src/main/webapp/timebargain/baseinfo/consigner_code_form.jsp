...
{
	if (confirm("您确定要提交吗？")) {
		if (document.forms(0).consignerID != "") {
		 // if(!tmp_baseinfo_up)
   // {
   //   baseinfo_onclick();
   // }
  ...
  //  }
    ...
    for(i=0; i < document.forms(0).lbRight.options.length; i++)
    {
    	
        permission += document.forms(0).lbRight.options[i].value + ",";
    }
    if(permission != "")
    {
      permission =  permission.substr(0, permission.length - 1) ;
    }
    document.forms(0).permission.value = permission;
...
// -->
	...
	<body leftmargin="0" topmargin="0" 
		...
		<table border="0" height="400" width="770" align="center">
			<tr>
				<td>
					<html:form action="/timebargain/baseinfo/consigner.do?funcflg=saveConsignerCode" method="post" styleClass="form">
		    ...
		    <tr><td >
		      <select  name="lbRight" size="16" onDblClick="moveSelected(lbRight,lbLeft);"   style="width : 100%" multiple>
		        <%=(String)request.getAttribute("operateCode")%>
						...
						<html:hidden property="consignerID"/>
