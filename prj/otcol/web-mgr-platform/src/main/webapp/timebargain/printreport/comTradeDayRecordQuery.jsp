...
function query(exportTo)
{
	...
	if(exportTo=="excel")
		window.open("reportContainerExcel.jsp?sign=comTradeDayRecord&startFirmID=" + zStart + "&endFirmID=" + zEnd +"&startClearDate=" + zDate + "&endClearDate=" + zEndDate + "&title=分交易商成交量统计表&brokerId="+brokerId+"&cateGoryId="+cateGoryId);
		else 
	window.showModalDialog("reportContainer.jsp?sign=comTradeDayRecord&startFirmID=" + zStart + "&endFirmID=" + zEnd + "&startClearDate=" + zDate + "&endClearDate=" + zEndDate + "&title=分交易商成交量统计表&brokerId="+brokerId+"&cateGoryId="+cateGoryId,"", "dialogWidth=900px; dialogHeight=600px; status=no;scroll=yes;help=no;resizable=yes");
