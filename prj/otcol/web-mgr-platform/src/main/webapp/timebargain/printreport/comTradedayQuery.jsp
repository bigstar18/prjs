...
function query(exportTo)
{
    ...
    if(exportTo=="excel")
		window.open("reportContainerExcel.jsp?sign=comTradeday&cleardate=" + zDate + "&commodityID=" + zStartCommodity + "&title=当日分商品成交量统计表");
		else 
	window.showModalDialog("reportContainer.jsp?sign=comTradeday" + "&cleardate=" + zDate  + "&commodityID=" + zStartCommodity + "&title=当日分商品成交量统计表","", "dialogWidth=900px; dialogHeight=600px; status=no;scroll=yes;help=no;resizable=yes");
