...
<%@ page import="gnnt.MEBS.finance.service.*"%>
<%
	...
	String sql = "";
	Map map=ClientLedger.queryClientLedgerTotals(cleardate,cleardate,startFirmID,endFirmID,"2",cateGoryId,brokerId);
	if(map!=null)
	{
	  		  ...
	  		  if(valueList!=null&&valueList.size()>0&&filedList!=null&&filedList.size()>0)
	  		  {
                    ...
                    List thFirmList=dao.queryBySQL(sql);   
			         ...
			         floatinglossAll+=Double.parseDouble(floatingloss.toString());
			          ...
			          <fmt:formatNumber value="<%=RuntimeMargin.toString()%>" pattern="#,##0.00"/></td>
			          <td class="td_reportMd1">&nbsp;
			          <fmt:formatNumber value="<%=RuntimeFL.toString()%>" pattern="#,##0.00"/></td>
			          <td class="td_reportMd1">&nbsp;
			          <fmt:formatNumber value="<%=RuntimeSettleMargin.toString()%>" pattern="#,##0.00"/></td>
			          <td class="td_reportMd1">&nbsp;
			          <fmt:formatNumber value="<%=floatingloss.toString()%>" pattern="#,##0.00"/></td>
			          <td class="td_reportMd1">&nbsp;
			          <fmt:formatNumber value="<%=quan+""%>" pattern="#,##0.00"/></td>
			          <td class="td_reportMd1">&nbsp;
			          <fmt:formatNumber value="<%=ClearAssure.toString()%>" pattern="#,##0.00"/></td>
			          <td class="td_reportMd1">&nbsp;
			          <fmt:formatNumber value="<%=ClearFL.toString()%>" pattern="#,##0.00"/></td>
			          <td class="td_reportRd1">&nbsp;
			          <fmt:formatNumber value="<%=ClearSettleMargin.toString()%>" pattern="#,##0.00"/></td>
                      <% 
		  		       %>
	  		             </tr>
		  		         <%
