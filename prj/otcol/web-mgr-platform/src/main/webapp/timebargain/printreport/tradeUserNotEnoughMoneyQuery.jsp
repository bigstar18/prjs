...
function query(exportTo)
{
    ...
    if(exportTo=="excel")
		window.open("reportContainerExcel.jsp?sign=tradeUserNotEnoughMoney&cleardate=" + zDate + "&title=资金不足交易商情况表");
		else
	window.showModalDialog("reportContainer.jsp?sign=tradeUserNotEnoughMoney&cleardate=" + zDate + "&title=资金不足交易商情况表","", "dialogWidth=900px; dialogHeight=650px; status=no;scroll=yes;help=no;resizable=yes");
}
