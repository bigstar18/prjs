...
function query(exportTo)
{
	...
	if(exportTo=="excel")
		window.open("reportContainerExcel.jsp?sign=tAPLStatistic&startFirmID=" + zStart + "&endFirmID=" + zEnd + "&startClearDate=" + zDate + "&endClearDate=" + zEndDate + "&startCommodityID=" + zStartCommodity + "&endCommodityID=" + zEndCommodity + "&title=转让和交收盈亏统计表&breedId="+breedId+"&brokerId="+brokerId+"&cateGoryId="+cateGoryId);
		else
	window.showModalDialog("reportContainer.jsp?sign=tAPLStatistic&startFirmID=" + zStart + "&endFirmID=" + zEnd + "&startClearDate=" + zDate + "&endClearDate=" + zEndDate + "&startCommodityID=" + zStartCommodity + "&endCommodityID=" + zEndCommodity + "&title=转让和交收盈亏统计表&breedId="+breedId+"&brokerId="+brokerId+"&cateGoryId="+cateGoryId,"", "dialogWidth=900px; dialogHeight=700px; status=no;scroll=yes;help=no;resizable=yes");
}
