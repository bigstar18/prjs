	...
	<tr>
	  ...
	  <tr>
        ...
        <td >
			<select name="orderBy" >
				<OPTION value="">请选择</OPTION>
				<OPTION value="1">按交易商</OPTION>
				<OPTION value="2">按成交量</OPTION>
			</select>
			<span class="req">*</span>
	  </tr>
    </table>
...
function query(exportTo)
{
	...
	//alert(cateGoryId);
	if(exportTo=="excel")
		window.open("reportContainerExcel.jsp?sign=tradeMonth&startFirmID=" + zStart + "&endFirmID=" + zEnd + "&startClearDate=" + zDate + "&endClearDate=" + zEndDate + "&orderBy=" + orderBy + "&title=成交量统计表&brokerId="+brokerId+"&cateGoryId="+cateGoryId);
		else
	window.showModalDialog("reportContainer.jsp?sign=tradeMonth&startFirmID=" + zStart + "&endFirmID=" + zEnd + "&startClearDate=" + zDate + "&endClearDate=" + zEndDate + "&orderBy=" + orderBy + "&title=成交量统计表&brokerId="+brokerId+"&cateGoryId="+cateGoryId,"", "dialogWidth=900px; dialogHeight=700px; status=no;scroll=yes;help=no;resizable=yes");
