...
<%
	...
	if(chcekNull(brokerId)){
		if(!brokerId.equals("null")){
			...
			filter1+=" and s.firmid in (select firmId from M_B_FIRMANDBROKER where brokerId ='"+brokerId+"')";
		}
	}
	...
	<td class="td_reportMd1">&nbsp;<fmt:formatNumber value="<%=turnToNum(innerMap.get("sumClose_PL")) %>" pattern="#,##0.00"/></td>
	<td class="td_reportMd1">&nbsp;<%=turnToNum(innerMap.get("sumTranAmount")) %></td>
	<td class="td_reportRd1">&nbsp;<fmt:formatNumber value="<%=turnToNum(innerMap.get("sumSettle_PL")) %>" pattern="#,##0.00"/></td>
	</tr>
	<%
