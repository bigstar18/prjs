...
<%
	...
	if(chcekNull(brokerId)){
		if(!brokerId.equals("null")){
			...
			filter1+=" and t.firmid in (select firmId from M_B_FIRMANDBROKER where brokerId ='"+brokerId+"')";
		}
	}
    ...
    for(int i=0;i<list.size();i++)
    {
		...
		String brokerSql="select brokerId from M_B_FIRMANDBROKER where firmId='"+firmId+"'";
...
<table width="750px" align="center" >
    ...
    <%
       ...
       sql="select '('||(case when f.fieldsign>0 then '+' else '-' end)||')'||f.name name,nvl(t.value,0) value  from (select * from f_clientledger where  firmId='"+firmId+"' "+filter+""+  
           " ) t,f_ledgerfield f "+
           "where f.code=t.code(+) and f.moduleid in (1,2) order by f.moduleid,f.ordernum";
       sql=sql.replaceAll("cleardate","b_date");
    ...
    %>
<table width="750px" height="200px" align="center" border="0" class="reportTemp">
	<tr>
	...
	<%
	  if(clientledgerList!=null&&clientledgerList.size()>0)
	  {
		...
		<td class="td_reportRd1" width="50%"><fmt:formatNumber value="<%=map.get("value").toString()%>" pattern="#,##0.00"/></td>
	     </tr>
	      <%
	    }
	  }
          sql="select nvl(sum(case when f.fieldsign>0 then t.value else -t.value end),0) value  from (select * from f_clientledger where  firmId='"+firmId+"' "+filter+""+  
           " ) t,f_ledgerfield f "+
           "where f.code=t.code(+) and f.moduleid not in (1,2) order by f.moduleid,f.ordernum";
          sql=sql.replaceAll("cleardate","b_date");
	...
	<tr>
		...
		<td class="td_reportRd1" width="50%"><fmt:formatNumber value="<%=runtimeRight+""%>" pattern="#,##0.00"/></td>
	</tr>
	<tr>
	...
	<tr>
		...
		%>
		<td class="td_reportRd1" width="50%"><fmt:formatNumber value="<%=superAdd+""%>" pattern="#,##0.00"/></td>
	</tr>
	<tr>
...
%>
<table width="750px"  align="center" border="0" class="reportTemp">
	<tr>
...
<br><center class="reportHead">成交记录表</center><br>
	...
	<td class="td_reportMd1">&nbsp;<fmt:formatNumber value="<%=turnToNum(innerMapBargainResult.get("price")) %>" pattern="#,##0.00"/></td>
	...
	<td class="td_reportRd1">&nbsp;<fmt:formatNumber value="<%=turnToNum(innerMapBargainResult.get("tradefee")) %>" pattern="#,##0.00"/></td>
	</tr>
	<%
...
<br><center class="reportHead">转让盈亏明细表</center><br>
	...
	<td class="td_reportMd1">&nbsp;<fmt:formatNumber value="<%=turnToNum(innerMap.get("holdprice")) %>" pattern="#,##0.00"/></td>
...
<br>
	...
	<td class="td_reportMd1">&nbsp;<fmt:formatNumber value="<%=turnToNum(innerMapIndentCollect.get("evenprice")) %>" pattern="#,##0.00"/></td>
	<td class="td_reportMd1">&nbsp;<fmt:formatNumber value="<%=turnToNum(innerMapIndentCollect.get("marketClearPrice")) %>" pattern="#,##0.00"/></td>
	<td class="td_reportMd1">&nbsp;<%=turnToNum(innerMapIndentCollect.get("HoldQty")) %></td>
	<td class="td_reportMd1">&nbsp;<fmt:formatNumber value="<%=turnToNum(innerMapIndentCollect.get("HoldMargin")) %>" pattern="#,##0.00"/></td>
	<td class="td_reportMd1">&nbsp;<fmt:formatNumber value="<%=turnToNum(innerMapIndentCollect.get("FloatingLoss")) %>" pattern="#,##0.00"/></td>
