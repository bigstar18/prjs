...
function query(exportTo)
{
	...
	if(exportTo=="excel")
		window.open("reportContainerExcel.jsp?sign=bargainResult&startFirmID=" + zStart + "&endFirmID=" + zEnd +"&cleardate="+ zDate + "&title=成交记录表&brokerId="+brokerId+"&cateGoryId="+cateGoryId);
		else 
	window.showModalDialog("reportContainer.jsp?sign=bargainResult&startFirmID=" + zStart + "&endFirmID=" + zEnd + "&cleardate=" + zDate + "&title=成交记录表&brokerId="+brokerId+"&cateGoryId="+cateGoryId,"", "dialogWidth=900px; dialogHeight=600px; status=no;scroll=yes;help=no;resizable=yes");
