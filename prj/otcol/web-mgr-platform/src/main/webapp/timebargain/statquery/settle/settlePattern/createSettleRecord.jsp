...
<head>
	<title>录入</title>
<%	//首次访问显示商品代码
		List cmdList=SettleManagePattern.getCommoditysNew();	
    ...
    if( opt!= null && "create".equals(opt))
    {
		...
		int returnResult = SettleManagePattern.createSettle(CommodityID,FirmID_B,FirmID_S,regsto,Quantity,result);
		if(returnResult==1){
		%>
	...
	function selectObj(v)
	{
		...
		else if(v == "Quantity")
		{
			...
			//填写数量后验证(第二次使用AJAX)
				...
				var objValue5 = frm.handleResult.value;
				var objValue6 = frm.regsto.value;
				...
				if(resultStutas){
					...
					filter = filter+"&FirmID_B="+objValue2+"&FirmID_S="+objValue3+"&Quantity="+objValue4+"&regsto="+objValue6+"&Result="+objValue5;
