...
<head>
<%	//首次访问显示商品代码
		List cmdList=SettleManageDelay.getCommoditysNew();
    ...
    if( opt!= null && "create".equals(opt))
    {
		...
		int returnResult = SettleManageDelay.createSettle(CommodityID,beginDate,FirmID_B,FirmID_S,Quantity,result);
		if(returnResult==1){
		%>
		...
		<%
		  ...
		  long checkResult=SettleManageDelay.checkInsertSettle(CommodityID,beginDate,FirmID_B,FirmID_S,Quantity); 
		%>
		<script type="text/javascript">
	...
	<fieldset width="100%">
		...
		<span>
			<table class="common" border="0" cellspacing="0" cellpadding="0" width="100%" align="center">
				...
				<!-- amount -->
				<tr class="common">
					<td align="right" width="50%" height="30">交收数量：<input type="text" name="Quantity" onkeyup="this.value=this.value.replace(/\D/g,'')">&nbsp;&nbsp;</td>
		...
		if(CommodityID!=""&&FirmID_B!=""&&FirmID_S!=""&&Quantity!=""&&result!=""){
		     ...
		     if(Quantity.search("^[1-9]*[1-9][0-9]*$")!=0 || Quantity  <= 0)
             {
			    ...
			    if(confirm("要提交的数据为：\n商品："+CommodityID+
								...
								"\n交收结果："+resultMSG+"，交收数量："+Quantity+"，交收日期："+beginDate+
								"\n确认要提交么？"))
			    {
	...
	function selectObj(v)
	{
		...
		else if(v == "Quantity")
		{
			...
			//填写数量后验证(第二次使用AJAX)
				...
				if(resultStutas){
					filter = filter+"&FirmID_B="+objValue2+"&FirmID_S="+objValue3+"&Quantity="+objValue4+"&settleDate="+objValue6;
