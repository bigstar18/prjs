...
<%
	...
	if(optValue != null){		
		...
		if("handleOK".equals(optValue)){//处理完成
			finishValue= SettleManage.finshSettle(matchID);
		}
		if("blankOut".equals(optValue)){//作废
			finishValue= SettleManage.cancelSettle(matchID);
		}
		if(finishValue==1){
		%>
	...
	// 获取可用资金
	String firmId = (String)matchMsg.get("FirmID_B");
	Map buyBalance = SettleManage.getFirmFunds(firmId);
...
<body>
	<form name="frm" method="post">
		...
		if(matchMsg!=null)
		{
			...
			boolean sign = (Status>1 || Status == -2);//处理完成、作废
		...
		<fieldset>
			...
			<span>
				...
				<tr class="common">
					<td align="right"><b>可用资金：</b></td><td align="left"><fmt:formatNumber value="<%=buyBalance.get("buyBalance") %>" pattern="#,##0.00"/></td>
				</tr>
				<tr class="common">
