...
<%
	...
	String bailToPayment = (String)request.getParameter("bailToPayment");
	if(bailToPayment== null){
		bailToPayment="0";
	}
	double bailToPaymentMoney = Double.parseDouble(bailToPayment);
	if(optValue!= null && "bailToPayment".equals(optValue))
	{
			...
			<table class="common" align="center" width="100%" height="100%" border="0">
				...
				<tr class="common">	
					...
					<td align="left" width="50%">
						<input type="text" name="bailToPayment" style="width: 100px">
						<font color="red">*</font>
					</td>
				</tr>
...
<script type="text/javascript">
	...
	{
		var bailToPayment = frm.bailToPayment.value;
		if(bailToPayment.search("^-?\\d+(\\.\\d+)?$")!=0 || parseFloat(bailToPayment) == parseFloat(0)){
	        alert("请输入一个非0数字!");
	        frm.bailToPayment.value="";
	        frm.bailToPayment.focus();
	        return false;
	     }else{
	     	var submark = false;
	     	if(confirm("确认提交的保证金转货款金额为："+bailToPayment+"？")){
	     		frm.subbutton.disabled = true;
	     		frm.opt.value="bailToPayment";
	     		submark = true;
	     	}
	     	if(submark){
	     		frm.submit();
