           var dispalyContent = new Array('<font color=yellow>●</font>','<font color=green>●</font>');
			...
			function refreshTable()
			{
				...
				//每页大小
				var pageSize = doc.getElementsByTagName("pageSize")[0].firstChild.data;
				
				var prePage = parseInt(pageIndex) - 1;
				var nextPage = parseInt(pageIndex) + 1;
				//起始索引
				var startIndex = (parseInt(pageIndex) - 1)*pageSize;
				...
				//add by yangpei 以前这里是在内存中取出所有数据，然后分页。现在是在内存中直接分页，然后将分页数据取出，这里直接显示就行
						...
						for(var i=1;i<colsCount;i++)
						{
							...
							if(colAbbr!=null && colAbbr.length>0)
							{
								...
								if(node!=null && node.hasChildNodes()){
									var tempVal = node.firstChild.data;
									if(colAbbr == "ORDERTYPE"&&tempVal == "1") str[i] = "订立";
									else if(colAbbr == "BS_FLAG"&&tempVal == "1") str[i] = "买";
									else if(colAbbr == "BS_FLAG"&&tempVal == "2") str[i] = "卖";
									else if(colAbbr == "ORDERTYPE"&&tempVal == "2") str[i] = "转让";
									else if(colAbbr == "ORDERTYPE"&&tempVal == "4") str[i] = "撤单";
									else if(colAbbr == "STATUS"&&tempVal == "1") str[i] = "已委托";
									else if(colAbbr == "STATUS"&&tempVal == "2") str[i] = "部分成交";
									else if(colAbbr == "STATUS"&&tempVal == "3") str[i] = "全部成交";
									else if(colAbbr == "STATUS"&&tempVal == "4") str[i] = "正在撤单";
									else if(colAbbr == "STATUS"&&tempVal == "5") str[i] = "全部撤单";
									else if(colAbbr == "STATUS"&&tempVal == "6") str[i] = "部分成交后撤单";
									else if(colAbbr == "STATUS"&&tempVal == "7") str[i] = "撤单委托成功";
									else if(colAbbr == "STATUS"&&tempVal == "8") str[i] = "撤单委托失败";
									else str[i] = tempVal; 
								}
								else str[i]='--';
							}
