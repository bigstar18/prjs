			...
			<c:choose>
				...
				<c:when test="${settleMatch.status<2 }">
					...
					<c:when test="${settleMatch.result==1 }">
						...
						<c:if test="${settleMatch.isChangeOwner==0 }">
...
<script type="text/javascript">
	...
	function event(v)
	{
		...
		switch(v)
		{
			...
			case 8:
				...
				break;
			case 9:
				window.location.href="<%=basePath%>servlet/settleMatchController.${POSTFIX}?funcflg=settleMatchReturn&moduleId=3";
				break;
			case 10:
	...
	function handleOK(v)
	{
		...
		if(subbtn7){
			frm.action="<%=basePath%>servlet/settleMatchController.${POSTFIX}?funcflg=settleHandleOK&matchId="+mid+"&dd="+Date();
			frm.submit();
		}
	}
	...
	function auto()
	{
		...
		if(subbtn18){
			frm.action="<%=basePath%>servlet/settleMatchController.${POSTFIX}?funcflg=autoSettleMatch&matchId="+mid+"&dd="+Date();
			frm.submit();
		}
	}
