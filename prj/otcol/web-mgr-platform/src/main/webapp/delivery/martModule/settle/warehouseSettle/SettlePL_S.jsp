	...
	<form name="frm" method="post">
		...
		<fieldset>
			...
			<table class="common" align="center" width="100%" height="100%" border="0">
				...
				<tr class="common">
					<td align="right">卖方基准货款：</td><td align="left">
					<fmt:formatNumber value="${settleMatch.sellIncome_Ref }" pattern="#,##0.00"/></td>
				</tr>
				<tr class="common">
					<td align="right">卖方加升贴水货款:</td><td align="left">
					<fmt:formatNumber value="${settleMatch.sellIncome_Ref+settleMatch.HL_Amount }" pattern="#,##0.00"/></td>
				</tr>
				<tr class="common">
					<td align="right">已付卖方货款：</td><td align="left">
					<fmt:formatNumber value="${settleMatch.sellIncome }" pattern="#,##0.00"/></td>
				</tr>
				<tr class="common">	
...
<script type="text/javascript">
	...
	{
		var SettlePL_S = frm.thisPayMent.value;
		if(SettlePL_S.search("^-?\\d+(\\.\\d+)?$")!=0 || parseFloat(SettlePL_S) == parseFloat(0)){
	        ...
	        return false;
	     	...
	     	var submark = false;
	     	if(confirm("确定卖方交收盈亏为："+SettlePL_S+"？")){
	     		frm.subbtn.disabled=true;
	     		frm.opt.value="SettlePL_S";
	     		submark = true;
	     	}
	     	if(submark){
				frm.action="<%=basePath %>servlet/settleMatchController.${POSTFIX}?funcflg=settleSettlePL_S&dd="+Date();
	     		frm.submit();
	     		}
	     }
	}
