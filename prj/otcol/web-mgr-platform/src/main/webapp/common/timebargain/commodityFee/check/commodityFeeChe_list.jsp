...
<script type="text/javascript">
	...
	function check_commodityFee(id){
		pTop("<%=basePath%>/timebargain/commodityFee/check/commodityFeeChe_U.jsp?id=" + id,600,550);
	}
	
	function search(){
		document.location.href = "<%=basePath%>/timebargain/commodityFee/check/commodityFeeChe.jsp";
	}
</script>
...
<table width="100%">
  <tr><td>
	<ec:table items="commodityFeeList" var="commodityFee" 
			action="${basePath}/timebargain/commodityFeeCheList.spr"	
	...
	>
      ...
      <ec:column property="feeAlgr" title="交易手续费算法" editTemplate="ecs_t_feeAlgr" mappingItem="FEEALGR" width="120" style="text-align:center;"/>
      <ec:column property="settleFeeAlgr" title="交收手续费算法" editTemplate="ecs_t_settleFeeAlgr" mappingItem="SETTLEFEEALGR" width="120" style="text-align:center;"/>
			<ec:column property="lowestSettleFee" title="最低交收手续费金额" width="140"  style="text-align:right;"/>
			...
			<c:choose>
					<c:when test="${commodityFee.status == 1}">
						<a href="#" onclick="check_commodityFee('<c:out value="${commodityFee.id}"/>')"><img height="20" title="审核与查看" src="<%=basePath%>/timebargain/images/weishen.gif"/></a>
					</c:when>
					<c:otherwise>
						<a href="#" onclick="check_commodityFee('<c:out value="${commodityFee.id}"/>')"><img height="20" title="审核与查看" src="<%=basePath%>/timebargain/images/yishen.gif"/></a>
					</c:otherwise>
			</c:choose>
			</ec:column>
		</ec:row>
