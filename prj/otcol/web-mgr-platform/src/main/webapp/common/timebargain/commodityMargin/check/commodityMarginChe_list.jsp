      ...
      <ec:column property="marginPriceType" title="保证金计算依据" editTemplate="ecs_t_marginPriceType" mappingItem="MARGINPRICETYPE" width="120" style="text-align:center;"/>
			...
			<c:choose>
					<c:when test="${commodityMargin.status == 1}">
						<a href="#" onclick="check_commodityMargin('<c:out value="${commodityMargin.id}"/>')"><img height="20" title="审核与查看" src="<%=basePath%>/timebargain/images/weishen.gif"/></a>
					</c:when>
					<c:otherwise>
						<a href="#" onclick="check_commodityMargin('<c:out value="${commodityMargin.id}"/>')"><img height="20" title="审核与查看" src="<%=basePath%>/timebargain/images/yishen.gif"/></a>
					</c:otherwise>
			</c:choose>
			</ec:column>
		</ec:row>
