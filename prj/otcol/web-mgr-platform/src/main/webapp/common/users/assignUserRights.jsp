			...
			<table border="0" cellspacing="0" cellpadding="0" width="100%">			  
				...
				<c:choose>
					...
					<c:when test="${roleDispatchMark }">
						<input type="checkbox"   id="main" name="ck" checked="checked" onclick="selectCks('${treeRight.id }');setParents('${treeRight.right.id }');" value="${treeRight.id }" disabled="disabled">
						<span style="font-weight: bold;cursor:hand;text-decoration: underline;" onclick="clkDiv('${treeRight.id }')">${treeRight.name }</span>
					</c:when>
					<c:otherwise>
...
function frmChk()
{
	...
	if(sign)
	{
		frm.action="<%=commonRightControllerPath %>commonRightSaveUserRights"
		frm.submit();
	}
}
