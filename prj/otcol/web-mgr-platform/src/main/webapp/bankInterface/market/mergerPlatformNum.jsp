...
{
	filter += " and fbfs.bankCode='" + bankCode + "'";
}
...
<html xmlns:MEBS>
  <head>
    ...
    <title>选择要合并到的平台号</title>
  </head>
  
  <body>
  <fieldset width="95%">
			<legend>请选择要合并到的平台号</legend>
					...
					<td align="left">
						<select name="bankCode" class="normal" onchange="changeBank();" style="width:120px">
							...
							for(int i=0;i<bankList.size();i++)
							{
								...
								%>
								<option value="<%=bankCodeMap.get(bv.bankID)%>" <%=bankCode.equals(bankCodeMap.get(bv.bankID))?"selected":""%>><%=bv.bankName%></option>		
								<%
								}
							}
							%>
		...
		<tBody>
			...
			for(int i=0;i<obj.getCurNum();i++)
			{
				...
				<tr height="22" align=center onclick="selectTr();">
					...
					<td class="underLine" align="left"><input name="ck" type="radio" value='<%=money.firmID%>'&nbsp;</td>
...
function doQuery()
{
	frm.action = "mergerPlatformNum.jsp";
	frm.pageIndex.value = 1;
	frm.submit();
}
