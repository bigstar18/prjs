...
<%
	...
	{//对签解约信息
		...
		if(!noAdapterBank(bankID)){
			firmbank = dao.getFirmBank(bankID,Tool.strToDate(s_time));
		}
		if(firmbank==null)
		{
			result = "匹配签约信息异常，银行代码："+bankID+"，对账时间："+s_time+"时间："+new java.util.Date();
		}
		else if(firmbank.size() == 0)
		{
			result = "签约信息匹配，银行代码："+bankID+"，对账时间："+s_time+"时间："+new java.util.Date();
		}
		else 
		{
			result = "银行信息不匹配："+bankID+"，对账时间："+s_time+"时间："+new java.util.Date();
			obj = ObjSet.getInstance(firmbank, pageSize, pageIndex);
		}
...
int maxpage = 0;
if(firmbank!=null){
	maxpage = firmbank.size()%pageSize==0 ? firmbank.size()/pageSize : firmbank.size()/pageSize+1;
}
  	...
  	<form id="frm" method="post">
		...
		<tBody>
  		<%
			...
			{
				FirmOpenCloseBank ocb = (FirmOpenCloseBank)obj.get(i);
				%>
