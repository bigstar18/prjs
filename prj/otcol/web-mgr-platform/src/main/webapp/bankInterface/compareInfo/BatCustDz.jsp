...
<%
	...
	{//查询银行对账不平信息
		...
		if(!noAdapterBank(bankID)){
			CustDzFail = dao.getBatCustDz(Tool.strToDate(s_time),bankID);
		}
		if(CustDzFail==null)
		{
			result = "查询银行对账不平信息异常，银行代码："+bankID+"，对账时间："+s_time+"时间："+new java.util.Date();
		}
		else if(CustDzFail.size() == 0)
		{
		...
		else 
		{
			...
			obj = ObjSet.getInstance(CustDzFail, pageSize, pageIndex);
		}
...
int maxpage = 0;
if(CustDzFail!=null){
	maxpage = CustDzFail.size()%pageSize==0 ? CustDzFail.size()/pageSize : CustDzFail.size()/pageSize+1;
}
...
<html xmlns:MEBS>
  <head>
    ...
    <title>银行对账失败信息列表</title>
  </head>
  
  <body>
  	<form id="frm" method="post">
		<fieldset width="100%">
			<legend>银行对账失败信息</legend>
		...
		<tBody>
  		<%
			...
			{
				BatCustDzFailChild ocb = (BatCustDzFailChild)obj.get(i);
				%>
