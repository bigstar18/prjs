	...
	if(isOK != null && isOK.equals("yes"))
	{
		...
		if(id != null && id.length > 0){
			for(int i=0;i<id.length;i++){
				...
				req.setBankID("");
				req.setMethodName("moneyAduitToBank");
				...
				if(result!=0) {
					failureIds+=""+id[i]+";";
					lv.setLogcontent("流水："+Long.parseLong(id[i])+"单边帐处理通过失败，时间："+Tool.fmtTime(new java.util.Date()));
				}else{
					lv.setLogcontent("流水："+Long.parseLong(id[i])+"单边帐处理通过成功，时间："+Tool.fmtTime(new java.util.Date()));
				}
				dao.log(lv);
			}
	...
	else if(isOK != null && isOK.equals("no"))
	{
		...
		if(id != null && id.length > 0){
			for(int i=0;i<id.length;i++){
				...
				if(result!=0) {
					failureIds+=""+id[i]+";";
					lv.setLogcontent("流水："+Long.parseLong(id[i])+"单边帐处理拒绝失败，时间："+Tool.fmtTime(new java.util.Date()));
				}else{
					lv.setLogcontent("流水："+Long.parseLong(id[i])+"单边帐处理拒绝成功，时间："+Tool.fmtTime(new java.util.Date()));
				}
				dao.log(lv);
			}
			if(!"".equals(failureIds))
