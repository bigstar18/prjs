...
function openDialog(url, args, width, height) {
    ...
    var returnVal = window.showModalDialog(url, args, "dialogWidth=" + width + "px; dialogHeight=" + height + "px; status=no;scroll=yes;help=no;");
...
function hideContent( eltID )
{
	...
	else
	{
		...
		function changeOrder(sign)
		{
		  if(sign)
		  {
		   var orderField=frm_query.orderField.value;
		   var orderDesc=frm_query.orderDesc.value;
		...
		function dtquery()   
       {
		 ...
		 if(orderField==o.abbr)
         {
           if(orderDesc=="true")
          {
               frm_query.orderDesc.value="false";
          }
           else 
          {
              frm_query.orderDesc.value="true"; 
          }
         }
         else
         {
             frm_query.orderDesc.value="false";
             frm_query.orderField.value=o.abbr;
         }
