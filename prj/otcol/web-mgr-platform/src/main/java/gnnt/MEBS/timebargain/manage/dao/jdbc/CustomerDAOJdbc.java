...
public class CustomerDAOJdbc extends BaseDAOJdbc implements CustomerDAO {
	...
	public List getCustomerCounts(String paramString) {
		StringBuffer localStringBuffer = new StringBuffer();
		localStringBuffer.append(
				"select a.firmID,a.firmname,b.counts customerCounts from T_firm a, (select T_customer.firmid, count(T_customer.customerid) counts from T_customer group by T_customer.firmid) b where a.firmid = b.firmid(+)");
		ArrayList localArrayList = new ArrayList();
	...
	public List customerQuery(Customer paramCustomer) {
		StringBuffer localStringBuffer = new StringBuffer();
		localStringBuffer.append("select a.*,case when a.Status = 0 then '正常' ").append("when a.Status = 1 then '禁止交易' ")
				.append("when a.Status = 2 then '退市' ").append("else '' end as Status,a.ModifyTime from T_Firm a ");
		this.log.debug("sql: " + localStringBuffer.toString());
		return getJdbcTemplate().queryForList(localStringBuffer.toString());
	}
	...
	public void addCustomerBalance(String paramString, Double paramDouble) {
		...
		if ((paramDouble != null) && (paramDouble.doubleValue() > 0.0D)) {
			...
			localStringBuffer.append("insert into T_dailymoney ")
					.append("(id, createtime, customerid, operation, a_tradeno, money, balance, frozenfunds, virtualfunds, runtimemargin, runtimefl) ")
					.append("select ").append("seq_dailymoney_id.nextval, sysdate, customerid, '101', null,").append(paramDouble)
					.append(", balance, frozenfunds, virtualfunds, runtimemargin, runtimefl ").append("from T_firmfunds where firmid = ? ");
		...
		if ((paramDouble != null) && (paramDouble.doubleValue() < 0.0D)) {
			...
			localStringBuffer.append("insert into T_dailymoney ")
					.append("(id, createtime, customerid, operation, a_tradeno, money, balance, frozenfunds, virtualfunds, runtimemargin, runtimefl) ")
					.append("select ").append("seq_dailymoney_id.nextval, sysdate, customerid, '102', null,").append(paramDouble)
					.append(", balance, frozenfunds, virtualfunds, runtimemargin, runtimefl ").append("from T_firmfunds where firmid = ? ");
			arrayOfObject = new Object[] { paramString };
			this.log.debug("sql: " + localStringBuffer.toString());
			this.log.debug("params[0]: " + arrayOfObject[0]);
	...
	public void updateStatusFirm(Customer paramCustomer) {
		if (paramCustomer.getStatus() != null) {
			String str = paramCustomer.getStatus().toString();
			if ("2".equals(str)) {
				String localObject = "select count(*) from T_FirmHoldSum where FirmID = " + paramCustomer.getFirmID();
	...
	public void insertNewApp(Customer paramCustomer) {
		...
		Object[] arrayOfObject = { paramCustomer.getFirmID(), paramCustomer.getVirtualFunds(), paramCustomer.getCreator(),
				paramCustomer.getRemark1() };
		this.log.debug("sql: " + str2);
		for (int i = 0; i < arrayOfObject.length; i++) {
			this.log.debug("params[" + i + "]: " + arrayOfObject[i]);
		}
	...
	public List getFirmPrivilege(Customer paramCustomer) {
		StringBuffer localStringBuffer = new StringBuffer();
		localStringBuffer.append("select t.id,t.type,t.typeid,t.kind,t.privilegecode_b,t.privilegecode_s,")
				.append("case when t.kind = 1 then c.breedName else t.kindid end kindid")
				.append(" from T_A_TRADEPRIVILEGE t,(select a.kindid,b.breedname breedName from T_A_TRADEPRIVILEGE a, T_A_breed b where a.kindid = to_char(b.breedid) and a.Type = 1 and a.TypeID = ?) c")
				.append(" where t.Type = 1 and t.TypeID = ? and t.kindid = c.kindid(+) order by t.id asc");
	...
	public void insertFirmPrivilege(Customer paramCustomer) {
		...
		Object[] arrayOfObject2 = { paramCustomer.getTypeID(), paramCustomer.getKindID(), paramCustomer.getPrivilegeCode_B(),
				paramCustomer.getPrivilegeCode_S(), paramCustomer.getKind() };
		this.log.debug("sql: " + str2);
	...
	public List getKHCustomerPrivilege(Customer paramCustomer) {
		...
		localStringBuffer.append("select t.id,t.type,t.typeid,t.kind,t.privilegecode_b,t.privilegecode_s,")
				.append("case when t.kind = 1 then c.breedName else t.kindid end kindid")
				.append(" from T_A_TRADEPRIVILEGE t,(select a.kindid,b.breedname breedName from T_A_TRADEPRIVILEGE a, T_A_breed b where a.kindid = to_char(b.breedid) and a.Type = 2 and a.TypeID = ?) c")
				.append(" where t.Type = 2 and t.TypeID = ? and t.kindid = c.kindid(+)");
	...
	public void exitMarketCheck(String paramString) {
		...
		if (((Double) getJdbcTemplate().queryForObject("select Balance from T_CustomerFunds where CustomerID=?", arrayOfObject, Double.class))
				.doubleValue() != 0.0D) {
	...
	private static String makeSql_tradeprivilege_delete(int paramInt1, String paramString1, int paramInt2, String paramString2) {
		String str1 = "";
		if (paramInt1 == 1) {
			String str2 = SqlUtil.makeFilterCondition("brokerid", paramString1, false);
	...
	private static String makeSql_tradeprivilege_insert(int paramInt1, String paramString1, int paramInt2, String paramString2, int paramInt3,
			int paramInt4) {
		String str1 = "";
		if (paramInt1 == 1) {
			String str2 = SqlUtil.makeFilterCondition("brokerid", paramString1, false);
	...
	class FirmPrivilegeMapRowMapper implements RowMapper {
		...
		private Customer rsToFirmPrivilegeMap(ResultSet paramResultSet) throws SQLException {
			Customer localCustomer = new Customer();
	...
	class KHCustomerRowMapper implements RowMapper {
		KHCustomerRowMapper() {
		}
