  ...
  extends SubmitAction
{
  ...
  public BargainVO[] getBargainList(String paramString)
    throws SQLException
  {
    BargainVO[] arrayOfBargainVO = this.TRADEDAO.getBargainList("where submitID in (select id from v_curSubmit where traderId='" + paramString + "') and tradepartition=" + this.PARTITIONID + " order by contractID desc");
    return arrayOfBargainVO;
  }
  
  public BargainVO getBargain(long paramLong)
    throws SQLException
  {
          ...
          if ((localTradeStatusValue.getLastPartID() == arrayOfCommodityVO[i].section) && (arrayOfCommodityVO[i].bargainFlag == 0))
          {
            ...
            if (arrayOfCommodityVO[i].countDownTime > 0) {
              if (arrayOfCommodityVO[i].lastTime != null) {
                arrayOfCommodityVO[i].countDownTime = ((int)((arrayOfCommodityVO[i].lastTime.getTime() + arrayOfCommodityVO[i].countDownTime * 1000 - l) / 1000L));
              } else {
                arrayOfCommodityVO[i].countDownTime = ((int)((localTradeStatusValue.getPartEndTime().getTime() - localTradeStatusValue.getCountdownTime() * 1000 + arrayOfCommodityVO[i].countDownTime * 1000 - l) / 1000L));
              }
            }
  ...
  public String getLastXML(String paramString1, String paramString2)
    throws SQLException
  {
    String str1 = "";
    String str2 = "";
    ...
    CurSubmitVO[] arrayOfCurSubmitVO = this.TRADEDAO.getCurSubmitList("where To_date('" + paramString2 + "','yyyy-MM-dd/HH24:mi:ss')<modifytime and traderid='" + paramString1 + "' and tradepartition=" + this.PARTITIONID + " order by modifytime desc");
  ...
  public BargainVO[] getMarketBargain()
    throws SQLException
  {
  ...
  public CurSubmitVO[] getSubmit(String paramString)
    throws SQLException
  {
    CurSubmitVO[] arrayOfCurSubmitVO = this.TRADEDAO.getCurSubmitList("where traderID='" + paramString + "' and tradepartition=" + this.PARTITIONID + " order by modifytime desc");
  ...
  public int chgTraderPwd(String paramString1, String paramString2, String paramString3)
    throws SQLException
  {
    int i = -1;
    ...
    throws Exception, SQLException, ClassNotFoundException, IllegalAccessException, NamingException, InstantiationException
  {
    ...
    {
      if ((paramDouble > localCommodityVO.alertPrice) || (paramDouble < localCommodityVO.beginPrice) || (paramDouble <= localCommodityVO.lastPrice) || (localBigDecimal1.subtract(localBigDecimal2).remainder(localBigDecimal3).doubleValue() > 0.0D)) {
        return 1;
      }
    }
    else if ((paramDouble < localCommodityVO.alertPrice) || (paramDouble > localCommodityVO.beginPrice) || ((paramDouble >= localCommodityVO.lastPrice) && (localCommodityVO.lastPrice > 0.0D)) || (localBigDecimal1.subtract(localBigDecimal2).remainder(localBigDecimal3).doubleValue() > 0.0D)) {
      return 1;
    }
    if (paramLong <= 0L) {
      return 7;
    ...
    synchronized (localCommodityVO)
    {
      ...
      try
      {
        ...
        else if ((localTradeUserVO1 == null) || ((paramString1.equals(localCommodityVO.lastUserID)) && (paramString2.equals(localCommodityVO.lastTraderID))))
        {
          m = 4;
        }
        else if (k == 0)
        ...
        else
        {
          ...
          try
          {
            synchronized (this.TRADELOCK)
            {
              ...
              d1 = localKernelEngine.getBail(paramString3, paramDouble, paramLong, paramString1, localConnection) + localKernelEngine.getFee(paramString3, paramDouble, paramLong, paramString1, localConnection);
              if (localTradeUserVO3 != null) {
                d2 = localKernelEngine.getBail(paramString3, localCommodityVO.lastPrice, paramLong, localCommodityVO.lastUserID, localConnection) + localKernelEngine.getFee(paramString3, localCommodityVO.lastPrice, paramLong, localCommodityVO.lastUserID, localConnection);
              }
              ...
              else
              {
                ...
                this.TRADEDAO.delTradeQuotation("where tradePartition=" + this.PARTITIONID + " and code='" + paramString3 + "'", localConnection);
  ...
  public Vector getSubmit_v(String paramString)
    throws SQLException
  {
    return null;
  }
