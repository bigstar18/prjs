  ...
  extends DaoHelperImpl
{
  ...
  public Firm getFirmLock(String id)
  {
    ...
    String sql = "select * from m_firm where firmId='" + id + 
      "' for update";
    List<Firm> list = queryBySQL(sql, null, null, new CommonRowMapper(
      new Firm()));
  ...
  public void createFirm(Firm firm)
  {
    ...
    Object[] params = { firm.getFirmId(), firm.getName(), 
      firm.getFirmCategoryId(), firm.getFullname(), firm.getBank(), 
      firm.getBankAccount(), firm.getAddress(), firm.getContactMan(), 
      firm.getPhone(), firm.getFax(), firm.getPostCode(), 
      firm.getEmail(), firm.getNote(), firm.getZoneCode(), 
      firm.getIndustryCode(), Integer.valueOf(firm.getType()), 
      firm.getExtendData(), firm.getTariffID() };
  ...
  public void log(Map map)
  {
    ...
    Object[] params = {
      (String)map.get("logopr"), (String)map.get("logcontent"), (String)map.get("logIp") };
  ...
  public List getTradeModules(QueryConditions conditions)
  {
    ...
    return queryBySQL(sql, params, null, new CommonRowMapper(
      new TradeModule()));
  }
  
  public List<FirmModule> getFirmModuleByFirmId(String firmId)
  {
    String sql = "select * from m_firmmodule t where t.enabled = 'Y' and t.firmid = '" + 
      firmId + "'";
    return queryBySQL(sql, null, null, 
      new CommonRowMapper(new FirmModule()));
  }
  ...
  public void addFirmModule(FirmModule module)
  {
    ...
    Object[] params = { module.getModuleId(), 
      module.getFirmId(), module.getEnabled() };
  ...
  public void addFirmLog(FirmLog firmLog)
  {
    ...
    Object[] params = { firmLog.getUserId(), 
      firmLog.getFirmId(), firmLog.getAction() };
  ...
  public int updateFirm(Firm firm)
  {
    ...
    sql = sql + 
      ",zoneCode=?,industryCode=?,type=?,EXTENDDATA=?,tariffID=? where firmId=?";
    Object[] params = { firm.getName(), firm.getFullname(), firm.getBank(), 
      firm.getBankAccount(), firm.getAddress(), firm.getContactMan(), 
      firm.getPhone(), firm.getFax(), firm.getPostCode(), 
      firm.getEmail(), firm.getNote(), firm.getFirmCategoryId(), 
      firm.getZoneCode(), firm.getIndustryCode(), 
      Integer.valueOf(firm.getType()), firm.getExtendData(), 
      firm.getTariffID(), firm.getFirmId() };
  ...
  {
    String sql = "update m_firmmodule set enabled='N' where firmId='" + 
      firmId + "'";
    updateBySQL(sql);
  }
  ...
  {
    String sql = "update m_firmmodule set enabled='Y' where firmId='" + 
      module.getFirmId() + "' and moduleID='" + 
      module.getModuleId() + "'";
    updateBySQL(sql);
  }
  ...
  public void updateFirmLog(Firm firm, String logonUser)
  {
    ...
    String sql = "insert into m_firmLog (occurTime,userId,firmId,action) values (sysdate,'" + 
      logonUser + "','" + firm.getFirmId() + "','" + remark + "')";
    this.logger.debug(sql);
    updateBySQL(sql);
  }
  ...
  {
    String sql = "update m_firm set status='" + firm.getStatus() + 
      "' where firmId='" + firm.getFirmId() + "'";
    updateBySQL(sql);
  }
  
  public void setStatusTrader(Firm firm)
  {
    String sql = "update m_Trader set status='D' where firmId='" + 
      firm.getFirmId() + "'";
    updateBySQL(sql);
  }
  ...
  {
    String sql = "";
    sql = "select count(tariffid) from (select * from t_a_tariff where status=0) t where tariffId='" + 
      user.getTariffID() + "'";
    ...
    if ((conditions != null) && (conditions.getFieldsSqlClause() != null))
    {
      ...
      if (bankid != null)
      {
        ...
        if (!bankid.equals("all")) {
          sql = 
            sql + " and a.firmId in (select firmid from F_B_FIRMIDANDACCOUNT where bankid='" + bankid + "')  ";
        }
      }
      if ((conditions != null) && (conditions.getFieldsSqlClause() != null))
      {
  ...
  public int setStatusFirm(String firmId, String firmStatus, String logonUser)
  {
    ...
    try
    {
      ...
      {
        sql = 
          "update m_firm set status='" + firmStatus + "' where firmId='" + firmId + "'";
        jdbcTemplate.update(sql);
        if ("U".equals(firmStatus))
        {
          sql = 
            "update m_Trader set status='U' where firmId='" + firmId + "'";
          jdbcTemplate.update(sql);
        }
        else if ("N".equals(firmStatus))
        {
          sql = 
            "update m_Trader set status='N' where firmId='" + firmId + "'";
          jdbcTemplate.update(sql);
        }
        ...
        sql = 
          "insert into m_firmLog (occurTime,userId,firmId,action) values (sysdate,'" + logonUser + "','" + firmId + "','" + remark + "')";
        jdbcTemplate.update(sql);
      ...
      else
      {
        ...
        CallableStatement cstmt = conn
          .prepareCall("{?=call FN_M_FirmExitCheck('" + firmId + 
          "')}");
        ...
        if (r == 1)
        {
          ...
          sql = "update m_Trader set status='U' where firmId='" + 
            firmId + "'";
          jdbcTemplate.update(sql);
          
          String remark = "交易商" + firmId + "设为退市";
          sql = "insert into m_firmLog (occurTime,userId,firmId,action) values (sysdate,'" + 
            logonUser + 
            "','" + 
            firmId + 
            "','" + 
  ...
  public int updatefirmbankMaxPertransMoney(String firmId, String firmStatus, String logonUser)
  {
    ...
    try
    {
      ...
      {
        sql = 
          "update F_B_FIRMIDANDACCOUNT set STATUS=0 where ISOPEN=1 and STATUS=1 and firmId='" + firmId + "'";
        jdbcTemplate.update(sql);
        String remark = "交易商" + firmId + "银行接口状态设为0";
        sql = "insert into m_firmLog (occurTime,userId,firmId,action) values (sysdate,'" + 
          logonUser + "','" + firmId + "','" + remark + "')";
        jdbcTemplate.update(sql);
      ...
      {
        sql = 
          "update F_B_FIRMIDANDACCOUNT set STATUS=1 where ISOPEN=1 and STATUS=0 and firmId='" + firmId + "'";
        jdbcTemplate.update(sql);
  ...
  public void createFirmCategory(FirmCategory firmCategory)
  {
    ...
    Object[] params = { firmCategory.getId(), firmCategory.getName(), 
      firmCategory.getNote() };
  ...
  {
    String sql = "update m_firmcategory  set id = ?, name = ?,note=? where id = " + 
      oldId;
    ...
    int[] dataType = { 2 };
    updateBySQL(sql, params, dataTypes);
    sql = "update m_firm set firmcategoryid = ? where firmcategoryid = " + 
      oldId;
    this.logger.debug("sql: " + sql);
  ...
  public void deleteFirmCategory(String firmId)
  {
    ...
    String sql1 = "update m_firm set firmcategoryid = 0 where firmcategoryid = '" + 
      firmId + "'";
