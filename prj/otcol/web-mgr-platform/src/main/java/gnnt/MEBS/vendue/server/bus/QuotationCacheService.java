...
public class QuotationCacheService {
	...
	private void clearPartitionQuotation(PartitionQuotation paramPartitionQuotation) {
		...
		for (int j = localList.size() - 1; j >= 0; j--) {
			...
			if ((!localQuotationValue.getSection().equals(paramPartitionQuotation.getSection()))
					|| (localQuotationValue.getPrice().doubleValue() < 0.0D)) {
				localList.remove(j);
				i = 1;
	...
	private PartitionQuotation mergePartitionQuotationModifiedAndOldMemory(PartitionQuotation paramPartitionQuotation1,
			PartitionQuotation paramPartitionQuotation2) {
		...
		if ((!paramPartitionQuotation2.getStatus().equals(paramPartitionQuotation1.getStatus()))
				|| (!paramPartitionQuotation2.getSection().equals(paramPartitionQuotation1.getSection()))) {
			localList2.clear();
			this.lastSeqId = null;
		...
		for (int j = 0; j < localList1.size(); j++) {
			...
			for (int k = localList2.size() - 1; k >= 0; k--) {
				...
				if ((localQuotationValue2.getCode().equals(localQuotationValue1.getCode()))
						&& (localQuotationValue2.getSection().equals(localQuotationValue1.getSection()))) {
					localList2.remove(k);
					this.sbForLogger.append("从内存中删除旧的   ");
					break;
				}
			}
			if ((localQuotationValue1.getPrice().doubleValue() < 0.0D)
					|| (!localQuotationValue1.getSection().equals(paramPartitionQuotation2.getSection()))) {
	...
	private StringBuffer buildQuotationValueXmlString_1(QuotationValue paramQuotationValue) {
		StringBuffer localStringBuffer = new StringBuffer();
		localStringBuffer.append("<Q>").append("<C>").append(paramQuotationValue.getCode()).append("</C>").append("<P>")
				.append(paramQuotationValue.getPrice()).append("</P>").append("<A>").append(paramQuotationValue.getValidAmount()).append("</A>")
				.append("<CD>");
	...
	private StringBuffer buildPartitionPartXmlString(PartitionQuotation paramPartitionQuotation) {
		...
		StringBuffer localStringBuffer = new StringBuffer();
		localStringBuffer.append("<N>").append(l).append("</N>").append("<S>").append(paramPartitionQuotation.getStatus()).append("</S>")
				.append("<U>").append(paramPartitionQuotation.getSection()).append("</U>").append("<T1>").append(str1).append("</T1>").append("<T2>")
				.append(str2).append("</T2>");
		return localStringBuffer;
	}
	...
	private String buildXmlForNullPartition(Long paramLong) {
		...
		localStringBuffer.append("<R>").append("<N>").append("0").append("</N>").append("<S>").append("</S>").append("<U>").append(str).append("</U>")
				.append("<T1>").append("</T1>").append("<T2>").append("</T2>").append("</R>");
