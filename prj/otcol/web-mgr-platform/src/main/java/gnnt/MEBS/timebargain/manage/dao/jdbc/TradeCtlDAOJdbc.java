...
public class TradeCtlDAOJdbc extends BaseDAOJdbc implements TradeCtlDAO {
	private Log log = LogFactory.getLog(TradeCtlDAOJdbc.class);

	public List getHolds() {
		StringBuffer sb = new StringBuffer();
		sb.append("select a.*,b.M_CustomerID,c.MarketName,d.commodityid,t.customername ").append(
				"from HoldPosition a , CM_CustomerMap b, Market c ,commodity d,customer t where b.MarketCode=c.MarketCode and b.CustomerID=a.CustomerID and t.CustomerID=a.CustomerID and a.Uni_Cmdty_Code like c.MarketCode || '%' and a.Uni_Cmdty_Code=d.Uni_Cmdty_Code");
	...
	public List getTrades() {
		StringBuffer sb = new StringBuffer();
		sb.append("select a.*,b.M_CustomerID,c.MarketName ").append(
				"from Trade a , CM_CustomerMap b, Market c where b.MarketCode=c.MarketCode and b.CustomerID=a.CustomerID and a.Uni_Cmdty_Code like c.MarketCode || '%' ");
	...
	public List getOrders() {
		StringBuffer sb = new StringBuffer();
		sb.append(
				"select a.*,b.M_CustomerID,c.MarketName from Orders a , CM_CustomerMap b, Market c where b.MarketCode=c.MarketCode and b.CustomerID=a.CustomerID and a.Uni_Cmdty_Code like c.MarketCode || '%' ");
	...
	public List getActions() {
		StringBuffer sb = new StringBuffer();
		sb.append("select a.*,")
				.append("case when a.Status in(2,3) then case when a.OrderType=1 then '订立' when a.OrderType=2 then '转让' when a.OrderType=3 then '强平' else '' end else '' end as OrderType,")
				.append("case when a.Status in(2,3) then a.TradeQty  else a.Quantity-a.TradeQty end as Quantity,")
				.append("case when a.Status in(5,6) then '撤单' when a.Status in(2,3) then '入单' else '' end as Action,")
				.append("b.M_CustomerID,c.MarketName from Orders a , CM_CustomerMap b, Market c where b.MarketCode=c.MarketCode and b.CustomerID=a.CustomerID and a.Uni_Cmdty_Code like c.MarketCode || '%' and a.OrderType<>4 and a.Status not in(1,4)");
	...
	public List getFunds(Date clearDate) {
		...
		sb.append(
				"select a.CustomerID,max(a.Balance) Balance,max(a.MaxOverdraft) MaxOverdraft,max(a.FrozenFunds) FrozenFunds,max(a.RuntimeFL) RuntimeFL,")
		...
		if (clearDate == null) {
			sb.append("from FundTradePropsView a, Trade c,")
					.append("(select CustomerID, nvl(sum(HoldQty),0) HoldQty from HoldPosition group by CustomerID) d,")
					.append("(select CustomerID,nvl(sum(Quantity-TradeQty),0) NotTradeQty from Orders where OrderType<>4 and Status in(1,2,4) group by CustomerID) e ")
					.append("where a.CustomerID = c.CustomerID(+) and a.CustomerID = d.CustomerID(+) and a.CustomerID = e.CustomerID(+) ")
					.append("group by a.CustomerID order by a.CustomerID ");
		} else {
			sb.append("from ( select d.CustomerName,e.*,nvl(c.MaxHoldQty,nvl(b.MaxHoldQty,a.MaxHoldQty)) MaxHoldQty,")
					.append("nvl(c.MinClearDeposit,nvl(b.MinClearDeposit,a.MinClearDeposit)) MinClearDeposit,")
					.append("nvl(c.MaxOverdraft,nvl(b.MaxOverdraft,a.MaxOverdraft)) MaxOverdraft ")
					.append("from CustomerDefProps a,GroupTradeProps b,CustomerTradeProps c,Customer d,HisCustomerFunds e ")
					.append("where d.CustomerID = c.CustomerID(+) and d.GroupID = b.GroupID and d.CustomerID = e.CustomerID and to_char(e.ClearDate,'yyyyMMdd')=to_char(?,'yyyyMMdd') and d.Status <> 2 ")
					.append(") a, HisTrade c,")
					.append("(select CustomerID, nvl(sum(HoldQty),0) HoldQty from HisHoldPosition where to_char(ClearDate,'yyyyMMdd')=to_char(?,'yyyyMMdd')  group by CustomerID) d,")
					.append("(select CustomerID,nvl(sum(Quantity-TradeQty),0) NotTradeQty from HisOrders where to_char(ClearDate,'yyyyMMdd')=to_char(?,'yyyyMMdd') and OrderType<>4 and Status in(1,2,4) group by CustomerID) e ")
					.append("where a.CustomerID = c.CustomerID(+) and a.CustomerID = d.CustomerID(+) and a.CustomerID = e.CustomerID(+) and to_char(?,'yyyyMMdd')=to_char(c.ClearDate(+),'yyyyMMdd') ")
					.append("group by a.CustomerID order by a.CustomerID ");
	...
	public List getSpecFrozenHolds() {
		StringBuffer sb = new StringBuffer();
		sb.append("select a.*,o.*").append(" from T_SpecFrozenHold a ,T_Orders o where a.A_OrderNo = o.A_OrderNo").append(" order by a.A_OrderNo");
	...
	public List getHoldPositions(QueryConditions conditions) {
		StringBuffer sb = new StringBuffer();
		sb.append("select a.*,m.name firmName,(a.GageQty+a.HoldQty) as dh, ");
	...
	public List getForceClose(String sectionId) {
		...
		sb.append(
				"and (((c.lastbalance + b.MaxOverdraft) < b.mincleardeposit) or ((FN_F_GetRealFunds(a.FirmID,0) + b.VirtualFunds + b.MaxOverdraft) < b.mincleardeposit )) ")
				.append("and a.holdqty > 0 ").append("order by a.firmID");
	...
	public double getForceCloseMoney(Orders order) {
		String sql = "select FN_T_COMPUTEFORCECLOSEQTY('" + order.getFirmID() + "','" + order.getCommodityID() + "'," + order.getBS_Flag() + "," + 1
				+ "," + order.getSpecPrice() + "," + order.getPrice() + ") from dual";
	...
	public int getHoldsCount(QueryConditions conditions) {
		...
		List<Map<String, Object>> list = getJdbcTemplate().queryForList(sb.toString(), params);
		if ((list == null) || (list.size() == 0)) {
			return 0;
		}
		Map map = (Map) list.get(0);
		return Integer.parseInt((String) map.get("sum"));
	}

	public List getHoldPositions(QueryConditions conditions, int rowStart, int rowEnd) {
		StringBuffer sb = new StringBuffer();
		sb.append(" select * from (  select rownum r,Q.* from (select a.*,m.name firmName,(a.GageQty+a.HoldQty) as dh ");
		...
		String orderby = "";
		if ((conditions != null) && (conditions.getFieldsSqlClause() != null) && (conditions.getConditionValue("orderby", "=") != null)) {
	...
	public int getSpecFrozenHoldsCount() {
		StringBuffer sb = new StringBuffer();
		sb.append("select count(*)  specCount").append(" from T_SpecFrozenHold a ,T_Orders o where a.A_OrderNo = o.A_OrderNo")
	...
	public List getSpecFrozenHolds(QueryConditions conditions, int rowStart, int rowEnd) {
		Object[] params = (Object[]) null;
		StringBuffer sb = new StringBuffer();
		sb.append("select * from ( ").append("select rownum r, o.ConsignerID,o.commodityid,o.price,a.a_orderno,a.a_holdno,a.frozenqty ")
	...
	public List getDetailForceClose() {
		StringBuffer sb = new StringBuffer();
		sb.append(
				"select a.commodityid,a.bs_flag,a.holdqty,a.evenprice,b.firmID,b.mincleardeposit,(FN_F_GetRealFunds(a.FirmID,0) + b.VirtualFunds + b.MaxOverdraft) usefulFunds,c.lastbalance leftoverPrice ")
				.append("from t_firmholdsum a, t_firm b, f_firmfunds c ").append("where a.firmid = b.firmid and a.firmid = c.firmid ")
	...
	public List getHoldPositionsDetail(QueryConditions conditions, int rowStart, int rowEnd) {
		StringBuffer sb = new StringBuffer();
		sb.append("select *").append("  from (select rownum r, Q.*")
				.append("          from (select * from (select ag.*, (s.holdqty - s.FrozenQty) maycloseQty")
				.append("                  from (select a.firmid,").append("                               a.customerid,")
				.append("                               a.commodityid,").append("                               a.bs_flag,")
				.append("                               sum(a.holdqty) holdqty,").append("                               sum(a.openqty) openqty,")
				.append("                               sum(a.gageqty) gageqty").append("                          from T_HoldPosition a");
		...
		sb.append("                         group by a.firmid,").append("                                  a.customerid,")
				.append("                                  a.commodityid,").append("                                  a.bs_flag) ag,")
				.append("                       t_customerholdsum s").append("                 where ag.commodityid = s.commodityid")
				.append("                   and ag.customerid = s.customerid").append("                   and ag.bs_flag = s.bs_flag ")
				.append("                   and ag.holdqty > 0").append("                   and (s.holdqty - s.FrozenQty) > 0)");
		if (orderby.equals("")) {
	...
	public long getSpecHoldqty(String firmID, String customerID, String commodityID, String bs_flag) {
		...
		firmID + "' " + "and h.customerid = '" + customerID + "' " + "and h.commodityid = '" + commodityID + "' " + "and h.bs_flag = '" + bs_flag
	...
	public long getNotradeqty(String firmID, String customerID, String commodityID, String bs_flag) {
		...
		firmID + "' " + "and o.customerid = '" + customerID + "' " + "and o.commodityid = '" + commodityID + "' " + "and o.bs_flag != '" + bs_flag
	...
	public int getHoldsDetailCount(QueryConditions conditions) {
		StringBuffer sb = new StringBuffer();
		sb.append("select count(firmid) sum").append("          from (select ag.*, (s.holdqty - s.FrozenQty) maycloseQty")
				.append("                  from (select a.firmid,").append("                               a.customerid,")
				.append("                               a.commodityid,").append("                               a.bs_flag,")
				.append("                               sum(a.holdqty) holdqty,").append("                               sum(a.openqty) openqty,")
				.append("                               sum(a.gageqty) gageqty").append("                          from T_HoldPosition a");
