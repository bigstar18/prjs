...
public class KernelEngineImpl
  extends KernelEngine
{
  public KernelEngineImpl()
      ...
      for (int i = 0; i < arrayOfCommodityVO.length; i++)
      {
        ...
        if ((localCommodityVO == null) || ((localCommodityVO.bargainFlag == 0) && (((this.TRADESTATUS.getCurStatus() == 2) && (localCommodityVO.lastPrice <= 0.0D)) || (this.TRADESTATUS.getCurStatus() != 2))))
        {
          ...
          if ((this.TRADESTATUS.getCurStatus() == 2) && (localCommodityVO.section == this.TRADESTATUS.getLastPartID())) {
            addQuotation(localCommodityVO.code, -1, new Timestamp(System.currentTimeMillis()), 0.0D, localCommodityVO.section, -1L, this.PARTITIONID, " ", 0L);
          }
        }
      }
    }
    ...
    throws SQLException
  {
    ...
    if (localCommodityVO.tradeMode == 0) {
      System.out.println("======保证金系数是 " + localCommodityVO.s_security + "  交易数量是 " + localCommodityVO.totalAmount + "  费用折扣是 " + localTradeUserVO.feecut + "  手续费系数是 " + localCommodityVO.s_fee);
    } else {
      System.out.println("======保证金系数是 " + localCommodityVO.b_security + "  交易数量是 " + localCommodityVO.totalAmount + "  费用折扣是 " + localTradeUserVO.feecut + "  手续费系数是 " + localCommodityVO.b_fee);
    }
    if (i1 == 0)
    {
      System.out.println("保证金====  绝对值");
      if (localCommodityVO.tradeMode == 0) {
        d1 = Arith.format(Arith.mul(Arith.mul(localCommodityVO.s_security, d6), localCommodityVO.tradeUnit) * localCommodityVO.totalAmount, 2);
      } else {
    ...
    else if (i1 == 1)
    {
      ...
      if (localCommodityVO.tradeMode == 0) {
        d1 = Arith.format(Arith.mul(Arith.mul(Arith.mul(d4, localCommodityVO.s_security), d6), localCommodityVO.tradeUnit) * localCommodityVO.totalAmount, 2);
      } else {
        d1 = Arith.format(Arith.mul(Arith.mul(Arith.mul(d4, localCommodityVO.b_security), d6), localCommodityVO.tradeUnit) * localCommodityVO.totalAmount, 2);
      }
    }
    if (j == 1) {
      if (i3 == 0)
      ...
      else if (i3 == 1)
      {
        ...
        if (localCommodityVO.tradeMode == 0) {
          d2 = Arith.format(Arith.mul(Arith.mul(Arith.mul(d4, localCommodityVO.s_fee), d5), localCommodityVO.tradeUnit) * localCommodityVO.totalAmount, 2);
        } else {
          d2 = Arith.format(Arith.mul(Arith.mul(Arith.mul(d4, localCommodityVO.b_fee), d5), localCommodityVO.tradeUnit) * localCommodityVO.totalAmount, 2);
        }
      }
    }
    ...
    for (;;)
    {
      try
      {
        try
        {
          ...
          if (((n != 4) && (n != 5) && (n != 6) && (this.COMMODITYTABLE != null) && (this.COMMODITYTABLE.size() > 0) && (n != 1)) || ((n == 1) && (i2 > 0)))
          {
            Object localObject1;
            if ((n != 8) && (n != 2))
            {
              localObject1 = this.TRADEDAO.getFlowControlList("where unitType=2 and tradePartition=" + this.PARTITIONID + " and unitID=" + (i2 - 1), localConnection);
              int i5;
              Object localObject2;
              ...
              localObject1 = this.TRADEDAO.getFlowControlList("where unitType=1 and tradePartition=" + this.PARTITIONID + " and unitID=" + i2, localConnection);
              if (localObject1 != null)
              {
                i5 = 0;
                if (i5 < localObject1.length)
            ...
            else
            {
                  ...
                  if (((Enumeration)localObject1).hasMoreElements())
                  {
                    ...
                    synchronized (localCommodityVO1)
                    {
                      if ((localCommodityVO1.section == i2) && (localCommodityVO1.bargainFlag == 0)) {
                        addQuotation(localCommodityVO1.code, -1, new Timestamp(l3), 0.0D, i2, -1L, this.PARTITIONID, " ", 0L, localConnection);
                      }
                    }
                    continue;
                  }
                  ...
                  if ((l3 >= l8 - j * 1000) && (m == 0))
                  {
                    ...
                    if (((Enumeration)localObject1).hasMoreElements())
                    {
                      ...
                      synchronized (localCommodityVO1)
                      {
                        ...
                        {
                          this.TRADEDAO.delTradeQuotation("where tradePartition=" + this.PARTITIONID + " and code='" + localCommodityVO1.code + "'", localConnection);
                          localObject4 = " ";
                          ...
                          addQuotation(localCommodityVO1.code, k, localTimestamp, localCommodityVO1.lastPrice, i1, localCommodityVO1.lastSubmitID, this.PARTITIONID, (String)localObject4, localCommodityVO1.amount, localConnection);
                        }
                      }
                      continue;
                    }
              ...
              else if (checkCommodityFinished())
              {
                ...
                if (n != 3)
                {
                  localObject1 = this.TRADEDAO.getCurBargainList(this.PARTITIONID, i1, localConnection);
                  int i6;
                  if (localObject1 != null)
                  {
                    i6 = 0;
                    if (i6 < localObject1.length)
                    {
                      ...
                      System.out.println();
                      ??? = localObject1[i6];
                      localObject4 = (CommodityVO)this.COMMODITYTABLE.get(((BargainVO)???).code);
                      if (((CommodityVO)localObject4).tradeMode == 0)
                      {
                        System.out.println();
                        System.out.println("==========竞买");
                        System.out.println();
                        ((BargainVO)???).b_bail = getBail(((BargainVO)???).code, ((BargainVO)???).price, ((BargainVO)???).amount, ((BargainVO)???).userID, localConnection);
                        ((BargainVO)???).s_bail = getBail(((BargainVO)???).code, ((BargainVO)???).price, ((BargainVO)???).amount, ((CommodityVO)localObject4).userID, localConnection);
                      }
                      else
                      {
                        System.out.println();
                        ...
                        System.out.println();
                        ((BargainVO)???).s_bail = getBail(((BargainVO)???).code, ((BargainVO)???).price, ((BargainVO)???).amount, ((BargainVO)???).userID, localConnection);
                        ((BargainVO)???).b_bail = getBail(((BargainVO)???).code, ((BargainVO)???).price, ((BargainVO)???).amount, ((CommodityVO)localObject4).userID, localConnection);
                      }
                      if (((CommodityVO)localObject4).tradeMode == 0)
                      {
                        ((BargainVO)???).b_poundage = getFee(((BargainVO)???).code, ((BargainVO)???).price, ((BargainVO)???).amount, ((BargainVO)???).userID, localConnection);
                        ((BargainVO)???).s_poundage = getFee(((BargainVO)???).code, ((BargainVO)???).price, ((BargainVO)???).amount, ((CommodityVO)localObject4).userID, localConnection);
                      }
                      else
                      {
                        ((BargainVO)???).s_poundage = getFee(((BargainVO)???).code, ((BargainVO)???).price, ((BargainVO)???).amount, ((BargainVO)???).userID, localConnection);
                        ((BargainVO)???).b_poundage = getFee(((BargainVO)???).code, ((BargainVO)???).price, ((BargainVO)???).amount, ((CommodityVO)localObject4).userID, localConnection);
                      }
                      long l9 = this.TRADEDAO.addBargain((BargainVO)???, localConnection);
                      ((BargainVO)???).contractID = l9;
                      setContractContent((BargainVO)???, localConnection);
                      this.TRADEDAO.modBargainContent((BargainVO)???, localConnection);
                      TradeUserVO localTradeUserVO = this.TRADEDAO.getTradeUserForUpdate(((BargainVO)???).userID, localConnection);
                      double d = localTradeUserVO.balance;
                      CommodityVO localCommodityVO2;
                      CurSubmitVO localCurSubmitVO;
                      if (((CommodityVO)localObject4).tradeMode == 0)
                      {
                        if (((BargainVO)???).userID.equals(((CommodityVO)localObject4).userID))
                        {
                          localTradeUserVO.totalSecurity += ((BargainVO)???).s_bail + ((BargainVO)???).s_poundage;
                          ...
                          localCallableStatement.setString(2, localTradeUserVO.userCode);
                          localCallableStatement.setDouble(3, -1.0D * (((BargainVO)???).s_bail + ((BargainVO)???).s_poundage));
                          localCallableStatement.setString(4, "4");
                          localCallableStatement.registerOutParameter(1, 8);
                          localCallableStatement.executeUpdate();
                          ...
                          localCallableStatement.setDouble(4, ((BargainVO)???).s_bail);
                          localCallableStatement.setLong(5, ((BargainVO)???).contractID);
                          localCallableStatement.registerOutParameter(1, 8);
                          localCallableStatement.executeUpdate();
                          d = localCallableStatement.getDouble(1);
                          addDailyMoney(d, localTradeUserVO.paymentForGoods, l9, ((BargainVO)???).userID, ((BargainVO)???).s_bail, 404, "", localConnection);
                          localCallableStatement = localConnection.prepareCall("{ ?=call FN_F_UpdateFunds(?,?,?,?) }");
                          localCallableStatement.setString(2, localTradeUserVO.userCode);
                          localCallableStatement.setInt(3, 403);
                          localCallableStatement.setDouble(4, ((BargainVO)???).s_poundage);
                          localCallableStatement.setLong(5, ((BargainVO)???).contractID);
                          localCallableStatement.registerOutParameter(1, 8);
                          localCallableStatement.executeUpdate();
                          d = localCallableStatement.getDouble(1);
                          addDailyMoney(d, localTradeUserVO.paymentForGoods, l9, ((BargainVO)???).userID, ((BargainVO)???).s_poundage, 403, "", localConnection);
                          System.out.println();
                          System.out.println("<<<<<<<<<<<<<<<<<<<  生成卖方流水 " + ((BargainVO)???).userID + " 保证金为 " + ((BargainVO)???).s_bail + "  手续费为 " + ((BargainVO)???).s_poundage + ">>>>>>>>>>");
                          System.out.println();
                          localCommodityVO2 = this.TRADEDAO.getCommodity(((BargainVO)???).commodityID, localConnection);
                          ...
                          localCallableStatement.setString(2, localTradeUserVO.userCode);
                          localCallableStatement.setInt(3, 404);
                          localCallableStatement.setDouble(4, ((BargainVO)???).b_bail);
                          localCallableStatement.setLong(5, ((BargainVO)???).contractID);
                          localCallableStatement.registerOutParameter(1, 8);
                          localCallableStatement.executeUpdate();
                          d = localCallableStatement.getDouble(1);
                          addDailyMoney(d, localTradeUserVO.paymentForGoods, l9, localCommodityVO2.userID, ((BargainVO)???).b_bail, 404, "", localConnection);
                          localCallableStatement = localConnection.prepareCall("{ ?=call FN_F_UpdateFunds(?,?,?,?) }");
                          localCallableStatement.setString(2, localTradeUserVO.userCode);
                          localCallableStatement.setInt(3, 403);
                          localCallableStatement.setDouble(4, ((BargainVO)???).b_poundage);
                          localCallableStatement.setLong(5, ((BargainVO)???).contractID);
                          localCallableStatement.registerOutParameter(1, 8);
                          localCallableStatement.executeUpdate();
                          d = localCallableStatement.getDouble(1);
                          addDailyMoney(d, localTradeUserVO.paymentForGoods, l9, localCommodityVO2.userID, ((BargainVO)???).b_poundage, 403, "", localConnection);
                          System.out.println();
                          System.out.println("<<<<<<<<<<<<<<<<<<<  生成买方方流水" + localCommodityVO2.userID + " 保证金为 " + ((BargainVO)???).b_bail + "  手续费为 " + ((BargainVO)???).b_poundage + ">>>>>>>>>>");
                          System.out.println();
                          this.TRADEDAO.dealCommodity(((BargainVO)???).commodityID, localConnection);
                          localCurSubmitVO = this.TRADEDAO.getCurSubmit(((BargainVO)???).submitID);
                          localCurSubmitVO.tradeFlag = 1;
                          this.TRADEDAO.modifyCurSubmit(localCurSubmitVO, localConnection);
                        }
                        else
                        {
                          localTradeUserVO.totalSecurity += ((BargainVO)???).b_bail + ((BargainVO)???).b_poundage;
                          localTradeUserVO.tradeCount += 1;
                          localCallableStatement = localConnection.prepareCall("{ ?=call FN_F_UpdateFrozenFunds(?,?,?) }");
                          localCallableStatement.setString(2, localTradeUserVO.userCode);
                          localCallableStatement.setDouble(3, -1.0D * (((BargainVO)???).b_bail + ((BargainVO)???).b_poundage));
                          localCallableStatement.setString(4, "4");
                          localCallableStatement.registerOutParameter(1, 8);
                          localCallableStatement.executeUpdate();
                          this.TRADEDAO.modifyTradeUser(localTradeUserVO, localConnection);
                          ...
                          localCallableStatement.executeUpdate();
                          d = localCallableStatement.getDouble(1);
                          addDailyMoney(d, localTradeUserVO.paymentForGoods, l9, ((BargainVO)???).userID, ((BargainVO)???).b_bail, 404, "", localConnection);
                          localCallableStatement = localConnection.prepareCall("{ ?=call FN_F_UpdateFunds(?,?,?,?) }");
                          localCallableStatement.setString(2, localTradeUserVO.userCode);
                          localCallableStatement.setInt(3, 403);
                          localCallableStatement.setDouble(4, ((BargainVO)???).b_poundage);
                          ...
                          localCallableStatement.executeUpdate();
                          d = localCallableStatement.getDouble(1);
                          addDailyMoney(d, localTradeUserVO.paymentForGoods, l9, ((BargainVO)???).userID, ((BargainVO)???).b_poundage, 403, "", localConnection);
                          System.out.println();
                          System.out.println("<<<<<<<<<<<<<<<<<<<  生成买方方流水" + ((BargainVO)???).userID + " 保证金为 " + ((BargainVO)???).b_bail + "  手续费为 " + ((BargainVO)???).b_poundage + ">>>>>>>>>>");
                          System.out.println();
                          localCommodityVO2 = this.TRADEDAO.getCommodity(((BargainVO)???).commodityID, localConnection);
                          delCommodityCharge(((BargainVO)???).commodityID, localConnection);
                          localTradeUserVO = this.TRADEDAO.getTradeUserForUpdate(localCommodityVO2.userID, localConnection);
                          ...
                          localCallableStatement.executeUpdate();
                          d = localCallableStatement.getDouble(1);
                          addDailyMoney(d, localTradeUserVO.paymentForGoods, l9, localCommodityVO2.userID, ((BargainVO)???).s_bail, 404, "", localConnection);
                          localCallableStatement = localConnection.prepareCall("{ ?=call FN_F_UpdateFunds(?,?,?,?) }");
                          localCallableStatement.setString(2, localTradeUserVO.userCode);
                          localCallableStatement.setInt(3, 403);
                          localCallableStatement.setDouble(4, ((BargainVO)???).s_poundage);
                          ...
                          localCallableStatement.executeUpdate();
                          d = localCallableStatement.getDouble(1);
                          addDailyMoney(d, localTradeUserVO.paymentForGoods, l9, localCommodityVO2.userID, ((BargainVO)???).s_poundage, 403, "", localConnection);
                          System.out.println();
                          System.out.println("<<<<<<<<<<<<<<<<<<<  生成卖方方流水" + localCommodityVO2.userID + " 保证金为 " + ((BargainVO)???).s_bail + "  手续费为 " + ((BargainVO)???).s_poundage + ">>>>>>>>>>");
                          System.out.println();
                          this.TRADEDAO.dealCommodity(((BargainVO)???).commodityID, localConnection);
                          localCurSubmitVO = this.TRADEDAO.getCurSubmit(((BargainVO)???).submitID);
                          localCurSubmitVO.tradeFlag = 1;
                      ...
                      else if (((BargainVO)???).userID.equals(((CommodityVO)localObject4).userID))
                      {
                        localTradeUserVO.totalSecurity += ((BargainVO)???).b_bail + ((BargainVO)???).b_poundage;
                        localTradeUserVO.tradeCount += 1;
                        localCallableStatement = localConnection.prepareCall("{ ?=call FN_F_UpdateFrozenFunds(?,?,?) }");
                  ...
                  if (localObject1 != null)
                  {
                    ...
                    if (i6 < localObject1.length)
                    {
                      ??? = getCurCommodity(localObject1[i6].code);
                      ((CommodityVO)???).bargainFlag = 1;
                      i6++;
                      continue;
                    }
                  }
                  modSysStatusDB(null, -1, null, 3, localConnection);
                  modSysStatus(null, -1, null, 3, new Timestamp(l3));
                }
              }
            }
            localConnection.commit();
