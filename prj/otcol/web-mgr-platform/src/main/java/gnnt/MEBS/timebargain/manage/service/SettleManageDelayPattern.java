...
public class SettleManageDelayPattern
{
  private static final transient Log logger = LogFactory.getLog(SettleManageDelayPattern.class);
  
  public static List getSettles(String filter)
  {
    DaoHelper dao = (DaoHelper)SysData.getBean("daoHelper");
    String sql = "select * from T_SettleMatch where 1=1 and matchid not like 'A%' " + filter + " order by createtime desc";
  ...
  {
    DaoHelper dao = (DaoHelper)SysData.getBean("daoHelper");
    String sql = "select s.* from T_SettleMatch s,t_commodity t where 1=1 and s.matchid not like 'A%' and t.SettleWay=1 and t.commodityId=s.commodityId " + filter + " order by createtime desc";
    List result = dao.queryBySQL(sql, null);
    return result;
  }
  ...
  {
    DaoHelper dao = (DaoHelper)SysData.getBean("daoHelper");
    String sql = "select * from T_SettleMatch where matchid not like 'A%' and  MatchID='" + id + "'";
  ...
  {
    DaoHelper dao = (DaoHelper)SysData.getBean("daoHelper");
    String sql = "select distinct(commodityid) commodityid,substr(to_char(settleprocessdate,'yyyy-mm-dd'),0,10) settleprocessdate  from t_settleholdposition where settletype<>2 order by settleprocessdate desc,commodityid desc";
    List result = dao.queryBySQL(sql, null);
    return result;
  }
  ...
  {
    DaoHelper dao = (DaoHelper)SysData.getBean("daoHelper");
    String sql = "select distinct(s.commodityid) commodityid,substr(to_char(settleprocessdate,'yyyy-mm-dd'),0,10) settleprocessdate  from t_settleholdposition s,t_commodity t where s.commodityId=t.commodityId and t.SettleWay=1 and settletype<>2 order by settleprocessdate desc,s.commodityid desc";
    List result = dao.queryBySQL(sql, null);
    return result;
  }
  
  public static List getBuyFirmIds(String commodityid, String settleDate)
  {
    DaoHelper dao = (DaoHelper)SysData.getBean("daoHelper");
    String sql = "select distinct(FirmID) bFirmId from t_settleholdposition t where settletype<>2 and BS_Flag=1 and trunc(SettleProcessDate)=to_date('" + settleDate + "','yyyy-MM-dd') and commodityid='" + commodityid + "' and " + 
    
      "(select nvl(sum(HoldQty+GageQty),0) bAmount from t_settleholdposition where settletype<>2 and " + 
      "BS_Flag=1 and commodityid='" + commodityid + "' and firmId= t.firmId and trunc(SettleProcessDate)=to_date('" + settleDate + "','yyyy-MM-dd'))" + 
      
      ">" + 
      
      "(select nvl(sum(Quantity),0) bSettleAmount from T_SettleMatch where matchid not like 'A%' and CommodityID='" + commodityid + "' " + 
      "and FirmID_B= t.firmId and Status>=0 and  settleDate=to_date('" + settleDate + "','yyyy-MM-dd'))" + 
  ...
  public static List getSellFirmIds(String commodityid, String settleDate)
  {
    DaoHelper dao = (DaoHelper)SysData.getBean("daoHelper");
    String sql = "select distinct(FirmID) sFirmId from t_settleholdposition t where settletype<>2 and BS_Flag=2 and trunc(SettleProcessDate)=to_date('" + settleDate + "','yyyy-MM-dd') and commodityid='" + commodityid + "' and " + 
    
      "(select nvl(sum(HoldQty+GageQty),0) sAmount from t_settleholdposition where settletype<>2 and " + 
      "BS_Flag=2 and commodityid='" + commodityid + "' and firmId= t.firmId and trunc(SettleProcessDate)=to_date('" + settleDate + "','yyyy-MM-dd'))" + 
      
      ">" + 
      
      "(select nvl(sum(Quantity),0) sSettleAmount from T_SettleMatch where matchid not like 'A%' and CommodityID='" + commodityid + "' " + 
      "and FirmID_S=t.firmId and Status>=0 and  settleDate=to_date('" + settleDate + "','yyyy-MM-dd'))" + 
  ...
  public static double checkInsertSettle(String commodityid, String settleDate, String bFirmId, String sFirmId, String regStockID, long settleAmount, int type)
  {
    double result = 0.0D;
    DaoHelper dao = (DaoHelper)SysData.getBean("daoHelper");
    String sql = "select nvl(sum(HoldQty+GageQty),0) sAmount,nvl(max(SettlePrice),0) SettlePrice from t_settleholdposition where settletype<>2 and BS_Flag=2 and commodityid='" + 
      commodityid + "' and firmId='" + sFirmId + "' and trunc(SettleProcessDate)=to_date('" + settleDate + "','yyyy-MM-dd')";
    List list = dao.queryBySQL(sql, null);
    long sAmount = 0L;
    long bAmount = 0L;
    ...
    sql = 
      "select nvl(sum(HoldQty+GageQty),0) bAmount,nvl(max(SettlePrice),0) SettlePrice from t_settleholdposition where settletype<>2 and BS_Flag=1 and commodityid='" + commodityid + "' and firmId='" + bFirmId + "' and trunc(SettleProcessDate)=to_date('" + settleDate + "','yyyy-MM-dd')";
    ...
    sql = "select nvl(sum(Quantity),0) bSettleAmount from T_SettleMatch where matchid not like 'A%' and CommodityID='" + commodityid + "' and FirmID_B='" + bFirmId + "' and Status>=0 and  settleDate=to_date('" + settleDate + "','yyyy-MM-dd')";
    list = dao.queryBySQL(sql, null);
    ...
    sql = "select nvl(sum(Quantity),0) sSettleAmount from T_SettleMatch where matchid not like 'A%' and CommodityID='" + commodityid + "' and FirmID_S='" + sFirmId + "' and Status>=0 and  settleDate=to_date('" + settleDate + "','yyyy-MM-dd')";
    list = dao.queryBySQL(sql, null);
    if ((list != null) && (list.size() > 0))
    {
      Map map = (Map)list.get(0);
  ...
  public static double getQualitySettle(String sFirmId, String regStockID)
  {
    String sql = "";
    ...
    if (regStockID != null)
    {
      String[] regStockIDs = regStockID.split(",");
      for (int i = 0; i < regStockIDs.length; i++)
      {
        sql = "select t.weight-t.frozenweight mayAmount from s_regstock t where t.regstockid='" + regStockIDs[i] + "' and t.firmid='" + sFirmId + "'";
        List list = dao.queryBySQL(sql, null);
        if ((list != null) && (list.size() > 0))
        {
          Map map = (Map)list.get(0);
  ...
  public static double checkQualitySettle(String commodityid, String settleDate, String bFirmId, String sFirmId, String regStockID, int type)
  {
    double result = 0.0D;
    DaoHelper dao = (DaoHelper)SysData.getBean("daoHelper");
    String sql = "select nvl(sum(HoldQty+GageQty),0) sAmount,nvl(max(SettlePrice),0) SettlePrice from t_settleholdposition where settletype<>2 and BS_Flag=2 and commodityid='" + 
  ...
  public static int createSettle(String commodityid, String settleDate, String bFirmId, String sFirmId, String regStockID, long settleAmount, int type)
    throws SQLException
  {
    int result = -1;
    double r = checkInsertSettle(commodityid, settleDate, bFirmId, sFirmId, regStockID, settleAmount, type);
    if (r < 0.0D)
    {
      ...
      try
      {
        ...
        long bAmount = 0L;
        long sAmount = 0L;
        sql = "select sum(Payout) Payout,sum(SettleMargin) SettleMargin,sum(HoldQty+GageQty) bamount,max(SettlePrice) SettlePrice from T_SettleHoldPosition where CommodityID='" + commodityid + "' and BS_Flag=1 and FirmID='" + bFirmId + "' and settletype<>2 and trunc(SettleProcessDate)=to_date('" + settleDate + "','yyyy-MM-dd')";
        list = dao.queryBySQL(sql, null);
        if ((list != null) && (list.size() > 0))
        {
          Map map = (Map)list.get(0);
        ...
        sql = "select sum(SettleMargin) SettleMargin,sum(HoldQty+GageQty) samount,max(SettlePrice) SettlePrice from T_SettleHoldPosition where CommodityID='" + commodityid + "' and BS_Flag=2 and FirmID='" + sFirmId + "' and settletype<>2 and trunc(SettleProcessDate)=to_date('" + settleDate + "','yyyy-MM-dd')";
        list = dao.queryBySQL(sql, null);
        if ((list != null) && (list.size() > 0))
        {
          Map map = (Map)list.get(0);
        ...
        String data = "{?=call FN_S_createT_SettleMatch('" + commodityid + "'," + ContractFactor + "," + settleAmount + ",0,'" + type + "','" + bFirmId + "','" + sFirmId + "'," + 
          SettlePrice_b + "," + SettlePrice_s + "," + BuyPayout + ",0," + BuyMargin + "," + SellMargin + ",'" + settleDate + "','" + regStockID + "')}";
