...
public class OrdersAction
  extends BaseAction
{
  public ActionForward chkLogin(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response)
    throws Exception
  {
    ...
    throws Exception
  {
    ...
    try
    {
      ...
      else
      {
        ...
        if ((mgr.getConsigner(request.getParameter("traderID")) != null) && (mgr.getConsigner(request.getParameter("traderID")).getType() != null)) {
    ...
    throws Exception
  {
    ...
    try
    {
      request.setAttribute("commoditySelect", lookupMgr
        .getSelectLabelValueByTable("Commodity", "CommodityID", 
        "Uni_Cmdty_Code", " where MarketDate<=sysdate and Status<>1 and MarketCode='" + request.getParameter("MarketCode") + "' order by CommodityID"));
    }
    catch (Exception err)
    {
    ...
    throws Exception
  {
    ...
    try
    {
      ...
      if (ordersForm.getOrderType().shortValue() == 2)
      {
        ...
        if ((ordersForm.getCloseMode().shortValue() == 1) && 
          (ordersForm.getCloseAlgr() != null)) {
    ...
    throws Exception
  {
    ...
    if (ids != null)
    {
      try
      {
        ...
        Long sessionID = (Long)request.getSession().getAttribute("sessionID");
        String traderID = remObject.getConsignerID(sessionID.longValue());
        if (traderID == null) {
          throw new Exception("未登录下单服务器！");
        }
      }
  ...
  public ActionForward outLine(ActionMapping mapping, ActionForm form, HttpServletRequest request, HttpServletResponse response)
    throws Exception
  {
    if (this.log.isDebugEnabled()) {
  ...
  {
    LookupManager lookupMgr = (LookupManager)getBean("lookupManager");
    request.setAttribute("marketSelect", lookupMgr
      .getSelectLabelValueByTable("Market", "MarketName", 
      "MarketCode", " where Status=1"));
  }
  
  private int submitOrder(AgencyRMIBean remObject, Long sessionID, Order ov, HttpServletRequest request)
    throws Exception
  {
