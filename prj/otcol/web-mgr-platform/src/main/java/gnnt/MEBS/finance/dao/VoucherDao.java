...
public class VoucherDao extends DaoHelperImpl {
	...
	public void createSummary(Summary paramSummary) {
		...
		Object[] arrayOfObject = { paramSummary.getSummaryNo(), paramSummary.getSummary(), paramSummary.getLedgerItem(), paramSummary.getFundDCFlag(),
				paramSummary.getAppendAccount() };
	...
	public void updateSummary(Summary paramSummary) {
		StringBuffer localStringBuffer = new StringBuffer("update f_Summary set summary='").append(paramSummary.getSummary()).append("'")
				.append(",ledgerItem=?,fundDCFlag='" + paramSummary.getFundDCFlag() + "',AppendAccount='" + paramSummary.getAppendAccount() + "'");
	...
	public void createVoucher(Voucher paramVoucher) {
		try {
			...
			Object[] arrayOfObject = { paramVoucher.getSummaryNo(), paramVoucher.getSummary(), paramVoucher.getVoucherNo(), paramVoucher.getNote(),
					paramVoucher.getInputUser(), paramVoucher.getInputTime(), paramVoucher.getAuditor(), paramVoucher.getAuditTime(),
					paramVoucher.getAuditNote(), paramVoucher.getStatus(), paramVoucher.getContractNo() };
	...
	public void deleteVoucher(Long paramLong, String paramString) {
		try {
			...
			if (i > 0) {
				String str2 = "delete from f_VoucherEntry where voucherNo=?";
	...
	public void updateVoucherNotEntrys(Voucher paramVoucher) {
		...
		Object[] arrayOfObject = { paramVoucher.getSummaryNo(), paramVoucher.getSummary(), paramVoucher.getNote(), paramVoucher.getInputUser(),
				paramVoucher.getInputTime(), paramVoucher.getAuditor(), paramVoucher.getAuditTime(), paramVoucher.getAuditNote(),
				paramVoucher.getStatus(), paramVoucher.getContractNo(), paramVoucher.getVoucherNo() };
	...
	private void updateVoucherEntry(VoucherEntry paramVoucherEntry) {
		...
		Object[] arrayOfObject = { paramVoucherEntry.getEntrySummary(), paramVoucherEntry.getAccountCode(), paramVoucherEntry.getDebitAmount(),
				paramVoucherEntry.getCreditAmount(), paramVoucherEntry.getEntryId() };
		this.logger.debug("sql: " + str);
		for (int i = 0; i < arrayOfObject.length; i++) {
			this.logger.debug("params[" + i + "]: " + arrayOfObject[i]);
		}
	...
	private void insertVoucherEntry(VoucherEntry paramVoucherEntry) {
		...
		Object[] arrayOfObject = { paramVoucherEntry.getEntryId(), paramVoucherEntry.getEntrySummary(), paramVoucherEntry.getVoucherNo(),
				paramVoucherEntry.getAccountCode(), paramVoucherEntry.getDebitAmount(), paramVoucherEntry.getCreditAmount() };
		this.logger.debug("sql: " + str);
		for (int i = 0; i < arrayOfObject.length; i++) {
			this.logger.debug("params[" + i + "]: " + arrayOfObject[i]);
		}
	...
	public int setVoucherBDate(String paramString1, String paramString2, String paramString3) {
		int i = -1;
		try {
			String str1 = "FN_F_SetVoucherBDate(to_date('" + paramString1 + " 23:59:59','yyyy-MM-dd hh24:mi:ss'),to_date('" + paramString2
					+ "','yyyy-MM-dd'))";
			try {
	...
	class channelRowMapper implements RowMapper {
		channelRowMapper() {
		}
		...
		private Channel rsToUser(ResultSet paramResultSet) throws SQLException {
	...
	class EntryRowMapper implements RowMapper {
		...
		private VoucherEntry rsToVoucherEntry(ResultSet paramResultSet) throws SQLException {
	...
	class VoucherRowMapper implements RowMapper {
		VoucherRowMapper() {
		}
		...
		private Voucher rsToVoucher(ResultSet paramResultSet) throws SQLException {
	...
	class SummaryRowMapper implements RowMapper {
		SummaryRowMapper() {
		}
		...
		private Summary rsToSummary(ResultSet paramResultSet) throws SQLException {
