...
public class AddRight {
	...
	public void doAction(HttpServletRequest paramHttpServletRequest, HttpServletResponse paramHttpServletResponse, long paramLong,
			String paramString1, int paramInt, String paramString2, JspWriter paramJspWriter) {
		try {
			...
			if (bool) {
				...
				for (int i = 0; i < localArrayList.size(); i++) {
					...
					if ((paramLong & l) > 0L) {
						paramJspWriter.print("<input type=\"checkbox\" id=\"r_" + str2 + "\" name=\"ck\" onclick=\"selectAll1('r_" + str3
								+ "')\"  value=\"" + l + "\" d_value=\"r_" + str3 + "\" checked>");
					} else {
						paramJspWriter.print("<input type=\"checkbox\" id=\"r_" + str2 + "\" name=\"ck\" onclick=\"selectAll1('r_" + str3
								+ "')\"  value=\"" + l + "\" d_value=\"r_" + str3 + "\">");
					}
