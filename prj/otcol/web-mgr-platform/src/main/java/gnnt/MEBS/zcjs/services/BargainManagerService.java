...
public class BargainManagerService
{
  ...
  public void match(List<KeyValue> paramList, String paramString, long paramLong)
    throws Exception
  {
    ...
    for (int i = 0; i < paramList.size(); i++)
    {
      ...
      int j = this.warehouseService.updateRegstockFrozen((String)localKeyValue.getKey(), QualityRatio.ratioObverse(d1, localBreed.getDeliveryRatio()), paramString, 1);
      if (j != 1) {
        throw new Exception();
      }
  ...
  @Transactional(propagation=Propagation.REQUIRED, readOnly=false, rollbackFor={Exception.class, RuntimeException.class})
  public void deleteMatch(String[] paramArrayOfString, String paramString)
    throws Exception
  {
  ...
  @Transactional(propagation=Propagation.REQUIRED, readOnly=false, rollbackFor={Exception.class, RuntimeException.class})
  public void send(long paramLong, String paramString)
    throws Exception
  {
    QueryConditions localQueryConditions = new QueryConditions();
    ...
    if ((localList != null) && (localList.size() > 0)) {
      for (int j = 0; j < localList.size(); j++)
      {
        ...
        if (localDelivery.getStatus() != 4)
        {
          ...
          {
            d = QualityUnit.uintObverse(localDelivery.getQuantity(), localBreed.getUnitVolume());
          ...
          {
            this.moneyDoDao.updateFundsFull(localHisTrade.getFirmId_B(), "308", -localHisTrade.getDeliveryPoundage_B(), localHisTrade.getTradeNo() + "", "");
            this.moneyDoDao.updateFundsFull(localHisTrade.getFirmId_S(), "308", -localHisTrade.getDeliveryPoundage_S(), localHisTrade.getTradeNo() + "", "");
            this.moneyDoDao.updateFundsFull(localHisTrade.getFirmId_B(), "303", -localHisTrade.getTradeBail_B(), localHisTrade.getTradeNo() + "", "");
            this.moneyDoDao.updateFundsFull(localHisTrade.getFirmId_S(), "303", -localHisTrade.getTradeBail_S(), localHisTrade.getTradeNo() + "", "");
          }
  ...
  @Transactional(propagation=Propagation.REQUIRED, readOnly=false, rollbackFor={Exception.class, RuntimeException.class})
  public void matchOne(Delivery paramDelivery, String paramString, long paramLong, double paramDouble)
    throws Exception
  {
    HisTrade localHisTrade = getHisTrade(paramLong);
    Breed localBreed = this.breedDao.getObject(localHisTrade.getBreedId());
    ...
    int i = this.warehouseService.updateRegstockFrozen(localWarehouseRegStock.getRegStockId(), QualityRatio.ratioObverse(d2, localBreed.getDeliveryRatio()), paramString, 1);
    if (i != 1) {
      throw new Exception();
    }
