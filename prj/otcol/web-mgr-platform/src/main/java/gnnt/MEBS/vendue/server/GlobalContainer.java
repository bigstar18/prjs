		...
		if (!LOADFLAG) {
			...
			if (arrayOfSysPartitionVO != null) {
				...
				for (int j = 0; j < arrayOfSysPartitionVO.length; j++) {
					...
					if (bool) {
						Thread localObject = new Thread(localKernelEngine, "kernelThread" + k);
						...
						localLogger.debug(
								"==============>" + localSysPartitionVO.description + "板块RMI服务已启动。" + "//" + str1 + ":" + i + "/KernelEngineRMI" + k);
					}
					SubmitAction localObject = null;
