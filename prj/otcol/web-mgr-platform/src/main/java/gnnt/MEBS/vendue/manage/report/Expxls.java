	...
	public void doDownload(HttpServletRequest paramHttpServletRequest, HttpServletResponse paramHttpServletResponse)
		...
		try {
			...
			try {
				...
				Context localObject1 = (Context) localInitialContext.lookup("java:/comp/env");
				DataSource localObject2 = (DataSource) ((Context) localObject1).lookup(this.jndi);
				...
				if (this.type == 0) {
					String localObject3 = "";
					String localObject4 = paramHttpServletRequest.getParameter("userCode");
					...
					((StringBuffer) localObject5).append(
							"u2.category2,u2.tradecount,u2.totalsecurity,u3.name as province,u4.name as market from tradeuserext u1,tradeuser u2");
					((StringBuffer) localObject5).append(",dictable u3,dictable u4 where u1.usercode=u2.usercode and u2.status<>3 and u3.type="
							+ this.proType + " and u1.str1=u3.value and u1.str2=");
					...
					String localObject3 = "select u1.*,u2.name as province,u3.name as ";
						...
						if (Integer.parseInt((String) localObject7) == 1) {
							if (((String) localObject5).equals(this.allMarket)) {
								...
								localObject4 = (String) localObject4
										+ " and u1.usercode in (select usercode from tradeusermarket where  balance>0 and marketid="
										+ (String) localObject5 + ") ";
							}
					...
					if ((localObject6 != null) && (!"".equals(localObject6))) {
						if (Integer.parseInt((String) localObject6) == 1) {
							localObject4 = (String) localObject4
									+ " and u1.usercode in (select usercode from tradeuser where (keycode is not null or keycode='" + this.defaultKey
									+ "'))";
						} else if (Integer.parseInt((String) localObject6) == 2) {
							localObject4 = (String) localObject4 + " and u1.usercode in (select usercode from tradeuser where  keycode='"
									+ this.defaultKey + "')";
					...
					for (int k = 1; localResultSet.next(); k++) {
						...
						localHSSFCell = localHSSFRow.createCell((short) 19);
						localHSSFCell.setEncoding((short) 1);
						localHSSFCell.setCellValue(localResultSet.getString("validperiod").replace(';', '至'));
						localHSSFCell = localHSSFRow.createCell((short) 20);
						localHSSFCell.setEncoding((short) 1);
