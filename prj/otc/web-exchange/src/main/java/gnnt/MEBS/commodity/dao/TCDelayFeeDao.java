...
@Repository("tcDelayFeeDao")
public class TCDelayFeeDao
  extends BaseDao<TCDelayFee>
{
  private final transient Log logger = LogFactory.getLog(TCDelayFeeDao.class);
  
  public List getTCDelayList(QueryConditions conditions, PageInfo pageInfo)
  {
    this.logger.debug("enter getList");
    String hql = "select distinct primary.commodityId,primary.firmId,commodity.name,primary.feeAlgr_v from " + getEntityClass().getName() + " as primary, gnnt.MEBS.commodity.model.Commodity as commodity where primary.commodityId=commodity.id ";
