...
public class CommodityJDBCDao
  extends ApplyJDBCDao
{
  public List getApplyList(QueryConditions conditions, PageInfo pageInfo)
  {
    String sql = "select * from(select primary.id as id,primary.applyType,primary.statusdiscribe as statusDiscribe,extractValue(xmlType(primary.content),'/root/name') as name ,extractValue(xmlType(primary.content),'/root/id') as commodityId from c_apply primary)";
