...
@Repository("customerDelayTradeDao")
public class CustomerDelayTradeDao
  extends BaseDao<DelayTrade>
{
  private final transient Log logger = LogFactory.getLog(CustomerDelayTradeDao.class);
  ...
  {
    String hql = "select new DelayTrade(primary.commodityId,primary.f_FirmId,commodity.name,customer.name,primary.delayTradeType,primary.delayTradeTime,primary.isslipPoint ) from DelayTrade as primary,Customer as customer,Commodity as commodity where primary.commodityId=commodity.id and primary.f_FirmId=customer.customerNo";
