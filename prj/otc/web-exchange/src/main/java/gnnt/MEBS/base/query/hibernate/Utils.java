...
public class Utils
{
  ...
  {
    return quote + source.replaceAll(quote, new StringBuilder("\\").append(quote).toString()) + quote;
  }
  
  public static String quoteDouble(String source, String quote, boolean enclose)
  {
    if (enclose) {
      return quote + source.replaceAll(quote, new StringBuilder(String.valueOf(quote)).append(quote).toString()) + quote;
    }
    ...
    throws Exception
  {
    ...
    if (type.equals("int")) {
      return Integer.valueOf(parseInt(source));
    }
    if (type.equals("long")) {
    ...
    if (type.equals("timestemp"))
    {
      source = combineDateTime(source);
      return parseDate(df, source);
    }
    if (type.equals("bigdecimal")) {
      return new BigDecimal(parseDouble(source).doubleValue());
    }
