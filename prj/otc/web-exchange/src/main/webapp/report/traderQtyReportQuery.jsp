						...
						<tr>
							<td>
								<div class="report_bor01">
									<form
										action="${basePath}/report/traderQty/traderQtyReportQuery.action"
										...
										<table width="820" border="0" class="table2_td_widthmax" cellspacing="0" cellpadding="0">
											...
											<tr class="report_w12h">
												<td class="table3_td_1" align="left">会员编号： 
													<input type="text" name="${gnnt_}primary.memberNo[=][String]" id="memberNo"
																value="${oldParams['primary.memberNo[=][String]'] }" class="input_textmin">
												</td>
												<td class="table3_td_1" align="left">
													开始日期：
													<MEBS:calendar eltID="startDate"
														eltName="${GNNT_}trunc(primary.clearDate)[>=][date]" eltCSS="date"
														eltStyle="width:80px" eltImgPath="<%=skinPath%>/images/"
														eltValue='${oldParams["trunc(primary.clearDate)[>=][date]"]}' />
												</td>
												<td class="table3_td_1" align="left">
													结束日期：
													<MEBS:calendar eltID="endDate"
														eltName="${GNNT_}trunc(primary.clearDate)[<=][date]" eltCSS="date"
														eltStyle="width:80px" eltImgPath="<%=skinPath%>/images/"
														eltValue='${oldParams["trunc(primary.clearDate)[<=][date]"]}' />
												</td>
...
<script type="text/javascript">
	function select1(){
		...
		frm.action="${basePath}/report/traderQty/traderQtyReport.action";
