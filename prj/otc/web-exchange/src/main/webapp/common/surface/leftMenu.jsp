...
CLASS_MSN_MESSAGE.prototype.rect = function(left,right,top,bottom){ 
 ...
 this.top = top !=null?top:this.bottom - this.height; 
			...
			var count=0;
			function PMstart() {
				positionMon.getFirmUpdateCount(function(isNum){
					count=isNum;
			    	checkInsert();
				});
			}
			function checkInsert(){
				positionMon.getFirmUpdateCount(function(isNum){
					if(isNum>count){						
						var result=isNum-count;
						//弹出窗口信息
						alertMsg("持仓监控提示", "新增" + result+ "条数据");
					}
					count=isNum;			    		
				});
				window.setTimeout("checkInsert()",10000);
			}
