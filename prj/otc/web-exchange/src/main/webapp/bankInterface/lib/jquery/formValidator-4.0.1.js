...
$.formValidator = 
{
	...
	inputValid : function(returnObj)
	{
		...
		switch(sType)
		{
			...
			case "radio":
				...
				if("true"==setting.required && val.length<=0){
					requiredflag = true;
				}
				...
				{
					lb_go_on = isDate(val);
					minflag = isDate(setting.min);
					maxflag = isDate(setting.max);
					if(lb_go_on){
						val = new Date(val.replace(/-/g,"\/"));
					}
					if(minflag){
						setting.min=new Date(setting.min.replace(/-/g,"\/"));
					}
					if(maxflag){
						setting.max=new Date(setting.max.replace(/-/g,"\/"));
					}
					if("true"==setting.lessToday){
						todayflag = true;
					}
				}else{
				...
				if(lb_go_on)
				{
					if((type == "date" || type == "datetime") && todayflag && val>new Date()){
						if(setting.onErrorToday){
							returnObj.errormsg=setting.onErrorToday;
						}
					}
					else if((minflag && val < setting.min) || (maxflag && val > setting.max)){
						if(minflag && val < setting.min && setting.onErrorMin){
							returnObj.errormsg= setting.onErrorMin;
						}
						if(maxflag && val > setting.min && setting.onErrorMax){
	...
	compareValid : function(returnObj)
	{
		...
		curvalue = srcjo.val();
		ls_data = desjo.val();
		if("true"==setting.required && curvalue.length<=0){
			requiredFlag = true;
		}else if(ls_data.length<=0){
			requiredFlag = true;
		}
