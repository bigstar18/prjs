...
<html>
			...
			<table class="table1_style" border="0" cellspacing="0"
				...
				<tr>
					<td>
						<div class="div_tj">
							<form name="frm"
								action="${basePath}/account/customerGeneral/customerListGeneral.action"
								method="post">
									...
									<td>
										<ec:table items="resultList"
											...
											<ec:row recordKey="${customer.customerNo}">
												...
												<ec:column property="primary.name[Like]" width="5%"
													title="客户名称"
												...
												<ec:column property="memberInfo.name[like]" width="6%"
													title="所属会员"
													style="text-align:left;overflow:hidden;text-overflow:ellipsis"
	...
	function forwardUpdate(id) {
		var url = "${basePath}/account/customerGeneral/forwardUpdate.action?obj.customerNo="
				+ id;
		ecsideDialog(url, "", 710, 530);
	}

	function addCustomer() {
		var url = "${basePath}/account/customerGeneral/forwardAdd.action";
		//ec.action=url;
		//ec.submit();
		ecsideDialog(url, "", 710, 580);
	}
	function select1() {
		frm.action="${basePath}/account/customerGeneral/customerListGeneral.action";
		frm.submit();
	}

	function deleteByCheckBox() {
		var url = "${basePath}/account/customerGeneral/delete.action";
		deleteEcside(ec.ids, url);
	}
	function forwardUpdatePW(id) {
		var url = "${basePath}/account/customerGeneral/forwardUpdatePassward.action?obj.traderID="
				+ id;
		ecsideDialog(url, "", 580, 260);
	}
	function forwardUpdatePhonePW(id) {
		var url = "${basePath}/account/customerGeneral/forwardUpdatePasswardPhone.action?obj.customerNo="
				+ id;
		ecsideDialog(url, "", 580, 260);
	}
</SCRIPT>
