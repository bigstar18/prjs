...
<div class="div_cxtjmid">
	<img src="<%=skinPath%>/cssimg/13.gif" />&nbsp;客户手续费
</div>
<table border="0" cellspacing="0" cellpadding="4" width="100%"
	...
	<tr height="40">
		...
		<td>
			<input class="input_text" type="text" id="m_FirmId" style="text-align:right"
...
function commodityId(userID){
	...
	if(firmId){
		checkAction.existCustomerTakeFee(commodityId, firmId, function(isExist){
			if(isExist){
				alert('信息已存在，请重新添加');
	...
	if(firmId){
		checkAction.existFirmId(firmId,function(isExist){
				if (!isExist) {
					alert('交易账号不存在，请重新添加');
					document.getElementById("m_FirmId").value="";
					document.getElementById("m_FirmId").focus();
				}
			});
