...
<html>
	...
	<body>
			...
			<table class="table1_style" border="0" cellspacing="0"
				...
				<tr>
					<td>
						<div class="div_tj">
							<form name="frm"
								action="${basePath}/commodity/memberDelayTrade/list.action?sortName=primary.commodityId"
								method="post">
						...
						<div class="div_gn">
							...
							<button class="anniu_btn" id='updateRMI' onclick="updateRMI()">
								实时生效
							</button>
						</div>
						<div class="div_list">
		...
		function update(f_FirmId,commodityId){
			var url = "${basePath}/commodity/memberDelayTrade/forwardUpdate.action?obj.commodityId="+commodityId+"&obj.f_FirmId="+f_FirmId;
		...
		function addDelayTradePoint() {
			var urlFee = "${basePath}/commodity/memberDelayTrade/forwardAdd.action";
			ecsideDialogDelayTrade(urlFee, "", 580, 415);
		}
		function updateRMI() {
			frm.action = "${basePath}/commodity/memberDelayTrade/updateRMI.action";
			if(affirm("交易商重新登录时生效，您确定要操作吗？")){
				frm.submit();
			}
		}
		function deleteDelayTrade(){
			var collCheck=document.getElementsByName("ids");
			var url = "${basePath}/commodity/memberDelayTrade/delete.action";
