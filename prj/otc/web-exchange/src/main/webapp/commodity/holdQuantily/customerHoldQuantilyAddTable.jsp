...
%>
<div class="div_cxtjd">
	<img src="<%=skinPath%>/cssimg/13.gif" />&nbsp;客户持仓数量
</div>
<table border="0" cellspacing="0" cellpadding="4" width="100%"
	...
	<tr height="40">
		...
		<td>			
			<input class="input_text" type="text" id="memberNo" style="text-align:right"
				onblur="myblur('memberNo')" onfocus="myfocus('memberNo')"
				name="obj.firmId">
			<span id="m_FirmId_span"></span>
	...
	<tr height="40">
		...
		<td>
			...
			<input type="hidden" name="obj.maxCleanQty" id="maxCleanQty" value="${obj.maxCleanQty}">
		</td>
		<td align="left" height="40">
...
function commodityName(userID){
	...
	if(firmId && commodityId){
		checkAction.existCustomerHoldQuantily(commodityId, firmId, function(isExist){
			if(isExist){
				alert('信息已存在，请重新添加');
...
if (isEmpty(user.value)) {
		...
		innerHTML = str;
			...
			if(userID=="maxHoldQty"){
				document.getElementById("maxCleanQty").value = user.value;
