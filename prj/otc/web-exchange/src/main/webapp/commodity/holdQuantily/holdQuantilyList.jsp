...
<html>
	...
	<body>
			...
			cellpadding="0">
				<tr>
					<td>
						<div class="div_tj">
						<form name="frm"
								action="${basePath}/commodity/memberHoldQuantily/list.action?sortName=primary.commodityId"
								method="post">
									...
									<td>
										<ec:table items="resultList"
											...
											<ec:row recordKey="${holdQuantily.firmId}">
												...
												<ec:column property="primary.firmId[like]" title="会员名称" width="10%"	style="text-align:left;" ellipsis="true" >${holdQuantily.memberName} </ec:column>
												...
												<ec:column property="primary.maxCleanQty[=][int]" title="最大净持仓量"
													width="16%" style="text-align:right;"
													value="${holdQuantily.maxCleanQty}" />
		...
		function update(firmId,commodityId){
			var url="${basePath}/commodity/memberHoldQuantily/forwardUpdate.action?obj.commodityId="+commodityId+"&obj.firmId="+firmId;
		...
		function addMemberQuotePoint(){
			var url="${basePath}/commodity/memberHoldQuantily/forwardAdd.action";
			ecsideDialog(url,"",580,453);
		}
		function deleteMemberQuotePoint(){
	  		var url="${basePath}/commodity/memberHoldQuantily/delete.action";
