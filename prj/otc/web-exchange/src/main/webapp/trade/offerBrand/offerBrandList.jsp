	...
	<body>
		<div id="main_body">
			<table class="table1_style" border="0" cellspacing="0" cellpadding="0">
				<tr>
					<td>
						<c:if test="${original_flag != 1 }">
							<div class="div_gn">
								<button class="anniu_btnmax" onclick="openBrand()">
									新窗口打开
								</button>
							</div>
						</c:if>
									...
									<td>
										<ec:table items="resultList"
											...
											<ec:row recordKey="">
												...
												<ec:column property="commodity.name[like]" styleClass="font-size:18" title="商品名称" width="10%"
												...
												<ec:column property="_1" title="买价" width="11%"
													...
													<c:if test="${quotation.minPriceMove=='0.01'}">
														<fmt:formatNumber value="${quotation.curPrice_B}" pattern=".00"/>
													</c:if>
													<c:if test="${quotation.minPriceMove != '0.01' && quotation.minPriceMove != 1}">
														<fmt:formatNumber value="${quotation.curPrice_B}" pattern=".0"/>
													</c:if>
													</font>
												</ec:column>
												<ec:column property="_2" title="卖价" width="11%"
													...
													<c:if test="${quotation.minPriceMove=='0.01'}">
														<fmt:formatNumber value="${quotation.curPrice_S}" pattern=".00"/>
													</c:if>
													<c:if test="${quotation.minPriceMove != '0.01' && quotation.minPriceMove != 1}">
														<fmt:formatNumber value="${quotation.curPrice_S}" pattern=".0"/>
													</c:if>
													</font>
												</ec:column>
												<ec:column property="curPrice[=][bigdecimal]" title="中间价" width="12%"
													...
													<c:if test="${quotation.minPriceMove=='0.01'}">
														<fmt:formatNumber value="${quotation.curPrice}" pattern="#.00"/>
													</c:if>
													<c:if test="${quotation.minPriceMove != '0.01' && quotation.minPriceMove != 1}">
												...
												<ec:column property="highPrice[=][bigdecimal]" title="最高价" width="12%"
													style="text-align:center;font-weight:bold;font-size:18">
													<c:if test="${quotation.minPriceMove==1}">
												...
												<ec:column property="lowPrice[=][bigdecimal]" title="最低价" width="12%"
													style="text-align:center;font-weight:bold;font-size:18">
												<c:if test="${quotation.minPriceMove==1}">
												...
												<ec:column property="createTime[=][date]" title="行情时间" width="29%"
...
<script type="text/javascript">
		...
		ecReloadF();
		function openBrand(){
			window.open('${basePath}/tradeManage/offerBrand/list.action?original_flag=1');
		}
	</script>
</html>
