...
<html>
	...
	<body>
			...
			<table class="table1_style" border="0" cellspacing="0" cellpadding="0">
				<tr>
					<td>
						<div class="div_tj">
						<form name="frm"
								action="${basePath}/tradeManage/notInMarket/list.action"
								method="post">
								...
								<tr>
									<td>
										<ec:table items="resultList"
											...
											var="commodity"
											action="${basePath}/tradeManage/notInMarket/list.action" title=""
											...
											<ec:row recordKey="${commodity.id}">
												<ec:column cell="checkbox" headerCell="checkbox" alias="ids"
													style="text-align:center; " value="${commodity.id}" width="4%"
													viewsAllowed="html" />
												...
												<ec:column property="marketDate[=][timestemp]" title="上市日期"
													width="9%" style="text-align:left;" value="${datefn:formatdate(commodity.marketDate)}">
												</ec:column>
											</ec:row>
										</ec:table>
									</td>
		...
		function update(id){
			var url="${basePath}/tradeManage/notInMarket/forwardUpdate.action?obj.id="+id;
			ecsideDialog(url,window,650,580);
		}
		function select1(){
			frm.submit();
		}
		...
		function deleteCommodity(){
			var url="${basePath}/tradeManage/notInMarket/delete.action";
