												...
												<table class="table3_style" border="0" cellspacing="0"
													...
													<tr>
														...
														<td class="table3_td_1tjcx" align="left">
															持仓单号:&nbsp;
															<label>
																<input type="text" name="${GNNT_}primary.openTradeNo[=][int]"
																	id="memberName" value="${oldParams['primary.openTradeNo[=][int]'] }"
																	class="input_textmin" />
															</label>
														</td>
								...
								<tr>
									<td>
										<ec:table items="resultList"
											...
											action="${basePath}/query/querySpecialMemberHoldSearch/list.action?sortName=primary.openTradeNo&sortOrder=true"
											title="" minHeight="325" listWidth="150%"
											...
											<ec:row ondblclick="">
												...
												<c:if test="${queryType=='H'}">
													<ec:column property="clearDate[=][date]" title="结算日期"
														...
														ellipsis="true" >
														<fmt:formatDate value="${speicalMemberHoldSearch.clearDate}"
															pattern="yyyy-MM-dd" />
													</ec:column>
												</c:if>
												...
												<ec:column property="holdQty[=][int]" title="数量" width="2%"
												...
												<c:if test="${queryType=='D'}">
													...
													width="4%" style="text-align:right; " >
													<fmt:formatNumber value="${speicalMemberHoldSearch.holdPl}"
														pattern="#,##0.00" />
												</ec:column>
												</c:if>
												<c:if test="${queryType=='H'}">
											...
											<c:if test="${ifHas eq 1}">		
												<ec:extendrow>
													...
													<td>&nbsp;</td><td>&nbsp;</td><td>&nbsp;</td>
													<c:if test="${queryType=='D'}">
														<td>&nbsp;</td><td align="right" style="font-weight:bold">${holdPlAll}</td>
													</c:if>
													<c:if test="${queryType=='H'}">
														<td>&nbsp;</td><td align="right" style="font-weight:bold">${holdPlAll}</td>
													</c:if>
