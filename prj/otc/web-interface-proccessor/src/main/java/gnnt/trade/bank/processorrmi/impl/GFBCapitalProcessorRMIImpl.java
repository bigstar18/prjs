...
import gnnt.trade.bank.GFBCapitalProcessor;
import gnnt.trade.bank.processorrmi.GFBCapitalProcessorRMI;
...
public class GFBCapitalProcessorRMIImpl
  extends UnicastRemoteObject
  implements GFBCapitalProcessorRMI
{
  private static final long serialVersionUID = -7531210850317443390L;
  GFBCapitalProcessor cp = new GFBCapitalProcessor();
  
  public GFBCapitalProcessorRMIImpl()
    throws Exception
  {}
    ...
    throws RemoteException
  {
    ...
    if (omm.firmID.equals("MarketIncomeOut"))
    {
      this.cp.log("----------交易市场资金划拨-----------");
      result = this.cp.outMoneyGS(omm.bankID, omm.money);
    }
    else
    {
      this.cp.log("----------交易商出金-----------");
