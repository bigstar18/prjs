...
import gnnt.trade.bank.processorrmi.HZCapitalProcessorRMI;
...
public class HZCapitalProcessorRMIImpl
  extends UnicastRemoteObject
  implements HZCapitalProcessorRMI
{
  private static final long serialVersionUID = 2L;
  HZCapitalProcessor cp = new HZCapitalProcessor();
  
  public HZCapitalProcessorRMIImpl()
  ...
  public void send(Date tradeDate)
    throws RemoteException
  {
    this.cp.log("-----------------------------------------send  适配器获取杭州清算");
    new HZExDataImpl().send(tradeDate);
    this.cp.log("-----------------------------------------send  适配器获取杭州清算");
  }
