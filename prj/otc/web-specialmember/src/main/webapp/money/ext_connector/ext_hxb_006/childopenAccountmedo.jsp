...
<%
	...
	//华夏银行他行签约定制>>>>>>>>>>>>取得开户行所在省份--end<<<<<<<<<<<
	HXBCapitalProcessorRMI cp = null;
	try{
		cp = getHXBBankUrl(bankID);
	}catch(Exception e){
		e.printStackTrace();
	}
	...
	if(message != null && message.equals("do")){
		...
		//String accountName1 = Tool.delNull(new String(request.getParameter("accountName1").getBytes("ISO-8859-1"),"GBK"));
		...
		//String RelatingAcctName = Tool.delNull(new String(request.getParameter("RelatingAcctName").getBytes("ISO-8859-1"),"GBK"));
		//String RelatingAcctBank = Tool.delNull(request.getParameter("RelatingAcctBank"));
		String RelatingAcctBank = Tool.delNull(request.getParameter("acctBankName"));
		//String RelatingAcctBank = Tool.delNull(new String(request.getParameter("acctBankName").getBytes("ISO-8859-1"),"GBK"));
		String RelatingAcctBankAddr = Tool.delNull(request.getParameter("RelatingAcctBankAddr"));
		//String RelatingAcctBankAddr = Tool.delNull(new String(request.getParameter("RelatingAcctBankAddr").getBytes("ISO-8859-1"),"GBK"));
		...
		//String PersonName = Tool.delNull(new String(request.getParameter("PersonName").getBytes("ISO-8859-1"),"GBK"));
		...
		try
		{
			retuenValue.result = cp.synchroAccount(corr).result;
		}
		catch(Exception e)
		{
			e.printStackTrace();
		...
		{
			msg = "交易商银行帐号签约成功！";
			//dao.changeBankID(bankID,contact);
		}
		else
		{
			msg = "交易商银行帐号签约失败！";
  ...
  <body style="overflow-y: hidden" onload="init()">
	...
	<div style="overflow:auto;height:485;">
	 <table border="0" width="95%" align="center">
		<tr>
		  <td>
			<div class="st_title">&nbsp;(华夏银行)交易所发起子账户签约</div>
			<table border="0" cellspacing="0" cellpadding="0" width="100%" class="st_bor">
				...
				<tr height="35">
					...
					<td align="left">
						<input name="accountName1" type=text maxlength="10" class="input_text" style="width: 150px"><span>*</span>				
					</td>
				</tr>
				<tr height="35">
				...
				<tr height="35">
					<td colspan="2">
						<!--table border="1" id="bankAddrAndNum"-->
							<div style="padding-left:38px" id="bankAddrAndNum"><div>
						<!--/table-->
					</td>
				</tr>
				<!--
				<tr height="35" id="TrForCompany" style="display: none;">
...
function doAdd()
{
	...
	else if(trim(frm.acctBankName.value) == "")
	{
		alert("请选开户行名称！");
