...
<html>
	<body>
		<form name="frm" method="post"
			...
			<div id="main_body">
				<table class="table1_style" border="0" cellspacing="0"
					cellpadding="0">
					<tr>
						<td><input type="hidden" name="roleId" value="${role.id }">
														...
														<td>
															<table border="0" cellspacing="0" cellpadding="0"
																...
																<tr>
																	...
																	<td class="table3_td2">
																		<input name="_userId[like]" type="text" class="text"
																			style="width: 150px;"
		...
		<div class="div_gn">
			<button id="addAs" class="anniu_btnmax" type="button" name="addbtn"
				onclick="addAssociation('${role.id}')">
				添加关联管理员
			</button>
			<button id="deleteAss" class="anniu_btnmax" type="button"
				name="bigbtn" onclick="deleteAssociation()">
				删除关联管理员
			</button>
						...
						<ec:table items="userSet"
							...
							<ec:row recordKey="${user.userId }">
								<ec:column cell="checkbox" headerCell="checkbox" alias="ids"
									value="${user.userId}" width="2%" viewsAllowed="html"
									style="text-align:center;" />
								<ec:column property="userId[like]" width="7%" title="用户代码"
									style="text-align:center; " value="${user.userId }">
								</ec:column>
								<ec:column property="name[Like]" width="7%" title="用户名称"
									style="text-align:center;" value="${user.name }" />
								<ec:column property="description[like]" width="7%" title="用户描述"
									style="text-align:center;" value="${user.description }" />
							</ec:row>
...
<SCRIPT LANGUAGE="JavaScript">
	function addAssociation(id)
	{
		...
		if(checkie()){
			result=ecsideDialog(url,"",550,600);
		}else{
			result=ecsideDialog(url,"",550,550);
		}
		if(result>0)
		{
			queryInfo();
