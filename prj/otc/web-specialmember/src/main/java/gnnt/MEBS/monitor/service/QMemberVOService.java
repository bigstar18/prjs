  ...
  extends BaseService
{
  ...
  {
    String sql = "select distinct m.memberno, m.name,r.s_memberno from m_memberinfo m, t_compmember t, m_memberrelation r where m.memberno=t.m_firmid(+) and m.memberno=r.memberno(+) and t.status in ('N','F') and " + condition + " order by m.memberno";
