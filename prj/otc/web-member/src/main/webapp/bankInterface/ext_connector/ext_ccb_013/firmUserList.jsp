...
if(firmid != null && firmid.trim().length()>0){
	filter1 += " and fbf.firmid  like '"+firmid.trim()+"%' ";
}
if(card != null && card.trim().length()>0){
	filter1 += " and fbf.card='"+card.trim()+"' ";
}
...
Vector dicList1 = cp.getFirmUserList(filter1,pageinfo,"member","CCBAc",CCBbankID);
			...
			for(int i=0;i<dicList1.size();i++) {
				...
				<tr align="center"  onclick="selectTr();" height="22">
					...
					<%if(firmer.firminfo.value!=null&&firmer.firminfo.value.length()>0&&firmer.firminfo.bankid!=null&&firmer.firminfo.bankid.equals(CCBbankID)){
						out.print("<a href='#' onclick='modFirm(this)' value='"+firmer.firmID+"'>"+firmer.firminfo.value+"</a>");
					}else{
						out.print("<a href='#' onclick='modFirm(this)' value='"+firmer.firmID+"'>新增</a>");
					}
					%>
					</td>
...
function modFirm(firmID){
	var result=window.showModalDialog("synchroCorr.jsp?firmID="+firmID.value,"","dialogWidth=530px; dialogHeight=500px; status=yes;scroll=yes;help=no;");
	if(result){
		frm.action="#";
		frm.submit();
	}
