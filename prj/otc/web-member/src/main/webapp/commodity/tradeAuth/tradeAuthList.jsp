...
<html>
	...
	<body>
			...
			<table class="table1_style" border="0" cellspacing="0" cellpadding="0">
				<tr>
					<td>
						<div class="div_tj">
						<form name="frm"
								action="${basePath}/commodity/tradeAuth/list.action?sortName=primary.commodityId"
								method="post">
								...
								<tr>
									<td>
										<ec:table items="resultList"
											...
											var="tradeAuth"
											action="${basePath}/commodity/tradeAuth/list.action?sortName=primary.commodityId" title=""
											...
											<ec:row recordKey="${tradeAuth.firmId}">
												...
												<ec:column property="commodity.name[Like]" title="商品名称" width="8%" style="text-align:left;" ellipsis="true"> <a href="#" class="blank_a" onclick="return update('${tradeAuth.firmId}','${tradeAuth.commodityId}');"><font color="#880000">${tradeAuth.commodityName }</font> </a> </ec:column>
												<ec:column property="primary.firmId[like]" title="类型" width="7%"
													style="text-align:left;" editTemplate="ecs_t_firmDis">
							  						${firmDisMap[tradeAuth.firmId]}
							  					</ec:column>
		...
		<!-- 编辑和过滤所使用的买入市建模板 -->
		<textarea id="ecs_m_S_Open" rows="" cols="" style="display: none">
		<select onblur="ECSideUtil.updateEditCell(this)" style="width: 100%;"
				name="primary.ecs_m_S_Open[=][int]">
			<ec:options items="authorityMap" />
		</select>
	    </textarea>
		<!-- 编辑和过滤所使用的买入市平模板 -->
		...
		function update(firmId,commodityId){
			var url="${basePath}/commodity/tradeAuth/forwardUpdate.action?obj.commodityId="+commodityId+"&obj.firmId="+firmId;
			ecsideDialog(url,"",580,620);
		}
