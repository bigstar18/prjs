									...
									<td>
										<ec:table items="resultList"
											...
											<ec:row recordKey="${user.userId}">
												...
												<ec:column property="organization.name[like][String]" width="8%"
													title="所属机构" style="text-align:left;" tipTitle="${user.organizationName}" sortable="false">
												...
												<ec:column property="isForbid[=]" width="8%" title="是否禁用"
												...
												<ec:column property="_1" width="8%" title="机构设置"
													style="text-align:center;" filterable="false" sortable="false">
													<a href="javascript:addOrganization('${user.userId }');"
														class="blank_a"><font color="#880000">设置</font></a>
												</ec:column>
												<ec:column property="_1" width="8%" title="关联角色"
													style="text-align:center;" filterable="false" sortable="false">
													<a href="javascript:relatedRight('${user.userId }');"
														class="blank_a"><font color="#880000">设置</font></a>
												</ec:column>
												<ec:column property="_2" width="8%" title="密码修改"
													style="text-align:center;" filterable="false" sortable="false">
...
<SCRIPT LANGUAGE="JavaScript">
	...
	function modPwd(userId){
	  ...
	  if(checkie()){
		  	result=ecsideDialog(url,null,580,375);
	  	}else{
	  		result=ecsideDialog(url,null,580,325);
	  	}
	  if(result)
		{		
			queryInfo();
	...
	function addOrganization(userId){
		var url="${basePath}/user/forRightTree.action?userId="+userId;
		//ec.action=url;
		//ec.submit();
		ecsideDialog(url,"",430,520);
	}
	//关联用户角色
