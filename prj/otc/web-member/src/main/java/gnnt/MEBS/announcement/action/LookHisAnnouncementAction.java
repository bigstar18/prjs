  ...
  extends BaseAction
{
  ...
  public String list()
  {
    ...
    if (((MemberInfo)this.memberInfoService.getById(AclCtrl.getUser(this.request).getMemberNo())).getCreateTime() != null) {
      qc.addCondition("notice.expiryTime", ">=", ((MemberInfo)this.memberInfoService.getById(AclCtrl.getUser(this.request).getMemberNo())).getCreateTime(), "date");
    }
    if ((this.request.getSession().getAttribute(ActionConstant.ORGANIZATIONID) != null) && (!"".equals(this.request.getSession().getAttribute(ActionConstant.ORGANIZATIONID)))) {
      this.resultList = this.hisNoticeService.getOrganizationOKNoticeList(qc, pageInfo, AclCtrl.getUser(this.request).getId());
    } else {
      this.resultList = this.hisNoticeService.getMemberOKNoticeList(qc, pageInfo, AclCtrl.getUser(this.request).getId());
    }
