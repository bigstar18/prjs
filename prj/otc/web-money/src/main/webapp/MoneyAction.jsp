...
<%	
	...
	if(cp==null) {
		results="调用Rmi错误";
	}else if(Double.parseDouble("10000000000")<=Double.parseDouble(money)){
		results="单笔出入金范围超出限制";
	}else {
		boolean flag = true;
		String filter = " ";
		...
		if(flag){
			if(!showPasswordBank(inoutMoney,bankID,cv.cardType)){
				result = cp.isPassword((String)session.getAttribute("FIRMID"),pwd);
				if(result == 1){
					flag = false;
						results="请先设置密码";
				}else if(result == -1){
		...
		if(flag){
			if((""+ProcConstants.outMoneyType).equals(inoutMoney)) {
				...
				omm.bankID = bankID;
				omm.firmID = (String)session.getAttribute("FIRMID");
				omm.trader = (String)session.getAttribute("FIRMID")+"_trader";
				omm.account = acount;
				...
				imm.firmID = (String)session.getAttribute("FIRMID");
				imm.trader = (String)session.getAttribute("FIRMID")+ "_trader";
				imm.bankID = bankID;
				imm.account = acount;
...
<SCRIPT LANGUAGE="JavaScript">
	window.location = "Money.jsp?fid=<%=fid%>&sid=<%=sid%>";
</SCRIPT>