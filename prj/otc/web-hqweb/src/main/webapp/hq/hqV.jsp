     ...
     +"  a.LowPrice,												 "
      +" a.CurPrice,												 "
       ...
       +" a.CreateTime,												 "
        ...
        +"c.CURPRICE_B,											 "
        +"c.CURPRICE_S        										 "
   +"from T_Quotation a, T_Commodity b,T_C_Quotation_RT c,			 "
   +"(select commodityid, closeprice from t_quotation_h h where h.cleardate = (select max(cleardate) from t_quotation_h)) LastClose "
  +"where b.Status = 1												 "
    +"and a.CommodityID = b.CommodityID and a.commodityid= c.commodityid and a.commodityid <> 'Ag15kg' and a.commodityid <> 'Ag100kg'";
	...
	//		out.print(df.format(rs.getDouble("BalancePrice"))+",");
			out.print(df.format(rs.getDouble("TotalMoney"))+",");            //总成交额
			out.print(rs.getInt("TotalAmount")+",");                         //总成交量
...
//			out.print(df.format(rs.getDouble("YESTERBALANCEPRICE"))+",");			//结算价
			...
			out.print(rs.getString("CreateTime"));
//			out.print(1+",");	//点差
//			out.print(1+",");	//涨跌幅
