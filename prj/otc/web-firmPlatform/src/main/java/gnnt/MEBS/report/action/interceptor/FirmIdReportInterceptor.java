...
public class FirmIdReportInterceptor
  extends AbstractInterceptor
{
  private final transient Log logger = LogFactory.getLog(FirmIdReportInterceptor.class);
  @Autowired
    ...
    throws Exception
  {
    ...
    if ("M".equals(type)) {
      request.setAttribute(ActionConstant.GNNT_ + "primary.firmId[=][String]", firm.getFirmId());
    } else {
      request.setAttribute(ActionConstant.GNNT_ + "primary.firmId[=]", trader.getFirmID());
    }
    this.logger.debug("primary.firmId[=]:" + request.getSession().getAttribute("username"));
