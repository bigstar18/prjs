  ...
  extends JdbcDaoSupport
{
  ...
  {
    String sql = "insert into customregistercheck(monicustomno) values('" + moNiCustomNo + "')";
    return getJdbcTemplate().update(sql);
  }
  
  public List checkCustomNo(String moNiCustomNo)
  {
    String sql = "select t.monicustomno from customregistercheck t where t.monicustomno='" + moNiCustomNo + "'";
  ...
  {
    String sql = "insert into f_b_firmidandaccount(BANKID,FIRMID,ACCOUNT,ACCOUNTNAME,ISOPEN,CARDTYPE,CARD) values (?,?,?,?,0,?,?)";
    Object[] params = { bankid, firmId, bankAccout, name, carType, cardNumber };
  ...
  {
    String sql = "SELECT t.memberno,t.name from m_memberinfo t where t.memberno='" + memberNo + "' ";
    List<Map> list = getJdbcTemplate().queryForList(sql);
    Map map = new HashMap();
    if ((list != null) && (list.size() > 0)) {
      map = (Map)list.get(0);
  ...
  {
    String sql = "select t.* from WX_AGREEMENT t";
    return getJdbcTemplate().queryForList(sql);
  }
  
  public List getFrimBindingInfo(String wxNo)
  {
    String sql = "select * from m_customerinfo s where s.DemiseTime  is null and wxno=? ";
    Object[] params = { wxNo };
    int[] types = { 12 };
  ...
  public boolean checkedCardNumbers(String cardNumber)
  {
    cardNumber = cardNumber.replace("x", "X");
    String sql = "select paperscode from m_customerinfo  where paperscode=? ";
  ...
  {
    return getJdbcTemplate().queryForList("select * from f_b_banks");
  }
  ...
  public int firmSign(String customerNo)
  {
    DoneFirmSign func = new DoneFirmSign(getDataSource());
    Map inputs = new HashMap();
  ...
  private class DoneFirmSign
    extends StoredProcedure
  {
    private static final String SFUNC_NAME = "SP_M_DoneFirmSign";
    
    public DoneFirmSign(DataSource ds)
    {
      super("SP_M_DoneFirmSign");
  ...
  public void addCustomer(Customer obj)
  {
    ...
    int[] dataTypes = { 12, 12, 
      ...
      12, 12, 12, 12, 
      12, 12 };
    System.out.println("sql++:" + sql + obj.getCustomerNo() + obj.getName() + 
      obj.getMemberNo() + obj.getPapersType() + obj.getPapersName() + 
      obj.getPhonePWD() + obj.getAddress() + obj.getPhone() + 
      obj.getFax() + obj.getPostCode() + obj.getEmail() + 
      obj.getExtendData());
    getJdbcTemplate().update(sql, params, dataTypes);
