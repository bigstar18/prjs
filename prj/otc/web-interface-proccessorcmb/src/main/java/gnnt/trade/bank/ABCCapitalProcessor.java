...
public class ABCCapitalProcessor
  extends CapitalProcessor
{
  ...
  private long handleOUtMoney(Connection conn, String bankID, String trader, double money, String firmID, String contact, String account, String funID, int type, long actionid)
    throws SQLException
  {
    log("检查交易账号是否符合出金条件,符合条件进行出金处理,记录流水 handleOUtMoney bankID[" + bankID + 
      "]trader[" + trader + "]money[" + money + "]firmID[" + firmID + 
      "]contact[" + contact + "]account[" + account + "]funID[" + 
      funID + "]type[" + type + "]");
    long result = -20042L;
    try
    {
      ...
      if ((funID != null) && (funID.trim().length() > 0))
      {
        Vector<CapitalValue> cv = DAO
          ...
          " and bankID='" + 
          bankID + 
          "' and createtime>=trunc(sysdate) and createtime<trunc(sysdate)+1 and funID='" + 
          funID + "'", conn);
    ...
    try
    {
      ...
      if (rows > 2)
      {
        for (int i = 1; i < rows - 1; i++)
        {
          ...
          {
            conn = DAO.getConnection();
            Vector<CapitalValue> list = DAO
              .getCapitalInfoList(" and bankID='" + 
              bankID.trim() + "' and funID='" + 
              val[1] + "' ", conn);
            for (int a = 0; a < list.size(); a++)
            {
              ...
              Vector<CorrespondValue> vcv = DAO
                .getCorrespondList(" and firmID='" + 
                cv.firmID + "' and bankID='" + 
                bankID.trim() + "'", conn);
              if ((vcv != null) || (vcv.size() >= 0)) {
                for (int b = 0; b < vcv.size(); b++)
                {
                  CorrespondValue cp = 
                    (CorrespondValue)vcv.get(b);
          ...
          moneyInfoValue.compareDate = new java.sql.Date(
            Tool.strToDate1(val[0]).getTime());
          if (moneyInfoValue.compareDate == null) {
            moneyInfoValue.compareDate = new java.sql.Date(date
              .getTime());
          }
