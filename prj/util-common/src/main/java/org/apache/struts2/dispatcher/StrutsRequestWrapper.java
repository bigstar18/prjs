...
public class StrutsRequestWrapper extends HttpServletRequestWrapper {
    ...
    public Object getAttribute(String key) {
        ...
        if (ctx != null && attribute == null) {
			...
			// 在struts源码基础上增加一下代码；
			// 如果仍未获取属性则从super.getAttribute("struts.valueStack")中继续查找
			if (attribute == null) {
				ValueStack vs = (ValueStack) super
				...
				if (vs != null) {
					...
					// logger.error("ValueStackkey=" + key);
					// logger.error("ValueStackvalue=" + map.get(key));
					//
					// }
					attribute = vs.findValue(key);
				}
			}
            }
        }
