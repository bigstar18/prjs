...
(function($){
	...
	_view = {
		...
		addRemoveBtn: function(setting, node) {//删除操作触发的事件
			...
			if(node.id!=-1&&node.treeType=="cate"){
			var aObj = $("#" + node.tId + consts.id.A);
			removeStr = "<button type='button' class='remove' id='" + node.tId + consts.id.REMOVE + "' title='删除分类' treeNode"+consts.id.REMOVE+" onfocus='this.blur();' onclick='dele("+node.id+",\""+node.name+"\");' style='display:none;'></button>";
			aObj.append(removeStr);
			$("#" + node.tId + consts.id.REMOVE).show();
			}
			if(node.type=="pro"){
				var aObj = $("#" + node.tId + consts.id.A);
				removeStr = "<button type='button' class='remove' id='" + node.tId + consts.id.REMOVE + "' title='删除品名' treeNode"+consts.id.REMOVE+" onfocus='this.blur();' onclick='dele("+node.id+",\""+node.name+"\");' style='display:none;'></button>";
				aObj.append(removeStr);
				$("#" + node.tId + consts.id.REMOVE).show();
			}
		},
