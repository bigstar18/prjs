	...
	<body class="leftframe" onLoad="start();" onselectstart="return false">
		...
		<div id="div_Drawer">
			...
			<div id="div_Container" align="left">
				...
				<div class="leftMenu_div">
					<div id="div_drw_Panel" align="left" curDrwNo="0"
						...
						<a href="#" onclick="setFastMenu();"><span
							style="font-size: 12; text-decoration: none; color: #336699;" curDrwNo="0">修改设置</span>
						</a>
					</div>
					...
					<div id="tree">
						<c:forEach items="${myMenuList}" var="myMenu">
							<c:set var="icoPath" value="${skinPath}/image/frame/menu/29_29.gif"/><%//设置我的菜单默认图片路径 %>
							<c:if test="${not empty myMenu.right.icon}">
							<c:set var="icoPath" value="${menuPicPath}${myMenu.right.icon}"/><%//如果数据库中配置了本菜单的图标，则采用数据库中配置的图标名称 %>
							</c:if>
							<c:set var="urlPath"
				...
				<c:forEach var="menu" items="${HaveRightMenu.childMenuSet}"><%//遍历抽屉菜单 %>
					...
					<c:if test="${mainMenu.parentID==RootRightID}">
						...
						<div id="div_drw_Panel" align="left"
							...
							<c:set var="icoPath" value="${skinPath}/image/frame/menu/06.gif"/><%//设置抽屉菜单默认图标 %>
							<c:if test="${not empty mainMenu.icon}">
							<c:set var="icoPath" value="${menuPicPath}${mainMenu.icon}"/><%//如果数据库中配置了抽屉菜单的图标，则采用数据库中配置的图标名称 %>
							</c:if>
						...
						<div id="tree">
							...
							<c:forEach var="firstMenu" items="${firstMenu}"><%//循环遍历抽屉菜单下的一级菜单 %>

							<c:set var="currentMenuIoc" value="${skinPath}/image/frame/menu/42_42.gif"/><%//设置一级菜单默认图标 %>
							<c:if test="${not empty firstMenu.icon}">
								<c:set var="currentMenuIoc" value="${menuPicPath}${firstMenu.icon}"/><%//如果数据库中配置了一级菜单的图标，则采用数据库中配置的图标名称 %>
							</c:if>
							...
							<script type="text/javascript">
								...
								<c:forEach var="secondMenu" items="${secondMenu}"><%//遍历一级菜单下的二级菜单 %>

								<c:set var="currentMenuIoc" value="${skinPath}/image/frame/menu/gif_07.gif" /><%//设置二级菜单默认图标 %>
								<c:if test="${not empty secondMenu.icon }">
									<c:set var="currentMenuIoc" value="${menuPicPath }${secondMenu.icon}" /><%//如果数据库中配置了二级菜单的图标，则采用数据库中配置的图标名称 %>
								</c:if>
