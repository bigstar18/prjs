	...
	<head>
		...
		<!--
			...
			function changePwd(id){
				//var url = "<%=basePath %>/self/passwordSelfMod.action?userId="+id;
				var url = "<%=basePath %>/broker/frame/modpasswordself.jsp";
			...
			jQuery(document).ready(function() {
				 ...
				 */
				 $('#skinStyle').click(function(){
					...
					if(showDialog(url, "", 500, 300)){
						changeStyle();
						window.location="${basePath}/broker/frame/topframe.jsp";
					}
				});
					//通知其它系统修改风格
...
.dock-container {
					...
					<div class="dock" id="dock" align="right">
						<div class="anniu_kuangCopy">
							<table>
								<tr>
									<td>
										<c:if test="${CurrentUser.type==0}">会员代码：${CurrentUser.broker.brokerId }</c:if>
										<c:if test="${CurrentUser.type==4}">居间商代码:${CurrentUser.brokerAge.brokerAgeId }</c:if>
									</td>
								</tr>
								<tr>
									<td>
										<c:if test="${CurrentUser.type==0}">会员名称：${CurrentUser.broker.name }</c:if>
										<c:if test="${CurrentUser.type==4}">居间商名称:${CurrentUser.brokerAge.name }</c:if>
									</td>
								</tr>
								<tr>
									<td>
	...
	var leftWidth = 350;<%//左侧会员管理占据的宽度 %>
	var rightWidth = 550;<%//右侧修改密码、帮助、退出按钮占据的宽度 %>
	var width = document.documentElement.scrollWidth - leftWidth - rightWidth - 29;<%//获取当前浏览器宽度去掉左右宽度后剩余的宽度 %>
