...
public class CEBBankDaoImpl extends CEBBankDao {
	public CEBBankDaoImpl() throws Exception {
	}
	...
	public Date getlastDate(Date date, Connection conn) throws SQLException {
		Date result = null;
	...
	public Vector<FirmRightsValue> getTradeDataMsg(String bankId, String firmid, Date qdate) throws SQLException, ClassNotFoundException {
	...
	public FirmUserValue getFirmUserValue(String contact) throws SQLException, ClassNotFoundException {
		...
		try {
			conn = getConnection();
			if (contact != null) {
				sql = " select t.account account,t.accountname accountname,fbf.cardtype cardtype,fbf.card card  from f_b_firmidandaccount t, f_b_firmuser fbf  where t.firmid = fbf.firmid and fbf.contact = "
						+ contact;
				state = conn.prepareStatement(sql);
				rs = state.executeQuery();
				if (rs.next()) {
	...
	public Vector<FirmUserValue> getFirmUserList2(String filter, int[] pageinfo, String bankId, String strKey)
		...
		try {
			...
			if ((filter != null) && (filter.indexOf("organizationno") >= 0)) {
				sql = "select fbf.firmID firmid,fbf.contact contact,fbf.firmName firmname,fbf.cardType cardtype,fbf.card card,fbf.status status,mf.firmType firmType,fbfa.account account,fbfa.isopen isopen,case when mf.firmtype<>'C' then fbf.firmid else (select nvl(memberno,'') from m_customerinfo mc where mc.customerno=fbf.firmid) end belevemember  from f_b_firmidandaccount fbfa,f_b_firmuser fbf,m_firm mf,v_customerrelateorganization mc where fbf.firmid=mf.firmid(+)  and mc.customerno=fbf.firmid and fbfa.firmid = fbf.firmid "
						+ filter;
			} else if ((filter != null) && (filter.indexOf("memberno") >= 0)) {
				sql = "select fbf.firmID firmid,fbf.contact contact,fbf.firmName firmname,fbf.cardType cardtype,fbf.card card,fbf.status status,mf.firmType firmType, fbfa.isopen isopen,fbfa.account account,case when mf.firmtype<>'C' then fbf.firmid else (select nvl(memberno,'') from m_customerinfo mc where mc.customerno=fbf.firmid) end belevemember  from f_b_firmidandaccount fbfa,f_b_firmuser fbf,m_firm mf,m_customerinfo mc where fbf.firmid=mf.firmid(+) and  mc.customerno=fbf.firmid and fbfa.firmid = fbf.firmid "
						+ filter;
			} else {
				sql = "select fbf.firmID firmid,fbf.contact contact,fbf.firmName firmname,fbf.cardType cardtype,fbf.card card,fbf.status status,mf.firmType firmType ,fbfa.isopen isopen, fbfa.account account,case when mf.firmtype<>'C' then fbf.firmid else (select nvl(memberno,'') from m_customerinfo mc where mc.customerno=fbf.firmid) end belevemember  from f_b_firmidandaccount fbfa , f_b_firmuser fbf,m_firm mf  where fbf.firmid=mf.firmid(+) and fbfa.firmid = fbf.firmid "
						+ filter;
			}
