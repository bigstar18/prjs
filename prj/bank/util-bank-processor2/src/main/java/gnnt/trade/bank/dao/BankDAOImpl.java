...
public class BankDAOImpl extends BankDAO {
	...
	public FirmBalanceValue availableBalance(String filter, Connection conn) throws SQLException {
		...
		String sql = null;
		try {
			sql = "select firmID,funds balance,runtimefundio,lastbalance,nvl(FN_F_CanOutFunds(firmID,0,bankcode),0) canOutFunds from f_firmfunds_bank t where 1=1 "
					+ filter;
			log("sql:" + sql);
			state = conn.prepareStatement(sql);
			rs = state.executeQuery();
	...
	public Vector<CorrespondValue> getCorrespondList2(String filter, int[] pageinfo) throws SQLException, ClassNotFoundException {
		...
		try {
			...
			if ((filter != null) && (filter.indexOf("organizationno") >= 0)) {
				...
				"select fbf.*,fbb.bankname name,mf.firmType firmType,ff.bankcode mainBank ,case when mf.firmtype<>'C' then fbf.firmid else (select nvl(memberno,'') from m_customerinfo mc where mc.customerno=fbf.firmid) end belevemember  from f_b_firmidandaccount fbf,f_b_banks fbb,m_firm mf,F_FirmFunds ff,v_customerrelateorganization mc  where fbf.bankid=fbb.bankid(+) and fbf.firmid=mf.firmid(+) and fbf.firmid=ff.firmid(+) and mc.customerno=fbf.firmid "
						+ filter;
			} else if ((filter != null) && (filter.indexOf("memberno") >= 0)) {
				sql =

				"select fbf.*,fbb.bankname name,mf.firmType firmType,ff.bankcode mainBank ,case when mf.firmtype<>'C' then fbf.firmid else (select nvl(memberno,'') from m_customerinfo mc where mc.customerno=fbf.firmid) end belevemember  from f_b_firmidandaccount fbf,f_b_banks fbb,m_firm mf,F_FirmFunds ff,m_customerinfo mc  where fbf.bankid=fbb.bankid(+) and fbf.firmid=mf.firmid(+) and fbf.firmid=ff.firmid(+) and mc.customerno=fbf.firmid "
						+ filter;
			} else {
				sql =

				"select fbf.*,fbb.bankname name,mf.firmType firmType,ff.bankcode mainBank ,case when mf.firmtype<>'C' then fbf.firmid else (select nvl(memberno,'') from m_customerinfo mc where mc.customerno=fbf.firmid) end belevemember  from f_b_firmidandaccount fbf,f_b_banks fbb,m_firm mf,F_FirmFunds ff  where fbf.bankid=fbb.bankid(+) and fbf.firmid=mf.firmid(+) and fbf.firmid=ff.firmid(+) "
						+ filter;
			}
			log("sql:" + sql);
			rs = PageQuery.executeQuery(conn, state, rs, sql, pageinfo);
	...
	public Vector<FirmBankMsg> getfirmBankMsg(String filter) throws SQLException, ClassNotFoundException {
		...
		try {
			...
			"select account.firmID firmID,account.contact contact,nvl(account.account,'') account ,nvl(account.accountName,'') accountName,nvl(account.account1,'') account1,nvl(account.frozenfuns,0) frozenfuns ,account.bankID bankID,nvl(bank.bankName,'') bankName,nvl(fund.bankCode,'') mainBank ,nvl(fundbank.TransferFund,0) transferFund,nvl(fundbank.Funds,0) funds,nvl(FN_F_CanOutFunds(account.firmID,0,account.bankID),0) canOutMoney from f_b_firmIDandAccount account,f_b_banks bank,F_FirmFunds_Bank fundbank,F_FirmFunds fund where account.bankID=bank.bankID(+) and account.bankID=fundbank.bankCode(+) and account.firmID=fundbank.firmID(+) and account.firmID=fund.firmID(+) and account.bankID is not null "
					+ filter + " order by account.firmID,account.bankID ";
			conn = getConnection();
			state = conn.prepareStatement(sql);
			rs = state.executeQuery();
			while (rs.next()) {
	...
	public int modCorrespondStatus(CorrespondValue val) throws SQLException, ClassNotFoundException {
		...
		try {
			conn = getConnection();
			sql = "update F_B_firmidandaccount set STATUS=(case when ISOPEN=1 then " + val.status + " else 1 end) where 1=1 and (firmID='"
					+ val.firmID + "' and CONTACT='" + val.contact + "') and bankID='" + val.bankID + "'";
			state = conn.prepareStatement(sql);
			result = state.executeUpdate();
		} catch (SQLException e) {
			throw e;
	...
	public int useBank(String bankID, int type) throws SQLException, ClassNotFoundException {
	...
	public Vector<CapitalValue> getCapitalInfoList2(String filter, int[] pageinfo) throws SQLException, ClassNotFoundException {
		...
		try {
			conn = getConnection();
			if ((filter != null) && (filter.indexOf("organizationno") >= 0)) {
				sql = "select fbc.*,mf.firmtype firmType,fbf.account account,fbf.accountName accountName,fbb.bankname bankName from f_b_capitalinfo fbc,m_firm mf,f_b_firmidandaccount fbf,f_b_banks fbb,v_customerrelateorganization mc where fbc.bankid=fbb.bankid(+) and fbc.bankid=fbf.bankid and fbc.firmid=mf.firmid(+) and fbc.firmid=fbf.firmid(+) and mc.customerno=fbf.firmid "
						+ filter;
			} else if ((filter != null) && (filter.indexOf("memberno") >= 0)) {
				sql = "select fbc.*,mf.firmtype firmType,fbf.account account,fbf.accountName accountName,fbb.bankname bankName from f_b_capitalinfo fbc,m_firm mf,f_b_firmidandaccount fbf,f_b_banks fbb,m_customerinfo mc where fbc.bankid=fbb.bankid(+) and fbc.bankid=fbf.bankid and fbc.firmid=mf.firmid(+) and fbc.firmid=fbf.firmid(+) and mc.customerno=fbf.firmid "
						+ filter;
			} else {
				sql = "select fbc.*,mf.firmtype firmType,fbf.account account,fbf.accountName accountName,fbb.bankname bankName from f_b_capitalinfo fbc,m_firm mf,f_b_firmidandaccount fbf,f_b_banks fbb where fbc.bankid=fbb.bankid(+) and fbc.bankid=fbf.bankid and fbc.firmid=mf.firmid(+) and fbc.firmid=fbf.firmid(+) "
						+ filter;
			}
		...
		try {
			...
			if ((filter != null) && (filter.indexOf("organizationno") >= 0)) {
				sql = "select count(*) rowcount,sum(fbc.money) money,fbc.type type,fbc.status status from f_b_capitalinfo fbc,m_firm mf,f_b_firmidandaccount fbf,f_b_banks fbb,v_customerrelateorganization mc where fbc.bankid=fbb.bankid(+) and fbc.bankid=fbf.bankid and fbc.firmid=mf.firmid(+) and fbc.firmid=fbf.firmid(+) and mc.customerno=fbf.firmid "
						+ filter + "  group by fbc.type,fbc.status ";
			} else if ((filter != null) && (filter.indexOf("memberno") >= 0)) {
				sql = "select count(*) rowcount,sum(fbc.money) money,fbc.type type,fbc.status status from f_b_capitalinfo fbc,m_firm mf,f_b_firmidandaccount fbf,f_b_banks fbb,m_customerinfo mc where fbc.bankid=fbb.bankid(+) and fbc.bankid=fbf.bankid and fbc.firmid=mf.firmid(+) and fbc.firmid=fbf.firmid(+) and mc.customerno=fbf.firmid "
						+ filter + "  group by fbc.type,fbc.status ";
			} else {
				sql = "select count(*) rowcount,sum(fbc.money) money,fbc.type type,fbc.status status from f_b_capitalinfo fbc,m_firm mf,f_b_firmidandaccount fbf,f_b_banks fbb where fbc.bankid=fbb.bankid(+) and fbc.bankid=fbf.bankid and fbc.firmid=mf.firmid(+) and fbc.firmid=fbf.firmid(+) "
						+ filter + "  group by fbc.type,fbc.status ";
			}
			log("sql:" + sql);
			state = conn.prepareStatement(sql);
	...
	public Vector<InterfaceLog> interfaceLogList(String filter, int[] pageinfo) throws SQLException, ClassNotFoundException {
		...
		String sql = "select fbi.*, fbf.firmID firmID, fbb.bankName bankName from F_B_INTERFACELOG fbi, F_B_FIRMUSER fbf, F_B_BANKS fbb where trim(fbi.CONTACT) = trim(fbf.CONTACT(+)) and trim(fbi.bankID) = trim(fbb.bankID) "
				+ filter;
		try {
			conn = getConnection();
			rs = PageQuery.executeQuery(conn, state, rs, sql, pageinfo);
	...
	public Vector<CompareSumMoney> sumCompareMoney(String bankID, java.util.Date date) throws SQLException, ClassNotFoundException {
		...
		try {
			...
			conn = getConnection();
			String bank = "select count(*) rowsize,nvl(sum(info.money),0) money,info.type type,info.bankid bankID,bank.bankname bankname,1 mb from f_b_bankcompareinfo info,f_b_banks bank where info.bankid=bank.bankid(+) and info.status=0 and info.bankid='"
					+ bankID + "' and trunc(info.comparedate)=to_date('" + Tool.fmtDate(date)
					+ "','yyyy-MM-dd') group by info.bankID,info.type,bank.bankname ";
			String market = "select count(*) rowsize,nvl(sum(info.money), 0) money,info.type type, info.bankid bankID, bank.bankname bankname, 2 mb from f_b_capitalinfo info, f_b_banks bank where info.bankid = bank.bankid(+) and info.status=0 and info.type in (0,1) and info.bankid = '"
					+ bankID + "' and trunc(info.banktime) = to_date('" + Tool.fmtDate(date)
					+ "', 'yyyy-MM-dd') group by info.bankID, info.type, bank.bankname ";
	...
	public Vector<CapitalCompare> sumResultInfo(String bankID, String[] firmIDs, java.util.Date date) throws SQLException, ClassNotFoundException {
		...
		try {
			...
			"select nvl(a.inmoney,0) minmoney,nvl(a.outmoney,0) moutmoney,nvl(b.inmoney,0) binmoney,nvl(b.outmoney,0) boutmoney,c.firmid firmid,case when a.tradedate is null then b.tradedate else a.tradedate end tradedate,case when a.bankid is null then b.bankid else a.bankid end bankid from (select nvl(sum(case when type=0 then money else 0 end),0) inmoney,nvl(sum(case when type=1 then money else 0 end),0) outmoney,bankid,trim(CONTACT) firmid,trunc(banktime) tradedate from f_b_capitalinfo t where banktime is not null and status=0 "
					+ filter2 + " group by trunc(banktime),bankid,firmid,trim(CONTACT) order by trunc(banktime) desc,bankid,firmid) a, "
					+ "(select nvl(sum(case when type=" + 0 + " then money else 0 end),0) inmoney,nvl(sum(case when type=" + 1
					+ " then money else 0 end),0) outmoney,trunc(comparedate) tradedate,trim(firmid) firmid,bankid from f_b_bankcompareinfo t where status="
					+ 0 + " " + filter3 + " group by trunc(comparedate),bankid,firmid order by trunc(comparedate) desc,bankid,firmid) b, "
					+ "(select distinct firmid,tradedate from "
					+ "(select distinct trim(CONTACT) firmid,trunc(banktime) tradedate from f_b_capitalinfo where 1=1 and status=" + 0 + " " + filter2
					+ "union  " + "select distinct trim(firmid) firmid,trunc(comparedate) tradedate from f_b_bankcompareinfo where 1=1 and status="
					+ 0 + " " + filter3 + ")where firmid is not null " + "order by firmid) c "
	...
	public Vector<FirmFundsBankValue> getFirmFundsBank(String firmID) throws SQLException, ClassNotFoundException {
		...
		try {
			...
			firmID + "' and ff.firmid='" + firmID + "' and fbf.firmid='" + firmID + "' " + " order by fbf.opentime ";
			state = conn.prepareStatement(sql);
			rs = state.executeQuery();
	...
	public Vector<FirmValue> getFirmList3(String filter, String type, String key, String bankid) throws SQLException, ClassNotFoundException {
		log("===>>>取得交易账号信息列表   getFirmList  ");
		Vector<FirmValue> list = new Vector();
		Connection conn = null;
		PreparedStatement state = null;
		...
		try {
			...
			if (type.equals("member")) {
				...
				"select fbf.* ,fbfi.value,fbfi.key,fbfi.bankid from f_b_firmuser fbf,m_customerinfo mc,(select * from f_b_firminfo where bankid='"
						+ bankid + "' and key='" + key + "') fbfi where mc.customerno=fbf.firmid " + " and fbfi.firmid(+)=fbf.firmid " + filter;
			} else if (type.equals("Backstage")) {
				sql =

				"select fbfu.*,fbfi.value,fbfi.key,fbfi.bankid from F_B_FirmUser fbfu,(select * from f_b_firminfo where bankid='" + bankid
						+ "' and key='" + key + "') fbfi where 1=1 and " + "fbfi.firmid(+)=fbfu.firmid " + filter;
			} else {
				return list;
			}
			log(sql);
	...
	public Map<String, Double> getAllVirtualFunds(String str, Connection conn) throws SQLException {
	...
	public java.util.Date getMaxvirtualfunds(String bankID, java.util.Date date, Connection conn) throws SQLException {
		log("查询虚拟资金日期表");
	...
	public Map<String, Double> getQsdateVirtualFunds(java.util.Date tradeDate, Connection conn) throws SQLException {
	...
	public int addDateVirtualFunds(DateVirtualFunds dv, Connection conn) throws SQLException {
		log("添加虚拟资金表");
		int result = 0;
		PreparedStatement state = null;
		ResultSet rs = null;
		String sql = null;
		try {
			state = conn.prepareStatement("select * from f_b_virtualfunds where to_char(qsDate,'yyyy-MM-dd')='" + Tool.fmtDate(dv.createDate)
					+ "' and contact = '" + dv.contact + "'");
			rs = state.executeQuery();
			if (!rs.next()) {
				closeStatement(rs, state, null);
	...
	public long delCCBQSResult(String bankID, java.util.Date tradeDate, Connection conn) throws SQLException {
		long result = 0L;
		String sql = null;
		PreparedStatement state = null;
		String filter = "";
	...
	public long addCCBQSResult(CCBQSResult qsResult, Connection conn) throws SQLException {
		long result = 0L;
		String sql = null;
		PreparedStatement state = null;
		try {
	...
	public FirmAuditValue getFirmAuditValue(String contact) throws SQLException, ClassNotFoundException {
	...
	public FirmAuditValue getFirmAuditValue(String contact, Connection conn) throws SQLException {
	...
	public String getFirmType(String firmID, Connection conn) throws SQLException {
		log("===>>>查询交易商类型" + firmID);
		PreparedStatement state = null;
		ResultSet rs = null;
		String sql = null;
	...
	public Vector<FirmAuditValue> getFirmAuditValue1(String filter, int[] pageinfo) throws SQLException, ClassNotFoundException {
	...
	public Vector<FirmAuditValue> getFirmAudit(String filter, Connection conn, int[] pageinfo) throws SQLException {
		log("===>>>取得交易账号   getFirm  ");
	...
	public int modFirmAuditValue(FirmAuditValue FirmAuditValue) throws SQLException, ClassNotFoundException {
		int result = -1;
		Connection conn = null;
		try {
			conn = getConnection();
	...
	public int modFirmAuditValue(FirmAuditValue val, Connection conn) throws SQLException {
		log("===>>>修改审核额度   modBank  ");
		int result = 0;
		PreparedStatement state = null;
		try {
			...
			String sql = "update f_b_firmaudit  set name = ?,maxpertransmoney = ?,maxpertranscount = ?,status = ?, maxPerSglTransMoney=?,maxAuditMoney=?,firmID=? where contact = '"
					+ val.contact + "'";
			int n = 1;
			state = conn.prepareStatement(sql);
	...
	public List<String> getRights(String userId) throws SQLException {
		log("===>>>获取菜单权限 ");
	...
	public Vector<FeeValue> getFeeList(String filter) throws SQLException, ClassNotFoundException {
		log("===>>>查询表   getDicList  " + new java.util.Date());
