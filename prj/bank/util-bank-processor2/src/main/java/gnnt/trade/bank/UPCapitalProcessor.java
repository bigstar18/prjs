...
public class UPCapitalProcessor extends CapitalProcessor {
		...
		try {
			...
			if (DAO.getFirm(correspondValue.firmID, conn) == null) {
				...
				log("银行账号注册，银行不存在，错误码=" + result);
					...
					.getCorrespondList(" and bankID='" + correspondValue.bankID + "' and firmID='" + correspondValue.firmID + "' and isopen=1", conn)
					.size() > 0) {
				result = -40004L;
				log("银行账号注册，账号已注册，错误码=" + result);
			}
			List<CorrespondValue> cvresult = DAO
					.getCorrespondList(" and bankID='" + correspondValue.bankID + "' and firmID='" + correspondValue.firmID + "'");
	...
	public ReturnValue inMoneyMarket(InMoneyMarket imm, String account, String bankCode, String bankName, String bankProvince, String bankCity,
			String accountType) {
	...
	public ReturnValue outMoneyMarket(OutMoneyMarket omm, String account, String bankCode, String bankName, String bankProvince, String bankCity,
			String accountType) {
		...
		try {
			...
			result = outMoneyAutoAudit(result.result, account, bankCode, bankName, bankProvince, bankCity, accountType);
		} catch (SQLException e) {
	...
	public ReturnValue outMoneyAutoAudit(long actionID, String account, String bankCode, String bankName, String bankProvince, String bankCity,
			String accountType) {
