...
public class JSBBankDAOImpl extends JSBBankDAO {
	private final transient Log logger = LogFactory.getLog("Processorlog");

	public JSBBankDAOImpl() throws Exception {
	}

	public Vector<MarketAcount> getMarketAcount(String filter) throws SQLException, ClassNotFoundException {
	...
	public int addMarketAcount(MarketAcount val) throws SQLException, ClassNotFoundException {
	...
	public int delMarketAcount(String account, String bankID) throws SQLException, ClassNotFoundException {
		...
		try {
			conn = getConnection();
			String sql = "delete from f_b_marketacount where account='" + account + "' and bankid='" + bankID + "'";
			state = conn.prepareStatement(sql);
			state.executeUpdate();
		} catch (SQLException e) {
			result = -1;
	...
	public int modMarketAcount(MarketAcount value) throws SQLException, ClassNotFoundException {
		...
		try {
			conn = getConnection();
			String sql = "update F_B_MARKETACOUNT set account = '" + value.account + "'," + " accountName = '" + value.accountName + "',"
					+ " accountOherName = '" + value.accountOherName + "'," + " bankOherName = '" + value.bankOherName + "'," + " bankName = '"
					+ value.bankName + "'," + " type = " + value.type + "," + " bankFlag = " + value.bankFlag + "," + " bankCode = '" + value.bankCode
					+ "' where account = '" + value.account + "' and bankid='" + value.bankId + "'";
	...
	public Vector<TransferInfo> getTransferList(String filter) throws SQLException, ClassNotFoundException {
	...
	public int addTransfer(TransferInfo param) throws SQLException, ClassNotFoundException {
		Connection conn = null;
		PreparedStatement state = null;
		String sql = null;
		int result = 0;
		try {
			conn = getConnection();
			sql = "insert into f_b_transferinfo values('" + param.funId + "','" + param.actionId + "'," + param.money + "," + param.type
					+ ",sysdate,sysdate,'" + param.outAccount + "','" + param.inAccount + "'," + param.status + ",'" + param.outAccountName + "','"
					+ param.inAccountName + "','" + param.bankid + "')";
			log(sql);
			state = conn.prepareStatement(sql);
			state.executeUpdate();
		} catch (SQLException e) {
	...
	public int modTransfer(String actionId, String funID, int status, Timestamp bankTime, Connection conn) throws SQLException {
	...
	public Vector<MarketCashBalance> getMarketCashBalance(String filter) throws SQLException, ClassNotFoundException {
