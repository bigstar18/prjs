...
public class JsExDataImpl {
	...
	private Vector<FirmBalance> getFirmBalance(java.util.Date tradeDate) {
		...
		JsConstant.bankID + "' and f.b_date=to_date('" + Tool.fmtDate(tradeDate) + "','yyyy-MM-dd')) ffb,"
				...
				+ " (select firmid,0-nvl(sum(mktfee),0) mktfee from t_trade_h where cleardate=to_date('" + Tool.fmtDate(tradeDate)
				+ "','yyyy/MM/dd') group by cleardate,firmid) th, " + " (select * from t_firm) tf" + " where fbf.firmID=ffb.firmID(+)"
				+ " and tf.firmid(+) = ffb.firmID" + " and ffb.firmid = th.firmid(+)";
		try {
			log("江苏银行清算sql:" + sql);
	...
	private MarketCashBalance getMarketBalance(String bankid, java.util.Date tradeDate) {
		...
		Tool.fmtDate(tradeDate) + "', 'yyyy-MM-dd')) b, " + "(select nvl(sum(ffb.capital - ffb.lastcapital - ffb.fundio), 0) marketcash, "
				+ "nvl(sum(case when ffb.tradefee<0 then ffb.tradefee*-1 else 0 end), 0) marketfee " + "from (select * "
				+ "from F_FIRMBALANCE_BANK f " + "where f.bankcode = '" + bankid + "' " + "and f.b_date = to_date('" + Tool.fmtDate(tradeDate)
				+ "', 'yyyy-MM-dd')) ffb, " + "(select * " + "from F_B_FIRMIDANDACCOUNT f " + "where f.bankID = '" + bankid + "' "
				+ "and f.isOpen = 1) fbf " + "where fbf.firmID = ffb.firmID(+)) a, "
				+ "(select nvl(sum(decode(type,0,money)),0) daycashio ,nvl(sum(decode(type,1,money)),0) "
				+ "dayfeeio from f_b_transferinfo where bankid='022' and to_char(banktime,'yyyy-MM-dd')='" + Tool.fmtDate(tradeDate)
				+ "' and status=0)c ,"
