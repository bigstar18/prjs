  ...
  extends CapitalProcessor
{
  ...
  public Map<String, Double> getFirmBalanceMap(String bankid, String date)
  {
    ...
    Connection conn = null;
    String filter = "select f.contact, b.capital+nvl(c.money,0), abs((case when m.firmtype='C' then b.tradefee + b.delayfee else 0 end))  fee from f_firmbalance_bank b, f_b_firmidandaccount f,m_firm m,(select firmid,sum(case when type = 0 then money else -money end) money from f_b_capitalinfo t where t.status =0 and t.type in (0,1) and t.createdate='" + 
      Common.df2.format(new Date()) + "' group by firmid) c" + 
      " where f.firmid = b.firmid(+)" + 
      "   and f.firmid = m.firmid(+)" + 
      "\tand f.firmid = c.firmid(+)" + 
      "and b.bankcode = '" + bankid + "' " + 
    ...
    try
    {
      ...
      try
      {
        ...
        map = DAO.getFirmBanlanceMap(conn, filter);
