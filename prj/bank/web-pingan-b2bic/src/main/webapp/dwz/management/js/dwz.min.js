...
if(typeof json == "string")json = $.parseJSON(json);	//by -- lyh
...
if(json.alertClose==false&&json.message&&alertMsg)alertMsg.correct(json.message, {alertClose:json.alertClose});	//by -- lyh
...
$.serializeParams=function($element,emptyValue,exceptSelector){	//by -- lyh -- start
var $fields;var serializer = ":text, :password, :file, :hidden, select, :checkbox:checked, :radio:checked, textarea, tr.selected :checkbox";
if(exceptSelector){$fields=$element.find(serializer).not(exceptSelector);} 
else{$fields=$element.find(serializer);}
var params=[];
if($fields.length > 0){$fields.each(function(){var val = $(this).val();
if(!val){
if(emptyValue==undefined){params.push({name : $(this).attr("name"), value : ""});} 
else if(emptyValue){params.push({name : $(this).attr("name"), value : emptyValue});}}
else{params.push({name : $(this).attr("name"), value : val});}});}
return params;
};	//by -- lyh -- end
...
if(json.statusCode==DWZ.statusCode.error){
if(json.message)alertMsg.error(json.message);if(op.errorCallback)op.errorCallback(json);}else{	//by -- lyh
...
loadUrl:function(url,data,callback,type,errorCallback){	//by -- lyh
$(this).ajaxUrl({url:url,data:data,callback:callback,type:type,errorCallback:errorCallback});},	//by -- lyh
initUI:function(){
...
if(!window.closeLoadFlag)ajaxbg.show();}).ajaxStop(function(){	//by -- lyh
ajaxbg.hide();});
...
if($this.hasClass("edit")){if($this.attr("multiSelect")!="true"){if($("table tr:not(.selected) :checkbox:checked, table tr.selected", $this.parents(".pageContent")).length>1){alertMsg.error($this.attr("warn")||DWZ.msg("alertSingleSelectMsg"));return false;}}}	//by -- lyh
var url=unescape($this.attr("href")).replaceTmById($(event.target).parents(".unitBox:first"));
var params=$this.attr("params");params?params=$.serializeParams($(params,$this.parents(".pageContent"))):params;	//by -- lyh
var callback=$this.attr("callback");	//by -- lyh
DWZ.debug(url);
if(!url.isFinishedTm()){
alertMsg.error($this.attr("warn")||DWZ.msg("alertSelectMsg"));
return false;}
navTab.openTab(tabid,url,{title:title,fresh:fresh,external:external,data:params,callback:callback});	//by -- lyh
...
options.params=$this.attr("params")||"";	//by -- lyh
options.pageContent=$p;	//by -- lyh
if($this.hasClass("edit")){if($this.attr("multiSelect")!="true"){if($("table tr:not(.selected) :checkbox:checked, table tr.selected", $this.parents(".pageContent")).length>1){alertMsg.error($this.attr("warn")||DWZ.msg("alertSingleSelectMsg"));return false;}}}	//by -- lyh
var url=unescape($this.attr("href")).replaceTmById($(event.target).parents(".unitBox:first"));
DWZ.debug(url);
if(!url.isFinishedTm()){
...
$(":text[prompt]",$p).each(function(){var $this=$(this);if(!$this.val()){var msg=$this.attr("prompt");$this.val(msg);$this.css("color","#b4b4b4");	//by -- lyh
$this.focus(function(){if($this.val()==msg){$this.val("");$this.css("color","");}});	//by -- lyh
$this.blur(function(){if(!$this.val()){$this.val(msg);$this.css("color","#b4b4b4");}});}});}(function($){	//by -- lyh
$.fn.extend({
...
if($.isFunction($.cookie))$.cookie("dwz_theme",themeName,{ expires:365});}	//by -- lyh
...
_open:function(type,msg,buttons,options){	//by -- lyh
$(this._boxId).remove();
var butsHtml="";
if(buttons){
...
if(options.alertClose||options.alertClose==undefined)this._closeTimer=setTimeout(function(){alertMsg.close()},3500);}else{	//by -- lyh
$(this._bgId).show();}
...
this._open(type,msg,buttons,op);},	//by -- lyh
...
$panel.ajaxUrl({type:"POST",url:url,data:args,callback:function(){navTab._loadUrlCallback($panel);}});}}},	//by -- lyh
...
$panel.ajaxUrl({
type:"POST",url:url,data:op.data,callback:function(){	//by -- lyh
navTab._loadUrlCallback($panel);if(op.callback)op.callback();},errorCallback:function(){navTab.closeCurrentTab();}});}}	//by -- lyh
...
var jDContent=dialog.find(".dialogContent");
jDContent.loadUrl(url,options.params&&options.pageContent?$.serializeParams($(options.params,options.pageContent)):{},function(response){	//by -- lyh
jDContent.find("[layoutH]").layoutH(jDContent);
$(".pageContent",dialog).width($(dialog).width()-14);
$("button.close").click(function(){
$.pdialog.close(dialog);
return false;});},"POST",function(){$.pdialog.close(dialog);});}}else{	//by -- lyh
...
$("button.close").click(function(){
$.pdialog.close(dialog);
return false;});},"POST",function(){$.pdialog.close(dialog);});}	//by -- lyh
if(op.mask){
...
var $th=$(oldThs[i]);
var style=[],width=$th.hasClass("selfWidth") ? $th.attr("width") : $th.innerWidth()-(100*$th.innerWidth()/tlength)-2;	//by -- lyh
...
if(args["rollPage"]&&form[DWZ.pageInfo.rollPage])form[DWZ.pageInfo.rollPage].value=args["rollPage"];	//by -- lyh
...
function ajaxTodo(url,callback,options){	//by -- lyh
var $callback=callback||navTabAjaxDone;
if(!$.isFunction($callback))$callback=eval('('+callback+')');
$.ajax({
type:'POST',
url:url,
data:(options&&options.data)?options.data:{},	//by -- lyh
dataType:"json",
cache:false,
success:$callback,
error:DWZ.ajaxError});}
...
return false;}
var options={};	//by -- lyh
var params=$this.attr("params");	//by -- lyh
if(params)options.data=$.serializeParams($(params,$this.parents(".pageContent")));	//by -- lyh
var title=$this.attr("title");
if(title){
alertMsg.confirm(title,{
okCall:function(){
ajaxTodo(url,$this.attr("callback"),options);}});}else{	//by -- lyh
ajaxTodo(url,$this.attr("callback"),options);}	//by -- lyh
event.preventDefault();});});},
dwzExport:function(){
...
var randPager=$(this).parents(".panelBar:first").parents("div:first").hasClass("noRandPager");	//by -- lyh
$this.find(setting.nums$).each(function(i){
if(!randPager)_bindEvent($(this),i+interval.start,pc.targetType(),pc.rel());});	//by -- lyh
$this.find(setting.jumpto$).each(function(){
if(randPager)return;	//by -- lyh
var $this=$(this);
...
dwzPageBreak({targetType:pc.targetType(),rel:pc.rel(),data:{pageNum:pageNum,rollPage:true}});}});	//by -- lyh
...
dwzPageBreak({targetType:targetType,rel:rel,data:{pageNum:event.data.pageNum,rollPage:true}});	//by -- lyh
event.preventDefault();});}},
...
dwzPageBreak({targetType:op.targetType,rel:op.rel,data:{orderField:orderField,orderDirection:orderDirection,rollPage:true}});});});},	//by -- lyh
