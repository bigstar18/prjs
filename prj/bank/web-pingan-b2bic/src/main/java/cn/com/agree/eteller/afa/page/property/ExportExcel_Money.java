...
public class ExportExcel_Money extends HibernateDaoSupport {
	public static final Log logger = LogFactory.getLog(ExportExcel_Money.class);
	...
	public ExportExcel_Money(String columnNames, String columns, String totalColumns, String reportFilePath, String reportTitle) {
	...
	public boolean createExcel() {
		try {
			...
			System.out.println("dddddd1111111=" + this.reportFilePath);
		...
		try {
			...
			if (this.map.size() != 0) {
				...
				for (Iterator itr = st.iterator(); itr.hasNext();) {
					...
					String tmp = "";
					tmp = ((String) this.map.get(str)).substring(0, 1);
					int colNo = 0;
					if (!tmp.equals("b")) {
						colNo = Integer.parseInt(this.map.get(str).toString());
					} else {
						colNo = Integer.parseInt(this.map.get(str).toString().substring(1));
					}
					if (isAddXh()) {
						colNo++;
					}
					String colvalue = String.valueOf((BigDecimal) this.map_value.get(str));
					Label labe2_value_total = null;
					if (tmp.equals("b")) {
						labe2_value_total = new Label(colNo, this.curRowIndex, colvalue);
					} else {
						labe2_value_total = new Label(colNo, this.curRowIndex, ComFunction.formatNumber(colvalue));
					}
			...
			if ((getReportHeader() != null) && (!getReportFooter().equals(""))) {
				...
				for (int i = 0; i < footers.length; i++) {
					String[] strArr = footers[i].split("\\Q|\\E");
					int rowNo = Integer.parseInt(strArr[0]);
					int colNo = Integer.parseInt(strArr[1]);
