...
public class ExportExcelbak extends HibernateDaoSupport {
	public static final Log logger = LogFactory.getLog(ExportExcelbak.class);
	...
	public ExportExcelbak(String columnNames, String columns, String totalColumns, String reportFilePath, String reportTitle) {
	...
	public boolean createExcel() {
		try {
			...
			File f = new File(this.reportFilePath.replaceAll("//", "/"));
		...
		try {
			...
			if (this.map.size() != 0) {
				...
				for (Iterator itr = st.iterator(); itr.hasNext();) {
					...
					Label labe2_value_total = new Label(colNo, this.curRowIndex, colvalue);
