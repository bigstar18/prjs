...
//		}
	...
	public long addCapitalInfo(CapitalValue val, Connection conn) throws SQLException
	{
		...
		try
		{
			...
			sql = "insert into F_B_capitalInfo(ID, FIRMID, FUNID, BANKID, DEBITID, CREDITID, TYPE," + " MONEY, OPERATOR, CREATETIME, BANKTIME, STATUS, NOTE, ACTIONID,EXPRESS,bankName,account,createdate,funid2) " + "values(?, ?, ?, ?, ?, ?, ?, ?, ?, sysdate, ?, ?, ?, ?,?,?,?,to_char(sysdate,'yyyy-MM-dd'),?)";
	...
	public Vector<CapitalValue> getCapitalInfoList2(String filter) throws SQLException, ClassNotFoundException
	{
		...
		try
		{
			...
			sql = "select c.*,f.accountname firmName,b.bankName bbankName from f_b_capitalinfo c,f_b_firmidandaccount f,f_b_banks b where f.firmid(+)=c.firmid and c.bankid=b.bankid(+) " + filter;
			log("sql:" + sql);
			state = conn.prepareStatement(sql);
			rs = state.executeQuery();
			while (rs.next())
	...
	public Vector<BankCompareInfoValue> getBankCapInfoList(String filter) throws SQLException, ClassNotFoundException
	{
		...
		try
		{
			...
			sql = "select id,funid,firmid,bankid,account,type,money," + "nvl(to_char(compareDate,'yyyy-MM-dd'),'') cDate,nvl(Note,'') note," + "nvl(to_char(createtime,'yyyy-MM-dd'),'') cTime,status from f_b_bankcompareinfo " + filter;
	...
	public void addBank(BankValue val) throws SQLException, ClassNotFoundException
	{
		...
		{
			conn = getConnection();
			sql = " insert into F_B_banks " + " (bankid, bankname, maxpertransmoney, maxpertranscount," + " adapterclassname, validflag, maxPerSglTransMoney,maxAuditMoney,beginTime,endTime,control) " + " values(?, ?, ?, ?, ?, ?, ?, ?,?,?,?)";
	...
	public void modBank(BankValue val) throws SQLException, ClassNotFoundException
	{
		...
		{
			conn = getConnection();
			sql = "update F_B_banks set bankname = ?,maxpertransmoney = ?,maxpertranscount = ?," + "adapterclassname = ?,validflag = ?,maxPerSglTransMoney=?,maxAuditMoney=?,beginTime=?,endTime=?,control=? where bankid = ?";
			state = conn.prepareStatement(sql);
	...
	public int modFirm(FirmValue val, Connection conn) throws SQLException
	{
		...
		try
		{
			sql = "update F_B_FirmUser  " + "set name = ?,maxpertransmoney = " + "?,maxpertranscount = ?,status = ?," + "logoutdate = ?,maxPerSglTransMoney=?,maxAuditMoney=?,password=? where firmid = '" + val.firmID + "'";
	...
	public Vector<MoneyInfoValue> getUnionMoneyInfoList(String filter) throws SQLException, ClassNotFoundException
	{
		...
		try
		{
			...
			sql = "select i.funid,c.id,i.bankid,i.type,i.money,c.money,i.account,i.firmID,i.comparedate  " + "from F_B_capitalInfo c full join F_B_BANKCOMPAREINFO i on c.funid=i.funid " + filter;
	...
	public int addCorrespond(CorrespondValue val, Connection conn) throws SQLException
	{
		...
		try
		{
			sql = "insert into F_B_firmidandaccount(bankid, firmid, account, status" + ",accountname, bankname, bankprovince, bankcity, mobile, email,account1,isopen,cardtype,card,accountName1,inMarketCode) " + "values(?,?, ?, ?, ?, ?, ?, ?, ?, ?, ?,?,?,?,?,?)";
			state = conn.prepareStatement(sql);
			state.setString(1, val.bankID);
	...
	public int delCorrespond(CorrespondValue val, Connection conn) throws SQLException
	{
		...
		try
		{
			sql = "delete F_B_firmidandaccount where bankid = '" + val.bankID + "'  " + "and firmid = '" + val.firmID + "'  and account = '" + val.account + "'";
	...
	public int modCorrespond(CorrespondValue val, Connection conn) throws SQLException
	{
		...
		try
		{
			sql = "update f_b_firmidandaccount set status=" + val.status + ",accountname = '" + val.accountName + "'" + ",bankname = '" + val.bankName + "',bankprovince = '" + val.bankProvince + "',bankcity = '" + val.bankCity + "'," + "mobile = '" + val.mobile + "',email = '" + val.email + "',isopen=" + val.isOpen + ",cardtype = '" + val.cardType + "'" + ",card='" + val.card + "',account1='" + val.account1 + "',account='" + val.account + "' ,accountName1 = '" + val.accountName1 + "'" + ",inMarketCode='" + val.inMarketCode + "'" + " where bankid = '" + val.bankID + "'  " + "and firmid = '" + val.firmID + "'";
			state = conn.prepareStatement(sql);
	...
	public int openCorrespond(CorrespondValue val) throws SQLException, ClassNotFoundException
	{
		...
		{
			conn = this.getConnection();
			sql = "update f_b_firmidandaccount set status=" + val.status + ",accountname = '" + val.accountName + "'" + ",bankname = '" + val.bankName + "',bankprovince = '" + val.bankProvince + "',bankcity = '" + val.bankCity + "'" + ",isopen=" + val.isOpen + ",cardtype = '" + val.cardType + "',openTime=sysdate" + ",card='" + val.card + "',account1='" + val.account1 + "',account='" + val.account + "' ,accountName1 = '" + val.accountName1 + "'" + ",inMarketCode='" + val.inMarketCode + "'" + " where bankid = '" + val.bankID + "'  " + "and firmid = '" + val.firmID + "'";
			System.out.println(sql);
			state = conn.prepareStatement(sql);
			state.executeUpdate();
		} catch (SQLException e)
	...
	public CorrespondValue getCorrespond(String bankID, String firmID, String account, Connection conn) throws SQLException
	{
		...
		try
		{
			sql = "select * from f_b_FirmidAndAccount where bankID='" + bankID + "' and firmID='" + firmID + "' and account='" + account + "' and status=0 and isopen=1";
			state = conn.prepareStatement(sql);
			rs = state.executeQuery();
			if (rs.next())
			{
	...
	public CorrespondValue getCorrespond(String bankID, String firmID, String account) throws SQLException, ClassNotFoundException
	{
		...
		try
		{
			...
			// sql =
			// "select t.*,(select b.bankname from f_b_banks b where b.bankid=t.bankid) headOffice from F_B_firmidandaccount t where t.bankID='"+bankID+"' and t.firmID='"+firmID+"' and t.account='"+account+"' and t.status=0";
			state = conn.prepareStatement(sql);
			rs = state.executeQuery();
			while (rs.next())
			{
	...
	public Vector<CompareResult> compareResultInfo(String bankID, String date) throws SQLException, ClassNotFoundException
	{
		...
		String fundflow=" and exists (select f.contractno from f_h_fundflow f where f.contractno = c.actionid and f.b_date = to_date('"+date+"', 'yyyy-MM-dd')) ";
		try
		{
			conn = getConnection();
			// 账面不平的数据
			...
			sql = "select c.firmID || '/' || b.firmID as firmID, c.bankID as bankID, b.account as account, c.funID as id , c.actionID as m_Id , " + " b.type as type, c.type as m_type, b.money as money ,c.money as m_money, b.comparedate as comparedate " + " from F_B_BANKCOMPAREINFO b,F_B_capitalInfo c " + " where c.status='0' and (c.type='0' or c.type='1') and c.bankid='" + bankID + "' and b.bankid='" + bankID + "' " +
			// " and trunc(b.COMPAREDATE)=to_date('"+ date
					// +"','yyyy-MM-dd') and trunc(c.bankTime)=to_date('"+ date
					// +"','yyyy-MM-dd')" +20110908 lipj
					// 修改对账条件，市场流水表使用createdate字段比较
					" and trunc(b.COMPAREDATE)=to_date('" + date + "','yyyy-MM-dd') and c.createdate='" + date + "'" + " and b.funid=c.funid  and (b.money<>c.money or (trim(b.firmID)<>trim(c.firmID) and b.firmID!='GSYTfirm'))";
			...
			" from F_B_BANKCOMPAREINFO b,F_B_capitalInfo c " + " where (c.status='0' and (c.type='0' or c.type='1') and c.bankid='" + bankID + "' and b.bankid='" + bankID + "' " +
			// " and trunc(b.COMPAREDATE)=to_date('"+ date
					// +"','yyyy-MM-dd') and trunc(c.bankTime)=to_date('"+ date
					// +"','yyyy-MM-dd')" +20110908 lipj
					// 修改对账条件，市场流水表使用createdate字段比较
			...
			sql = "select c.firmID as firmID, c.bankID as bankID, b.account as account, c.funID as id, c.actionID as m_Id,  " + " b.type as type ,c.type as m_type, b.money as money ,c.money as m_money ,b.comparedate as comparedate " + " from F_B_BANKCOMPAREINFO b,F_B_capitalInfo c " + " where (c.status='0' and (c.type='0' or c.type='1') and c.bankid='" + bankID + "' and b.bankid='" + bankID + "' " +
			// " and trunc(b.COMPAREDATE)=to_date('"+ date
					// +"','yyyy-MM-dd') and trunc(c.bankTime)=to_date('"+ date
					// +"','yyyy-MM-dd')" +20110908 lipj
					// 修改对账条件，市场流水表使用createdate字段比较
					" and trunc(b.COMPAREDATE)=to_date('" + date + "','yyyy-MM-dd') and c.createdate='" + date + "'" + " and b.funid=c.funid  and b.type<>c.type )";
			...
			if("true".equals(Tool.getConfig("CompareWithFundflow"))){
			sql = "select * from F_B_BANKCOMPAREINFO b where trunc(b.comparedate)=to_date('" +
			date + "','yyyy-mm-dd') and b.bankID='" + bankID + 
			"' " + " and b.funid<>'-1' and not exists(select funid from F_B_capitalInfo c ,f_h_fundflow f where b.funid=c.funid  and  c.actionid =f.contractno  and f.b_date = to_date('" + date + "', 'yyyy-MM-dd') and c.status='0' and (c.type='0' or c.type='1') " +
			...
			sql = "select * from F_B_BANKCOMPAREINFO b where trunc(b.comparedate)=to_date('" + date + "','yyyy-mm-dd') and b.bankID='" + bankID + "' " + " and b.funid<>'-1' and not exists(select funid from F_B_capitalInfo c where b.funid=c.funid and c.status='0' and (c.type='0' or c.type='1') " +
			// " and c.bankID='"+bankID+"' and trunc(c.bankTime)=to_date('"+date+"','yyyy-mm-dd') and funid is not null) ";//20110908
					// lipj 修改对账条件，市场流水表使用createdate字段比较
					" and c.bankID='" + bankID + "' and c.createdate='" + date + "' and funid is not null) ";
			}
			...
			sql = "select * from F_B_capitalInfo c where c.createdate='" + date + "' and c.bankID='" + bankID + "' " + " and c.status='0' and (c.type='0' or c.type='1') and not exists " + " (select funid from F_B_BANKCOMPAREINFO b where b.funid=c.funid and b.bankid='" + bankID + "' and trunc(b.comparedate)=to_date('" + date + "','yyyy-mm-dd'))  ";
	...
	public int modFeeInfo(FeeInfoVO feeInfoVO, Connection conn) throws SQLException, ClassNotFoundException
	{
		...
		try
		{
			sql = "update  F_B_feeInfo set upLimit=?, downLimit=?, tMode=?, rate=?, maxRateValue=?,minRateValue=?,type=?, " + "uptateTime=sysdate,userID=?) where id = ?";
	...
	public List<TradeResultValue> getTradeDataInList(String filter, int moduleid) throws SQLException, ClassNotFoundException
	{
		...
		try
		{
			...
			if (moduleid == 2)
			{
				sql = " select f.fundflowid,f.firmid,f.oprcode,f.amount,f.balance,f.appendamount,f.b_date,b.bankid,b.account " + " from f_h_fundflow f, f_b_firmidandaccount b where f.firmid = b.firmid(+) and to_char(f.oprcode) like '2%' and " + filter + " order by f.fundflowid ";
			} else if (moduleid == 3)
			{
				sql = " select f.fundflowid,f.firmid,f.oprcode,f.amount,f.balance,f.appendamount,f.b_date,b.bankid,b.account " + " from f_h_fundflow f, f_b_firmidandaccount b where f.firmid = b.firmid(+) and to_char(f.oprcode) like '3%' and " + filter + " order by f.fundflowid ";
			} else if (moduleid == 4)
			{
				sql = " select f.fundflowid,f.firmid,f.oprcode,f.amount,f.balance,f.appendamount,f.b_date,b.bankid,b.account " + " from f_h_fundflow f, f_b_firmidandaccount b where f.firmid = b.firmid(+) and to_char(f.oprcode) like '4%' and " + filter + " order by f.fundflowid ";
			} else
			{
				sql = " select f.fundflowid,f.firmid,f.oprcode,f.amount,f.balance,f.appendamount,f.b_date,b.bankid,b.account " + " from f_h_fundflow f, f_b_firmidandaccount b where f.firmid = b.firmid(+) and " + filter + " order by f.fundflowid ";
			}
			System.out.println(sql);
			state = conn.prepareStatement(sql);
			rs = state.executeQuery();
	...
	public Hashtable<String, Double> getFundsAndInterests(java.util.Date date, int moduleid) throws SQLException, ClassNotFoundException
	{
		...
		{
			conn = getConnection();
			sql = " select a.firmid,a.todaybalance,b.RuntimeMargin,b.RuntimeFL,b.RuntimeSettleMargin,nvl(c.floatingloss,0) floatingloss from " + " (select firmid, todaybalance from f_firmbalance where b_date=to_date('" + new java.sql.Date(date.getTime()) + "','yyyy-MM-dd')) a," + " (select firmid,(RuntimeMargin - RuntimeAssure) RuntimeMargin, RuntimeFL, RuntimeSettleMargin from t_h_firm where cleardate=to_date('" + date + "','yyyy-MM-dd')) b," + " (select firmid,nvl(sum(floatingloss),0) floatingloss from t_h_firmholdsum where cleardate=to_date('" + date + "','yyyy-MM-dd') group by firmid) c" + " where a.firmid=b.firmid(+) and a.firmid=c.firmid(+) order by a.firmid ";
			state = conn.prepareStatement(sql);
			rs = state.executeQuery();
			while (rs.next())
			{
	...
	public Vector<FundsAndInterests> getFundsAndInterestsInVector(java.util.Date date, int moduleid) throws SQLException, ClassNotFoundException
	{
		...
		{
			conn = getConnection();
			sql = " select a.firmid,a.todaybalance,b.RuntimeMargin,b.RuntimeFL,b.RuntimeSettleMargin,nvl(c.floatingloss,0) floatingloss from " + " (select firmid, todaybalance from f_firmbalance where b_date=to_date('" + new java.sql.Date(date.getTime()) + "','yyyy-MM-dd')) a," + " (select firmid,(RuntimeMargin - RuntimeAssure) RuntimeMargin, RuntimeFL, RuntimeSettleMargin from t_h_firm where cleardate=to_date('" + new java.sql.Date(date.getTime()) + "','yyyy-MM-dd')) b," + " (select firmid,nvl(sum(floatingloss),0) floatingloss from t_h_firmholdsum where cleardate=to_date('" + new java.sql.Date(date.getTime()) + "','yyyy-MM-dd') group by firmid) c" + " where a.firmid=b.firmid(+) and a.firmid=c.firmid(+) order by a.firmid ";
			state = conn.prepareStatement(sql);
			rs = state.executeQuery();
			while (rs.next())
			{
...
//			sql = "select d.*,a.balance todayFunds,a.frozenfunds frozenfunds,a.lastbalance yesFunds,b.zvmargin zvMargin,c.rsm settleMargin,c.rma runtimeMargin,c.fl,c.cleardate from " + "(select firmid,balance,frozenfunds,lastbalance from f_firmfunds where 1=1 " + filter + ") a, " + "(select firmID,sum(case when code in ('Margin_V','Margin_Z') then value else -value end) zvmargin from f_clientledger where code in ('MarginBack_V','Margin_V','MarginBack_Z','Margin_Z') and trunc(b_date)<to_date('" + Tool.fmtDate(yesDate) + "','yyyy-MM-dd') " + filter + " group by firmid) b, " + "(select firmID,RuntimeSettleMargin rsm,(RuntimeMargin-RuntimeAssure) rma,RuntimeFL fl,cleardate from T_h_firm where 1=1 and trunc(cleardate)=to_date('" + Tool.fmtDate(yesDate) + "','yyyy-MM-dd') " + filter + ") c, " + "(select * from f_b_firmidandaccount where 1=1 " + filter + ") d  " + "where d.firmid=a.firmid(+) and d.firmid=b.firmid(+) and d.firmid=c.firmid(+)";
			//TODO　交易商资金情况中用到了远期的数据
			sql = "select d.*,a.balance todayFunds,a.frozenfunds frozenfunds,a.lastbalance yesFunds,b.zvmargin zvMargin from " + "(select firmid,balance,frozenfunds,lastbalance from f_firmfunds where 1=1 " + filter + ") a, " + "(select firmID,sum(case when code in ('Margin_V','Margin_Z') then value else -value end) zvmargin from f_clientledger where code in ('MarginBack_V','Margin_V','MarginBack_Z','Margin_Z') and trunc(b_date)<to_date('" + Tool.fmtDate(yesDate) + "','yyyy-MM-dd') " + filter + " group by firmid) b, " + "(select * from f_b_firmidandaccount where 1=1 " + filter + ") d  " + "where d.firmid=a.firmid(+) and d.firmid=b.firmid(+)";
			state = conn.prepareStatement(sql);
			rs = state.executeQuery();
			while (rs.next())
			{
	...
	public Vector<FirmOpenCloseBank> getFirmBank(String bankID, java.util.Date date) throws SQLException, ClassNotFoundException
	{
		...
		try
		{// 查询市场没有的签约
			sql = "select FIRMID from f_b_firmkxh a where bankid='" + bankID + "' and status=1 and trunc(TRADEDATE)=to_date('" + Tool.fmtDate(date) + "','yyyy-MM-dd') and not exists(select FIRMID from f_b_firmidandaccount b where bankid='" + bankID + "' and trunc(opentime)=to_date('" + Tool.fmtDate(date) + "','yyyy-MM-dd') and b.firmid=a.firmid and b.bankid=a.bankid or b.firmid='D_'||a.firmid and b.bankid='D_'||a.bankid)";
			log("sql:" + sql);
			System.out.println("sql:" + sql);
			conn = this.getConnection();
			state = conn.prepareStatement(sql);
		...
		try
		{// 查询市场没有的解约
			sql = "select FIRMID from f_b_firmkxh a where bankid='" + bankID + "' and status=2 and trunc(TRADEDATE)=to_date('" + Tool.fmtDate(date) + "','yyyy-MM-dd') and not exists(select FIRMID from f_b_firmidandaccount b where (bankid='" + bankID + "' or bankid='D_" + bankID + "') and trunc(deltime)=to_date('" + Tool.fmtDate(date) + "','yyyy-MM-dd') and b.firmid='D_'||a.firmid and b.bankid='D_'||a.bankid)";
			log("sql:" + sql);
			System.out.println("sql:" + sql);
			conn = this.getConnection();
			state = conn.prepareStatement(sql);
		...
		try
		{// 查询银行没有的签约
			sql = "select firmid from f_b_firmidandaccount a where (bankid='" + bankID + "' or bankid='D_" + bankID + "') and trunc(opentime)=to_date('" + Tool.fmtDate(date) + "', 'yyyy-MM-dd') and not exists (select firmid from f_b_firmkxh b where bankid='" + bankID + "' and status=1 and trunc(TRADEDATE)=to_date('" + Tool.fmtDate(date) + "', 'yyyy-MM-dd') and (b.firmid=a.firmid or 'D_'||b.firmid=a.firmid))";
			log("sql:" + sql);
			System.out.println("sql:" + sql);
			conn = this.getConnection();
			state = conn.prepareStatement(sql);
		...
		try
		{// 查询银行没有的解约
			sql = "select firmid from f_b_firmidandaccount a where bankid='D_" + bankID + "' and trunc(deltime)=to_date('" + Tool.fmtDate(date) + "', 'yyyy-MM-dd') and not exists (select firmid from f_b_firmkxh b where bankid='" + bankID + "' and status=2 and trunc(TRADEDATE)=to_date('" + Tool.fmtDate(date) + "', 'yyyy-MM-dd') and 'D_'||b.firmid=a.firmid)";
			log("sql:" + sql);
			System.out.println("sql:" + sql);
			conn = this.getConnection();
			state = conn.prepareStatement(sql);
	...
	/** 查询交易商余额信息 */
	public Vector<BatCustDzBChild> getFirmBalanceFile(String ThirdCustId, String bankID, java.util.Date date) throws SQLException, ClassNotFoundException
	{
	...
	/** 查询银行对账失败信息 */
	public Vector<BatFailResultChild> getFirmBalanceError(String[] firmIDs, String bankID, java.util.Date date) throws SQLException, ClassNotFoundException
	{
	...
	/** 获取某日的清算信息 */
	public Map<String, BatQsChild> getQSChild(String bankID, Set<String> firmIDs, Set<String> notFirmIDs, java.util.Date qdate, Connection conn) throws SQLException
	{
		...
		try
		{
			...
			// sql =
			// "select nvl(a.value,0)+(case when (nvl(j.valued,0)-nvl(j.valuez,0))>0 then (nvl(j.valued,0)-nvl(j.valuez,0)) else 0 end)+(case when nvl(k.value,0)>0 then nvl(k.value,0) else 0 end) AddTranAmount,"
			// +// --销售收入
			// "nvl(b.value,0)+(case when (nvl(j.valuez,0)-nvl(j.valued,0))>0 then (nvl(j.valuez,0)-nvl(j.valued,0)) else 0 end)+(case when nvl(k.value,0)<0 then nvl(-k.value,0) else 0 end) CutTranAmount,"
			// +// --购货支出
			...
			// "from " +
			// "(select fc.firmid,sum(nvl(fc.value,0)) value from f_ledgerfield fl,f_clientledger fc where fl.code = fc.code and fc.b_date=to_date('"+Tool.fmtDate(qdate)+"','yyyy-MM-dd') and fl.code in ('Income','Income_Z','Income_V') "+filter+" group by fc.firmid) a, "
			// +// --销售收入
			// "(select fc.firmid,sum(nvl(fc.value,0)) value from f_ledgerfield fl,f_clientledger fc where fl.code = fc.code and fc.b_date=to_date('"+Tool.fmtDate(qdate)+"','yyyy-MM-dd') and fl.code in ('Payout','Payout_Z','Payout_V') "+filter+" group by fc.firmid) b, "
			// +// --购货支出
			// "(select fc.firmid,sum(nvl(fc.value,0)) value from f_ledgerfield fl,f_clientledger fc where fl.code = fc.code and fc.b_date=to_date('"+Tool.fmtDate(qdate)+"','yyyy-MM-dd') and fl.code in ('TradePL','SettlePL') and value>=0 "+filter+" group by fc.firmid) c, "
			// +// --盈利
			// "(select fc.firmid,sum(nvl(-fc.value,0)) value from f_ledgerfield fl,f_clientledger fc where fl.code = fc.code and fc.b_date=to_date('"+Tool.fmtDate(qdate)+"','yyyy-MM-dd') and fl.code in ('TradePL','SettlePL') and value<0 "+filter+" group by fc.firmid) d, "
			// +// --亏损
			// "(select fc.firmid,sum(nvl(fc.value,0)) value from f_ledgerfield fl,f_clientledger fc where fl.code = fc.code and fc.b_date=to_date('"+Tool.fmtDate(qdate)+"','yyyy-MM-dd') and fl.code in ('BankFee','TradeFee','SettleFee','TradeFee_Z','TradeFee_V') "+filter+" group by fc.firmid) e, "
			// +// --手续费
			// "( " +
			// "select nvl(fb.value,0)+nvl(fc.value,0)+nvl(fd.value,0)+nvl(ff.value,0)+nvl(fg.value,0) valued,nvl(fb1.value,0)+nvl(fc1.value,0)+nvl(fd1.value,0)+nvl(ff1.value,0)+nvl(fg1.value,0) valuez, fe.firmid from "
			// +
			// "(select nvl(RuntimeSettleMargin,0) value,firmid from t_h_firm where cleardate=to_date('"+Tool.fmtDate(qdate)+"','yyyy-MM-dd') "+filter+") fb, "
			// +// --当日交收保证金
			// "(select nvl(RuntimeFL,0) value,firmid from t_h_firm where cleardate=to_date('"+Tool.fmtDate(qdate)+"','yyyy-MM-dd') "+filter+") fc, "
			// +// --当日浮亏
			// "(select nvl(RuntimeMargin,0)-nvl(RuntimeAssure,0) value,firmid from t_h_firm where cleardate=to_date('"+Tool.fmtDate(qdate)+"','yyyy-MM-dd') "+filter+") fd, "
			// +// --当日远期保证金
			// "(select nvl(payout_Z,0)+nvl(Payout_V,0)+nvl(payOut_T,0) value,firmid from F_firmbalance where trunc(B_Date)=to_date('"+Tool.fmtDate(qdate)+"','yyyy-MM-dd') "+filter+") ff, "
			// + //当日待付货款
			// "(select nvl(sum(case when code = 'Margin_V' then value when code = 'Margin_Z' then value when code = 'MarginBack_V' then -value when code = 'MarginBack_Z' then -value else 0 end), 0) value,firmid from f_clientledger where code in ('MarginBack_V', 'Margin_V','MarginBack_Z', 'Margin_Z') and to_char(b_date,'yyyy-MM-dd')<='"+Tool.fmtDate(qdate)+"' "+filter+" group by firmid) fg, "
			// +//当日现货竞价保证金
			//	      
			// "(select nvl(RuntimeSettleMargin,0) value,firmid from t_h_firm where cleardate=to_date('"+Tool.fmtDate(yestody)+"','yyyy-MM-dd') "+filter+") fb1, "
			// +// --上日交收保证金
			// "(select nvl(RuntimeFL,0) value,firmid from t_h_firm where cleardate=to_date('"+Tool.fmtDate(yestody)+"','yyyy-MM-dd') "+filter+") fc1, "
			// +// --上日浮亏
			// "(select nvl(RuntimeMargin,0)-nvl(RuntimeAssure,0) value,firmid from t_h_firm where cleardate=to_date('"+Tool.fmtDate(yestody)+"','yyyy-MM-dd') "+filter+") fd1, "
			// +// --上日远期保证金
			// "(select nvl(payout_Z,0)+nvl(Payout_V,0)+nvl(payOut_T,0) value,firmid from F_firmbalance where trunc(B_Date)=to_date('"+Tool.fmtDate(yestody)+"','yyyy-MM-dd') "+filter+") ff1, "
			// + //上日待付货款
			// "(select nvl(sum(case when code = 'Margin_V' then value when code = 'Margin_Z' then value when code = 'MarginBack_V' then -value when code = 'MarginBack_Z' then -value else 0 end), 0) value,firmid from f_clientledger where code in ('MarginBack_V', 'Margin_V','MarginBack_Z', 'Margin_Z') and to_char(b_date,'yyyy-MM-dd')<='"+Tool.fmtDate(yestody)+"' "+filter+" group by firmid) fg1, "
			// +//上日现货竞价保证金
			//	      
			// "(select firmid from f_b_firmidandaccount where 1=1 and (account1 is not null) "+filter2+") fe "
			// +//--和银行接口连接
			// "where fe.firmid=fb.firmid(+) and fe.firmid=fc.firmid(+) and fe.firmid=fd.firmid(+) and fe.firmid=ff.firmid(+) and fe.firmid=fg.firmid(+) and fe.firmid=fb1.firmid(+) and fe.firmid=fc1.firmid(+) and fe.firmid=fd1.firmid(+) and fe.firmid=ff1.firmid(+) and fe.firmid=fg1.firmid(+) "
			// +
			// ") f, " +// --冻结解冻资金
			// "(" +
			// "select nvl(ia.value,0) valued,nvl(ib.value,0) valuez,ic.firmid from "
			// +
			// "(select sum(nvl(floatingloss,0)) value,firmid from t_h_firmholdsum where trunc(cleardate)=to_date('"+Tool.fmtDate(qdate)+"','yyyy-MM-dd') "+filter+" group by firmid) ia,"
			// + //当日浮动盈亏
			// "(select sum(nvl(floatingloss,0)) value,firmid from t_h_firmholdsum where trunc(cleardate)=to_date('"+Tool.fmtDate(yestody)+"','yyyy-MM-dd') "+filter+" group by firmid) ib,"
			// + //上日浮动盈亏
			...
			// +
			// "(select nvl(Overdraft,0) value,firmid from F_firmbalance where trunc(B_Date)=to_date('"+Tool.fmtDate(qdate)+"','yyyy-MM-dd') "+filter+") ja,"
			// +//当日欠款
			// "(select nvl(Overdraft,0) value,firmid from F_firmbalance where trunc(B_Date)=to_date('"+Tool.fmtDate(yestody)+"','yyyy-MM-dd') "+filter+") ja1,"
			// +//上日欠款
			...
			// ") j," +
			// "(select nvl(value,0) value,firmid  from f_clientledger where code='OtherItem_Z' and trunc(B_Date)=to_date('"+Tool.fmtDate(qdate)+"','yyyy-MM-dd') "
			// + filter + ") k, " +
			// "(select todaybalance value,firmid from f_firmbalance where b_date=to_date('"+Tool.fmtDate(qdate)+"','yyyy-MM-dd') "+filter+") g,"
			// +// --当日余额
			// "(select account1,ACCOUNTNAME1,firmid from f_b_firmidandaccount where 1=1 and (account1 is not null) and account1<>'null' "+filter2+") h "
			// +// --和银行接口连接
			// "where h.firmid=a.firmid(+) and h.firmid=b.firmid(+) and h.firmid=c.firmid(+) and h.firmid=d.firmid(+) and h.firmid=e.firmid(+) and h.firmid=f.firmid(+) and h.firmid=g.firmid(+) and h.firmid=i.firmid(+) and h.firmid=j.firmid(+) and h.firmid=k.firmid(+)";
			...
			sql = "select nvl(a.value,0)" + /*
					...
					"h.account1 CustAcctId," + // --交易商子账户
					"h.ACCOUNTNAME1 CustName " + // --子账户名称
					"from " + "(select fc.firmid,sum(nvl(fc.value,0)) value from f_ledgerfield fl,f_clientledger fc where fl.code = fc.code and fc.b_date=to_date('" + Tool.fmtDate(qdate) + "','yyyy-MM-dd') and fl.code in ('Income','Income_Z','Income_V') " + filter + " group by fc.firmid) a, " + // --销售收入
					"(select fc.firmid,sum(nvl(fc.value,0)) value from f_ledgerfield fl,f_clientledger fc where fl.code = fc.code and fc.b_date=to_date('" + Tool.fmtDate(qdate) + "','yyyy-MM-dd') and fl.code in ('Payout','Payout_Z','Payout_V') " + filter + " group by fc.firmid) b, " + // --购货支出
					"(select fc.firmid,sum(nvl(fc.value,0)) value from f_ledgerfield fl,f_clientledger fc where fl.code = fc.code and fc.b_date=to_date('" + Tool.fmtDate(qdate) + "','yyyy-MM-dd') and fl.code in ('TradePL','SettlePL') and value>=0 " + filter + " group by fc.firmid) c, " + // --盈利
					"(select fc.firmid,sum(nvl(-fc.value,0)) value from f_ledgerfield fl,f_clientledger fc where fl.code = fc.code and fc.b_date=to_date('" + Tool.fmtDate(qdate) + "','yyyy-MM-dd') and fl.code in ('TradePL','SettlePL') and value<0 " + filter + " group by fc.firmid) d, " + // --亏损
					"(select fc.firmid,sum(nvl(case when fl.code='SettleCompens' then -fc.value else fc.value end,0)) value from f_ledgerfield fl,f_clientledger fc where fl.code = fc.code and fc.b_date=to_date('" + Tool.fmtDate(qdate) + "','yyyy-MM-dd') and fl.code in ('BankFee','TradeFee','SettleFee','TradeFee_Z','TradeFee_V','SettleFee_Z','SettleCompens') " + filter + " group by fc.firmid) e, " + // --手续费
					"( " + "select nvl(fb.value,0)+nvl(fc.value,0)+nvl(fd.value,0)" + /*
																					 ...
																					 */"+nvl(fg.value,0) valued,nvl(fb1.value,0)+nvl(fc1.value,0)+nvl(fd1.value,0)" + /*
																																									 ...
																																									 */"+nvl(fg1.value,0) valuez" + /*
																																																	 ...
																																																	 * )
																																																	 * ffvalue
																																																	 */", fe.firmid from " + "(select nvl(RuntimeSettleMargin,0) value,firmid from t_h_firm where cleardate=to_date('" + Tool.fmtDate(qdate) + "','yyyy-MM-dd') " + filter + ") fb, " + // --当日交收保证金
					"(select nvl(RuntimeFL,0) value,firmid from t_h_firm where cleardate=to_date('" + Tool.fmtDate(qdate) + "','yyyy-MM-dd') " + filter + ") fc, " + // --当日浮亏
					"(select nvl(RuntimeMargin,0)-nvl(RuntimeAssure,0) value,firmid from t_h_firm where cleardate=to_date('" + Tool.fmtDate(qdate) + "','yyyy-MM-dd') " + filter + ") fd, " + // --当日远期保证金
					// "(select nvl(payout_Z,0)+nvl(Payout_V,0)+nvl(payOut_T,0) value,firmid from F_firmbalance where trunc(B_Date)=to_date('"+Tool.fmtDate(qdate)+"','yyyy-MM-dd') "+filter+") ff, "
					// + //当日待付货款
					"(select nvl(sum(case when code = 'Margin_V' then value when code = 'Margin_Z' then value when code = 'MarginBack_V' then -value when code = 'MarginBack_Z' then -value else 0 end), 0) value,firmid from f_clientledger where code in ('MarginBack_V', 'Margin_V','MarginBack_Z', 'Margin_Z') and to_char(b_date,'yyyy-MM-dd')<='" + Tool.fmtDate(qdate) + "' " + filter + " group by firmid) fg, " + // 当日现货竞价保证金

					"(select nvl(RuntimeSettleMargin,0) value,firmid from t_h_firm where cleardate=to_date('" + Tool.fmtDate(yestody) + "','yyyy-MM-dd') " + filter + ") fb1, " + // --上日交收保证金
					"(select nvl(RuntimeFL,0) value,firmid from t_h_firm where cleardate=to_date('" + Tool.fmtDate(yestody) + "','yyyy-MM-dd') " + filter + ") fc1, " + // --上日浮亏
					"(select nvl(RuntimeMargin,0)-nvl(RuntimeAssure,0) value,firmid from t_h_firm where cleardate=to_date('" + Tool.fmtDate(yestody) + "','yyyy-MM-dd') " + filter + ") fd1, " + // --上日远期保证金
					// "(select nvl(payout_Z,0)+nvl(Payout_V,0)+nvl(payOut_T,0) value,firmid from F_firmbalance where trunc(B_Date)=to_date('"+Tool.fmtDate(yestody)+"','yyyy-MM-dd') "+filter+") ff1, "
					// + //上日待付货款
					"(select nvl(sum(case when code = 'Margin_V' then value when code = 'Margin_Z' then value when code = 'MarginBack_V' then -value when code = 'MarginBack_Z' then -value else 0 end), 0) value,firmid from f_clientledger where code in ('MarginBack_V', 'Margin_V','MarginBack_Z', 'Margin_Z') and to_char(b_date,'yyyy-MM-dd')<='" + Tool.fmtDate(yestody) + "' " + filter + " group by firmid) fg1, " + // 上日现货竞价保证金
					...
					"(" + "select nvl(ia.value,0) valued,nvl(ib.value,0) valuez,ic.firmid from " + "(select sum(nvl(floatingloss,0)) value,firmid from t_h_firmholdsum where trunc(cleardate)=to_date('" + Tool.fmtDate(qdate) + "','yyyy-MM-dd') " + filter + " group by firmid) ia," + // 当日浮动盈亏
					"(select sum(nvl(floatingloss,0)) value,firmid from t_h_firmholdsum where trunc(cleardate)=to_date('" + Tool.fmtDate(yestody) + "','yyyy-MM-dd') " + filter + " group by firmid) ib," + // 上日浮动盈亏
					...
					// +
					// ") j," +
					"(select nvl(sum(nvl(value,0)),0) value,firmid  from f_clientledger where code in ('OtherItem','OtherItem_Z','OtherItem_V') and trunc(B_Date)=to_date('" + Tool.fmtDate(qdate) + "','yyyy-MM-dd') " + filter + " group by firmid) k, " + "(select todaybalance value,firmid from f_firmbalance where b_date=to_date('" + Tool.fmtDate(qdate) + "','yyyy-MM-dd') " + filter + ") g," + // --当日余额
					"(select account1,ACCOUNTNAME1,firmid from f_b_firmidandaccount where 1=1 and (account1 is not null) and account1<>'null' " + filter2 + ") h " + // --和银行接口连接
	 ...
	 */
	public Vector<KXHfailChild> getBankOpen(String bankID, String[] firmIDs, int status, java.util.Date tradeDate) throws SQLException, ClassNotFoundException
	{
	...
	public Vector<FirmBalance> getFirmBalance(String bankID, String[] firmIDs, java.util.Date qdate) throws SQLException, Exception
	{
		...
		try
		{
			...
			// sql =
			// "select Bank.firmid firmid,Bank.bankID bankid,Bank.account account,Bank.accountName accountName,Bank.cardType cardType,Bank.card card,nvl(Fee.value,0) Fee,"
			// +
			// "nvl(Fee.value,0)+nvl(Income.value,0)-nvl(Payout.value,0)+nvl(PLY.value,0)-nvl(PLK.value,0)+nvl(Margin.value,0)+nvl(Balance.value,0)-nvl(InMoney.value,0)+nvl(OutMoney.value,0)+nvl(Other.value,0) QYChangeMoney from "
			// +
			// "(" +
			// "select fc.firmid firmid,nvl(sum(nvl(fc.value,0)),0) value from f_clientledger fc where exists (select code from f_ledgerfield fl where code like '%Fee%' and fc.code=fl.code ) "+Fee_filter+" group by firmid"
			// +
			// ") Fee," +//交易商手续费
			// "(" +
			// "select fc.firmid firmid,nvl(sum(nvl(fc.value,0)),0) value from f_clientledger fc where exists (select code from f_ledgerfield fl where code like '%Income%' and fc.code=fl.code ) "+Income_filter+" group by firmid"
			// +
			// ") Income," +//销售收入
			// "(" +
			// "select fc.firmid firmid,nvl(sum(nvl(fc.value,0)),0) value from f_clientledger fc where exists (select code from f_ledgerfield fl where code like '%Payout%' and fc.code=fl.code ) "+Payout_filter+" group by firmid"
			// +
			// ") Payout," +//购货支出
			// "(" +
			// "select fc.firmid firmid,nvl(sum(nvl(fc.value,0)),0) value from f_clientledger fc where value>0 and exists (select code from f_ledgerfield fl where code like '%PL%' and fc.code=fl.code ) "+PLY_filter+" group by firmid"
			// +
			// ") PLY," +//盈利金额
			// "(" +
			// "select fc.firmid firmid,nvl(sum(nvl(fc.value,0)),0) value from f_clientledger fc where value<0 and exists (select code from f_ledgerfield fl where code like '%PL%' and fc.code=fl.code ) "+PLK_filter+" group by firmid"
			// +
			// ") PLK," +//亏损金额
			// "(" +
			// "select fc.firmid firmid,sum(case when (code='MarginBack_Z' or code='MarginBack_V') then -fc.value else fc.value end) value from f_clientledger fc where exists (select fl.code from f_ledgerfield fl where code like '%Margin%' and fc.code=fl.code) "+Margin_filter+" group by fc.firmid"
			// +
			// ") Margin," +//保证金变化
			// "(" +
			...
			// ") OutMoney," +//交易商出金
			// "(" +
			// "select Bank.firmid firmid,nvl(JSD.value,0)-nvl(JSY.value,0)+nvl(YQD.value,0)-nvl(YQY.value,0)+nvl(FLD.value,0)-nvl(FLY.value,0)+nvl(FTD.value,0)-nvl(FTY.value,0) value from "
			// +
			// "(" +
			...
			if("qy".equalsIgnoreCase(Tool.getConfig("QSMode")))  {
				sql = "select i.firmid , " + "i.bankid, " + "i.account, " + "i.accountname, " + "i.cardtype, " + "i.card, " +
						...
						"nvl(Fee.value,0) Fee ," +
						"nvl(InMoney.value, 0) - nvl(OutMoney.value, 0) crjsum," +
						"nvl(todayb.todaybalance, 0) qymoney," +
						"nvl(a.todaybalance, 0) yesqymoney," +
						"nvl(a1.todaybalance, 0) + nvl(b1.RuntimeMargin, 0) + nvl(b1.RuntimeFL, 0) + nvl(b1.RuntimeSettleMargin, 0) + nvl(c1.floatingloss, 0) + nvl(d1.money, 0)+ " + "nvl(e1.money, 0) + nvl(Fee.value,0) + nvl(OutMoney.value, 0) - nvl(InMoney.value, 0) - (nvl(a.todaybalance, 0) + nvl(b.RuntimeMargin, 0) + nvl(b.RuntimeFL, 0) + nvl(b.RuntimeSettleMargin, 0) + nvl(c.floatingloss, 0) + nvl(d.money, 0)+ " + "nvl(e.money, 0)) QYChangeMoney " + 
						"from " +
						"(select firmid, todaybalance from f_firmbalance where 1 = 1 " + filter_f_fb + ") a," +
						"(select firmid,(RuntimeMargin - RuntimeAssure) RuntimeMargin, RuntimeFL, RuntimeSettleMargin from t_h_firm where 1 = 1 " + filter_t_h_f + ") b," +
						"(select firmid,nvl(sum(floatingloss), 0) floatingloss from t_h_firmholdsum where 1 = 1 " + filter_t_h_f + " group by firmid) c," +
						"(select firmid,nvl(sum(case when code = 'Margin_V' then value when code = 'MarginBack_V' then -value else 0 end), 0) money from f_clientledger where code in ('MarginBack_V', 'Margin_V') " + filter_z_v + " group by firmid) d," +
						"(select firmid,nvl(sum(case when code = 'Margin_Z' then value when code = 'MarginBack_Z' then -value else 0 end), 0) money from f_clientledger where code in ('MarginBack_Z', 'Margin_Z') " + filter_z_v + " group by firmid) e," +
						"(select firmid, nvl(sum(value), 0) money from f_clientledger where code like 'Payout%' " + filter_f_fb + " group by firmid) f," +
						"(select firmid,nvl(sum(value),0) value from f_clientledger where 1=1 and code='Deposit' " + filter_Fee + " group by firmid " + ") InMoney," + // 交易商入金
						"(select firmid,nvl(sum(value),0) value from f_clientledger where 1=1 and code='Fetch' " + filter_Fee + " group by firmid " + ") OutMoney," + // 交易商出金
						"(select firmid,bankid,accountname,account,isopen,cardType, card from f_b_Firmidandaccount) i ," +
						"(select fc.firmid firmid,nvl(sum(nvl((case when value <0 then 0 else value end), 0)), 0) value from f_clientledger fc where exists (select code from f_ledgerfield fl where code like '%Fee%' and fc.code=fl.code ) "+Fee_filter+" group by firmid" +
		                ") Fee," +//交易商手续费
						"(select firmid, todaybalance from f_firmbalance where 1 = 1 and b_date = to_date('" + Tool.fmtDate(qdate) + "', 'yyyy-MM-dd')) todayb ," + // 当日可用
						"(select firmid, todaybalance from f_firmbalance where 1 = 1 "+filter_f_fb_today+") a1,"+
						"(select firmid,(RuntimeMargin - RuntimeAssure) RuntimeMargin, RuntimeFL, RuntimeSettleMargin from t_h_firm where 1 = 1 "+filter_t_h_f_today+") b1,"+
						"(select firmid,nvl(sum(floatingloss), 0) floatingloss from t_h_firmholdsum where 1 = 1 "+filter_t_h_f_today+" group by firmid) c1,"+
						"(select firmid,nvl(sum(case when code = 'Margin_V' then value when code = 'MarginBack_V' then -value else 0 end), 0) money from f_clientledger where code in ('MarginBack_V', 'Margin_V') "+filter_z_v_today+" group by firmid) d1,"+
						"(select firmid,nvl(sum(case when code = 'Margin_Z' then value when code = 'MarginBack_Z' then -value else 0 end), 0) money from f_clientledger where code in ('MarginBack_Z', 'Margin_Z') "+filter_z_v_today+" group by firmid) e1 "+
						"where " + "i.firmid = a.firmid(+) " +
						"and i.firmid = b.firmid(+) " +
						"and i.firmid = c.firmid(+) " +
						"and i.firmid = d.firmid(+) " +
						"and i.firmid = e.firmid(+) " +
						"and i.firmid = f.firmid(+) " +
				...
				sql = "select i.firmid , " + "i.bankid, " + "i.account, " + "i.accountname, " + "i.cardtype, " + "i.card, " +
						...
						"(select firmid,nvl(sum(value),0) value from f_clientledger where 1=1 and code='Fetch' " + filter_Fee + " group by firmid " + ") OutMoney," + // 交易商出金
						"(select firmid,bankid,accountname,account,isopen,cardType, card from f_b_Firmidandaccount) i ," +
						//"(select fc.firmid firmid,nvl(sum(nvl(fc.value,0)),0) value from f_clientledger fc where exists (select code from f_ledgerfield fl where code like '%Fee%' and fc.code=fl.code ) " + Fee_filter + " group by firmid" + ") Fee," + // 交易商手续费
						"(select fc.firmid firmid,nvl(sum(nvl((case when value <0 then 0 else value end), 0)), 0) value from f_clientledger fc where exists (select code from f_ledgerfield fl where code like '%Fee%' and fc.code=fl.code ) "+Fee_filter+" group by firmid" +
		                ") Fee," +//交易商手续费
						"(select firmid, todaybalance from f_firmbalance where 1 = 1 and b_date = to_date('" + Tool.fmtDate(qdate) + "', 'yyyy-MM-dd')) todayb " + // 当日可用
						"where " + "i.firmid = a.firmid(+) " +
	 ...
	 * 查询银行错误信息表
	 */
	public Vector<QSRresult> getQSList(String bankID, String[] firmIDs, int status, java.util.Date tradeDate) throws SQLException, ClassNotFoundException
	{
	 ...
	 * @throws ClassNotFoundException
	 */
	public Vector<HXSentQSMsgValue> getHXQSMsg(String bankID, String[] firmIDs, java.util.Date tradeDate) throws SQLException, ClassNotFoundException
	{
		...
		{
			conn = this.getConnection();
			sql = "select bank.*,nvl(rsm.value,0) rsm,nvl(rm.value,0) rm,nvl(Margin.value,0)-nvl(MarginBack.value,0) Margin,nvl(rfl.value,0) rfl,nvl(fl.value,0) fl," + "(nvl(rsm.value,0)-nvl(rsmy.value,0)+nvl(rm.value,0)-nvl(rmy.value,0)+nvl(Marginch.value,0)-nvl(MarginBackch.value,0)+nvl(rfl.value,0)-nvl(rfly.value,0)" + "+nvl(fl.value,0)-nvl(fly.value,0)+nvl(balance.todaybalance,0)-nvl(balance.lastbalance,0)-nvl(inmoney.value,0)+nvl(outmoney.value,0)) balanceChange," + "(nvl(rsm.value,0)+nvl(rm.value,0)+nvl(Margin.value,0)-nvl(MarginBack.value,0)+nvl(rfl.value,0)+nvl(fl.value,0)) frozen," + "nvl(balance.todaybalance,0) balance,nvl(balance.lastbalance,0) lastbalance,nvl(inmoney.value,0) inmoney,nvl(outmoney.value,0) outmoney," + "(nvl(rsm.value,0)+nvl(rm.value,0)+nvl(Margin.value,0)-nvl(MarginBack.value,0)+nvl(rfl.value,0)+nvl(fl.value,0)+nvl(balance.todaybalance,0)) quanyi " + "from " + "(select sum(nvl(RuntimeSettleMargin,0)) value,firmid from t_h_firm where 1=1 " + djfilter + " group by firmid) rsm, " + // --当日交收保证金
					"(select sum(nvl(RuntimeMargin,0)-nvl(RuntimeAssure,0)) value,firmid from t_h_firm where 1=1 " + djfilter + " group by firmid) rm, " + // --当日远期保证金

					"(select sum(nvl(RuntimeSettleMargin,0)) value,firmid from t_h_firm where 1=1 " + djfiltery + " group by firmid) rsmy, " + // --上日交收保证金
					"(select sum(nvl(RuntimeMargin,0)-nvl(RuntimeAssure,0)) value,firmid from t_h_firm where 1=1 " + djfiltery + " group by firmid) rmy, " + // --上日远期保证金
					...
					"(select sum(nvl(value,0)) value,firmid from f_clientledger fc where code in ('Margin_V','Margin_Z') " + marginfilter + " group by firmid) Margin, " + // --收取现货竞价保证金
					"(select sum(nvl(value,0)) value,firmid from f_clientledger fc where code in ('MarginBack_V','MarginBack_Z') " + marginfilter + " group by firmid) MarginBack, " + // --返还现货竞价保证金
					"(select sum(nvl(value,0)) value,firmid from f_clientledger fc where code in ('Margin_V','Margin_Z') " + bhfilter + " group by firmid) Marginch, " + // --收取现货竞价保证金
					"(select sum(nvl(value,0)) value,firmid from f_clientledger fc where code in ('MarginBack_V','MarginBack_Z') " + bhfilter + " group by firmid) MarginBackch, " + // --返还现货竞价保证金
					...
					"(select * from f_b_firmidandaccount where isopen=1 " + bankfilter + ") bank " + "where bank.firmid=rsm.firmid(+) and bank.firmid=rm.firmid(+) and bank.firmid=Margin.firmid(+) and bank.firmid=MarginBack.firmid(+) " + " and bank.firmid=income.firmid(+) and bank.firmid=payout.firmid(+) and bank.firmid=rsmy.firmid(+) and bank.firmid=rmy.firmid(+)  " + "and bank.firmid=rfl.firmid(+) and bank.firmid=fl.firmid(+) and bank.firmid=balance.firmid(+) and bank.firmid=inmoney.firmid(+) " + "and bank.firmid=outmoney.firmid(+) and bank.firmid=Marginch.firmid(+) and bank.firmid=MarginBackch.firmid(+) and bank.firmid=rfly.firmid(+) " + " and bank.firmid=fly.firmid(+) ";
			System.out.println("sql:" + sql);
			state = conn.prepareStatement(sql);
			rs = state.executeQuery();
			while (rs.next())
	...
	public int addChangeFirmRights(String bankID, String[] firmIDs, java.util.Date tradeDate, Connection conn) throws SQLException
	{
		...
		try
		{
			...
			sql = "insert into F_B_Tradelist (TRADE_MONEY,TRADE_TYPE,B_MEMBER_CODE,B_MEMBER_NAME,S_MEMBER_CODE,S_MEMBER_NAME,TRADE_DATE,BARGAIN_CODE,SERIAL_ID,GOOD_CODE,GOOD_NAME,GOOD_QUANTITY,FLAG,BANKID,CREATEDATE) " + "select TRADE_MONEY,TRADE_TYPE,B_MEMBER_CODE,B_MEMBER_NAME,S_MEMBER_CODE,S_MEMBER_NAME,to_date('" + Tool.fmtTime(tradeDate) + "','yyyy-MM-dd hh24:mi:ss') TRADE_DATE,'' BARGAIN_CODE,'' || SEQ_F_B_TRADELIST.nextval SERIAL_ID,'' GOOD_CODE,'' GOOD_NAME,0 GOOD_QUANTITY,'N' FLAG,BANKID,sysdate CREATEDATE from " + "(" + "select nvl(SettleCompens.value,0)+nvl(Income.value,0)-nvl(Payout.value,0)+nvl(PL1.value,0)-nvl(PL2.value,0) TRADE_MONEY,1 TRADE_TYPE,'666666' B_MEMBER_CODE,'市场' B_MEMBER_NAME " + ",bank.firmid S_MEMBER_CODE,bank.accountName S_MEMBER_NAME,bank.bankid BANKID from " + "(select firmid,sum(nvl(value,0)) value from f_clientledger fc where fc.code='SettleCompens' " + filter1 + " group by firmid) SettleCompens," + // --
					// 交割补偿费
					"(select firmid,sum(nvl(value,0)) value from f_clientledger fc where exists(select code from f_ledgerfield fl where fc.code=fl.code and fl.code like 'Income%') " + filter1 + " group by firmid) Income," + // --
					// 交易商收货款
					"(select firmid,sum(nvl(value,0)) value from f_clientledger fc where exists(select code from f_ledgerfield fl where fc.code=fl.code and fl.code like 'Payout%') " + filter1 + " group by firmid) Payout," + // --
					// 交易商付货款
					"(select firmid,sum(nvl(value,0)) value from f_clientledger fc where exists(select code from f_ledgerfield fl where fc.code=fl.code and fl.code like '%PL') and value>=0 " + filter1 + " group by firmid) PL1," + // --
					// 盈利
					"(select firmid,-sum(nvl(value,0)) value from f_clientledger fc where exists(select code from f_ledgerfield fl where fc.code=fl.code and fl.code like '%PL') and value<0 " + filter1 + " group by firmid) PL2," + // --
					// 亏损
					"(select * from f_b_firmidandaccount where 1=1 and firmid not like 'D_%' " + bankfilter + ") bank " + "where bank.firmid=SettleCompens.firmid(+) and bank.firmid=Income.firmid(+) and bank.firmid=Payout.firmid(+) and bank.firmid=PL1.firmid(+) and bank.firmid=PL2.firmid(+) " + // --
					// 交易变化量
					"union all " + "select nvl(rfl.value,0)-nvl(rfl1.value,0) TRADE_MONEY,4 TRADE_TYPE,bank.firmid B_MEMBER_CODE,bank.accountName B_MEMBER_NAME,'555555' S_MEMBER_CODE,'市场' S_MEMBER_NAME,bank.bankid BANKID from " + "(select sum(nvl(RuntimeFL,0)) value,firmid from t_h_firm where 1=1 " + filter2 + " group by firmid) rfl," + // --当日浮亏
					"(select sum(nvl(RuntimeFL,0)) value,firmid from t_h_firm where 1=1 " + filter21 + " group by firmid) rfl1," + // --上日浮亏
					"(select * from f_b_firmidandaccount where 1=1 and firmid not like 'D_%' " + bankfilter + ") bank " + "where bank.firmid=rfl.firmid(+) and bank.firmid=rfl1.firmid(+)" + // --
					// 浮亏变化量
					"union all " + "select nvl(tf.value,0)+nvl(bf.value,0) TRADE_MONEY,2 TRADE_TYPE,bank.firmid B_MEMBER_CODE,bank.accountName B_MEMBER_NAME,'888888' S_MEMBER_CODE,'市场' S_MEMBER_NAME,bank.bankid BANKID from " + "(select firmid,sum(nvl(value,0)) value from f_clientledger fc where exists(select code from f_ledgerfield fl where fc.code=fl.code and fl.code like 'TradeFee%') " + filter1 + " group by firmid) tf," + // --
					// 交易手续费
					"(select firmid,sum(nvl(value,0)) value from f_clientledger fc where fc.code='BankFee' group by firmid) bf," + // --
					// 银行接口手续费
					"(select * from f_b_firmidandaccount where 1=1 and firmid not like 'D_%' " + bankfilter + ") bank " + "where bank.firmid=tf.firmid(+) and bank.firmid=bf.firmid(+) " + // --
					// 交易手续费
					"union all " + "select nvl(sf.value,0) TRADE_MONEY,3 TRADE_TYPE,bank.firmid B_MEMBER_CODE,bank.accountName B_MEMBER_NAME,'101011' S_MEMBER_CODE,'市场' S_MEMBER_NAME,bank.bankid BANKID from " + "(select firmid,sum(nvl(value,0)) value from f_clientledger fc where exists(select code from f_ledgerfield fl where fc.code=fl.code and fl.code like 'SettleFee%') " + filter1 + " group by firmid) sf," + "(select * from f_b_firmidandaccount where 1=1 and firmid not like 'D_%' " + bankfilter + ") bank " + "where bank.firmid=sf.firmid(+) " + // --
					// 交收手续费
					"union all " + "select nvl(OtherItem.value,0) TRADE_MONEY,5 TRADE_TYPE,'444444' B_MEMBER_CODE,'市场' B_MEMBER_NAME,bank.firmid S_MEMBER_CODE,bank.accountName S_MEMBER_NAME,bank.bankid BANKID from " + "(select firmid,sum(nvl(value,0)) value from f_clientledger fc where exists(select code from f_ledgerfield fl where fc.code=fl.code and fl.code like 'OtherItem%') " + filter1 + " group by firmid) OtherItem," + // --
					// 当日其他项
					"(select * from f_b_firmidandaccount where 1=1 and firmid not like 'D_%' " + bankfilter + ") bank " + "where bank.firmid=OtherItem.firmid(+)" + // --
					// 当日其他项
					")";// +
			// "where TRADE_MONEY<>0";
			System.out.println(sql);
	...
	public int addChangeFirmFrozen(String bankID, String[] firmIDs, java.util.Date qdate, Connection conn) throws SQLException
	{
		...
		try
		{
			...
			sql = "insert into F_B_MARGINS (SERIAL_ID,BARGAIN_CODE,TRADE_TYPE,TRADE_MONEY,MEMBER_CODE,MEMBER_NAME,TRADE_DATE,GOOD_CODE,GOOD_NAME,GOOD_QUANTITY,FLAG,BANKID,CREATEDATE) " + "select '' || SEQ_F_B_MARGINS.nextval SERIAL_ID,'' BARGAIN_CODE,case when TRADE_MONEY>0 then 1 else 2 end TRADE_TYPE, abs(TRADE_MONEY) TRADE_MONEY,MEMBER_CODE,MEMBER_NAME,to_date('" + Tool.fmtTime(qdate) + "','yyyy-MM-dd hh24:mi:ss') TRADE_DATE,'' GOOD_CODE,'' GOOD_NAME,0 GOOD_QUANTITY,'N' FLAG,BANKID,sysdate CREATEDATE from " + "(" + "select nvl(rsm.value,0)+nvl(rm.value,0)+nvl(Margin.value,0)-nvl(rm2.value,0) TRADE_MONEY,bank.firmid MEMBER_CODE,bank.accountName MEMBER_NAME,bank.bankid bankid from " + "(select sum(nvl(RuntimeSettleMargin,0)-nvl(ClearSettleMargin,0)) value,firmid from t_h_firm where 1=1 " + filter1 + " group by firmid) rsm, " + // --
					// 当日交收保证金
					"(select sum(nvl(RuntimeMargin,0)-nvl(RuntimeAssure,0)) value,firmid from t_h_firm where 1=1 " + filter1 + " group by firmid) rm, " + // --
					// 当日远期保证金
					"(select nvl(sum(case when code = 'Margin_V' then value when code = 'Margin_Z' then value when code = 'MarginBack_V' then -value when code = 'MarginBack_Z' then -value else 0 end), 0) value,firmid from f_clientledger fc where exists (select code from f_ledgerfield fl where fc.code=fl.code and fl.code like 'Margin%') " + filter2 + " group by firmid) Margin, " + // --
					// 当日现货竞价保证金
					"(select sum(nvl(RuntimeMargin,0)-nvl(RuntimeAssure,0)) value,firmid from t_h_firm where 1=1 " + filter11 + " group by firmid) rm2, " + // --
					// 上日远期保证金
					"(select * from f_b_firmidandaccount where 1=1 and firmid not like 'D_%' " + bankfilter + ") bank " + "where bank.firmid=rsm.firmid(+) and bank.firmid=rm.firmid(+) and bank.firmid=Margin.firmid(+) and bank.firmid=rm2.firmid(+) " + ")";// +
			// " where TRADE_MONEY<>0";
			state = conn.prepareStatement(sql);
			result = state.executeUpdate();
		} catch (SQLException e)
	...
	public RZQSValue getXYQSValue(String bankID, String[] firmIDs, java.util.Date tradeDate) throws SQLException, ClassNotFoundException
	{
		...
		try
		{
			...
			sql = "select " + " bank.*" + ",nvl(Fee.value,0) maketMoney" + // 自有资金
					",0-(nvl(Income.value,0)-nvl(Payout.value,0)+nvl(PLY.value,0)-nvl(PLK.value,0)+nvl(OtherItem.value,0)+nvl(thf.value,0)-nvl(thfy.value,0)) bankErrorMoney" + // 银行间扎差
					",nvl(Income.value,0)-nvl(Payout.value,0)+nvl(PLY.value,0)-nvl(PLK.value,0)-nvl(Fee.value,0)+nvl(OtherItem.value,0)+nvl(thf.value,0)-nvl(thfy.value,0) firmErrorMoney" + // 权益变化量
					",nvl(hmsg.rsm,0)-nvl(hmsgy.rsm,0)+nvl(hmsg.rma,0)-nvl(hmsgy.rma,0)+nvl(hmsg.fl,0)-nvl(hmsgy.fl,0)+nvl(zvm.value,0)-nvl(zvmy.value,0)+nvl(thf.value,0)-nvl(thfy.value,0) cash" + // 冻结资金变化量
					...
					" from " + "(select firmID,sum(value) value from f_clientledger where code like 'Income%' " + fcfilter1 + " group by firmID) Income," + // 销售收入
					...
					"(select firmID,sum(value) value from f_clientledger where code like 'OtherItem%' " + fcfilter1 + "  group by firmID) OtherItem," + // 当日其他项
					"(select firmID,sum(case when code in ('Margin_V','Margin_Z') then value else -value end) value from f_clientledger where code in ('MarginBack_V','Margin_V','MarginBack_Z','Margin_Z') " + fcfilter2 + " group by firmID) zvm," + // 当日现货竞价保证金
					"(select firmID,sum(case when code in ('Margin_V','Margin_Z') then value else -value end) value from f_clientledger where code in ('MarginBack_V','Margin_V','MarginBack_Z','Margin_Z') " + fcfilter2y + " group by firmID) zvmy," + // 上日现货竞价保证金
					...
					"(select firmID,RuntimeSettleMargin rsm,(RuntimeMargin-RuntimeAssure) rma,RuntimeFL fl from T_h_firm where 1=1 " + thfilter + ") hmsg," + // 当日交收、远期保证金，浮亏
					"(select firmID,RuntimeSettleMargin rsm,(RuntimeMargin-RuntimeAssure) rma,RuntimeFL fl from T_h_firm where 1=1 " + thfiltery + ") hmsgy," + // 上日交收、远期保证金，浮亏
					...
					" where bank.firmID=Income.firmID(+) and bank.firmID=Payout.firmID(+) and bank.firmID=PLY.firmID(+) and bank.firmID=PLK.firmID(+) " + " and bank.firmID=Fee.firmID(+) and bank.firmID=OtherItem.firmID(+) and bank.firmID=zvm.firmID(+) and bank.firmID=zvmy.firmID(+) " + " and bank.firmID=inMoney.firmID(+) and bank.firmID=outMoney.firmID(+) and bank.firmID=hmsg.firmID(+) and bank.firmID=hmsgy.firmID(+) " + " and bank.firmID=balance.firmID(+) and bank.firmID=thf.firmID(+) and bank.firmID=thfy.firmID(+) order by bank.firmID";
	...
	public RZDZValue getXYDZValue(String bankID, String[] firmIDs, java.util.Date tradeDate) throws SQLException, ClassNotFoundException
	{
		...
		try
		{
			...
			sql = "select " + " bank.*" + ",nvl(Fee.value,0) marketMoney" + // 自有资金
					",nvl(Income.value,0)-nvl(Payout.value,0)+nvl(PLY.value,0)-nvl(PLK.value,0)+nvl(OtherItem.value,0)+nvl(thf.value,0)-nvl(thfy.value,0) bankErrorMoney" + // 银行间扎差
					",nvl(Income.value,0)-nvl(Payout.value,0)+nvl(PLY.value,0)-nvl(PLK.value,0)-nvl(Fee.value,0)+nvl(OtherItem.value,0)+nvl(thf.value,0)-nvl(thfy.value,0) firmErrorMoney" + // 权益变化量
					",nvl(hmsg.rsm,0)-nvl(hmsgy.rsm,0)+nvl(hmsg.rma,0)-nvl(hmsgy.rma,0)+nvl(hmsg.fl,0)-nvl(hmsgy.fl,0)+nvl(zvm.value,0)-nvl(zvmy.value,0)+nvl(thf.value,0)-nvl(thfy.value,0) cash" + // 冻结资金变化量
					",nvl(balance.todaybalance,0)-nvl(balance.lastbalance,0)-nvl(inMoney.value,0)+nvl(outMoney.value,0) availableBalance" + // 可用资金变化量
					",nvl(hmsg.rsm,0)+nvl(hmsg.rma,0)+nvl(hmsg.fl,0)+nvl(zvm.value,0)+nvl(thf.value,0) frozen" + // 冻结资金
	...
	public Vector<FirmBalance> getFirmBalance(String bankID, java.util.Date qdate) throws SQLException, Exception
	{
		...
		try
		{
			...
			sql = "select i.firmid , " + "i.bankid, " + "i.account, " + "i.accountname, " + "i.cardtype, " + "i.card, " + "nvl(a1.todaybalance, 0) + " + "nvl(b1.RuntimeMargin, 0) + nvl(b1.RuntimeFL, 0) + " + "nvl(b1.RuntimeSettleMargin, 0) + nvl(c1.floatingloss, 0) + " + "nvl(d1.money, 0)+ nvl(e1.money, 0) money ," + "nvl(Fee.value,0) Fee ," + "nvl(OutMoney.value, 0) ," + "nvl(InMoney.value, 0) ," + "nvl(a1.todaybalance, 0) + nvl(b1.RuntimeMargin, 0) + nvl(b1.RuntimeFL, 0) + nvl(b1.RuntimeSettleMargin, 0) + nvl(c1.floatingloss, 0) + nvl(d1.money, 0)+ nvl(e1.money, 0) -" + "nvl(a.todaybalance, 0) - nvl(b.RuntimeMargin, 0) - nvl(b.RuntimeFL, 0) - nvl(b.RuntimeSettleMargin, 0) - nvl(c.floatingloss, 0) - nvl(d.money, 0)- nvl(e.money, 0) +" + "nvl(Fee.value,0) + nvl(OutMoney.value, 0) - nvl(InMoney.value, 0) QYChangeMoney " + "from " + "(select firmid, todaybalance from f_firmbalance where 1 = 1 " + filter_f_fb + ") a," + "(select firmid,(RuntimeMargin - RuntimeAssure) RuntimeMargin, RuntimeFL, RuntimeSettleMargin from t_h_firm where 1 = 1 " + filter_t_h_f + ") b," + "(select firmid,nvl(sum(floatingloss), 0) floatingloss from t_h_firmholdsum where 1 = 1 " + filter_t_h_f + " group by firmid) c," + "(select firmid,nvl(sum(case when code = 'Margin_V' then value when code = 'MarginBack_V' then -value else 0 end), 0) money from f_clientledger where code in ('MarginBack_V', 'Margin_V') " + filter_z_v + " group by firmid) d," + "(select firmid,nvl(sum(case when code = 'Margin_Z' then value when code = 'MarginBack_Z' then -value else 0 end), 0) money from f_clientledger where code in ('MarginBack_Z', 'Margin_Z') " + filter_z_v + " group by firmid) e," +
			...
			"(select firmid,nvl(sum(value),0) value from f_clientledger where 1=1 and code='Deposit' " + filter_Fee + " group by firmid " + ") InMoney," + // 交易商入金
					...
					"(select firmid, todaybalance from f_firmbalance where 1 = 1 " + filter_f_fb_today + ") a1," + "(select firmid,(RuntimeMargin - RuntimeAssure) RuntimeMargin, RuntimeFL, RuntimeSettleMargin from t_h_firm where 1 = 1 " + filter_t_h_f_today + ") b1," + "(select firmid,nvl(sum(floatingloss), 0) floatingloss from t_h_firmholdsum where 1 = 1 " + filter_t_h_f_today + " group by firmid) c1," + "(select firmid,nvl(sum(case when code = 'Margin_V' then value when code = 'MarginBack_V' then -value else 0 end), 0) money from f_clientledger where code in ('MarginBack_V', 'Margin_V') " + filter_z_v_today + " group by firmid) d1," + "(select firmid,nvl(sum(case when code = 'Margin_Z' then value when code = 'MarginBack_Z' then -value else 0 end), 0) money from f_clientledger where code in ('MarginBack_Z', 'Margin_Z') " + filter_z_v_today + " group by firmid) e1 " + "where " + "i.firmid = a.firmid(+) " + "and i.firmid = b.firmid(+) " + "and i.firmid = c.firmid(+) " + "and i.firmid = d.firmid(+) " + "and i.firmid = e.firmid(+) " + "and i.firmid = OutMoney.firmid(+) " + "and i.firmid = InMoney.firmid(+) " + "and i.firmid = FEE.firmid(+) " + "and i.firmid = a1.firmid(+) " + "and i.firmid = b1.firmid(+) " + "and i.firmid = c1.firmid(+) " + "and i.firmid = d1.firmid(+) " + "and i.firmid = e1.firmid(+) " + "and i.isopen = '1' " + filter + " order by a.firmid";
	...
	public Vector<BankQSNetChild> getQSBankDate(String bankID, java.util.Date qdate) throws Exception
	{
		...
		try
		{
			...
			sql = "select h.firmid," + "nvl(a.value,0)" + "+(case when nvl(k.value,0)>0 then nvl(k.value,0) else 0 end)" + " AddTranAmount," + // --销售收入
					...
					"nvl(a1.todaybalance, 0) + nvl(b1.RuntimeMargin, 0) + nvl(b1.RuntimeFL, 0) + nvl(b1.RuntimeSettleMargin, 0) + nvl(c1.floatingloss, 0) + nvl(d1.money, 0)+ nvl(e1.money, 0) todayQY," + // 当日权益
					"nvl(a2.todaybalance, 0) + nvl(b2.RuntimeMargin, 0) + nvl(b2.RuntimeFL, 0) + nvl(b2.RuntimeSettleMargin, 0) + nvl(c2.floatingloss, 0) + nvl(d2.money, 0)+ nvl(e2.money, 0) lastDayQY," + // 上日权益
					"nvl(a1.todaybalance, 0) + nvl(b1.RuntimeMargin, 0) + nvl(b1.RuntimeFL, 0) + nvl(b1.RuntimeSettleMargin, 0) + nvl(c1.floatingloss, 0) + nvl(d1.money, 0)+ nvl(e1.money, 0) -" + "(nvl(a2.todaybalance, 0) + nvl(b2.RuntimeMargin, 0) + nvl(b2.RuntimeFL, 0) + nvl(b2.RuntimeSettleMargin, 0) + nvl(c2.floatingloss, 0) + nvl(d2.money, 0)+ nvl(e2.money, 0)) +" + "nvl(Fee.value,0) + nvl(OutMoney.value, 0) - nvl(InMoney.value, 0) QYChangeMoney " + // 当日交易盈亏变化量
					// en
					"from " + "(select fc.firmid,sum(nvl(fc.value,0)) value from f_ledgerfield fl,f_clientledger fc where fl.code = fc.code and fc.b_date=to_date('" + Tool.fmtDate(qdate) + "','yyyy-MM-dd') and fl.code in ('Income','Income_Z','Income_V') " + filter + " group by fc.firmid) a, " + // --销售收入
					"(select fc.firmid,sum(nvl(fc.value,0)) value from f_ledgerfield fl,f_clientledger fc where fl.code = fc.code and fc.b_date=to_date('" + Tool.fmtDate(qdate) + "','yyyy-MM-dd') and fl.code in ('Payout','Payout_Z','Payout_V') " + filter + " group by fc.firmid) b, " + // --购货支出
					"(select fc.firmid,sum(nvl(fc.value,0)) value from f_ledgerfield fl,f_clientledger fc where fl.code = fc.code and fc.b_date=to_date('" + Tool.fmtDate(qdate) + "','yyyy-MM-dd') and fl.code in ('TradePL','SettlePL') and value>=0 " + filter + " group by fc.firmid) c, " + // --盈利
					...
					// st
					"(select firmid, todaybalance from f_firmbalance where 1 = 1 " + filter_f_fb + ") a2," + "(select firmid,(RuntimeMargin - RuntimeAssure) RuntimeMargin, RuntimeFL, RuntimeSettleMargin from t_h_firm where 1 = 1 " + filter_t_h_f + ") b2," + "(select firmid,nvl(sum(floatingloss), 0) floatingloss from t_h_firmholdsum where 1 = 1 " + filter_t_h_f + " group by firmid) c2," + "(select firmid,nvl(sum(case when code = 'Margin_V' then value when code = 'MarginBack_V' then -value else 0 end), 0) money from f_clientledger where code in ('MarginBack_V', 'Margin_V') " + filter_z_v + " group by firmid) d2," + "(select firmid,nvl(sum(case when code = 'Margin_Z' then value when code = 'MarginBack_Z' then -value else 0 end), 0) money from f_clientledger where code in ('MarginBack_Z', 'Margin_Z') " + filter_z_v + " group by firmid) e2," +
					...
					"(select firmid, todaybalance from f_firmbalance where 1 = 1 " + filter_f_fb_today + ") a1," + "(select firmid,(RuntimeMargin - RuntimeAssure) RuntimeMargin, RuntimeFL, RuntimeSettleMargin from t_h_firm where 1 = 1 " + filter_t_h_f_today + ") b1," + "(select firmid,nvl(sum(floatingloss), 0) floatingloss from t_h_firmholdsum where 1 = 1 " + filter_t_h_f_today + " group by firmid) c1," + "(select firmid,nvl(sum(case when code = 'Margin_V' then value when code = 'MarginBack_V' then -value else 0 end), 0) money from f_clientledger where code in ('MarginBack_V', 'Margin_V') " + filter_z_v_today + " group by firmid) d1," + "(select firmid,nvl(sum(case when code = 'Margin_Z' then value when code = 'MarginBack_Z' then -value else 0 end), 0) money from f_clientledger where code in ('MarginBack_Z', 'Margin_Z') " + filter_z_v_today + " group by firmid) e1 ," + "(select bankid,bankName from f_b_banks where validflag = 0 ) j " +
					// en
					...
					" " + "and h.firmid=k.firmid(+) " +
					// st
					"and h.firmid = a2.firmid(+) " + "and h.firmid = b2.firmid(+) " + "and h.firmid = c2.firmid(+) " + "and h.firmid = d2.firmid(+) " + "and h.firmid = e2.firmid(+) " + "and h.firmid = OutMoney.firmid(+) " + "and h.firmid = InMoney.firmid(+) " + "and h.firmid = FEE.firmid(+) " + "and h.firmid = a1.firmid(+) " + "and h.firmid = b1.firmid(+) " + "and h.firmid = c1.firmid(+) " + "and h.firmid = d1.firmid(+) " + "and h.firmid = e1.firmid(+) " + "and h.bankid = j.bankid";
			// en
			log("sql:" + sql);
			System.out.println("sql:" + sql);
			state = conn.prepareStatement(sql);
	 ...
	 * @throws SQLException
	 */
	public int modRgstCapitalValue(String bankid, String firmid, String account, Timestamp banktime, int status, long actionid, int type, Connection conn) throws SQLException
	{
		...
		try
		{
			...
			if (status == 0)
			{
				...
				if (type == 1)
				{// 签约
					sql = " update f_b_firmidandaccount  set status='0' , isopen='1' , " + " opentime=sysdate ,account='" + account + "' " + "where firmid='" + firmid + "' and bankid='" + bankid + "'";
				} else if (type == 2)
				{// 解约
					...
					state = conn.prepareStatement(sql);
					state.executeUpdate();
					sql = " update f_b_firmidandaccount " + " set firmID='D_'||firmID,bankID='D_'||bankID," + " deltime=sysdate  where firmid='" + firmid + "' and bankid='" + bankid + "' ";
				}
	...
	public int addClientState(ClientState state) throws SQLException, ClassNotFoundException {
		...
		try {
			...
			sql = "INSERT INTO F_B_ACCOUNTSTATUS(BANK_ID,EXCHANGE_CODE,AREA_CODE," +
					"TRADE_DATE,ACCOUNT,FUNDS_ACCOUNT,FIRM_NAME,CCYCODE,FLAG,STATUS) " +
					" VALUES(?,?,?,?,?,?,?,?,?,?)";
	...
	public List<TransferAccountsTransactionDetailed> getZZJYMX(String bankID, String[] firmIDs, Date tradeDate, Connection conn) throws SQLException {
		...
		String sql = "SELECT" +
				" a.createtime TransDateTime," + //交易日期时间 yyyyMMDD hhmmss
				...
				" FROM F_B_capitalInfo a, F_B_FIRMIDANDACCOUNT b" +
				" WHERE 1=1 " + filter2 + filter1 + filter +
	...
	/**客户账户状态信息*/
	public List<AccountStatusReconciliation> getKHZHZT(String bankID,String[] firmIDs, Date tradeDate,Connection conn) throws SQLException, ClassNotFoundException {
		...
		try {
			sql = "SELECT" +
					...
					" b.accountname certificationName," +
					" (CASE b.isopen WHEN  0 THEN 1 WHEN 1 THEN 0 END) CashExCode" +
					" FROM  f_b_firmidandaccount b" +
					" WHERE 1=1 " +
					" AND (trunc(b.opentime) = to_date('"+Tool.fmtDate(tradeDate)+"','yyyy-MM-dd')" +
					" OR trunc(b.deltime) = to_date('"+Tool.fmtDate(tradeDate)+"','yyyy-MM-dd'))"+filter;
			System.out.println(sql);
			state = conn.prepareStatement(sql);
			rs = state.executeQuery();
			while (rs.next()) {
	...
	/**存管客户资金交收明细*/
	public List<StorageMoneySettlementList> getCGKHZJJSMX(String bankID,Date tradeDate,Connection conn) throws SQLException,ClassNotFoundException {
		...
		try {
			sql = "SELECT" +
					...
					" FROM f_b_firmidandaccount a," +
					"(" +
					" SELECT c.firmid,nvl(sum(d.fieldsign*c.value),0) VALUE" +
					" FROM" +
					" F_LEDGERFIELD d, F_CLIENTLEDGER c" +
					" WHERE d.code = c.code and  c.code not in('Deposit','Fetch') and trunc(c.b_date) = to_date('"+Tool.fmtDate(tradeDate)+"','yyyy-MM-dd') group by c.firmid" +
					") b" +
	...
	/**存管客户资金台账余额明细*/
	public List<StorageMoneyLedgerBalanceList> getCGKHZJTZYEMX(String bankID,Date tradeDate, Connection conn) throws SQLException,ClassNotFoundException {
		...
		try {
			sql = " SELECT" +
				" a.firmid bondAcc," +
				" a.accountname certificationName," +
				" b.todaybalance money" +
				" FROM f_b_firmidandaccount a,f_firmbalance b" +
				" WHERE a.firmid = b.firmid" +
				" AND a.isopen=1 and a.firmid not like '%D%'"+
	...
	/** 查询交易商扎差数据 */
    public List<BankQSNetChild> getQSFirmDate(String bankID, String firmID,
            java.util.Date qdate) throws Exception
    {
        log("获取某日的清算信息  getQSChild 时间：" + Tool.fmtDate(new java.util.Date()));
        ...
        java.util.Date yes_qdate = getlastDate(qdate, conn);
        String filter_Fee = " and b_date=to_date('" + Tool.fmtDate(qdate)
                + "','yyyy-MM-dd') ";
        String Fee_filter = filter_Fee;
        if (qdate != null)
        {
            filter_f_fb = filter_f_fb + " and b_date = to_date('"
                    + Tool.fmtDate(yes_qdate) + "', 'yyyy-MM-dd') ";
            filter_t_h_f = filter_t_h_f + " and cleardate = to_date('"
                    + Tool.fmtDate(yes_qdate) + "', 'yyyy-MM-dd') ";
            filter_z_v = filter_z_v + " and b_date <= to_date('"
                    + Tool.fmtDate(yes_qdate) + "', 'yyyy-MM-dd') ";

            filter_f_fb_today = filter_f_fb_today + " and b_date = to_date('"
                    + Tool.fmtDate(qdate) + "', 'yyyy-MM-dd') ";
            filter_t_h_f_today = filter_t_h_f_today
                    + " and cleardate = to_date('" + Tool.fmtDate(qdate)
                    + "', 'yyyy-MM-dd') ";
            filter_z_v_today = filter_z_v_today + " and b_date <= to_date('"
                    + Tool.fmtDate(qdate) + "', 'yyyy-MM-dd') ";
        }
        String filter2 = filter
                + " and isopen=1 and firmid not like '%/_D%' escape '/' and firmid not like '%D/_%' escape '/' ";
        if (bankID != null && bankID.trim().length() > 0
                && !bankID.trim().equals("-1"))
        {
            filter2 += " and bankID='" + bankID + "' ";
        }
        if (firmID != null && firmID.trim().length() > 0
                && !firmID.trim().equals("-1"))
        {
        ...
        try
        {
            ...
            sql = "select h.firmid firmID ,"
                    ...
                    + " CutTranAmount,"
                    + // --购货支出
                    "nvl(c.value,0) ProfitAmount,"
                    + // --盈利
                    "nvl(d.value,0) LossAmount,"
                    + // --亏损
                    ...
                    + "j.bankname bankName ,"
                    + "nvl(Fee.value,0) Fee ,"
                    + "nvl(OutMoney.value, 0) OutMoney,"
                    + // 当日出金
                    "nvl(InMoney.value, 0) InMoney,"
                    + // 当日入金
                    ...
                    + Tool.fmtDate(qdate)
                    + "','yyyy-MM-dd') and fl.code in ('Income','Income_Z','Income_V') "
                    + filter
                    + " group by fc.firmid) a, "
                    + // --销售收入
                    "(select fc.firmid,sum(nvl(fc.value,0)) value from f_ledgerfield fl,f_clientledger fc where fl.code = fc.code and fc.b_date=to_date('"
                    + Tool.fmtDate(qdate)
                    + "','yyyy-MM-dd') and fl.code in ('Payout','Payout_Z','Payout_V') "
                    + filter
                    + " group by fc.firmid) b, "
                    + // --购货支出
                    "(select fc.firmid,sum(nvl(fc.value,0)) value from f_ledgerfield fl,f_clientledger fc where fl.code = fc.code and fc.b_date=to_date('"
                    + Tool.fmtDate(qdate)
                    + "','yyyy-MM-dd') and fl.code in ('TradePL','SettlePL') and value>=0 "
                    + filter
                    ...
                    + "','yyyy-MM-dd') and fl.code in ('BankFee','TradeFee','SettleFee','TradeFee_Z','TradeFee_V','SettleFee_Z','SettleCompens') "
                    + filter
                    + " group by fc.firmid) e, "
                    + // --手续费
                    "( "
                    + "select nvl(fb.value,0)+nvl(fc.value,0)+nvl(fd.value,0)"
                    + /* +nvl(ff.value,0) */"+nvl(fg.value,0) valued,nvl(fb1.value,0)+nvl(fc1.value,0)+nvl(fd1.value,0)"
                    + /* +nvl(ff1.value,0) */"+nvl(fg1.value,0) valuez"
                    + /* , nvl(ff.value,0)-nvl(ff1.value,0) ffvalue */", fe.firmid from "
                    + "(select nvl(RuntimeSettleMargin,0) value,firmid from t_h_firm where cleardate=to_date('"
                    + Tool.fmtDate(qdate)
                    + "','yyyy-MM-dd') "
                    + filter
                    + ") fb, "
                    + // --当日交收保证金
                    "(select nvl(RuntimeFL,0) value,firmid from t_h_firm where cleardate=to_date('"
                    + Tool.fmtDate(qdate)
                    + "','yyyy-MM-dd') "
                    + filter
                    + ") fc, "
                    + // --当日浮亏
                    "(select nvl(RuntimeMargin,0)-nvl(RuntimeAssure,0) value,firmid from t_h_firm where cleardate=to_date('"
                    + Tool.fmtDate(qdate)
                    + "','yyyy-MM-dd') "
                    + filter
                    + ") fd, "
                    ...
                    "(select firmid from f_b_firmidandaccount where 1=1 "
                    + filter2
                    + ") fe "
                    + // --和银行接口连接
                    "where fe.firmid=fb.firmid(+) and fe.firmid=fc.firmid(+) and fe.firmid=fd.firmid(+)"
                    ...
                    + /* and fe.firmid=ff1.firmid(+) */" and fe.firmid=fg1.firmid(+) "
                    + ") f, "
                    + // --冻结解冻资金
                    "("
                    + "select nvl(ia.value,0) valued,nvl(ib.value,0) valuez,ic.firmid from "
                    + "(select sum(nvl(floatingloss,0)) value,firmid from t_h_firmholdsum where trunc(cleardate)=to_date('"
                    + Tool.fmtDate(qdate)
                    + "','yyyy-MM-dd') "
                    + filter
                    + " group by firmid) ia,"
                    ...
                    + filter
                    + " group by firmid) k, "
                    + "(select todaybalance value,firmid from f_firmbalance where b_date=to_date('"
                    + Tool.fmtDate(qdate)
                    + "','yyyy-MM-dd') "
                    + filter
                    + ") g,"
                    + // --当日余额
                    "(select account1,ACCOUNTNAME1,firmid,bankid from f_b_firmidandaccount where 1=1 "
                    + filter2
                    + ") h ,"
                    + // --和银行接口连接

                    // st
                    "(select firmid, todaybalance from f_firmbalance where 1 = 1 "
                    + filter_f_fb
                    + ") a2,"
                    + "(select firmid,(RuntimeMargin - RuntimeAssure) RuntimeMargin, RuntimeFL, RuntimeSettleMargin from t_h_firm where 1 = 1 "
                    + filter_t_h_f
                    + ") b2,"
                    + "(select firmid,nvl(sum(floatingloss), 0) floatingloss from t_h_firmholdsum where 1 = 1 "
                    + filter_t_h_f
                    + " group by firmid) c2,"
                    ...
                    "(select firmid,nvl(sum(value),0) value from f_clientledger where 1=1 and code='Deposit' "
                    + filter_Fee
                    + " group by firmid "
                    + ") InMoney,"
                    + // 交易商入金
                    "(select firmid,nvl(sum(value),0) value from f_clientledger where 1=1 and code='Fetch' "
                    + filter_Fee
                    + " group by firmid "
                    + ") OutMoney,"
                    + // 交易商出金
                    ...
                    + filter_z_v_today
                    + " group by firmid) e1 ,"
                    + "(select bankid,bankName from f_b_banks where validflag = 0 ) j "
                    +
                    // en

                    " where h.firmid=a.firmid(+) "
                    + "and h.firmid=b.firmid(+) "
                    + "and h.firmid=c.firmid(+) "
                    + "and h.firmid=d.firmid(+) "
                    + "and h.firmid=e.firmid(+) "
                    + "and h.firmid=f.firmid(+) "
                    + "and h.firmid=g.firmid(+) "
                    + "and h.firmid=i.firmid(+)"
                    + /* and h.firmid=j.firmid(+) */" "
                    + "and h.firmid=k.firmid(+) "
                    +
                    // st
                    "and h.firmid = a2.firmid(+) "
                    + "and h.firmid = b2.firmid(+) "
                    + "and h.firmid = c2.firmid(+) "
                    + "and h.firmid = d2.firmid(+) "
                    + "and h.firmid = e2.firmid(+) "
                    + "and h.firmid = OutMoney.firmid(+) "
                    + "and h.firmid = InMoney.firmid(+) "
                    + "and h.firmid = FEE.firmid(+) "
                    + "and h.firmid = a1.firmid(+) "
                    + "and h.firmid = b1.firmid(+) "
                    + "and h.firmid = c1.firmid(+) "
                    + "and h.firmid = d1.firmid(+) "
                    + "and h.firmid = e1.firmid(+) "
                    + "and h.bankid = j.bankid";
            // en
            log("sql:" + sql);
            System.out.println("sql:" + sql);
            state = conn.prepareStatement(sql);
    ...
    public Vector<BankTransferValue> getBankTransferList(String filter) throws SQLException, ClassNotFoundException
	{
		...
		try
		{
			...
			"where bt.paybankid = b1.bankid and bt.recbankid = b2.bankid " +
			"and bt.payacc = a1.code and bt.recacc = a2.code " + filter;
	...
	public long addBankTransfer(BankTransferValue val, Connection conn) throws SQLException, ClassNotFoundException
	{
		...
		try
		{
			...
			state.executeUpdate();
			System.out.println("【新增银行间资金划转流水】:流水号[" + id + "]" + "银行流水号[" + val.funId + "]" + "市场流水号[" + val.actionId + "]" + "金额[" + val.money + "]");
		} catch (SQLException e)
		{
			e.printStackTrace();
			id = -1;
	...
	//-------------------------------跨行定制  2013.03.21 end------------------------------------
	@Override
	public Vector<CitysValue> getCityOfBank(String filter) throws SQLException,
			ClassNotFoundException {
