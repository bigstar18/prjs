...
public class CapitalProcessor {
	...
	private double getTotalTransMoney(String bankID, String firmID, int type, Connection conn) throws SQLException {
		...
		System.out.println("}}}}}}}}}}}}}}}}}时间：" + Tool.fmtTime(new java.util.Date()) + "}}}}}}}}}}}}}}}}}}}}}}}}}");
		double result = 0.0D;
		Vector<CapitalValue> capitalList = DAO
				.getCapitalInfoList("where status=0 and bankID='" + bankID + "' and firmid='" + firmID + "' and type=" + type, conn);
	...
	public ReturnValue rgstAccountforWeb(String firmid, String pwd, String type) {
		...
		Vector<CorrespondValue> vec = getCorrespondValue(filer);
	...
	private int getDayTransCount(String bankID, String firmID, int type, Timestamp time, Connection conn) throws SQLException {
		...
		System.out.println("bankID[" + bankID + "]firmID[" + firmID + "]type[" + type + "]time[" + time + "]");
		System.out.println("}}}}}}}}}}}}}}}}}}}}}时间：" + Tool.fmtTime(new java.util.Date()) + "}}}}}}}}}}}}}}}}}}}}}}}}");
		Vector<CapitalValue> capitalList = DAO.getCapitalInfoList("where (status!=1 and status !=-1) and to_char(createtime,'yyyy-mm-dd')='"
				+ Tool.convertTS(time) + "'" + " and bankID='" + bankID + "' and firmid='" + firmID + "' and type=" + type, conn);
	...
	public long delAccountMaket(CorrespondValue correspondValue) {
		...
		try {
			conn = DAO.getConnection();
			Vector<CorrespondValue> vector = DAO.getCorrespondList(" where bankID='" + correspondValue.bankID + "' and firmID='"
					+ correspondValue.firmID + "' and account='" + correspondValue.account + "'", conn);
			if ((vector == null) || (vector.size() <= 0)) {
				...
				if (cv.isOpen == 1) {
					...
					BankAdapterRMI bankadapter = getAdapter(correspondValue.bankID);
					if (bankadapter != null) {
						correspondValue.accountName = cv.accountName;
			...
			if (result == 0) {
				try {
					...
					for (int i = 0; i < list.size(); i++) {
						...
						System.out.println("=========" + i + "=====firmid=[" + moneyInfoValue.firmID + "]");
						moneyInfoValue.firmID = getFirmID(moneyInfoValue.firmID.trim(), moneyInfoValue.bankID);

						Vector<MoneyInfoValue> curList = DAO.getMoneyInfoList(
								"where funID='" + moneyInfoValue.id + "' and trunc(comparedate)=to_date('" + Tool.fmtDate(moneyInfoValue.compareDate)
										+ "','yyyy-MM-dd') and bankid='" + moneyInfoValue.bankID + "'",
								conn);
						if (curList.size() == 0) {
		...
		try {
			...
			if ((firmIDs != null) && (firmIDs.size() > 0)) {
				for (String firmID : firmIDs) {
					Vector<CorrespondValue> correspondList = DAO.getCorrespondList(" where firmid='" + firmID + "' and bankid='" + bankID + "' ",
							conn);
	...
	public ReturnValue chargeAgainst(ChargeAgainstValue cav) {
		...
		synchronized (cav) {
			...
			try {
				...
				filter0 = " where bankID='" + cav.bankID
						+ "' and (trunc(createtime)=trunc(sysdate) or trunc(bankTime)=trunc(sysdate)) and status=0 and (type=0 or type=1 or type=2) ";
				if (rv.result == 0L) {
					...
					if ((capList != null) && (capList.size() > 0)) {
						filter0 = filter0 + " and actionID=" + ((CapitalValue) capList.get(0)).actionID;
					} else {
						Tool.log("到系统流水表中为找到对应流水");
						rv.result = -1L;
						rv.remark = "市场端未找到全部对应流水";
					...
					Vector<CapitalValue> capListPT = DAO.getCapitalInfoListPT(filter0);
					if ((capList != null) && (capList.size() > 0) && (capListPT != null) && (capListPT.size() > 0)) {
						conn.setAutoCommit(false);
						FirmBalanceValue fbv = null;
						CapitalValue cv = new CapitalValue();
						CapitalValue capVal = null;
						CapitalValue capValPT = null;
						...
						for (int i = 0; i < capList.size(); i++) {
							...
							capValPT = (CapitalValue) capListPT.get(i);
							cv.systemID = capVal.systemID;
							cv.sysFirmID = capVal.sysFirmID;
							fbv = null;
							...
							DAO.modCapitalInfoStatusPT(capValPT.iD, capValPT.funID, capValPT.status, capValPT.bankTime, conn);
						}
						...
						TradeProcessorRMI system = getTradeCaptialProcessor(capValPT.systemID);
						if (system == null) {
							Tool.log("获取交易系统[" + capValPT.systemID + "]处理器失败");
							rv.result = -1L;
						...
						req.setBankID(capValPT.bankID);
						req.setMethodName("chargeAgainst");
						req.setParams(new Object[] { capValPT });
						rv = system.doWork(req);
						if (rv.result == 0L) {
							conn.commit();
						} else {
							conn.rollback();
			...
			if (mv != null) {
				try {
					for (int a = 0; a < mv.size(); a++) {
						...
						Vector<MoneyInfoValue> curList = DAO.getMoneyInfoList("where funID='" + miv.id + "' and trunc(compareDate)=to_date('"
						...
						if ((curList == null) || (curList.size() == 0)) {
							...
							miv.firmID = getFirmID(miv.firmID, miv.bankID);
	...
	public long outMoneyForAccess(int rvResult, String bankid, String firmid, String account, String actionid, String funcid) {
		...
		try {
			...
			filter = filter + "  and actionid=" + actionid + " or funid='" + funcid + "' ";
			System.out.println(filter);

			Vector<CapitalValue> list = DAO.getCapitalInfoList(filter, conn);
			Vector<CapitalValue> listPT = DAO.getCapitalInfoListPT(filter);
			if ((list.size() > 0) && (listPT.size() > 0)) {
				for (int i = 0; i < list.size(); i++) {
				...
				for (int i = 0; i < listPT.size(); i++) {
					CapitalValue val = (CapitalValue) listPT.get(i);
					if (val.type == 1) {
						capitalPT = val;
					}
				}
				...
				if (result == 0L) {
					dataProcess.updateFrozenFunds(capital.firmID, capital.bankID, -1.0D * Arith.add(capital.money, rate.money), conn);
					...
					DAO.modCapitalInfoStatusPT(capitalPT.iD, capitalPT.funID, 0, curTime, conn);
					DAO.modCapitalInfoNotePT(capitalPT.iD, "market_out_Audit_1审核通过，银行处理成功，出金成功", conn);

					TradeProcessorRMI tradeSystem = getTradeCaptialProcessor(capitalPT.systemID);
					if (tradeSystem == null) {
						Tool.log("获取交易系统[" + capitalPT.systemID + "]的处理器链接失败");
					} else {
						RequestMsg req = new RequestMsg();
						req.setBankID(capitalPT.bankID);
						req.setMethodName("noticeOutMoneyResult");
						req.setParams(new Object[] { capitalPT.funID, String.valueOf(capitalPT.actionID), Boolean.valueOf(true) });
						try {
							ReturnValue rv = tradeSystem.doWork(req);
							Tool.log("通知交易系统[" + capitalPT.systemID + "]处理流水结果[" + rv.remark + "]");
						} catch (RemoteException e) {
							Tool.log("通知交易系统[" + capitalPT.systemID + "]处理流水异常：" + Tool.getExceptionTrace(e));
						}
					}
				} else {
					...
					if (tradeSystem == null) {
						...
						req.setBankID(capitalPT.bankID);
						req.setMethodName("noticeOutMoneyResult");
						req.setParams(new Object[] { capitalPT.funID, String.valueOf(capitalPT.actionID), Boolean.valueOf(false) });
						try {
							ReturnValue rv = tradeSystem.doWork(req);
							Tool.log("通知交易系统[" + capitalPT.systemID + "]处理流水结果[" + rv.remark + "]");
						} catch (RemoteException e) {
	...
	public HashMap<Integer, Long> handleOUtMoney(long result, Connection conn, long checkTrans, double outRate, String bankID, double money,
		...
		try {
			...
			if ((funID != null) && (funID.trim().length() > 0)) {
				Vector<CapitalValue> cv = DAO
						.getCapitalInfoList(" where bankID='" + bankID + "' and trunc(createtime)=trunc(sysdate) and funID='" + funID + "'", conn);
	...
	public long chenckCorrespondValue(CorrespondValue cv) {
		...
		if ((bankid == null) || (firmid == null) || (account == null)) {
			...
			try {
				List<CorrespondValue> cvresult = DAO.getCorrespondList(" where bankId='" + bankid + "' and firmId='" + firmid
						+ "' and (account like '%" + account.trim() + "%' or account='" + Tool.getConfig("DefaultAccount") + "')");
	...
	public long chenckCorrespondValue(CorrespondValue cv, Connection conn) {
		...
		if ((bankid == null) || (firmid == null) || (account == null)) {
			...
			try {
				List<CorrespondValue> cvresult = DAO.getCorrespondList(" where bankId='" + bankid + "' and firmId='" + firmid
						+ "' and (account like '%" + account.trim() + "%' or account='" + Tool.getConfig("DefaultAccount") + "')", conn);
				if ((bankid.equals("20")) || (bankid.equals("005")) || (bankid.equals("66"))) {
	...
	public ReturnValue synchroAccountMarket(CorrespondValue cv) {
		...
		if (checkResult == 0L) {
			...
			try {
				cv.isOpen = 1;
				List<CorrespondValue> cvresult = DAO.getCorrespondList(" where bankId='" + cv.bankID.trim() + "' and firmId='" + cv.firmID.trim()
						+ "' and account like '%" + cv.account.trim() + "%'");
				if ((cvresult == null) || (cvresult.size() <= 0)) {
					rv.result = -40001L;
				} else {
	...
	public ReturnValue ifQuitFirm(String firmID, String bankID) {
		...
		ReturnValue result = new ReturnValue();
		try {
			Vector<CorrespondValue> vecCorr = DAO.getCorrespondList(" where contact='" + firmID + "' and bankid='" + bankID + "'");
	...
	public ReturnValue saveFirmKXH(Vector<KXHfailChild> vector, String bankID) {
		...
		if (vector == null) {
			...
			for (KXHfailChild kxh : vector) {
				kxh.firmID = getFirmID(kxh.firmID, bankID);
				try {
	...
	private Vector<BatQsChild> getQSChild(java.util.Date date, String bankID) {
		...
		try {
			...
			Set<String> notFirmIDs = new HashSet();
			...
			while (flag) {
				...
				if (usdate == null) {
					...
					if ((toBank != null) && (toBank.size() > 0)) {
						...
						while (it.hasNext()) {
							Map.Entry<String, BatQsChild> ent = (Map.Entry) it.next();
					...
					if ((lastQSDate != null) && (usdate.after(lastQSDate))) {
						...
						while (it.hasNext()) {
							...
							todays = (BatQsChild) today.get(key);
							if (todays != null) {
		...
		if (bcd == null) {
			...
			try {
				...
				for (BatCustDzFailChild cd : bcd) {
					cd.ThirdCustId = getFirmID(cd.ThirdCustId, bankID);
		...
		if (fbf == null) {
			...
			try {
				for (BatCustDzBChild bc : fbf) {
					bc.ThirdCustId = getFirmID(bc.ThirdCustId, bankID);
		...
		if (fbe == null) {
			...
			try {
				for (int i = 0; i < fbe.size(); i++) {
					...
					bfr.ThirdCustId = getFirmID(bfr.ThirdCustId, bankID);
					if (i == 0) {
		...
		try {
			...
			if (bankadapter == null) {
				...
				if (Tool.fmtDate(new java.util.Date()).equalsIgnoreCase(Tool.fmtDate(date))) {
					Vector<FirmBankFunds> vecHis = DAO.getFirmBankFundsHis(" and f.b_date=to_date('" + Tool.fmtDate(date) + "','yyyy-MM-dd')");
				...
				if ("XH".equalsIgnoreCase(Tool.getConfig("qs"))) {
					Vector<FirmBalance> vector1 = DAO.getCCBQS(conn,
							" and bankiD='" + bankID + "' and flag ='1' and trunc(CREATEDATE)=to_date('" + Tool.fmtDate(date) + "','yyyy-MM-dd') ");
					if (vector1.size() > 0) {
						result.result = -1L;
						result.remark = ("给银行[" + bankID + "]发送数据，已发送过");
						ReturnValue localReturnValue1 = result;
						return localReturnValue1;
					}
					vector1 = DAO.getCCBQS(conn,
							" and bankiD='" + bankID + "' and flag ='0' and trunc(CREATEDATE)=to_date('" + Tool.fmtDate(date) + "','yyyy-MM-dd') ");
					if (vector1.size() > 0) {
						DAO.delCCBQS(date, conn);
					}
					Tool.log("第一次查询当日数据的条数[" + (vector == null ? 0 : vector.size()) + "]");
					DAO.addCCBQS(vector, conn);
					vector = DAO.getCCBQS(conn, " and bankiD='" + bankID + "' and trunc(CREATEDATE)<=to_date('" + Tool.fmtDate(date)
							+ "','yyyy-MM-dd')and flag=0 order by qychangemoney-fee");
					Tool.log("第二次查询当日数据的条数[" + (vector == null ? 0 : vector.size()) + "]");
		...
		try {
			...
			if (bankadapter == null) {
				...
				if ((firmIDs != null) && (firmIDs.length > 0)) {
					Vector<MarketFirmMsg> mfms = new Vector();
					for (String firmID : firmIDs) {
						if ((firmID != null) && (firmID.trim().length() > 0)) {
							Vector<CorrespondValue> cvs = DAO
									.getCorrespondList(" where firmid='" + firmID.trim() + "' and bankID='" + bankID.trim() + "'", conn);
	...
	public ReturnValue hxSentQS(String bankID, java.util.Date date) {
		...
		Vector<HXSentQSMsgValue> vector = hxGetQS(bankID, date);
		BankAdapterRMI bankadapter = getAdapter(bankID);
		if (bankadapter == null) {
			result.result = -920000L;
	...
	public ReturnValue pfdbQS(String bankID, java.util.Date date) {
		...
		try {
			...
			if ((v1 != null) && (v1.size() > 0)) {
				try {
					conn.setAutoCommit(false);
					Vector<TradeList> v11 = DAO.getChangeFirmRights(
			...
			if ((v2 != null) && (v2.size() > 0)) {
				try {
					conn.setAutoCommit(false);
					Vector<Margins> v21 = DAO.getChangeFirmFrozen(
							" and trunc(Trade_Date)=to_date('" + Tool.fmtDate(date) + "','yyyy-MM-dd') and flag<>'Y' for update", conn);
	...
	public ReturnValue getTradesDateDetailsList(String bankID, java.util.Date date, int sendCount, int timeOutCount, int faileCount) {
		...
		Vector<TradeList> balance = getTradesDateDetailsList(bankID, date, new String[] { "N", "F" });
		BankAdapterRMI bankadapter = getAdapter(bankID);
		if (bankadapter == null) {
			result.result = -920000L;
		} else {
	...
	public ReturnValue getDongjieDetailList(String bankID, java.util.Date date, int sendCount, int timeOutCount, int faileCount) {
		...
		Connection conn = null;
		Vector<Margins> margins = getDongjieDetailList(bankID, date, new String[] { "N", "F" });
		try {
			BankAdapterRMI bankadapter = getAdapter(bankID);
			if (bankadapter == null) {
				result.result = -920000L;
	...
	public ReturnValue sendGHQS(String bankId, String firmId, java.util.Date qdate) {
		...
		if (bankAdapter == null) {
			...
			List<FirmRights> frs = new ArrayList();
			List<TradingDetailsValue> tds = new ArrayList();
			try {
				Vector<FirmBalance> vecFB = DAO.getRZDateByBank(bankId, qdate);
				int i = 1;
				for (FirmBalance fb : vecFB) {
					FirmRights fr = new FirmRights();
					fr.firmId = fb.firmID;
					fr.account = fb.account;
					fr.firmName = fb.accountName;
					fr.money = fb.QYMoney;
					frs.add(fr);
					TradingDetailsValue td = new TradingDetailsValue();
					td.firmId = fb.firmID;
					td.firmName = fb.accountName;
					td.account = fb.account;
					td.maketNum = i;
					if (fb.QYChangeMoney - fb.CRJSum > 0.0D) {
						td.money = (fb.QYChangeMoney - fb.CRJSum);
						td.updown = "2";
					} else {
						td.money = (-1.0D * (fb.QYChangeMoney - fb.CRJSum));
						td.updown = "1";
					}
					tds.add(td);
					i++;
				...
				Tool.log("获取工行日志数据异常" + Tool.getExceptionTrace(e1));
			}
			List<OpenOrDelFirmValue> opdf = getOpenOrDropMaket(bankId, qdate);
			try {
	...
	public List<OpenOrDelFirmValue> getOpenOrDropMaket(String bankId, java.util.Date qdate) {
		...
		if ((bankId != null) && (!bankId.trim().equals(""))) {
			...
			delFilter = delFilter + "and (bankId='" + bankId + "_D' or bankId='D_" + bankId + "')";
		}
		if (qdate != null) {
		...
		if (bankAdapter == null) {
			...
			try {
				try {
					...
					Vector<BankQSVO> vt = DAO.getBankQSDate(
	...
	public RZQSValue getHRBQSValue(String bankID, String[] firmIDs, java.util.Date tradeDate) {
		...
		try {
			...
			RZQSValue localRZQSValue1;
			if (rv.result != 0L) {
				localRZQSValue1 = result;
				return localRZQSValue1;
			}
			if ((Tool.fmtDate(new java.util.Date()).equalsIgnoreCase(Tool.fmtDate(tradeDate))) && (!getTraderStatus())) {
				localRZQSValue1 = result;
				return localRZQSValue1;
	...
	public RZDZValue getHRBDZValue(String bankID, String[] firmIDs, java.util.Date tradeDate) {
		...
		try {
			ReturnValue rv = isTradeDate(tradeDate);
			RZDZValue localRZDZValue1;
			if (rv.result != 0L) {
				localRZDZValue1 = result;
				return localRZDZValue1;
			}
			if ((Tool.fmtDate(new java.util.Date()).equalsIgnoreCase(Tool.fmtDate(tradeDate))) && (!getTraderStatus())) {
				localRZDZValue1 = result;
				return localRZDZValue1;
	...
	public ReturnValue saveZFPH(ZFPHValue zfph) {
		Tool.log("保存[" + Tool.fmtDate(zfph.tradeDate) + "]的总分平衡数据");
		ReturnValue result = new ReturnValue();
		Connection conn = null;
		try {
			conn = DAO.getConnection();
	...
	public ReturnValue saveFFHD(FFHDValue ffhd) {
		Tool.log("写入" + Tool.fmtDate(ffhd.tradeDate) + "分分核对信息");
		ReturnValue result = new ReturnValue();
		Connection conn = null;
		try {
			conn = DAO.getConnection();
			try {
				...
				conn.commit();
				Tool.log("分分核对信息写入成功，"
						+ (ffhd.getFdv() == null ? "根根核对信息为空" : new StringBuilder("一共[").append(ffhd.getFdv().size()).append("]条信息").toString()));
			} catch (SQLException e) {
				conn.rollback();
				result.result = -1L;
	...
	public ReturnValue sendCMBCQSValue(String bankID, java.util.Date date) {
		...
		try {
			conn = DAO.getConnection();
			Vector<FirmBalance> vector = DAO.getRZDateByBank(bankID, date);
			double QYHZ = 0.0D;
			double FeeHZ = 0.0D;
	...
	public Hashtable<String, List> getZHQSValue(String bankID, java.util.Date tradeDate) {
		...
		try {
			...
			try {
				...
				List<TransferAccountsTransactionDetailed> tatd = getZZJYMX(bankID, null, tradeDate, conn);
				valueTmp.put("S01", tatd);

				List<AccountStatusReconciliation> asr = getKHZHZT(bankID, null, tradeDate, conn);
				valueTmp.put("S02", asr);

				List<StorageMoneySettlementList> smsl = getCGKHZJJSMX(bankID, null, tradeDate, conn);
				valueTmp.put("S06", smsl);

				List<StorageMoneyLedgerBalanceList> smlbl = getCGKHZJTZYEMX(bankID, null, tradeDate, conn);
		...
		if ((ready == 1) || (ready == 2)) {
			if ((states != null) && (states.size() != 0)) {
				for (ClientState state : states) {
					state.setTransFundAcc(getFirmID(state.getTransFundAcc().trim(), state.getBankNo()));
		...
		if (bankAdapter == null) {
			...
			try {
				...
				try {
					...
					smsl = new ArrayList();
					smlbl = new ArrayList();
					Vector<FirmBalance> vec = DAO.getRZDateByBank(bankID, tradeDate);
					for (FirmBalance fb : vec) {
						StorageMoneySettlementList sms = new StorageMoneySettlementList();
						sms.setBankCode("");
						sms.setMarketCode("");
						sms.setTransAddressCode("");
						sms.setTransDateTime(Tool.fmtDate(tradeDate));
						sms.setTaiZhangZhangHao("333");
						sms.setBondAcc(fb.firmID);
						sms.setCertificationName(fb.accountName);
						sms.setMoneyType("001");
						sms.setCashExCode("0");
						StorageMoneyLedgerBalanceList sml = new StorageMoneyLedgerBalanceList();
						sml.setBankCode("");
						sml.setMarketCode("");
						sml.setTransAddressCode("");
						sml.setTransDateTime(Tool.fmtDate(tradeDate));
						sml.setTaiZhangZhangHao("");
						sml.setBondAcc(fb.firmID);
						sml.setCertificationName(fb.accountName);
						sml.setMoneyType("001");
						sml.setCashExCode("0");
						if ("qy".equals(Tool.getConfig("QSMode_ZH"))) {
							if (fb.QYChangeMoney - fb.CRJSum >= 0.0D) {
								sms.setTradeDifference("0");
							} else {
								sms.setTradeDifference("1");
							}
							sms.setMoney(fb.QYChangeMoney - fb.CRJSum);
							sml.setMoney(fb.QYMoney);
							sml.setLastMoney(fb.yesQYMoney);
						} else {
							if (fb.kyChangeMoney - fb.CRJSum >= 0.0D) {
								sms.setTradeDifference("0");
							} else {
								sms.setTradeDifference("1");
							}
							sms.setMoney(fb.kyChangeMoney - fb.CRJSum);
							sml.setMoney(fb.kyMoney);
							sml.setLastMoney(fb.yesKyMoney);
						}
						smsl.add(sms);
						smlbl.add(sml);
					}
	...
	public long bankTransfer(long id, int optFlag) {
		...
		try {
			...
			try {
				conn.setAutoCommit(false);
				Vector<BankTransferValue> bankTransferList = DAO.getBankTransferList(" and bt.id = " + id + " and bt.status = 2", conn);
	...
	public CEB_RSP CEB(CEB_param param, String id) {
		...
		try {
			...
			if ("CEB_F610".equals(id)) {
				Vector<CorrespondValue> cList = DAO.getCorrespondList("where bankid='21' and firmid='" + param.firmId + "'", conn);
				if (cList.size() > 0) {
			...
			if ("CEB_F612".equals(id)) {
				...
				if (("2".equals(param.flag)) || ("4".equals(param.flag))) {
					Vector<CorrespondValue> MarketAcountList01 = DAO.getMarketAcount(" and type = '01' ");
					source = (CorrespondValue) MarketAcountList01.get(0);
					Vector<CorrespondValue> MarketAcountList02 = DAO.getMarketAcount(" and type = '02' ");
					target = (CorrespondValue) MarketAcountList02.get(0);
					param.isCrossLine = "01";
				} else if ("9".equals(param.flag)) {
					Vector<CorrespondValue> MarketAcountList01 = DAO.getMarketAcount(" and type = '02' ");
					source = (CorrespondValue) MarketAcountList01.get(0);
					Vector<CorrespondValue> MarketAcountList02 = DAO.getMarketAcount(" and type = '01' ");
					target = (CorrespondValue) MarketAcountList02.get(0);
					param.isCrossLine = "01";
					...
					Vector<CorrespondValue> MarketAcountList02 = DAO
							.getMarketAcount(" and (type = '03' or account = '" + param.targetAccount + "') ");
					target = (CorrespondValue) MarketAcountList02.get(0);
					Vector<CorrespondValue> MarketAcountList01 = DAO.getMarketAcount(" and type = '01' ");
					source = (CorrespondValue) MarketAcountList01.get(0);
	...
	public ReturnValue sendFile_CEB(String bankID, java.util.Date date) {
		...
		try {
			conn = DAO.getConnection();
			Vector<FirmBalance_CEB> vector = DAO.getFirmBalance_CEB(bankID, date);
		...
		try {
			...
			if ((list == null) || (list.size() == 0)) {
				...
				if ((cv.status == 2) || (cv.status == 5)) {
					...
					try {
						conn.setAutoCommit(false);
						Vector<CapitalValue> ll = DAO.getCapitalInfoList("where actionID=" + actionID, conn);
		...
		try {
			...
			for (int i = 1; i < rows - 1; i++) {
				...
				if (row != null) {
					...
					try {
						conn = DAO.getConnection();
						Vector<CapitalValue> list = DAO.getCapitalInfoList(" where bankID='" + bankID.trim() + "' and funid='" + val[1] + "' ", conn);
						for (int a = 0; a < list.size(); a++) {
							CapitalValue cv = (CapitalValue) list.get(a);
							moneyInfoValue.firmID = cv.firmID;
							Vector<CorrespondValue> vcv = DAO.getCorrespondList(" where (firmID='" + cv.firmID + "' or firmID='D_'||'" + cv.firmID
									+ "') and (bankID='" + bankID.trim() + "'or bankID='D_'||'" + bankID.trim() + "')", conn);
	...
	public Vector<FirmBalance> firmBalance(String bankID, java.util.Date date) {
		log("判断不是当天，累加数据");
		Vector<FirmBalance> vector = new Vector();
		Connection conn = null;
		try {
			conn = DAO.getConnection();
			...
			Map<String, FirmBalance> mapFirmBalance = new HashMap();
			...
			Set<String> key = mapFirmBalance.keySet();
			for (Object it = key.iterator(); ((Iterator) it).hasNext();) {
				String s = (String) ((Iterator) it).next();
	...
	public double getCanOutBalance(String firmID, String bankID, Connection conn) {
		...
		try {
			...
			Vector<FirmBankFunds> vecFBF = DAO.getFirmBankFunds(" and f.firmID='" + firmID + "' and f.bankcode='" + bankID + "'", conn);
			...
			if (!"true".equals(Tool.getConfig("todayYLNoOut"))) {
				Tool.log("处理器模式为盈利及时可出模式");
				return result;
			}
			Tool.log("继续判断上日结算后余额+当日入金-当日出金结果");

			Vector<FirmBankFunds> vecHis = DAO.getFirmBankFundsHis(" and f.firmID='" + firmID + "' and f.bankcode='" + bankID
			...
			if ((vecHis == null) || (vecHis.size() <= 0)) {
				...
				money2 = Arith.add(((FirmBankFunds) vecHis.get(0)).balance, ((FirmBankFunds) vecFBF.get(0)).OutInMoney);
			}
			if (result > money2) {
				result = money2;
			}
	...
	public double getCanOutBalanceByBank(String firmID, String bankID, Connection conn) {
		double result = 0.0D;
		try {
			Vector<FirmBankFunds> fbf = DAO.getFirmBankFunds(" and f.firmid = '" + firmID + "' and f.bankcode ='" + bankID + "'");
			if ((fbf != null) && (fbf.size() == 1)) {
				double balance = ((FirmBankFunds) fbf.get(0)).balance;
				double frozen = ((FirmBankFunds) fbf.get(0)).outMoneyFrozenFunds;
				result = Arith.sub(balance, frozen);
			}
		} catch (Exception e) {
			log(Tool.getExceptionTrace(e));
			e.printStackTrace();
	...
	public String getPlatNum(String sysFirmID, String systemID) {
		...
		Vector<FirmID2SysFirmID> mappings = null;
		try {
	...
	public ReturnValue synchFirmAccount(CorrespondValue corr) {
		...
		try {
			...
			Vector<CorrespondValue> vecCorr = DAO.getCorrespondList(" where firmID='" + firmID + "' and bankID='" + corr.bankID + "'");
			if ((vecCorr == null) || (vecCorr.size() <= 0)) {
				Vector<CorrespondValue> vecCorr2 = DAO.getCorrespondList(" where contact='" + firmID + "' and bankID='" + corr.bankID + "'");
			...
			Vector<FirmBankFunds> vecFBF = DAO.getFirmBankFunds(" and f.firmID='" + firmID + "' and f.bankcode='" + corr.bankID + "'", conn);
			if ((vecFBF == null) || (vecFBF.size() <= 0)) {
				log("平台号[" + firmID + "]还没有签约银行[" + corr.bankID + "]");

				Vector<FirmBankFunds> vecFBF2 = DAO.getFirmBankFunds(" and f.firmID='" + firmID + "' and f.primarybankflag=1", conn);
				...
				Vector<CorrespondValue> vecCorrIsOpen = DAO
						.getCorrespondList(" where firmID='" + firmID + "' and bankID='" + corr.bankID + "' and isOpen=1");
	...
	public ReturnValue openAccountBank(CorrespondValue corr) {
		...
		try {
			if (("1".equals(corr.cardType)) && (corr.card != null) && (corr.card.length() == 15)) {
				String cardFilter = corr.card.substring(0, 6) + "%" + corr.card.substring(6, 15) + "%";
				vec = DAO.getCorrespondList(" where bankID='" + corr.bankID + "' and card like '" + cardFilter + "'");
			} else {
		...
		try {
			...
			if (((FirmBankFunds) vecFBF.get(0)).primaryBankFlag == 1) {
				...
				Vector<FirmBankFunds> vecFBF2 = DAO.getFirmBankFunds(" and f.firmID='" + corr.firmID + "' and f.primaryBankFlag=1", conn);
				...
				Vector<CorrespondValue> vecOpen = DAO
	...
	public ReturnValue delCorrBank(CorrespondValue corr) {
		...
		try {
			conn = DAO.getConnection();
			CorrespondValue corrClose = (CorrespondValue) vecCorr.get(0);
			if (DAO.closeCorrespond(corrClose.bankID, corrClose.firmID, corrClose.card, conn) != 1) {
	...
	public boolean canDelCorr(CorrespondValue corr) {
		...
		Vector<FirmBankFunds> fbfNow = null;
		try {
			fbfNow = DAO.getFirmBankFunds(" and f.firmid='" + corr.firmID + "' and f.bankcode='" + corr.bankID + "'");
		} catch (SQLException e2) {
			Tool.log("查询交易商和银行签约记录异常:" + Tool.getExceptionTrace(e2));
			return false;
		}
		if ((fbfNow != null) && (fbfNow.size() > 0) && (((FirmBankFunds) fbfNow.get(0)).balance != 0.0D)) {
			Tool.log("交易商在该银行相爱实时余额不为零，不允许解约");
			result = false;
			return result;
		}
		Vector<FirmBankFunds> fbfOld = null;
		try {
		...
		Vector<CapitalValue> vecCV = null;
		try {
		...
		if (((FirmBankFunds) vecFBF.get(0)).primaryBankFlag == 1) {
			...
			if ((vecFBF2 != null) && (vecFBF2.size() > 0)) {
				try {
					...
					Vector<CorrespondValue> vecCorr = DAO.getCorrespondList(filter);
					if ((vecCorr != null) && (vecCorr.size() > 0)) {
	...
	public ReturnValue outMoneyBank(InOutMoney outMoney) {
		...
		try {
			...
			try {
				...
				cVal.status = 2;
				cVal.type = 1;
				cVal.systemID = outMoney.defaultSystem;
	...
	public ReturnValue inMoneyBank(InOutMoney inMoney) {
		...
		try {
			conn = DAO.getConnection();
			ReturnValue localReturnValue1;
			if ((inMoney.firmID == null) || (inMoney.systemID == null) || (inMoney.sysFirmID == null) || ("".equals(inMoney.firmID))
					|| ("".equals(inMoney.systemID)) || ("".equals(inMoney.sysFirmID))) {
				log("银行端入金参数非法");
				result.result = -1L;
			...
			Vector<SystemMessage> vecSystem = DAO.getSystemMessages(" where systemID='" + ((FirmID2SysFirmID) vecf2f.get(0)).defaultSystem + "'");
		...
		try {
			...
			if (result.result == 0L) {
				...
				if (checkOutMoney <= outMoney.money) {
					if (outMoney.type == 1) {
						Tool.log("银行[" + outMoney.bankID + "]不支持银行端异步出金，出金金额超出审核额度，审核拒绝");
						result.result = -1L;
						result.remark = "出金审核拒绝";
						try {
							conn.setAutoCommit(false);
							DAO.modCapitalInfoStatusPT(capitalID, null, 1, curTime, conn);
							DAO.modCapitalInfoNotePT(capitalID, "银行端出金，审核拒绝", conn);
							conn.commit();
						} catch (Exception e) {
					...
					try {
						conn.setAutoCommit(false);
						DAO.modCapitalInfoStatusPT(capitalID, null, 3, curTime, conn);
	...
	public Vector<SystemMessage> getSystemMsg(String filter) {
		Vector<SystemMessage> sysMsg = null;
		try {
	...
	public ReturnValue saveQSData(Vector<SystemQSValue> vec) {
		...
		try {
			...
			for (int i = 0; i < vec.size(); i++) {
				...
				Vector<SystemQSValue> vecHiven = DAO.getSystemQSData(filter, conn);
	...
	public ReturnValue getSystemDetailsFile(String systemID) {
		ReturnValue result = new ReturnValue();
		Vector<SystemMessage> vecSysMsg = null;
		try {
	...
	public ReturnValue fundsTransfer(InOutMoney outMoney, String systemID) {
		...
		try {
			...
			Vector<FirmID2SysFirmID> vecf2f2 = DAO.getFirmID2SysFirmID(" where firmID='" + outMoney.firmID + "' and systemID='" + systemID + "'");
			...
			try {
				...
				dataProcess.updateFrozenFunds(outMoney.firmID, outMoney.bankID, outMoney.money, conn);
			...
			try {
				...
				if (rv.result == 0L) {
					...
					dataProcess.updateFrozenFunds(outMoney.firmID, outMoney.bankID, -1.0D * outMoney.money, conn);
				} else {
	...
	public ReturnValue associatedFirmInfo(String firmID, String tradeFirmID, String oldFirmID, String systemID) throws RemoteException {
		...
		try {
			...
			Vector<FirmID2SysFirmID> vec = DAO
			...
			Vector<FirmID2SysFirmID> vec3 = DAO.getFirmID2SysFirmID(" where firmid='" + oldFirmID + "'");
			...
			Vector<FirmID2SysFirmID> vec2 = DAO.getFirmID2SysFirmID(" where firmid='" + firmID + "' and systemid='" + systemID + "'");
			...
			Vector<CorrespondValue> corr = DAO
					.getCorrespondList(" where bankid in (select bankid from f_b_firmidandaccount where isopen=1 and firmid='" + firmID
			...
			Vector<CorrespondValue> corr1 = DAO
					.getCorrespondList(" where bankid in (select bankid from f_b_firmidandaccount where isopen=1 and firmid='" + oldFirmID
			...
			Vector<CapitalValue> bcap = DAO.getCapitalInfoList(" where firmid in ('" + oldFirmID + "','" + firmID + "')");
			if ((bcap == null) || (bcap.size() == 0)) {
				rv.result = -1L;
				rv.remark = "当日有银行资金转账不允许合并账号";
			}
			Vector<CapitalValue> mcap = DAO.getCapitalInfoListPT(" where firmid in ('" + oldFirmID + "','" + firmID + "')");
			...
			Vector<CorrespondValue> vecCorr = DAO.getCorrespondList(" where firmID='" + oldFirmID + "' and bankID='" + fsf.bankID + "'", conn);
			if ((vecCorr == null) || (vecCorr.size() <= 0)) {
	...
	public ReturnValue getSystemMsgs(String firmID) {
		...
		Vector<SystemMessage> sysMsgs = new Vector();
		try {
	...
	public ReturnValue allotbankfunds(java.util.Date date) {
		...
		try {
			...
			Vector<RZQS> vec = DAO.checkRZQSData(date);
			ReturnValue localReturnValue1;
			if ((vec == null) || (vec.size() <= 0)) {
	...
	public String getFirmID(String contact, String bankID) {
		...
		String result = "";
		try {
			Vector<CorrespondValue> vec = DAO.getCorrespondList(" where contact='" + contact + "' and bankid= '" + bankID + "'");
			if ((vec == null) || (vec.size() <= 0)) {
	...
	public boolean bankClose(String bankID) {
		Tool.log("查询银行是否是启用状态");
		boolean result = false;
		try {
			BankValue bv = DAO.getBank(bankID);
			if ((bv != null) && (bv.validFlag == 0)) {
				result = true;
			}
		} catch (Exception e) {
			Tool.log("查询银行状态异常：" + Tool.getExceptionTrace(e));
		}
		return result;
	}

	public ReturnValue outMoneyCheck(String bankID, String firmID, double money) {
		Tool.log("判断出金金额[" + money + "]是否符合银行[" + bankID + "]，平台账号[" + firmID + "]的出金条件");
		ReturnValue result = new ReturnValue();
		if (money == 0.0D) {
			Tool.log("出金金额[" + money + "]不符合要求");
			result.result = -1L;
			result.remark = "出金金额不能为0";
			return result;
		}
		result = outMoneyCheckFirm(firmID, money);
		if (result.result != 0L) {
			return result;
		}
		result = outMoneyCheckBank(bankID, money);
		return result;
	}

	public ReturnValue outMoneyCheckBank(String bankID, double money) {
		Tool.log("判断出金金额[" + money + "]是否符合银行[" + bankID + "]的出金条件");
		ReturnValue result = new ReturnValue();

		BankValue bank = null;
		try {
			bank = DAO.getBank(bankID);
		} catch (Exception e) {
			Tool.log("获取银行[" + bankID + "]对象出现异常：" + Tool.getExceptionTrace(e));
		}
		if (bank == null) {
			Tool.log("获取银行[" + bankID + "]对象失败");
			result.result = -1L;
			result.remark = "系统异常";
			return result;
		}
		if (money > bank.maxPerSglTransMoney) {
			Tool.log("出金金额[" + money + "]大于银行[" + bankID + "]设置的单笔最大转账金额[" + bank.maxPerSglTransMoney + "]");
			result.result = -1L;
			result.remark = "出金金额大于单笔最大转账金额";
			return result;
		}
		Vector<CapitalValue> vec = null;
		try {
			vec = DAO.getCapitalInfoList(" where bankid=' + bankID + ' and status=0 and type=1");
		} catch (Exception e) {
			Tool.log("获取流水异常：" + Tool.getExceptionTrace(e));
		}
		if ((vec == null) || (vec.size() < 0)) {
			Tool.log("获取银行[" + bankID + "]当日出金流水失败");
			result.result = -1L;
			result.remark = "系统异常";
			return result;
		}
		if (vec.size() > bank.maxPerTransCount) {
			Tool.log("超过银行[" + bankID + "]设置的每日最大转账次数[" + bank.maxPerTransCount + "]");
			result.result = -1L;
			result.remark = "超出每日最大转账次数";
			return result;
		}
		double maxOutMoney = 0.0D;
		for (CapitalValue cv : vec) {
			maxOutMoney = Arith.add(cv.money, maxOutMoney);
		}
		if (Arith.add(money, maxOutMoney) > bank.maxPerTransMoney) {
			Tool.log("转账金额[" + money + "]和历史成功转出金额[" + maxOutMoney + "]的总值超出银行[" + bankID + "]设置的每日最大转账金额[" + bank.maxPerTransMoney + "]");
			result.result = -1L;
			result.remark = "超出当日最大转账金额";
			return result;
		}
		result.result = 0L;
		result.remark = ("符合银行[" + bankID + "]的出金限制");
		return result;
	}

	public ReturnValue outMoneyCheckFirm(String firmID, double money) {
		Tool.log("判断出金金额[" + money + "]是否符合平台号[" + firmID + "]的出金条件");
		ReturnValue result = new ReturnValue();
		...
		try {
			...
			Tool.log("获取平台号[" + firmID + "]对象出现异常：" + Tool.getExceptionTrace(e));
		}
		if (fv == null) {
			Tool.log("获取平台号[" + firmID + "]对象失败");
			result.result = -1L;
			result.remark = "系统异常";
			return result;
		}
		if ((money > fv.maxPerSglTransMoney) && (fv.maxPerSglTransMoney > 0.0D)) {
			Tool.log("出金金额[" + money + "]大于平台号[" + firmID + "]设置的单笔最大转账金额[" + fv.maxPerSglTransMoney + "]");
			result.result = -1L;
			result.remark = "出金金额大于单笔最大转账金额";
			return result;
		}
		Vector<CapitalValue> vec = null;
		try {
			vec = DAO.getCapitalInfoList(" where firmid=' + firmID + ' and status=0 and type=1");
		} catch (Exception e) {
			Tool.log("获取流水异常：" + Tool.getExceptionTrace(e));
		}
		if ((vec == null) || (vec.size() < 0)) {
			Tool.log("获取平台号[" + firmID + "]当日出金流水失败");
			result.result = -1L;
			result.remark = "系统异常";
			return result;
		}
		if ((vec.size() > fv.maxPerTransCount) && (fv.maxPerTransCount > 0)) {
			Tool.log("超过平台号[" + firmID + "]设置的每日最大转账次数[" + fv.maxPerTransCount + "]");
			result.result = -1L;
			result.remark = "超出每日最大转账次数";
			return result;
		}
		...
		if ((Arith.add(money, maxOutMoney) > fv.maxPerTransMoney) && (fv.maxPerTransMoney > 0.0D)) {
			Tool.log("转账金额[" + money + "]和历史成功转出金额[" + maxOutMoney + "]的总值超出平台号[" + firmID + "]设置的每日最大转账金额[" + fv.maxPerTransMoney + "]");
			result.result = -1L;
			result.remark = "超出当日最大转账金额";
			return result;
		}
		result.result = 0L;
		result.remark = ("符合平台号[" + firmID + "]的出金限制");
		return result;
	}

	public ReturnValue checkFirmStats(String firmid, String bankID) {
		Tool.log("检查平台号[" + firmid + "]签约的银行[" + bankID + "]的记录是否可用");
		ReturnValue result = new ReturnValue();
		Vector<CorrespondValue> vec = null;
		try {
			vec = DAO.getCorrespondList(" where firmid='" + firmid + "' and bankid='" + bankID + "'");
		} catch (Exception e) {
		...
		if ((vec == null) || (vec.size() != 1)) {
			Tool.log("查询签约关系失败");
			result.result = -1L;
			result.remark = "未找到签约关系";
			return result;
		}
		if (((CorrespondValue) vec.get(0)).status != 0) {
			Tool.log("签约关系不可用");
			result.result = -1L;
			result.remark = "签约关系不可用";
			return result;
		}
		result.result = 0L;
		result.remark = "签约关系可用";
		return result;
	}
		...
		try {
			...
			try {
				...
				for (Iterator<String> it = key.iterator(); it.hasNext();) {
					...
					DAO.updateCCBQS(s, (String) map.get(s), conn);
				}
				conn.commit();
			} catch (SQLException e) {
				conn.rollback();
				result.result = -1L;
				result.remark = "修改现货交易状态数据库异常";
				e.printStackTrace();
			} finally {
				conn.setAutoCommit(true);
			}
			...
			result.result = -1L;
			result.remark = "修改现货交易状态取得数据库连接时数据库异常";
			e.printStackTrace();
		} catch (Exception e) {
			result.result = -2L;
			result.remark = "修改现货交易状态时系统异常";
			e.printStackTrace();
		} finally {
			DAO.closeStatement(null, null, conn);
		}
	...
	public Vector<FirmBalance> getCCBQS(String filter) {
		...
		try {
			...
			try {
				...
				result = DAO.getCCBQS(conn, filter);
	...
	public ReturnValue addCCBQS(Vector<FirmBalance> vec) {
		...
		try {
			conn = DAO.getConnection();
			result.result = DAO.addCCBQS(vec, conn);
			if (result.result > 0L) {
				result.result = 0L;
				result.remark = "保存成功";
			} else {
