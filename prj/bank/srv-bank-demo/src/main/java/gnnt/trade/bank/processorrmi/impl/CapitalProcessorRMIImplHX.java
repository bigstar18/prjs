...
import gnnt.trade.bank.CapitalProcessorHX;
import gnnt.trade.bank.processorrmi.CapitalProcessorRMI;
import gnnt.trade.bank.processorrmi.CapitalProcessorRMIhx;
...
public class CapitalProcessorRMIImplHX extends UnicastRemoteObject implements CapitalProcessorRMIhx {
	private static final long serialVersionUID = 1L;
	CapitalProcessorHX cp = new CapitalProcessorHX();
	public CapitalProcessorRMIImplHX() throws RemoteException {
		super();
	}
	...
	public ReturnValue relevanceAccount(CorrespondValue cv)throws RemoteException{
		cp.log("-----------------------------------------supplyCapitalInfo 银行发起绑定");
		ReturnValue result = cp.relevanceAccount(cv);
		cp.log("=========================================supplyCapitalInfo  添加成功银行发起绑定");
		return result;
	}
	public ReturnValue synchroAccount(CorrespondValue correspondValue) throws RemoteException {
		cp.log("-----------------------------------------synchroAccount  子账户同步");
		ReturnValue result = cp.synchroAccount(correspondValue);
		cp.log("=========================================synchroAccount  子账户同步");
		return result;
	}
	public ReturnValue modCapitalInfoStatus(long id, String funID, int status,Timestamp bankTime) throws RemoteException{
		cp.log("-----------------------------------------interfaceLog  修改银行流水号信息");
		ReturnValue result =  cp.modCapitalInfoStatus(id, funID, status,bankTime);
		cp.log("=========================================interfaceLog  修改银行流水号信息");
		return result;
	}
	public ReturnValue ifQuitFirm(String firmID, String bankID)
			throws RemoteException {
		cp.log("-----------------------------------------interfaceLog  判断是否可以解约");
		ReturnValue result =  cp.ifQuitFirm(firmID, bankID);
		cp.log("=========================================interfaceLog  判断是否可以解约");
		return result;
	}
	public Map<String,CapitalValue> getCapitalValue(Vector<String> contacts,String bankID)
		throws RemoteException {
		cp.log("-----------------------------------------interfaceLog  查询流水信息");
		Map<String,CapitalValue> result =  cp.getCapitalValue(contacts,bankID);
		cp.log("=========================================interfaceLog  查询流水信息");
		return result;
	}
}
