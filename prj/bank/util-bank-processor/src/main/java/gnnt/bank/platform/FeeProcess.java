...
public class FeeProcess
{
  ...
  {
    Hashtable allFee = new Hashtable();
    try
    {
      Vector dicList = DAO.getDicList("where type=3 and bankid='" + bankID + "'");
  ...
  private Hashtable<String, String> getFeeList()
  {
    ...
    try
    {
      Vector dicList = DAO.getDicList("where type=3 and ( bankid IS  NULL or length(bankid)=0 ) ");
      for (int i = 0; i < dicList.size(); i++)
      {
        DicValue dVal = (DicValue)dicList.get(i);
        allFee.put(dVal.name, dVal.value);
  ...
  private boolean isTownsAccont(String bankID, String account)
  {
    ...
    try
    {
      Vector dicList = DAO.getDicList("where type=2 and name='townsAccount' and bankid='" + bankID + "'");
      if (dicList.size() > 0)
      {
  ...
  private double calculateFee(Vector<FeeInfoVO> feeList, double money, double example)
  {
    ...
    for (int i = 0; i < feeList.size(); i++)
    {
        ...
        (money > feeInfoVO.downLimit) && (feeInfoVO.upLimit == -1)))
      {
        ...
        if ((feeInfoVO.maxRateValue > 0.0D) && (Arith.compareTo(result, feeInfoVO.maxRateValue) == 1)) result = feeInfoVO.maxRateValue;
        if (Arith.compareTo(result, feeInfoVO.minRateValue) != -1) break; result = feeInfoVO.minRateValue;
        break;
      }
    }
    return result;
  ...
  protected double getInRate(String bankID, String firmID, double money, int express, String account, Connection conn)
  {
    ...
    try
    {
      Hashtable fFee = getFeeList();
      Hashtable bFee = getBankFeeList(bankID);
      if (bFee.containsKey("inRateFee"))
      {
        Vector feeList = DAO.getFeeInfoList(" where type='" + (String)bFee.get("inRateFee") + "' and userID='" + bankID + "'", conn);
      ...
      {
        Vector feeList = DAO.getFeeInfoList(" where type='" + (String)fFee.get("inRateFee") + "' and userID='" + firmID + "'", conn);
        if ((feeList != null) && (feeList.size() > 0))
        {
          result = calculateFee(feeList, money, money);
        }
  ...
  protected double getOutRate(String bankID, String firmID, double money, int express, String account, Connection conn)
  {
    ...
    try
    {
      ...
      {
        Vector feeList = DAO.getFeeInfoList(" where type='" + (String)bFee.get("outRateFee") + "' and userID='" + bankID + "'", conn);
        if ((feeList != null) && (feeList.size() > 0))
        {
      ...
      {
        Vector feeList = DAO.getFeeInfoList(" where type='" + (String)fFee.get("outRateFee") + "' and userID='" + firmID + "'", conn);
        if ((feeList != null) && (feeList.size() > 0))
        {
          outRateFee = calculateFee(feeList, money, money);
        }
      ...
      if (isTownsAccont(bankID, account))
      {
        ...
        {
          Vector feeList = DAO.getFeeInfoList(" where type='" + (String)bFee.get("townsOutRateFee") + "' and userID='" + bankID + "'", conn);
          if ((feeList != null) && (feeList.size() > 0))
          {
      ...
      else
      {
        if (bFee.containsKey("difTransFee"))
        {
          ...
          Vector feeList = DAO.getFeeInfoList(" where type='" + (String)bFee.get("difTransFee") + "' and userID='" + bankID + "'", conn);
          if ((feeList != null) && (feeList.size() > 0))
          {
        ...
        {
          Vector feeList = DAO.getFeeInfoList(" where type='" + (String)bFee.get("difOutRateFee") + "' and userID='" + bankID + "'", conn);
          if ((feeList != null) && (feeList.size() > 0))
          {
            result = Arith.add(result, calculateFee(feeList, money, money));
          }
