...
public class YjfCapitalProcessor extends CapitalProcessor {
	...
	public ReturnValue inMoneyMarket(InMoneyMarket imm) {
		...
		try {
			...
			InMoneyVO inMoneyInfo = new InMoneyVO(imm.amoutDate, imm.bankName, imm.outAccount, imm.personName, imm.inOutStart, imm.bankID, null,
					imm.money, imm.contact, payInfo, imm.accountPwd, receiveInfo, imm.remark, result.actionId, imm.payChannel);
	...
	public ReturnValue synchroAccountMarket(CorrespondValue cv) {
		...
		if (checkResult == 0L) {
			...
			try {
				List<CorrespondValue> cvresult = DAO.getCorrespondList(" and bankId='" + cv.bankID.trim() + "' and firmId='" + cv.firmID.trim()
	...
	public ReturnValue removeChro(CorrespondValue corr) {
		...
		long result = 0L;
		Connection conn = null;
		if ((corr.bankID == null) || (corr.bankID.length() == 0) || (corr.firmID == null) || (corr.firmID.length() == 0) || (corr.account == null)
				|| (corr.account.length() == 0)) {
			rv.result = -40011L;
			rv.remark = "账户信息不完整";
			return rv;
		}
		label665: try {
			conn = DAO.getConnection();
			Vector<CorrespondValue> vector = DAO.getCorrespondList(
					" where bankID='" + corr.bankID + "' and firmID='" + corr.firmID + "' and account='" + corr.account + "'", conn);
