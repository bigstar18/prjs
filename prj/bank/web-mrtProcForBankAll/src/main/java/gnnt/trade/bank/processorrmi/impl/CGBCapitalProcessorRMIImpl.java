...
import gnnt.trade.bank.processorrmi.CGBCapitalProcessorRMI;
...
public class CGBCapitalProcessorRMIImpl
  extends UnicastRemoteObject
  implements CGBCapitalProcessorRMI
{
  private static final long serialVersionUID = 1L;
  CGBCapitalProcessor cp = new CGBCapitalProcessor();
  
  public CGBCapitalProcessorRMIImpl()
  ...
  public ReturnValue CommunicationsTest(String bankID)
    throws RemoteException
  {
    this.cp.log("-----------------------------------------CommunicationsTest  银行发起通讯检测");
    ReturnValue result = this.cp.CommunicationsTest(bankID);
    this.cp.log("=========================================CommunicationsTest  银行发起通讯检测");
    return result;
  }
  ...
  public CorrespondValue getFirmIDbyCardAndAccount(String card, String cardtype, String accountname)
    throws RemoteException
  {
    this.cp.log("-----------------------------------------getFirmIDbyCardAndAccount  通过证件类型、号码获得交易商信息");
    CorrespondValue result = this.cp.getFirmIDbyCardAndAccount(card, cardtype, accountname);
    this.cp.log("=========================================getFirmIDbyCardAndAccount  通过证件类型、号码获得交易商信息");
    return result;
  }
  ...
  public void send(Date tradeDate)
    throws RemoteException
  {
    this.cp.log("-----------------------------------------send  适配器获取中行清算");
    new BOCExDataImpl().send(tradeDate);
    this.cp.log("-----------------------------------------send  适配器获取中行清算");
  }
    ...
    throws RemoteException
  {
    ...
    ReturnValue result = new BOCExDataImpl().send(date);
    this.cp.log("========================================sendOldQS   发送指定日期的清算");
    return result;
  }
  ...
  public double getfirmQY(String BankID, String firmID)
    throws RemoteException
  {
    this.cp.log("----------------------------------------getfirmQY   获取交易商【" + firmID + "】的期初签约");
    return this.cp.getfirmQY(BankID, firmID);
  }
  
  public void sendCGBQS(Date tradeDate)
    throws RemoteException
  {
    this.cp.log("-----------------------------------------sendCGBQS  适配器获取广发清算");
    new CGBExDataImpl().send(tradeDate);
    this.cp.log("-----------------------------------------sendCGBQS  适配器获取广发清算");
  }
}
