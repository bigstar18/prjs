...
public class CEBCapitalProcessor extends CapitalProcessor {
		...
		try {
			...
			if ("CEB_F610".equals(id)) {
				if ("1".equals(param.depositType)) {
					Vector<CorrespondValue> cList = DAO
							.getCorrespondList("and bankid='" + CebConstant.bankID + "' and contact='" + param.firmId + "'", conn);
					if (cList.size() > 0) {
			...
			if ("CEB_F612".equals(id)) {
				...
				if (("2".equals(param.flag)) || ("4".equals(param.flag))) {
					...
					Vector MarketAcountList02 = DAO.getMarketAcount(" and (type = '03' and account = '" + param.targetAccount + "') ");
					target = (CorrespondValue) MarketAcountList02.get(0);
					Vector MarketAcountList01 = DAO.getMarketAcount(" and type = '01' ");
					source = (CorrespondValue) MarketAcountList01.get(0);
	...
	public ReturnValue chargeAgainstMaket_ceb(ChargeAgainstValue cav) {
		...
		try {
			...
			filter = " and bankID='" + cav.bankID + "' and status=0 and (type=0 or type=1 or type=2) ";
			if (cav.actionID > 0L) {
				...
				if ((capList != null) && (capList.size() > 0)) {
					...
					for (int i = 0; i < capList.size(); i++) {
						...
						if (firmMsg.get(capVal.firmID) != null) {
							...
							String filter2 = " and firmid='" + capVal.firmID + "' for update ";
