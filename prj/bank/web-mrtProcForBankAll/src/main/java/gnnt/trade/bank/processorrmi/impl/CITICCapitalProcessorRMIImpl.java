...
import gnnt.trade.bank.CITICCapitalProcessor;
import gnnt.trade.bank.processorrmi.CITICCapitalProcessorRMI;
...
public class CITICCapitalProcessorRMIImpl
  extends UnicastRemoteObject
  implements CITICCapitalProcessorRMI
{
  private static final long serialVersionUID = 1L;
  CITICCapitalProcessor cp = new CITICCapitalProcessor();
  
  public CITICCapitalProcessorRMIImpl()
  ...
  public ReturnValue modCapitailOutMoney(long actionID, String funID, boolean funFlag, String note, int type)
    throws RemoteException
  {
    this.cp.log("-----------------------------------------modCapitailOutMoney  审核处理中的出金/提现");
    ReturnValue result = new ReturnValue();
    result.result = -1L;
    if (type == 1) {
      result = this.cp.modCaptailInfoOutMoney(actionID, funID, funFlag, note);
    } else if (type == 3) {
      result = this.cp.modCaptailInfoOutMoney2Account(actionID, funID, funFlag, note);
    }
    this.cp.log("-----------------------------------------modCapitailOutMoney  审核处理中的出金/提现");
    return result;
  }
  ...
  public ReturnValue outMoney2Account(OutMoneyMarket omm, String isCorressline, String reciveAccount, String reciveName, String reciveBankName, String reciveBankCode)
    throws RemoteException
  {
    this.cp.log("-----------------------------------------outMoney2Account  提现");
    ReturnValue result = this.cp.outMoney2Account(omm, isCorressline, reciveAccount, reciveName, reciveBankName, reciveBankCode);
    this.cp.log("-----------------------------------------outMoney2Account  提现");
    return result;
  }
