    ...
    try
    {
      ...
      if ((filter != null) && (filter.indexOf("organizationno") >= 0)) {
        ...
        sql = 
          ...
          "select fbc.CREATETIME CREATETIME,fbc.ACTIONID ACTIONID,fbb.bankname bankName,fbf.contact contact,fbf.account account,fbc.MONEY MONEY,fbc.NOTE NOTE ,case when fbc.FUNID is null then '-' else fbc.FUNID end FUNID ,case when mf.firmtype='C' then '-' else fbc.FIRMID end FIRMID ,case when mf.firmtype='C' then '客户' when mf.firmtype='M' then '会员' else '特别会员' end firmtype ,case when fbf.accountName is null then fbf.accountName else fbf.accountName end accountName ,case when fbc.TYPE=0 then '入金' when fbc.TYPE=1 then '出金' when fbc.TYPE=4 then '入金冲正' when fbc.TYPE=5 then '出金冲正' else '其他' end TYPE ,case when fbc.STATUS=0 then '成功' when fbc.STATUS=1 then '失败' when fbc.STATUS=9 then '被冲正' else '处理中' end STATUS  from f_b_capitalinfo fbc,m_firm mf,f_b_firmidandaccount fbf,f_b_banks fbb , m_customerinfo    mc   where fbc.bankid=fbb.bankid(+) and fbc.bankid=fbf.bankid and fbc.firmid=mf.firmid(+) and fbc.firmid=fbf.firmid(+) and mc.customerno = fbf.firmid " + filter;
      } else {
        sql = 
          ...
          "select fbc.CREATETIME CREATETIME,fbc.ACTIONID ACTIONID,fbb.bankname bankName,fbf.contact contact,fbf.account account,fbc.MONEY MONEY,fbc.NOTE NOTE ,case when fbc.FUNID is null then '-' else fbc.FUNID end FUNID ,case when mf.firmtype='C' then '-' else fbc.FIRMID end FIRMID ,case when mf.firmtype='C' then '客户' when mf.firmtype='M' then '会员' else '特别会员' end firmtype ,case when fbf.accountName is null then fbf.accountName else fbf.accountName end accountName ,case when fbc.TYPE=0 then '入金' when fbc.TYPE=1 then '出金' when fbc.TYPE=4 then '入金冲正' when fbc.TYPE=5 then '出金冲正' else '其他' end TYPE ,case when fbc.STATUS=0 then '成功' when fbc.STATUS=1 then '失败' when fbc.STATUS=9 then '被冲正' else '处理中' end STATUS  from f_b_capitalinfo fbc,m_firm mf,f_b_firmidandaccount fbf,f_b_banks fbb    where fbc.bankid=fbb.bankid(+) and fbc.bankid=fbf.bankid and fbc.firmid=mf.firmid(+) and fbc.firmid=fbf.firmid(+)  " + filter;
      }
