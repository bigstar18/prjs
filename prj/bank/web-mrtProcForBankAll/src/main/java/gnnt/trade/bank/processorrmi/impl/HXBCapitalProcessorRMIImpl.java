...
import gnnt.trade.bank.HXBCapitalProcessor;
import gnnt.trade.bank.data.hxb.ExchangeDataImpl;
import gnnt.trade.bank.processorrmi.HXBCapitalProcessorRMI;
...
public class HXBCapitalProcessorRMIImpl
  extends UnicastRemoteObject
  implements HXBCapitalProcessorRMI
{
  private static final long serialVersionUID = 1L;
  HXBCapitalProcessor cp = new HXBCapitalProcessor();
  
  public HXBCapitalProcessorRMIImpl()
  ...
  public ReturnValue moneyInAudit(long actionID, boolean funFlag)
    throws RemoteException
  {
    this.cp.log("-----------------------------------------moneyInAudit  入金申请审核");
    ReturnValue rv = this.cp.moneyInAudit(actionID, funFlag);
    this.cp.log("=========================================moneyInAudit  入金申请审核");
    return rv;
  }
  ...
  {
    this.cp.log("-----------------------------------------inMoneyMarket  入金(市场方调用)");
    ReturnValue result = this.cp.inMoneyMarketHx(imm);
    this.cp.log("=========================================inMoneyMarket  入金(市场方调用)");
    return result;
  }
  ...
  public ReturnValue send(String bankId, Date tradeDate)
    throws RemoteException
  {
    this.cp.log("-----------------------------------------send  发送清算");
    return new ExchangeDataImpl().send(bankId, tradeDate);
  }
