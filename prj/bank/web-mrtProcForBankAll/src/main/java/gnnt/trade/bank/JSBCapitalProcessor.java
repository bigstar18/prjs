...
public class JSBCapitalProcessor extends CapitalProcessor {
				...
				if ((cvresult == null) || (cvresult.size() <= 0)) {
					...
					if (bankadapter == null) {
						...
						if (rv.result == 0L) {
							...
							cvresult = DAO
									.getCorrespondList(" and contact='" + cv2.contact + "' and (bankid is null or bankid='" + cv2.bankID + "')");
							if ((cvresult != null) && (cvresult.size() <= 0)) {
	...
	public ReturnValue auditPositions(String actionId, String bankId, Boolean flag) {
		System.out.println(">>>>江苏银行头寸、收益划转审核>>>>时间：" + Tool.fmtTime(new Date()) + ">>>>");
		ReturnValue rv = new ReturnValue();
		...
		try {
			...
			if (flag.booleanValue()) {
				...
				if (bankadapter == null) {
					...
					TransferInfoValue pay = new TransferInfoValue();
					pay.account = transferInfo.outAccount;
					pay.accountName = transferInfo.outAccountName;
					TransferInfoValue receive = new TransferInfoValue();
					receive.account = transferInfo.inAccount;
					receive.accountName = transferInfo.inAccountName;
					TransferMoneyVO rmo = new TransferMoneyVO(bankId, null, transferInfo.money, pay, receive, 0L);
					rmo.type = transferInfo.type + "";
					rmo.actionID = Long.parseLong(transferInfo.actionId);
					try {
				...
				try {
					conn = DAO.getConnection();
					conn.setAutoCommit(false);
					DAO.modTransfer(transferInfo.actionId, "", 1, curTime, conn);

					log(actionId + "转账流水银行确认审核拒绝");
					conn.commit();
				} catch (SQLException e) {
					conn.rollback();
					log(actionId + "审核转账SQLException，错误码=" + rv + "  " + Tool.getExceptionTrace(e));
					throw e;
				} finally {
					conn.setAutoCommit(true);
				}
