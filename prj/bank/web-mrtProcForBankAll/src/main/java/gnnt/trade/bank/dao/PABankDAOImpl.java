...
public class PABankDAOImpl extends PABankDAO {
	public PABankDAOImpl() throws Exception {
	}
	...
	public Vector<KXHfailChild> getFirmKXH1(String filter) throws SQLException, ClassNotFoundException {
	...
	public int addBankQS(BankQSVO bq, Connection conn) throws SQLException {
		...
		String sql = null;
		try {
			state = conn.prepareStatement("select * from F_B_BankQSDate where to_char(TRADEDATE,'yyyy-MM-dd')='" + Tool.fmtDate(bq.tradeDate)
					+ "' and bankid = '" + bq.bankID + "'");
			rs = state.executeQuery();
			if (!rs.next()) {
	...
	public Map<String, BatQsChild> getQSChild(String bankID, Set<String> firmIDs, Set<String> notFirmIDs, java.util.Date qdate, Connection conn)
		...
		PAConstant.bankID + "' and f.b_date=to_date('" + Tool.fmtDate(qdate) + "','yyyy-MM-dd')) ffb,"
				...
				+ " (select firmid,0-nvl(sum(mktfee),0) mktfee from t_trade_h where cleardate=to_date('" + Tool.fmtDate(qdate)
	...
	public BankQSVO getBankQS(String bankID, java.util.Date date, Connection conn) throws SQLException {
		log("查询某天清算结果");
		...
		String sql = null;
		try {
			sql = "select * from F_B_BankQSDATE where bankid='" + bankID + "' and trunc(tradeDate)=to_date('" + Tool.fmtDate(date)
					+ "','yyyy-MM-dd')";
			state = conn.prepareStatement(sql);
			rs = state.executeQuery();
			if (rs.next()) {
