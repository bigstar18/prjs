...
public class CITICCapitalProcessor extends CapitalProcessor {
	...
	public ReturnValue outMoney2Account(OutMoneyMarket omm, String isCorressline, String reciveAccount, String reciveName, String reciveBankName,
			String reciveBankCode) {
	...
	public ReturnValue openAccountMarket(CorrespondValue cv) {
		...
		try {
			rv.result = -1L;
			ReturnValue localReturnValue1;
			if ((cv.bankID == null) || (cv.bankID.length() == 0) || (cv.firmID == null) || (cv.firmID.length() == 0) || (cv.account1 == null)
					|| (cv.account1.length() == 0) || (cv.accountName1 == null) || (cv.accountName1.length() == 0)) {
				rv.result = -1L;
	...
	public FirmBalanceValue getFirmBalance(String bankid, String firmid, String pwd) {
		...
		try {
			...
			if (rv.result < 0L) {
				...
				FirmBalanceValue localFirmBalanceValue1 = fv;
