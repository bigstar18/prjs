...
public class HXBCapitalProcessor extends CapitalProcessor {
			...
			if (DAO.getCorrespondList(" and firmID='" + correspondValue.firmID + "' ", conn).size() > 0) {
				...
				if ((returnValue != null) && (returnValue.result == 0L)) {
					List<CorrespondValue> cvresult = DAO
							.getCorrespondList(" and trim(firmid)='" + correspondValue.firmID.trim() + "' and trim(bankID) is  null");
					if ((cvresult == null) || (cvresult.size() <= 0)) {
						cvresult = DAO.getCorrespondList(
								" and trim(firmid)='" + correspondValue.firmID.trim() + "' and trim(bankID)='" + correspondValue.bankID.trim() + "'");
						if ((cvresult == null) || (cvresult.size() <= 0)) {// TODO
