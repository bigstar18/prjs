...
public class BankDAOImpl extends BankDAO {
	...
	public boolean getTradeDate(java.util.Date tradeDate) throws SQLException, ClassNotFoundException {
		...
		try {
			conn = getConnection();
			state = conn.prepareStatement(sql);
			for (rs = state.executeQuery(); rs.next();) {
				...
				String as[];
				int k = (as = weeks).length;
				for (int i = 0; i < k; i++) {
					String week = as[i];
					if ((new StringBuilder(String.valueOf(dayOfWeek))).toString().equalsIgnoreCase(week))
						flag = false;
				}
	...
	public Vector<CompareResult> compareResultInfo(String bankID, String date) throws SQLException, ClassNotFoundException {
		...
		try {
			...
			if ("true".equals(Tool.getConfig("CompareWithFundflow")))
				sql = "select c.firmID as firmID, c.bankID as bankID, b.account as account, c.funID as id, c.actionID as m_Id,   b.type as type ,c.type as m_type, b.money as money ,c.money as m_money ,b.comparedate as comparedate  from F_B_BANKCOMPAREINFO b,F_B_capitalInfo c  where (c.status='0' and (c.type='0' or c.type='1') and c.bankid='"
						+ bankID + "' and b.bankid='" + bankID + "' " + " and trunc(b.COMPAREDATE)=to_date('" + date + "','yyyy-MM-dd') "
			...
			if ("true".equals(Tool.getConfig("CompareWithFundflow")))
				sql = "select * from F_B_capitalInfo c where  c.bankID='" + bankID + "' "
	...
	public Map<String, BatQsChild> getQSChild(String bankID, Set<String> firmIDs, Set<String> notFirmIDs, java.util.Date qdate, Connection conn)
		...
		if ((firmIDs != null) && (firmIDs.size() > 0)) {
			...
			String firms = "('aa'";
			while (it.hasNext()) {
				firms = firms + ",'" + (String) it.next() + "'";
			}
			firms = firms + ")";
			filter = filter + " and firmid in " + firms;
		}
		if ((notFirmIDs != null) && (notFirmIDs.size() > 0)) {
			Iterator it = notFirmIDs.iterator();
			String firms = "('aa'";
			while (it.hasNext()) {
				firms = firms + ",'" + (String) it.next() + "'";
			}
			firms = firms + ")";
			filter = filter + " and firmid not in " + firms;
		}
		...
		try {
			...
			sql = "select nvl(a.value,0)+(case when nvl(k.value,0)>0 then nvl(k.value,0) else 0 end) AddTranAmount,nvl(b.value,0)+(case when nvl(k.value,0)<0 then nvl(-k.value,0) else 0 end) CutTranAmount,nvl(c.value,0) ProfitAmount,nvl(d.value,0) LossAmount,nvl(e.value,0) TranHandFee,nvl(f.valued,0) FreezeAmount,nvl(f.valuez,0) UnfreezeAmount,nvl(i.valued,0) toDhykAmount,nvl(i.valuez,0) yeDhykAmount,nvl(g.value,0) NewBalance,nvl(f.valued,0) NewFreezeAmount,h.firmid ThirdCustId,h.account1 CustAcctId,h.ACCOUNTNAME1 CustName from (select fc.firmid,sum(nvl(fc.value,0)) value from f_ledgerfield fl,f_clientledger fc where fl.code = fc.code and fc.b_date=to_date('"
					...
					+ "(select nvl(RuntimeSettleMargin,0) value,firmid from t_h_firm where cleardate=to_date('" + Tool.fmtDate(yestody)
					+ "','yyyy-MM-dd') " + filter + ") fb1, " + "(select nvl(RuntimeFL,0) value,firmid from t_h_firm where cleardate=to_date('"
					+ Tool.fmtDate(yestody) + "','yyyy-MM-dd') " + filter + ") fc1, "
	...
	public Vector<HXSentQSMsgValue> getHXQSMsg(String bankID, String[] firmIDs, java.util.Date tradeDate)
		...
		try {
			...
			sql = "select bank.*,nvl(rsm.value,0) rsm,nvl(rm.value,0) rm,nvl(Margin.value,0)-nvl(MarginBack.value,0) Margin,nvl(rfl.value,0) rfl,nvl(fl.value,0) fl,(nvl(rsm.value,0)-nvl(rsmy.value,0)+nvl(rm.value,0)-nvl(rmy.value,0)+nvl(Marginch.value,0)-nvl(MarginBackch.value,0)+nvl(rfl.value,0)-nvl(rfly.value,0)+nvl(fl.value,0)-nvl(fly.value,0)+nvl(balance.todaybalance,0)-nvl(balance.lastbalance,0)-nvl(inmoney.value,0)+nvl(outmoney.value,0)) balanceChange,(nvl(rsm.value,0)+nvl(rm.value,0)+nvl(Margin.value,0)-nvl(MarginBack.value,0)+nvl(rfl.value,0)+nvl(fl.value,0)) frozen,nvl(balance.todaybalance,0) balance,nvl(balance.lastbalance,0) lastbalance,nvl(inmoney.value,0) inmoney,nvl(outmoney.value,0) outmoney,(nvl(rsm.value,0)+nvl(rm.value,0)+nvl(Margin.value,0)-nvl(MarginBack.value,0)+nvl(rfl.value,0)+nvl(fl.value,0)+nvl(balance.todaybalance,0)) quanyi from (select sum(nvl(RuntimeSettleMargin,0)) value,firmid from t_h_firm where 1=1 "
					...
					+ " group by firmid) rm, " + "(select sum(nvl(RuntimeSettleMargin,0)) value,firmid from t_h_firm where 1=1 " + djfiltery
					+ " group by firmid) rsmy, " + "(select sum(nvl(RuntimeMargin,0)-nvl(RuntimeAssure,0)) value,firmid from t_h_firm where 1=1 "
					+ djfiltery + " group by firmid) rmy, " + "(select sum(value) value,firmid from f_clientledger where code like '%Income%' "
					+ bhfilter + " group by firmid) income," + "(select sum(value) value,firmid from f_clientledger where code like '%Payout%' "
					+ bhfilter + " group by firmid) payout,"
	...
	public int addChangeFirmRights(String bankID, String[] firmIDs, java.util.Date tradeDate, Connection conn) throws SQLException {
		...
		try {
			...
			sql = "insert into F_B_Tradelist (TRADE_MONEY,TRADE_TYPE,B_MEMBER_CODE,B_MEMBER_NAME,S_MEMBER_CODE,S_MEMBER_NAME,TRADE_DATE,BARGAIN_CODE,SERIAL_ID,GOOD_CODE,GOOD_NAME,GOOD_QUANTITY,FLAG,BANKID,CREATEDATE) select TRADE_MONEY,TRADE_TYPE,B_MEMBER_CODE,B_MEMBER_NAME,S_MEMBER_CODE,S_MEMBER_NAME,to_date('"
					...
					+ "(select firmid,-sum(nvl(value,0)) value from f_clientledger fc where exists(select code from f_ledgerfield fl where fc.code=fl.code and fl.code like '%PL') and value<0 "
					+ filter1 + " group by firmid) PL2," + "(select * from f_b_firmidandaccount where 1=1 and firmid not like 'D_%' " + bankfilter
					+ ") bank "
					...
					+ "where bank.firmid=rfl.firmid(+) and bank.firmid=rfl1.firmid(+)" + "union all "
					...
					+ "(select * from f_b_firmidandaccount where 1=1 and firmid not like 'D_%' " + bankfilter + ") bank "
					+ "where bank.firmid=tf.firmid(+) and bank.firmid=bf.firmid(+) " + "union all "
					...
					+ ") bank " + "where bank.firmid=sf.firmid(+) " + "union all "
					...
					+ filter1 + " group by firmid) OtherItem," + "(select * from f_b_firmidandaccount where 1=1 and firmid not like 'D_%' "
					+ bankfilter + ") bank " + "where bank.firmid=OtherItem.firmid(+)" + ")";
	...
	public int addChangeFirmFrozen(String bankID, String[] firmIDs, java.util.Date qdate, Connection conn) throws SQLException {
		...
		try {
			...
			sql = "insert into F_B_MARGINS (SERIAL_ID,BARGAIN_CODE,TRADE_TYPE,TRADE_MONEY,MEMBER_CODE,MEMBER_NAME,TRADE_DATE,GOOD_CODE,GOOD_NAME,GOOD_QUANTITY,FLAG,BANKID,CREATEDATE) select '' || SEQ_F_B_MARGINS.nextval SERIAL_ID,'' BARGAIN_CODE,case when TRADE_MONEY>0 then 1 else 2 end TRADE_TYPE, abs(TRADE_MONEY) TRADE_MONEY,MEMBER_CODE,MEMBER_NAME,to_date('"
					...
					+ " group by firmid) rsm, " + "(select sum(nvl(RuntimeMargin,0)-nvl(RuntimeAssure,0)) value,firmid from t_h_firm where 1=1 "
					+ filter1 + " group by firmid) rm, "
					...
					+ "(select sum(nvl(RuntimeMargin,0)-nvl(RuntimeAssure,0)) value,firmid from t_h_firm where 1=1 " + filter11
					+ " group by firmid) rm2, " + "(select * from f_b_firmidandaccount where 1=1 and firmid not like 'D_%' " + bankfilter + ") bank "
	...
	public Vector<FirmRightsValue> getTradeDataMsg(String bankId, String firmid, java.util.Date qdate) throws SQLException, ClassNotFoundException {
		...
		if ((bankId != null) && (!"".equals(bankId.trim()))) {
			filter = filter + " and i.bankId='" + bankId.trim() + "' and i.isopen ='1'" + "' and i.firmid not like '%D'";
		}
		...
		try {
			...
			sql = "select i.firmid qy_firmid,i.accountname accountname,i.account account, nvl(a.todaybalance, 0) f_balance, nvl(b.RuntimeMargin, 0) + nvl(b.RuntimeFL, 0) + nvl(b.RuntimeSettleMargin, 0) + nvl(c.floatingloss, 0) t_money, nvl(d.money, 0) v_money, nvl(e.money, 0) z_money, nvl(f.money, 0) pay_money, nvl(g.money, 0) in_money, nvl(h.money, 0) fee_money, nvl(jie.money, 0) jie_money, nvl(dai.money, 0) dai_money from (select firmid, todaybalance from f_firmbalance where 1 = 1 "
					...
					+ filter_f_fb + " group by firmid) dai," + "(select firmid,bankid,accountname,account,isopen from f_b_Firmidandaccount) i "
					+ "where " + "i.firmid = a.firmid(+) " + "and i.firmid = b.firmid(+) " + "and i.firmid = c.firmid(+) "
					+ "and i.firmid = d.firmid(+) " + "and i.firmid = e.firmid(+) " + "and i.firmid = f.firmid(+) " + "and i.firmid = g.firmid(+) "
					+ "and i.firmid = h.firmid(+) " + "and i.firmid = jie.firmid(+) " + "and i.firmid = dai.firmid(+) " + filter
					+ " order by a.firmid";
			state = conn.prepareStatement(sql);
	...
	public RZQSValue getXYQSValue(String bankID, String[] firmIDs, java.util.Date tradeDate) throws SQLException, ClassNotFoundException {
		...
		try {
			conn = getConnection();
			StringBuffer sbfSQL = new StringBuffer("select ");
			...
			while (rs.next()) {
				...
				value.account = rs.getString("account");
				value.availableBalance = (rs.getDouble("balance") - rs.getDouble("lastBalance") - rs.getDouble("crjsum"));
				value.billMoney = 0.0D;
				value.cash = (rs.getDouble("frozen") - rs.getDouble("lastFrozen"));
				value.cashMoney = 0.0D;
				value.credit = 0.0D;
				value.firmErrorMoney = (rs.getDouble("rights") - rs.getDouble("lastRights") - rs.getDouble("crjsum"));
				mrv.bankErrorMoney += value.firmErrorMoney + rs.getDouble("fee");
				...
				Tool.log("交易商[" + rs.getString("firmID") + "]的自有资金[" + rs.getBigDecimal("maketMoney") + "]总自有资金[" + mrv.maketMoney + "]");
				result.putFrv(value);
			}
	...
	public RZDZValue getXYDZValue(String bankID, String[] firmIDs, java.util.Date tradeDate) throws SQLException, ClassNotFoundException {
		...
		try {
			...
			while (rs.next()) {
				...
				value.credit = 0.0D;
				value.quanyibh = (rs.getDouble("rights") - rs.getDouble("lastRights") - rs.getDouble("crjsum"));
				result.putFdv(value);
			}
		} catch (SQLException e) {
			throw e;
		...
		try {
			...
			if ((ffhd != null) && (ffhd.getFdv() != null)) {
				...
				for (FirmDateValue ff : vv) {
					try {
						ff.firmID = ((CorrespondValue) getCorrespondList(" where contact='" + ff.firmID + "' and bankid= '" + ff.bankID + "'")
								.get(0)).firmID;
					} catch (ClassNotFoundException e) {
	...
	public Vector<BankQSNetChild> getQSBankDate(String bankID, java.util.Date qdate) throws Exception {
		...
		try {
			...
			sql = sql
					...
					+ "','yyyy-MM-dd') " + filter + ") fd1, "
					+ "(select nvl(sum(case when code = 'Margin_V' then value when code = 'Margin_Z' then value when code = 'MarginBack_V' then -value when code = 'MarginBack_Z' then -value else 0 end), 0) value,firmid from f_clientledger where code in ('MarginBack_V', 'Margin_V','MarginBack_Z', 'Margin_Z') and to_char(b_date,'yyyy-MM-dd')<='"
					+ Tool.fmtDate(yestody) + "' " + filter + " group by firmid) fg1, " + "(select firmid from f_b_firmidandaccount where 1=1 "
					+ filter2 + ") fe " + "where fe.firmid=fb.firmid(+) and fe.firmid=fc.firmid(+) and fe.firmid=fd.firmid(+)"
					+ " and fe.firmid=fg.firmid(+) and fe.firmid=fb1.firmid(+) and fe.firmid=fc1.firmid(+) and fe.firmid=fd1.firmid(+)"
					+ " and fe.firmid=fg1.firmid(+) " + ") f, " + "(" + "select nvl(ia.value,0) valued,nvl(ib.value,0) valuez,ic.firmid from "
					+ "(select sum(nvl(floatingloss,0)) value,firmid from t_h_firmholdsum where trunc(cleardate)=to_date('" + Tool.fmtDate(qdate)
					+ "','yyyy-MM-dd') " + filter + " group by firmid) ia,"
					...
					+ "and h.firmid=k.firmid(+) " + "and h.firmid = a2.firmid(+) " + "and h.firmid = b2.firmid(+) " + "and h.firmid = c2.firmid(+) "
					+ "and h.firmid = d2.firmid(+) " + "and h.firmid = e2.firmid(+) " + "and h.firmid = OutMoney.firmid(+) "
					+ "and h.firmid = InMoney.firmid(+) " + "and h.firmid = FEE.firmid(+) " + "and h.firmid = a1.firmid(+) "
					+ "and h.firmid = b1.firmid(+) " + "and h.firmid = c1.firmid(+) " + "and h.firmid = d1.firmid(+) "
			...
			Map<String, BankQSNetChild> map = new HashMap<String, BankQSNetChild>();
			while (rs.next()) {
	...
	public List<AccountStatusReconciliation> getKHZHZT(String bankID, String[] firmIDs, java.util.Date tradeDate, Connection conn)
		...
		String sql = "";
		try {
			sql = "SELECT b.firmid bondAcc, b.account BankAccount, b.accountname certificationName, (CASE b.isopen WHEN  0 THEN 1 WHEN 1 THEN 0 END) CashExCode FROM  f_b_firmidandaccount b WHERE 1=1  AND (trunc(b.opentime) = to_date('"
	...
	public List<StorageMoneyLedgerBalanceList> getCGKHZJTZYEMX(String bankID, java.util.Date tradeDate, Connection conn)
		...
		if (bankID != null)
			filter = filter + " and bankid = '" + bankID + "'";
		try {
			sql = " SELECT a.firmid bondAcc, a.accountname certificationName, b.todaybalance money, b.lastbalance lastMoney FROM f_b_firmidandaccount a,f_firmbalance b WHERE a.firmid = b.firmid AND a.isopen=1 and a.firmid not like '%D%' AND trunc(b.b_date) = to_date('"
	...
	public int addPlatRzqs(java.util.Date date, Connection conn) throws SQLException {
		...
		try {
			...
			bf.append("t2.firmid, ");
			bf.append("nvl(t2.balance + t1.qychange - t1.frozen, 0) balance, ");
			bf.append("nvl(t1.frozen, 0) rightsfrozenfunds, ");
			bf.append("nvl(t2.balance + t1.qychange, 0) rights, ");
			bf.append("nvl(t1.fee, 0) fee, ");
			bf.append("0 marketfee ");
			bf.append("from  ");
			bf.append("(select firmid,sum(rights) - sum(lastrights) - sum(fundio) qychange,sum(rights) - sum(balance) frozen,sum(fee) fee from ");
			bf.append(
	...
	public Vector<FirmID2SysFirmID> getfirmIDMgs(String filter) throws SQLException {
		...
		try {
			conn = getConnection();
			String sql = "select fbfs.*,fbf.account account,fbf.accountName accountName,fbf.card card,fbf.cardtype cardtype from f_b_firmid_sysfirmid fbfs,f_b_firmidandaccount fbf where fbfs.firmid=fbf.firmid "
					+ filter;
			state = conn.prepareStatement(sql);
			rs = state.executeQuery();
			while (rs.next()) {
	...
	public Vector<RZQS> checkRZQSData(java.util.Date date) throws SQLException, ClassNotFoundException {
		...
		try {
			conn = getConnection();
			String sql = "select * from f_b_firmrzqs where to_char(createdate,'yyyy-MM-dd')=?";
			st = conn.prepareStatement(sql);
			st.setString(1, Tool.fmtDate(date));
			rs = st.executeQuery();
			while (rs.next()) {
