...
<head>
<%
    String reportTiltle = "交易商入金出金记录表";
%>
...
<fieldset class="pickList" >
	...
	<table border="0" align="center" cellpadding="5" cellspacing="5" class="common">
		<tr>
				<td>
				<rightButton:rightButton name="查看" onclick="return query('pdf')" className="btn_sec" action="${basePath}/timebargain/cushInAndOutReport/cushInAndOutReport.action" id="add"></rightButton:rightButton>
	     		</td>
	     		<td>
	     		<rightButton:rightButton name="保存为excel" onclick="return query('excel')" className="btn_sec1" action="${basePath}/timebargain/cushInAndOutReport/cushInAndOutReportExcel.action" id="saveExcel"></rightButton:rightButton>
	     		</td>
		</tr>
	</table>
	</fieldset>
...
function query(exportTo)
{
    ...
    if(cateGoryId == ""){
	 ...
	 if(exportTo=="excel"){
			//获取配置权限的 URL
			var url = "${mgrPath}/app/timebargain/printReport/cushInAndOutReportExcel.jsp?startFirmID="+zStart +"&endFirmID="+ zEnd +"&startClearDate="+zDate+"&endClearDate="+zEndDate+"&title=交易商入金出金记录表&brokerId="+brokerId+"&cateGoryId="+cateGoryId;
			document.location.href = url;
		 }else{
			//获取配置权限的 URL
			var url = "${mgrPath}/app/timebargain/printReport/cushInAndOutReport.jsp?startFirmID="+zStart +"&endFirmID="+ zEnd +"&startClearDate="+zDate+"&endClearDate="+zEndDate+"&title=交易商入金出金记录表&brokerId="+brokerId+"&cateGoryId="+cateGoryId;
