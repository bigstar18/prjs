		...
		<div class="main">
			...
			<div class="warning">
				...
				<div class="content">　该仓单【仓单号：${entity.stockId}】为
						未注册仓单。未注册仓单可以进行出库和拆单处理。未注册仓单不可直接用于交易，您需要将未注册仓单申请注册并经审核后生成注册仓单，注册仓单才可用于交易。
	    <%--  			<c:if test="${entity.stockStatus == 1}">
	     				注册仓单；注册仓单可以进行交收、卖仓单和融资操作；
	     			</c:if>
	     			<c:if test="${entity.stockStatus == 2}">
	     				已经出库的仓单；
	     			</c:if>
	     			<c:if test="${entity.stockStatus == 3}">
	     				拆单仓单；拆单仓单分为已拆仓单、拆单申请中仓单和拆单失败仓单；<br/>
						&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;1、拆单仓单为已经拆分的仓单，已拆分仓单只作为一种追溯记录；<br/>
						&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;2、拆单申请中仓单等待仓库进行拆单处理，拆单申请中仓单不用用于交易；<br/>
