...
(function( $ ) {
  $.fn.promptumenu = function(options) {
	...
	var a = this.each(function(i){
		...
		var funObj = {};
		funObj.next_page = function () {
		    if (cursor.page > cells.current_page) {
		        methods.go_to(cells.current_page + 1);
		    } else {
		        //methods.go_to(1);
		    	//alert("已经是最后一页了！");
		...
		funObj.prev_page = function () {
		    if (cells.current_page > 1) {
		        methods.go_to(cells.current_page - 1);
		    } else {
		        //methods.go_to(cursor.page);
		    	//alert("已经是第一页了！");
		    }
		...
		if($this.data('promptumenu')){
			...
			$this.bind('mousedown.promptumenu', function(mdown){
				...
				$(document).bind('mouseup.promptumenu', function(mup){
					...
					var delta_start = delta_end;
					if(mmove_event.length>0){
						delta_start = mmove_event[0];
					}
					var event_delta = {
