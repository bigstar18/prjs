...
public class LogonTimeOutThread
  extends BaseThread
{
  ...
  private void logoffLogonTypeMap(AULogonTypeManageVO logonTypeManageVO, long expireTime)
  {
    ...
    List<AUUserManageVO> logoutList = new ArrayList();
    ...
    for (AUUserManageVO userManageVO : logoutList) {
      ...
      {
        this.logger.error("登录超时，退出[" + userManageVO.getUserID() + "]在[" + userManageVO.getLogonType() + "]的登录[" + userManageVO.getSessionID() + "]异常", e);
      }
    }
  }
  ...
  private long getAUExpireTime(String logonType)
  {
    ...
    if ((this.auExpireTimeMap != null) && 
      (this.auExpireTimeMap.get(logonType) != null)) {
