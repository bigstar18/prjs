...
public class ActiveUserManage
{
  ...
  public AULogonResultVO logon(AULogonVO logonVO)
  {
    ...
    result.setResult(-1);
    if ((logonVO == null) || (logonVO.getUserID() == null) || 
      (logonVO.getLogonType() == null))
    {
      result.setRecode("-1001");
      result.setMessage("传入信息不完整");
      if (logonVO != null) {
        this.logger.info("用户[" + logonVO.getUserID() + "]在模块[" + 
          logonVO.getModuleID() + "]以[" + 
          logonVO.getLogonType() + "]类型登录信息不完整");
      } else {
        this.logger.info("用户登录，传入信息为空");
      }
      return result;
    }
    AULogonTypeManageVO logonTypeManageVO = getLogonTypeManageVO(logonVO
      .getLogonType());
    

    List<AUUserManageVO> userManageList = 
      (List)logonTypeManageVO.getUserMap().get(logonVO.getUserID());
    if (userManageList == null) {
      userManageList = new ArrayList();
    }
    if ((this.logonMode != LogonMode.MULTI_MODE) && 
      (userManageList.size() > 0))
    {
      ...
      logoff(logoffVO);
      this.logger.info("新用户[" + logonVO.getUserID() + "]以[" + 
        logonVO.getLogonType() + "]类型登录，踢出原用户 SessionID[" + 
        userManageVO.getSessionID() + "]");
    }
    ...
    logonTypeManageVO.getSessionMap().put(Long.valueOf(newUserManageVO.getSessionID()), 
      newUserManageVO);
    
    putLogonOrLogoffUserList(newUserManageVO, 1);
    
    this.logger.info("用户[" + logonVO.getUserID() + logonVO.getLastLogonTime() + 
      "]在模块[" + logonVO.getModuleID() + "]以[" + 
      logonVO.getLogonType() + "]类型登录成功，返回 SessionID [" + 
  ...
  public AULogoffResultVO logoff(AULogoffVO logoffVO)
  {
    ...
    result.setResult(-1);
    if ((logoffVO == null) || (logoffVO.getLogonType() == null) || 
      (logoffVO.getUserID() == null))
    {
      result.setRecode("-1101");
      result.setMessage("传入信息不完整");
      if (logoffVO != null) {
        this.logger.info("用户[" + logoffVO.getUserID() + "]退出在[" + 
          logoffVO.getLogonType() + "]类型登录");
      } else {
        this.logger.info("退出登录，传入信息为空");
      }
      return result;
    }
    AULogonTypeManageVO logonTypeManageVO = getLogonTypeManageVO(logoffVO
      .getLogonType());
    
    AUUserManageVO userManageVO = (AUUserManageVO)logonTypeManageVO.getSessionMap().get(
      Long.valueOf(logoffVO.getSessionID()));
    if (userManageVO == null)
    {
      result.setResult(1);
      this.logger.info("用户[" + logoffVO.getUserID() + "]退出在[" + 
        logoffVO.getLogonType() + "]类型登录，发现用户已退出");
      return result;
    }
    if (!userManageVO.getUserID().equals(logoffVO.getUserID()))
    {
      ...
      this.logger.info("用户[" + logoffVO.getUserID() + "]退出在[" + 
        logoffVO.getLogonType() + "]类型登录，AU SessionID 不符");
      return result;
    }
    logonTypeManageVO.getSessionMap().remove(Long.valueOf(userManageVO.getSessionID()));
    List<AUUserManageVO> list = (List)logonTypeManageVO.getUserMap().get(
      userManageVO.getUserID());
    if (list != null)
    {
      if (list.size() <= 1) {
    ...
    this.logger.info("用户[" + logoffVO.getUserID() + "]退出在[" + 
      logoffVO.getLogonType() + "]类型登录的 SessionID[" + 
      userManageVO.getSessionID() + "]成功");
  ...
  public AUCheckUserResultVO checkUser(AUCheckUserVO checkUserVO)
  {
    ...
    AULogonTypeManageVO logonTypeManageVO = getLogonTypeManageVO(checkUserVO
      .getLogonType());
    
    List<AUUserManageVO> userManageList = 
      (List)logonTypeManageVO.getUserMap().get(checkUserVO.getUserID());
    ...
    if (userManageList.size() > 0) {
      if (this.logonMode == LogonMode.MULTI_MODE)
      {
        for (AUUserManageVO userManageVO : userManageList) {
          if (userManageVO.getSessionID() == checkUserVO
            .getSessionID())
          {
            initUserMap = true;
      ...
      else
      {
        ...
        {
          Object moduleIDList = userManageVO.getModuleIDList();
          
          boolean enableModule = false;
          for (Integer moduleID : (List)moduleIDList) {
          ...
          if (!enableModule) {
            ((List)moduleIDList).add(Integer.valueOf(checkUserVO.getToModuleID()));
          }
          userManageVO.setLastTime(System.currentTimeMillis());
    ...
    if (!initUserMap)
    {
      ...
      logonTypeManageVO.getUserMap().put(newUserManageVO.getUserID(), 
        userManageList);
      logonTypeManageVO.getSessionMap().put(
        Long.valueOf(newUserManageVO.getSessionID()), newUserManageVO);
  ...
  public AUGetUserResultVO getUserBySessionID(AUGetUserVO auGetUserVO)
  {
    ...
    AULogonTypeManageVO logonTypeManageVO = getLogonTypeManageVO(auGetUserVO
      .getLogonType());
    
    AUUserManageVO userManageVO = (AUUserManageVO)logonTypeManageVO.getSessionMap().get(
      Long.valueOf(auGetUserVO.getSessionID()));
    if (userManageVO == null)
    {
  ...
  public AUISLogonResultVO isLogon(AUISLogonVO isLogonVO)
  {
    ...
    result.setResult(-1);
    if ((isLogonVO == null) || (isLogonVO.getLogonType() == null) || 
      (isLogonVO.getUserID() == null))
    {
      result.setRecode("-1301");
      result.setMessage("传入信息不完整");
      return result;
    }
    AULogonTypeManageVO logonTypeManageVO = getLogonTypeManageVO(isLogonVO
      .getLogonType());
    
    List<AUUserManageVO> userManageList = 
      (List)logonTypeManageVO.getUserMap().get(isLogonVO.getUserID());
    ...
    AUUserManageVO userManageVO = (AUUserManageVO)logonTypeManageVO.getSessionMap().get(
      Long.valueOf(isLogonVO.getSessionID()));
    if (userManageVO == null)
    {
      if (this.logonMode == LogonMode.MULTI_MODE)
      {
  ...
  public AUCompulsoryLogoffResultVO compulsoryLogoff(AUCompulsoryLogoffVO compulsoryLogoffVO)
  {
    ...
    if ((compulsoryLogoffVO == null) || 
      (compulsoryLogoffVO.getLogonIP() == null) || 
      (compulsoryLogoffVO.getOperator() == null) || 
      (compulsoryLogoffVO.getUserIDList() == null) || 
      (compulsoryLogoffVO.getUserIDList().size() <= 0))
    {
      result.setRecode("-1401");
      result.setMessage("传入信息不完整");
      if (compulsoryLogoffVO != null) {
        this.logger.info("管理员[" + compulsoryLogoffVO.getOperator() + "]在[" + 
          compulsoryLogoffVO.getLogonIP() + "]地址强制退出用户[" + 
    ...
    for (String userID : compulsoryLogoffVO.getUserIDList())
    {
      ...
      {
        AULogonTypeManageVO logonTypeManageVO = 
          (AULogonTypeManageVO)this.logonTypeMap.get(logonType);
        
        List<AUUserManageVO> userManageList = 
          (List)logonTypeManageVO.getUserMap().get(userID);
        if (userManageList != null)
        {
          for (AUUserManageVO userManageVO : userManageList) {
            logonTypeManageVO.getSessionMap().remove(
              Long.valueOf(userManageVO.getSessionID()));
          }
          logonTypeManageVO.getUserMap().remove(userID);
        }
      }
      this.logger.info("管理员[" + compulsoryLogoffVO.getOperator() + "]在[" + 
        compulsoryLogoffVO.getLogonIP() + "]地址强制退出用户[" + userID + 
        "]成功");
    }
    result.setResult(1);
    return result;
  ...
  public List<AULogonOrLogoffUserVO> getAllLogonUserList()
  {
    ...
    for (String key : this.logonTypeMap.keySet()) {
      synchronized (getClass())
      {
        ...
        Map<Long, AUUserManageVO> map = auLogonTypeManageVO
          .getSessionMap();
