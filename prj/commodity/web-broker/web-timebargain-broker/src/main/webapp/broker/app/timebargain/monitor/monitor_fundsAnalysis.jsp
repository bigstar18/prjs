...
<html>
  <head>
	...
	<script src="${mgrPath}/app/timebargain/monitor/js/monitorAjax2.js" type="text/javascript"></script>
    ...
    <script type="text/javascript">
	   //设置数据源地址
		function winload() 
		{
			...
			setRequestUrl("${basePath}/ajaxcheck/monitor/monitorFundsAnalysis.action");
		}
    ...
    function PopMenu(id)
    {
        ...
        oPopBody.innerHTML =TableBegin + Tr2 + TrBr2 + Tr3+ TableEnd;
        //oPopBody.innerHTML =TableBegin + Tr1 + TableEnd;
        oPopup.show(event.x, event.y, 100, 57, document.body);
        return false;
    }
    function firmMonitor(firmID){
    	window.location = "${basePath}/timebargain/monitor/monitorGetCommodityList.action?firmID=" + firmID+"&type=firmMonitor";
    }
    
    function tradeList(firmID){
    	window.location = "${basePath}/timebargain/monitor/monitorGetCommodityList.action?firmID=" + firmID+"&type=tradeList";
    }
    
    function orderMonitor(firmID){
    	window.location = "${basePath}/timebargain/monitor/monitorGetCommodityList.action?firmID=" + firmID+"&type=orderMonitor";
    }
    
    function firmInfo(firmID){
    	var url="${basePath}/timebargain/monitor/monitorFundsAnalysisInfo.action?firmID=" + firmID;
