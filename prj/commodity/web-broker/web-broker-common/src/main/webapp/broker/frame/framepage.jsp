...
<html>
	<head>
		...
		var lastToModuleID = '<%=Global.getSelfModuleID()%>';
			 ...
			 * 将路径结尾加上登陆用户类型
			 */
			function srcAddUserType(src){
				if(src.indexOf("?")>0){
					src += "&<%=Global.CURRENTUSERSTRTYPE%>=${CurrentUser.type}";
				}else{
					src += "?<%=Global.CURRENTUSERSTRTYPE%>=${CurrentUser.type}";
				}
				return src;
			...
			*/
			function clickChange(obj,allobj){
				...
				if(affirmrst){
					allobj.css("background-color","");
					obj.css("background-color","#050505");
	...
	<frameset rows="65,*" cols="*" frameborder="NO" border="0" framespacing="0">
		...
		<frameset id=mainwork rows="100%,*" cols="*" frameborder="NO" border="0" framespacing="0" >
			<frame src="<%=framePath %>/waiting.jsp" id='common_broker' visibility='visible'>
		</frameset>
	</frameset>
	<noframes>
		<body>
