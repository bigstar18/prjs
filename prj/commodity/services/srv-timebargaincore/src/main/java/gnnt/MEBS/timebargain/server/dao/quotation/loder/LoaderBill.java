...
public class LoaderBill extends BaseLoader {
	private final transient Log log = LogFactory.getLog(LoaderBill.class);

	public LoaderBill(Config paramConfig) {
		super(paramConfig);
	}
	...
	public List loadCommodity() {
		...
		localStringBuilder.append("select a.commodityid,a.name,decode(a.status,0,'N',1,'D','S') status,a.contractfactor,a.minpricemove,")
				...
				.append("where a.BreedID = b.BreedID").append(" and a.SettleWay =2 ");
	...
	public List loadHisCommodity() {
		...
		localStringBuilder.append("select a.commodityid,a.name,'D' status,a.contractfactor,a.minpricemove,")
				...
				.append("to_char(a.settledate+1,'yyyyMMdd') settledate,a.FirmMaxHoldQty from T_SettleCommodity a ").append(" where a.SettleWay =2 ");
		return executeQuery(localStringBuilder.toString());
	}

	public List loadProductData(Calendar paramCalendar) {
		String str = "Select q.* from T_QUOTATION q,T_Commodity c Where q.CommodityID=c.CommodityID and c.SettleWay =2 and q.CreateTime>? Order By q.CreateTime ";
	...
	public List loadAllProductData() {
		String str = "Select q.* from T_QUOTATION q,T_Commodity c Where q.CommodityID=c.CommodityID and c.SettleWay =2 Order By q.CreateTime ";
		List localArrayList = new ArrayList();
		try {
			getConnection();
			this.ps = this.conn.prepareStatement(str);
	...
	public void loadCodeSet(HashSet paramHashSet) {
		String str = "select commodityid from t_commodity where SettleWay =2 ";
		try {
			getConnection();
			this.ps = this.conn.prepareStatement(str);
			this.rs = this.ps.executeQuery();
	...
	public HashMap<String, String> getCommodityTradeSec() {
		String str1 = "select a.CommodityID,a.SectionID from T_A_CommodityTradeProp a,T_Commodity c where a.CommodityID=c.CommodityID and c.SettleWay =2 ";
