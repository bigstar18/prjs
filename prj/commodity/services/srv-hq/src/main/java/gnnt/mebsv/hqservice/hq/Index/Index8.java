...
public class Index8 extends Index
{
  public Index8(HQDAO paramHQDAO)
  {
    super(paramHQDAO);
  }
  ...
  boolean calculate(int paramInt)
  {
    ...
    {
      if (this.indexData[paramInt].code.startsWith("SS"))
        f = 0.6F;
      else if (this.indexData[paramInt].code.startsWith("GJ"))
        f = 5.0F;
      else
        f = 1.0F;
      this.indexData[paramInt].curPrice = (this.indexData[paramInt].balancePrice = (float)(100.0D * d / ((float)l * f)));
      System.out.println("curPrice:" + this.indexData[paramInt].curPrice);
    }
    ...
    for (int i = 0; i < this.indexData.length; i++)
      try
      {
        ...
        if (localDayDataVO == null)
        {
          ...
          this.indexData[i].curPrice = 10000.0F;
          this.indexData[i].yesterBalancePrice = 10000.0F;
        }
        else
        {
          this.indexData[i].time = ((Date)this.dao.getHqTime().get(Integer.valueOf(0)));
